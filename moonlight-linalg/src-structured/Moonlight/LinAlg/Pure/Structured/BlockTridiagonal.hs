...
mkRowMajorBlock rowCount columnCount payload
  ...
  | U.length payload /= rowCount * columnCount =
      Left
        ( InvariantViolation
  ...
  | U.any (not . isFiniteDouble) payload =
      Left (InvariantViolation "row-major block entries must be finite")
  | otherwise =
      Right
        RowMajorBlock
...
symmetrizeRowMajorBlockLower blockValue
  ...
  | otherwise =
      mkRowMajorBlock
        blockSize
        blockSize
        (U.generate (blockSize * blockSize) mirroredLowerEntry)
  where
...
mkSymmetricBlockTridiagonal diagonalBlocks lowerCouplingBlocks = do
  ...
  Right
    SymmetricBlockTridiagonal
      { blockOffsets = offsetsFromSizes (Box.toList blockSizes),
        diagonalPayloadOffsets = offsetsFromSizes (U.length <$> Box.toList diagonalPayloads),
        diagonalLowerPacked = U.concat (Box.toList diagonalPayloads),
        couplingPayloadOffsets = offsetsFromSizes (U.length <$> Box.toList couplingPayloads),
...
validateDiagonalBlock blockValue
  ...
  | otherwise =
      if U.and (U.generate (rowMajorBlockRows blockValue * rowMajorBlockRows blockValue) symmetricEntry)
        then Right ()
...
offsetsFromSizes :: [Int] -> U.Vector Int
offsetsFromSizes sizes =
  U.fromList (scanl (+) 0 sizes)
