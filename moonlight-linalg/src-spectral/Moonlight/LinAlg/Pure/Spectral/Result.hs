...
mkEigenpairs dimension values vectors residuals
  ...
  | U.length vectors /= dimension * U.length values =
      Left (InvariantViolation "Eigenpair vector payload length must equal dimension * eigenvalue count")
  | otherwise =
      Right
        Eigenpairs
...
certifySelectedEigenpairResiduals residualBound pairs =
  case filter (\(_, residualValue) -> not (finiteDouble residualValue) || residualValue < 0.0 || residualValue > residualBound) indexedResiduals of
    [] ->
      Right
        SelectedEigenpairResidualEvidence
...
certifySelectedEigenpairOrthonormality orthonormalityBound columns =
  case filter (\(_, _, deviationValue) -> not (finiteDouble deviationValue) || abs deviationValue > orthonormalityBound) deviations of
    [] ->
      Right
        SelectedEigenpairOrthonormalityEvidence
...
orderedAdjacent spectrumEnd (_, leftValue, rightValue) =
  finiteDouble leftValue
    && finiteDouble rightValue
