...
data GF2PackedMatrixFailure
  ...
  | GF2PackedMatrixNegativeShape !Int !Int
...
mkGF2PackedMatrix rowCountValue columnCountValue entries =
  let rowCount =
        fromIntegral rowCountValue
      columnCount =
        fromIntegral columnCountValue
   in case firstOutOfBoundsEntry rowCount columnCount entries of
        Just entryValue ->
          Left
...
mkGF2PackedMatrixFromRowMajor ::
  ...
  [GF2] ->
  Either GF2PackedMatrixFailure GF2PackedMatrix
mkGF2PackedMatrixFromRowMajor rowCountValue columnCountValue values =
  let expectedEntryCount =
        fromIntegral (rowCountValue * columnCountValue)
      actualEntryCount =
        length values
   in if actualEntryCount /= expectedEntryCount
