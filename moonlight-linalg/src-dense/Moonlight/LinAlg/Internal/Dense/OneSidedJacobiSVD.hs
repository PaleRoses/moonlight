...
thinSvdFullColumnRank matrixValue
  ...
  | otherwise =
      runST (thinSvdFullColumnRankST rowCount columnCount matrixValue)
  where
    !(rowCount, columnCount) = denseDoubleMatrixShape matrixValue
    payload = denseDoubleMatrixToRowMajorVector matrixValue

thinSvdFullColumnRankST :: Int -> Int -> DenseDoubleMatrix -> ST s (Either ThinSvdFailure ThinSvdResult)
thinSvdFullColumnRankST !rowCount !columnCount matrixValue = do
  ...
  case sweepResult of
    ...
    Right () -> do
      ...
      case firstRankDeficiency rankTolerance singularValues of
        ...
        Nothing -> Right <$> projectThinSvdResult rowCount columnCount leftColumns rightVectors singularValues
...
projectThinSvdResult ::
  ...
  S.Vector Double ->
  ST s ThinSvdResult
projectThinSvdResult !rowCount !columnCount leftColumns rightVectors singularValues = do
  uBuffer <- SM.new (rowCount * columnCount)
  sigmaBuffer <- SM.new columnCount
  vtBuffer <- SM.new (columnCount * columnCount)
  let orderedColumns =
        fmap fst
