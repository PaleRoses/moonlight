...
smithStep ::
  ...
  SmithState a ->
  Either MoonlightError (SmithState a)
smithStep pivotIndex rowCount columnCount stateValue
  ...
  | otherwise = do
      ...
      case pivotCandidate of
        ...
        Just (pivotRow, pivotCol, _) -> do
          ...
          let maxRounds = max 1 (rowCount * columnCount * 2)
          normalized <- normalizePivot pivotRowIndex pivotColumnIndex maxRounds pivotMoved
          smithStep (pivotIndex + 1) rowCount columnCount normalized
...
enforceDivisibilityChain ::
  ...
  SmithState a ->
  Either MoonlightError (SmithState a)
enforceDivisibilityChain rowCount columnCount stateValue =
  go budget stateValue
  where
    diagSize = min rowCount columnCount
    budget = diagSize * diagSize
      ...
      | otherwise =
          case findViolation 0 (smithStateMatrix currentState) of
            ...
            Just violationIndex -> do
              ...
              let maxRounds = max 1 (rowCount * columnCount * 2)
              normalizedI <- normalizePivot rowI colI maxRounds combined
              normalizedJ <- normalizePivot rowJ colJ maxRounds normalizedI
...
runSmithState witnessValue matrixValue = do
  ...
  steppedState <- smithStep 0 rowCount columnCount initialState
  repairedState <- enforceDivisibilityChain rowCount columnCount steppedState
