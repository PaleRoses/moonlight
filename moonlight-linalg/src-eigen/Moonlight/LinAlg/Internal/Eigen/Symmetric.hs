...
symmetricEigenPairsDenseUnchecked !matrixSize matrixValue
  ...
  | otherwise =
      runST (symmetricEigenPairsST matrixSize matrixValue)
  where
...
symmetricEigenPairsST :: Int -> DenseDoubleMatrix -> ST s (Either MoonlightError SymmetricEigenResult)
symmetricEigenPairsST !matrixSize matrixValue = do
    ...
    Right () -> do
      ...
      case clusterResult of
        ...
        Right () -> do
          ...
          Right <$> eigenResultFromMutable matrixSize diagonalValues eigenvectors
...
eigenResultFromMutable !matrixSize diagonalValues (MutableDenseWork rowCount _ eigenvectorPayload) = do
  eigenvalueBuffer <- SM.new matrixSize
  eigenvectorBuffer <- SM.new (matrixSize * matrixSize)
