...
checkedColumnPayloadLength dimension columnCount
  ...
  | columnCount /= 0
      && dimension > (maxBound :: Int) `quot` columnCount =
      Left
        ( InvariantViolation
            "native eigenvector payload exceeds Int range"
        )
  | otherwise = Right (dimension * columnCount)
...
blockTridiagonalEigenpairsFromRawPairs spectrumEnd blockValue rawPairs = do
  ...
  matrixNorm <- blockFrobeniusNorm blockValue
...
blockFrobeniusNorm :: SymmetricBlockTridiagonal -> Either MoonlightError Double
blockFrobeniusNorm blockValue =
  let dimension = symmetricBlockTridiagonalDimension blockValue
   in sqrt . U.sum
        <$> U.generateM
          (dimension * dimension)
          ( \payloadIndex ->
              let rowIndex = payloadIndex `quot` dimension
                  columnIndex = payloadIndex `rem` dimension
               in (\entryValue -> entryValue * entryValue)
