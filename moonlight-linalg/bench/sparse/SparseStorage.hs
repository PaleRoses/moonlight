...
sparseStorageBenchmark dimension =
  bgroup
    ("n=" <> show dimension)
    [ sparseStorageCaseBenchmark dimension "dense 32x32 -> COO" denseToCooWeight,
      sparseStorageCaseBenchmark dimension "dense 32x32 -> CSR" denseToCsrWeight,
      sparseStorageCaseBenchmark dimension "dense 32x32 -> CSC" denseToCscWeight,
      sparseStorageCaseBenchmark dimension "CSR -> COO" csrToCooWeight,
      sparseStorageCaseBenchmark dimension "CSR -> CSC" csrToCscWeight,
      sparseStorageCaseBenchmark dimension "CSR matvec" csrMatvecWeight,
      sparseStorageCaseBenchmark dimension "packed sparse apply" packedSparseWeight,
      sparseStorageCaseBenchmark dimension "graph Laplacian construction" graphLaplacianWeight
    ]

sparseStorageOnceBenchmark :: Int -> [OnceBenchmark]
sparseStorageOnceBenchmark dimension =
  [ sparseStorageCaseOnceBenchmark dimension "dense 32x32 -> COO" denseToCooWeight,
    sparseStorageCaseOnceBenchmark dimension "dense 32x32 -> CSR" denseToCsrWeight,
    sparseStorageCaseOnceBenchmark dimension "dense 32x32 -> CSC" denseToCscWeight,
    sparseStorageCaseOnceBenchmark dimension "CSR -> COO" csrToCooWeight,
    sparseStorageCaseOnceBenchmark dimension "CSR -> CSC" csrToCscWeight,
    sparseStorageCaseOnceBenchmark dimension "CSR matvec" csrMatvecWeight,
    sparseStorageCaseOnceBenchmark dimension "packed sparse apply" packedSparseWeight,
    sparseStorageCaseOnceBenchmark dimension "graph Laplacian construction" graphLaplacianWeight
  ]

sparseStorageCaseBenchmark :: Int -> String -> (SparseStorageCase -> BenchmarkWeight) -> Benchmark
sparseStorageCaseBenchmark dimension label measure =
  env (prepareBenchmarkSetup (prepareSparseStorageCase dimension)) $ \benchmarkCase ->
    bench label (nf measure benchmarkCase)

sparseStorageCaseOnceBenchmark :: Int -> String -> (SparseStorageCase -> BenchmarkWeight) -> OnceBenchmark
sparseStorageCaseOnceBenchmark dimension label measure =
  OnceBenchmark
    { onceBenchmarkLabel = "sparse storage and packed kernels.n=" <> show dimension <> "." <> label,
      onceBenchmarkAction =
        pure
          (runBenchmarkSetup (prepareSparseStorageCase dimension) >>= benchmarkWeightEither . measure)
    }
