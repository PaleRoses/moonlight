...
blockedToSparseMat :: (Eq a, Num a) => BlockedMat a -> SparseMat a
...
canonicalSparseMatEntries :: (Eq a, Num a) => SparseMat a -> [SparseMatrixEntry a]
canonicalSparseMatEntries SparseMat {smEntries} =
  ...
  where
    accumulatedEntries =
      foldr
        ( \SparseMatrixEntry {smeRow, smeColumn, smeValue} ->
...
sparseMatColumnEntries :: (Eq a, Num a) => SparseMat a -> Vector [(Int, a)]
...
denseBlockEntries :: (Eq a, Num a) => DenseMat a -> SparseBlockEntries a
denseBlockEntries denseMat =
  ...
  where
    insertEntry SparseMatrixEntry {smeRow, smeColumn, smeValue} =
      insertSparseBlockCoefficient smeRow smeColumn smeValue
...
fromExpanded rowLabelsVec colLabelsVec mat =
  let rows = fromLabels rowLabelsVec
      ...
      rIndex = V.ifoldl' (\acc i (FinObjectId objectKey) -> IM.insertWith (\new old -> old ++ new) objectKey [i] acc) IM.empty rowLabelsVec
      cIndex = V.ifoldl' (\acc j (FinObjectId objectKey) -> IM.insertWith (\new old -> old ++ new) objectKey [j] acc) IM.empty colLabelsVec
  ...
  where
    updateOne acc c =
      let tgtBlk = blockAt targetLab c acc
          ...
          srcRow = if dmRows srcBlk == 0 then V.replicate (dmCols tgtBlk) 0 else rowAt srcBlk sourceIx
  ...
  where
    updateOne acc r =
      let tgtBlk = blockAt r targetLab acc
          ...
          srcCol = if dmCols srcBlk == 0 then V.replicate (dmRows tgtBlk) 0 else colAt srcBlk sourceIx
