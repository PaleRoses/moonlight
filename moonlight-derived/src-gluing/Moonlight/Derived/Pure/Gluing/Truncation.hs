...
import Moonlight.Derived.Pure.Site.Poset (DerivedPoset (derivedPosetNodes))
...
exactTail posetValue seedDiff =
  unfoldM
    step
    (V.length (derivedPosetNodes posetValue) + 1, Just seedDiff)
  where
    step (_, Nothing) =
      Right Nothing
    step (remainingFuel, Just previousDifferential)
      | remainingFuel <= 0 =
          Left
            ( InvariantViolation
                "canonical truncation: exact tail exceeded the finite-poset dimension bound"
            )
      | otherwise = do
          nextDifferential <- resolutionStep posetValue previousDifferential
          Right
            ( Just
                ( nextDifferential
                , ( remainingFuel - 1
                  , if axisEmpty (blockedMatRows nextDifferential)
                      then Nothing
                      else Just nextDifferential
                  )
                )
            )
