...
denseMul ::
  ...
  DenseMat a ->
  Either MoonlightError (DenseMat a)
denseMul context leftMatrix rightMatrix = do
  if dmCols leftMatrix /= dmRows rightMatrix
    then
      Left
        ( InvariantViolation
            ( context
      ...
      leftSparseRows <-
        traverse
          (denseRowNonZeros (context <> ": left sparse row") leftMatrix)
          (indexRange (dmRows leftMatrix))
      resultRows <-
        traverse
          (buildProductRow rightMatrix)
          leftSparseRows
      Right
        DenseMat
          { dmRows = dmRows leftMatrix
          , dmCols = dmCols rightMatrix
          , dmData = V.fromList resultRows
          }
  where
    buildProductRow rightFactor leftEntries =
      fmap V.fromList
        ( traverse
            (denseMulCell rightFactor leftEntries)
            (indexRange (dmCols rightFactor))
        )

    denseMulCell rightFactor leftEntries columnIndex =
      foldM
        ( \ !accumulated (innerIndex, leftValue) -> do
            rightValue <-
              denseIndex
                (context <> ": right factor entry")
                rightFactor
                innerIndex
                columnIndex
            let !nextValue =
