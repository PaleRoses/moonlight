...
reduceFully :: forall a. (Field a, Num a) => Reduction a -> Reduction a
reduceFully = reduceFullyFrom 0
  where
    reduceFullyFrom :: Int -> Reduction a -> Reduction a
    reduceFullyFrom resumeDiffIndex reductionValue =
      case findPivot resumeDiffIndex reductionValue of
        Nothing -> reductionValue
        Just pivotValue@(diffIndex, _, _, _) ->
          reduceFullyFrom (max 0 (diffIndex - 1)) (eliminate pivotValue reductionValue)

findPivot :: Field a => Int -> Reduction a -> Maybe (Int, Int, Int, a)
findPivot resumeDiffIndex Reduction {redLabels, redDiffs} =
  listToMaybe
    [ (diffIndex, rowIndex, colIndex, inverseValue)
    | diffIndex <- [resumeDiffIndex .. V.length redDiffs - 1]
...
eliminate (diffIndex, pivotRow, pivotCol, inverseValue) Reduction {redLabels, redDiffs, redPhi, redPsi} =
  materializeDenseUpdates diffUpdates
    `seq` materializeDenseUpdates phiUpdates
    `seq` materializeDenseUpdates psiUpdates
    `seq` Reduction
      { redLabels =
          redLabels
      ...
      , redDiffs = redDiffs V.// diffUpdates
      , redPhi = redPhi V.// phiUpdates
      , redPsi = redPsi V.// psiUpdates
      }
  where
    diffValue = redDiffs V.! diffIndex

    diffUpdates =
      [ (diffIndex, schurValue) ]
    ...
    phiUpdates =
      [ (diffIndex, deleteDenseRow pivotCol (redPhi V.! diffIndex))
      , (diffIndex + 1, correctedPhi)
      ]

    psiUpdates =
      [ (diffIndex, correctedPsi)
...
materializeDenseUpdates :: [(Int, DenseMat a)] -> ()
materializeDenseUpdates =
  foldl'
    (\() (_, DenseMat {dmData}) ->
      V.foldl'
        (\() rowValue -> V.foldl' (\() entryValue -> entryValue `seq` ()) () rowValue)
        ()
        dmData
    )
    ()
