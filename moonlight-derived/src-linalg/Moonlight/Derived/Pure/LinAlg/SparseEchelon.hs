...
appendSparseRowEntries context entries rowValue =
  ...
  where
    insertEntry rowValue (indexValue, coefficientValue)
      | indexValue < 0 =
          Left
            ( InvariantViolation
                ( context
      ...
      | otherwise =
          Right (insertCoefficient indexValue coefficientValue rowValue)
...
prependStableSparseSpanRows context prefixRows suffixRows suffixSpan =
  Just
    <$> prependSparseSpanRows
      context
      prefixRows
...
prependStableTrackedLeftKernelRows context prefixRows suffixRows suffixKernel suffixKernelRows =
  Just
    <$> prependTrackedLeftKernelRows
      context
      prefixRows
...
reduceAgainstSpan (SparseSpan basisRows) initialRow =
  foldl'
    ( \rowValue (pivotIndex, basisRow) ->
        eliminatePivot pivotIndex basisRow rowValue
    )
    initialRow
    (IntMap.toAscList basisRows)
...
reduceTrackedAgainstBasis basisRows initialData initialWitness =
  ...
  where
    reduceOne ::
      (IntegralDomain value, Num value) =>
      (SparseRow value, SparseRow value) ->
      (Int, TrackedEchelonRow value) ->
      (SparseRow value, SparseRow value)
    reduceOne
      (dataRow, witnessRow)
      (pivotIndex, TrackedEchelonRow {terData, terWitness}) =
        case coefficientAt pivotIndex dataRow of
          Nothing ->
            (dataRow, witnessRow)
...
eliminatePivot ::
  ...
  SparseRow a ->
  SparseRow a
eliminatePivot pivotIndex basisRow targetRow =
  case coefficientAt pivotIndex targetRow of
    Nothing ->
      targetRow
    Just coefficientValue ->
      addScaledSparseRow
...
coefficientAt :: Int -> SparseRow a -> Maybe a
coefficientAt indexValue (SparseRow entries) =
  IntMap.lookup indexValue entries
