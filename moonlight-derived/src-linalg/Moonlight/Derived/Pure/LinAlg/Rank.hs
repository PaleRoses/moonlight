...
sparseMatStableDigest :: forall a. (Eq a, Num a) => SparseMat a -> SparseMatStableDigest
sparseMatStableDigest sparseMat =
  ...
  where
    supportEntry :: SparseMatrixEntry a -> String
...
sparseRankRows :: forall a. (Eq a, Num a) => SparseMat a -> [SparseRankRow a]
sparseRankRows =
  fmap rowMap
    . IntMap.toAscList
    . foldl' insertEntry IntMap.empty
    . canonicalSparseMatEntries
  where
    insertEntry :: IntMap (SparseRankRow a) -> SparseMatrixEntry a -> IntMap (SparseRankRow a)
    insertEntry rowsByIndex SparseMatrixEntry {smeRow, smeColumn, smeValue} =
      IntMap.alter
        (Just . insertRankCoefficient smeColumn smeValue . maybe IntMap.empty id)
        smeRow
        rowsByIndex

    rowMap :: (Int, SparseRankRow a) -> SparseRankRow a
    rowMap =
      snd
...
reduceSparseRankRow spanValue initialRow
  | IntMap.null spanValue =
      initialRow
  | otherwise =
      reduceAtOrAfter 0 initialRow
  where
    reduceAtOrAfter !cursor !rowValue =
      case IntMap.lookupGE cursor rowValue of
        Nothing ->
          rowValue
        Just (columnIndex, coefficientValue) ->
          let !nextRow =
                case IntMap.lookup columnIndex spanValue of
                  Nothing ->
                    rowValue
                  Just basisRow ->
