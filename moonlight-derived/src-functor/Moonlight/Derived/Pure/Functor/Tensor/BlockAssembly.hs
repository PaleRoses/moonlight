...
addScaledSparseMatAt context rowOffsetValue columnOffsetValue coefficientValue sparseValue accumulated
  ...
  | otherwise =
      validateSparseShape context sparseValue
        *> do
          rowPlacements <-
            placementSlice
...
validateSparseShape :: String -> SparseMat a -> Either MoonlightError ()
validateSparseShape context SparseMat {smRows, smCols, smEntries}
  | smRows < 0 || smCols < 0 =
      Left
        ( InvariantViolation
            ( context
                <> ": sparse shape has negative extent "
                <> show (smRows, smCols)
            )
        )
  | otherwise =
      traverse
        (validateSparseEntry context smRows smCols)
        smEntries
        *> Right ()
...
validateSparseEntry context rowCount columnCount SparseMatrixEntry {smeRow, smeColumn}
  | smeRow < 0 || smeRow >= rowCount =
      Left
        ( InvariantViolation
            ( context
                <> ": sparse row coordinate out of bounds "
                <> show (smeRow, rowCount)
            )
        )
  | smeColumn < 0 || smeColumn >= columnCount =
      Left
        ( InvariantViolation
            ( context
                <> ": sparse column coordinate out of bounds "
                <> show (smeColumn, columnCount)
            )
        )
  | otherwise =
      Right ()
