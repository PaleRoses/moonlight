...
import Moonlight.EGraph.Pure.Context.Core (cegContextRevision)
...
import Moonlight.EGraph.Pure.Extraction (liftCostAlgebra)
...
advanceHarvest config ingested planUsedGlobalFallback stageReport stagedGraph saturated previousHarvest =
  ...
  where
    ...
    dirtyContexts =
      crrDirtyContexts stageReport <> cmtDirtyContexts (smMutationTrace saturated)
    ...
    saturationReport =
      ContextRebaseReport
        { crrScope = crrScope stageReport,
          crrDirtyContexts = cmtDirtyContexts (smMutationTrace saturated),
