...
sizeExtractionSections config contextGraph contexts =
  either
    (const (nebulaSizeExtractionSectionsFromCache (emptyContextualSectionCache contextGraph)))
    nebulaSizeExtractionSectionsFromCache
    sectionCacheResult
  where
    sectionCacheResult =
...
choiceLabelsForSection section =
  let choiceSection =
        cesChoiceSection section
   in choiceLabels
        (ecsTable choiceSection)
        (ecsChoices choiceSection)
        tagSignatureChoiceLabels

tagSignatureChoiceLabels :: ChoiceLabelAlgebra HsExprF TagSignature
tagSignatureChoiceLabels =
  ChoiceLabelAlgebra
    { choiceLabelNode = \(ENode nodeValue) -> tagSignatureFromTag (hsExprRootTag nodeValue),
      choiceLabelJoin = (<>)
    }
