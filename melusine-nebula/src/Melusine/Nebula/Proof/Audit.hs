...
baseTypeEvidenceCensus graph =
  foldMap
    ...
    (IntMap.toAscList (materializeEGraphClasses graph))
...
classEvidenceByKey contextGraph =
  IntMap.fromList
    ...
    | (classKey, classValue) <- IntMap.toAscList (materializeEGraphClasses (cegBase contextGraph)),
...
replayStepVerdicts ::
  ...
  IntMap (Set StableDigest128) ->
  [ReplayStep] ->
  Either UnionFindAllocationError [TypeVerdict]
...
replayStep :: ReplayState -> ReplayStep -> Either UnionFindAllocationError (ReplayState, TypeVerdict)
replayStep state step = do
  (anchoredLhs, lhsState) <- anchorKey (repLhsWitness step) state
  (anchoredRhs, anchoredState) <- anchorKey (repRhsWitness step) lhsState
  let lhsRep = representative (rsParent anchoredState) anchoredLhs
...
anchorKey :: Maybe (Fix HsExprF) -> ReplayState -> Either UnionFindAllocationError (Int, ReplayState)
anchorKey maybeWitness state =
  case maybeWitness of
    Just witness -> do
      (anchorClass, anchoredGraph) <- addTerm witness (rsAnchorGraph state)
      pure (classIdKey anchorClass, state {rsAnchorGraph = anchoredGraph})
    Nothing ->
      pure
