...
sizeSectionsFor :: NebulaConfig -> IngestedModule -> SaturatedModule -> NebulaSizeExtractionSections
...
requireChosenWith config workload = do
  ...
  requireRight "binding extraction" (chooseBindings config ingested saturated (sizeSectionsFor config ingested saturated))
...
chooseCases =
  testGroup
    ...
    [ testCase "extraction never inflates a binding under the size cost" $ do
        (ingested, saturated) <- requireSaturatedFixture
        chosen <- requireRight "binding extraction" (chooseBindings defaultNebulaConfig ingested saturated (sizeSectionsFor defaultNebulaConfig ingested saturated))
        assertEqual
      ...
      testCase "anti-unification surfaces the shared application spine first" $ do
        (ingested, saturated) <- requireSaturatedFixture
        chosen <- requireRight "binding extraction" (chooseBindings defaultNebulaConfig ingested saturated (sizeSectionsFor defaultNebulaConfig ingested saturated))
        sites <- requireRight "candidate sites" (candidateSites defaultNebulaConfig ingested saturated chosen (sizeSectionsFor defaultNebulaConfig ingested saturated))
