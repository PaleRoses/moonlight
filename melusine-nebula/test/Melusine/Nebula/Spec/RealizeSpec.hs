...
measureProductionPath workload =
  measureFreshSample
    ...
    (\receipt -> productionPathReceiptDemand receipt `seq` ())
...
timedProductionPath measurement =
  TimedProductionPath
    { tppElapsedNs = freshMeasurementElapsedNanoseconds measurement,
      tppAllocatedBytes = freshRtsDeltaAllocatedBytes (freshMeasurementRtsDelta measurement),
      tppPeakLiveBytes = freshMeasurementPeakLiveBytesThroughAction measurement,
...
freshMeasurementFailure = \case
  ...
  FreshMeasurementRtsDeltaFailed obstruction ->
    failedTimedProductionPath ("RTS counter regression: " <> show obstruction)
