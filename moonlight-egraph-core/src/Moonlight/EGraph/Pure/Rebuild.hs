...
import Moonlight.Delta.Normalize (DeltaNormalize (deltaNull))
...
rebuild graph =
  let (_delta, result) = rebuildWithDelta graph in result
  ...
  let (rebuildDelta, tuplePatch, updatedGraph) =
        if eGraphEditDeltaNull (egPendingDelta graph)
          then (mempty, emptyStructuralTuplePatch, graph)
            ...
            let (delta, patchValue, nextGraph) =
...
rebuildWithDelta :: Language f => EGraph f a -> (EGraphRebuildDelta, EGraph f a)
rebuildWithDelta graph
  | eGraphEditDeltaNull (egPendingDelta graph) =
      (mempty, graph)
  | otherwise =
      let (delta, tuplePatch, updatedGraph) = rebuildWithTuplePatch graph
       in tuplePatch `seq` (delta, updatedGraph)

rebuildWithTuplePatch :: Language f => EGraph f a -> (EGraphRebuildDelta, StructuralTuplePatch f, EGraph f a)
rebuildWithTuplePatch graph
  | eGraphEditDeltaNull (egPendingDelta graph) =
      (mempty, emptyStructuralTuplePatch, graph)
  | otherwise =
      let preRepair =
            drainPendingEditDeltaPre (egPendingDelta graph) graph
       ...
       in case obstructions of
            [] ->
              (drainedDelta, proTuplePatch preRepair, graphAfterDrain)
            _ ->
              let (recDelta, recPatch, finalGraph) =
