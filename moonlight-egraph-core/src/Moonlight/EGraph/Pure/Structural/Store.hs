...
structuralParentKeysByChild :: StructuralStore f -> IntMap IntSet
structuralParentKeysByChild =
  ssParentKeysByChild
{-# INLINE structuralParentKeysByChild #-}
...
structuralChildrenByResult =
  IntMap.mapMaybe nonEmptyIntMap . IntMap.map nodesChildMultiplicity . ssNodesByResult
...
structuralChildrenByResultWithin resultKeys store =
  IntMap.mapMaybe nonEmptyIntMap (IntMap.fromSet childrenAt resultKeys)
  where
...
deleteStructuralRow resultKey enode store =
  ...
  where
    ...
    retainedChildKeys =
      Set.foldl'
        (\childKeys retainedNode -> IntSet.union childKeys (IntSet.fromList (enodeChildKeys retainedNode)))
        IntSet.empty
        retainedNodes
    ...
    nextParentKeysByChild =
      IntSet.foldl'
        ( \parents childKey ->
            if IntSet.member childKey retainedChildKeys
              then parents
