...
data ExtractionChoiceCache f a cost = ExtractionChoiceCache
  { eccRevision :: !EGraphRevision,
    eccTable :: !(ExtractionTable f a),
    eccChoices :: !(IntMap (Maybe (BestChoice f cost)))
  }
...
data ExtractionCacheObstruction
  ...
  | ExtractionCacheDidNotConverge !ExtractionConvergenceReport
  ...
  case stableExtractionSnapshotFromEGraph graph of
    ...
    Just snapshot ->
      let table =
       ...
       in case worklistChoices budget costAlgebraValue table IntMap.empty SeedAllClasses of
            Left report ->
              Left (ExtractionCacheDidNotConverge report)
            Right choices ->
              Right
                ExtractionChoiceCache
...
advanceExtractionChoiceCache ::
  ...
  Either ExtractionCacheObstruction (ExtractionChoiceCache f a cost)
advanceExtractionChoiceCache budget costAlgebraValue traceValue graph cacheValue
      ...
      case stableExtractionSnapshotFromEGraph graph of
        ...
        Just snapshot ->
          let table =
           ...
           in case coneAdvanceChoices budget costAlgebraValue table parentIndex (mutationTraceDirtyKeys traceValue) (eccChoices cacheValue) of
                Left report ->
                  Left (ExtractionCacheDidNotConverge report)
                Right choices ->
                  Right
...
coneAdvanceChoices budget costAlgebraValue table parentIndex dirtyKeys previousChoices =
  worklistChoices budget costAlgebraValue table boundary SeedAllClasses
  where
    cone :: IntSet
    cone =
...
extractionChoiceCacheSection cacheValue =
  ExtractionChoiceSection
    { ecsTable = eccTable cacheValue,
      ecsChoices = eccChoices cacheValue
    }
