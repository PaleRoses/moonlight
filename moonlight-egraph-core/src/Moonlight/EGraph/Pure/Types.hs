...
-- | Cost-explicit diagnostic projection of every class. Point queries must use
-- 'lookupEClass' so an observation of one class does not materialize the graph.
materializeEGraphClasses :: Language f => EGraph f a -> IntMap (EClass f a)
...
-- | Cost-explicit diagnostic projection of the complete hash-cons relation.
materializeEGraphHashCons :: Language f => EGraph f a -> Map (ENode f) ClassId
materializeEGraphHashCons graph =
...
lookupEClass graph classId =
  let canonicalKey =
   ...
   in fmap
        (\analysisValue ->
           EClass
             { eClassId = ClassId canonicalKey,
               eClassNodes = Set.fromList (structuralTuplesForResultKey canonicalKey store),
               eClassData = analysisValue,
               eClassParents = materializeParentsAt canonicalKey store
             }
        )
        (IntMap.lookup canonicalKey (egAnalysis graph))

materializeParentsAt :: Language f => Int -> StructuralStore f -> [(ClassId, ENode f)]
materializeParentsAt childKey store =
  [ (ClassId parentKey, enode)
    | parentKey <-
        IntSet.toAscList
          (structuralParentKeysOf store (IntSet.singleton childKey)),
      enode@(ENode children) <- structuralTuplesForResultKey parentKey store,
      any ((== childKey) . classIdKey) children
  ]
