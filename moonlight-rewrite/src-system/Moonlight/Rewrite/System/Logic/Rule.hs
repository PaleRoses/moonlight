...
import Moonlight.Core (ClassId, Pattern, PatternVar, patternVariables)
...
data CompiledFactRule capability f = CompiledFactRule
  { cfrId :: FactRuleId,
    cfrName :: String,
    cfrQuery :: PatternQuery (RewriteCondition capability f) f,
    cfrCompiledQuery :: CompiledPatternQuery (CompiledGuard capability f) f,
    cfrProjection :: [GuardRef],
    cfrFactId :: FactId
  }
...
selectFactDerivations factStore derivationIndex =
  foldMap
    (\factWitness ->
       FactDerivationIndex
         (Map.singleton factWitness (lookupFactDerivations factWitness derivationIndex))
    )
    (factWitnesses factStore)
        ...
        then
          fmap
            (\query ->
               CompiledFactRule
                 { cfrId = frId factRule,
                   cfrName = frName factRule,
                   cfrQuery = patternQuery,
                   cfrCompiledQuery = query,
                   cfrProjection = frProjection factRule,
                   cfrFactId = frFactId factRule
                 }
            )
            compiledQuery
