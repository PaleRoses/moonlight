...
data GuardEvidence = GuardEvidence
  { geClauses :: ![GuardClauseEvidence],
    geFactWitnesses :: !(Set FactWitness)
  }
  deriving stock (Eq, Ord, Show, Read)
...
guardEvidenceFromClauses clauseEvidences =
  GuardEvidence
    { geClauses = clauseEvidences,
      geFactWitnesses = foldMap guardClauseFactWitnesses clauseEvidences
    }
  where
...
insertFacts factId factTuples (FactStore storeEntries) =
