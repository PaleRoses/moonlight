...
import Moonlight.Rewrite.System.Variable (RuleVariables, untypedRuleVariables)
...
rule :: Foldable f => RuleName -> Pattern f -> Pattern f -> RuleSpec capability f
rule =
  ruleWithMaybeId Nothing

ruleWithId :: Foldable f => RewriteRuleId -> RuleName -> Pattern f -> Pattern f -> RuleSpec capability f
ruleWithId =
  ruleWithMaybeId . Just

ruleWithMaybeId :: Foldable f => Maybe RewriteRuleId -> RuleName -> Pattern f -> Pattern f -> RuleSpec capability f
ruleWithMaybeId maybeRewriteRuleId name leftPattern rightPattern =
  RuleSpec
    { rsId = maybeRewriteRuleId,
      rsName = name,
      rsVariables = untypedRuleVariables (patternVariables leftPattern),
      rsLhs = leftPattern,
      rsRhs = rightPattern,
      rsCondition = Nothing,
...
withRuleVariables :: RuleVariables -> RuleSpec capability f -> RuleSpec capability f
withRuleVariables variables ruleValue =
  ruleValue {rsVariables = variables}
