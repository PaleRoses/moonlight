...
data TestHostFailure = TestHostFailure
...
pbpoEngineTests =
  testGroup
    ...
    [ testCase "rule constructor rejects mismatched legs" testRuleEndpointMismatch,
      ...
      testCase "unit admission accepts admitted hosts and rejects crafted violations" testAdmitPBPOUnitAdmissibilityLaw,
      ...
      testCase "unit admission runs the real step and preserves admission semantics" testAdmitPBPOUnit
    ]
...
testRuleSquareDisagreement = do
  contextComposite <-
    assertRight (composeMor RuleTestCat (plContextLeg ruleTestLegs) (plInterfaceTyping ruleTestLegs))
  leftComposite <-
    assertRight (composeMor RuleTestCat (plLeftTyping ruleTestLegs) (plLeftLeg ruleTestLegs))
...
testIdentityTypedRule = do
  ...
  _rule <-
    assertRight (identityTypedRule RefCat ("identity-typed" :: String) leftLeg rightLeg)
  pure ()

testIdentityTypedRuleActsAsIdentity :: Assertion
testIdentityTypedRuleActsAsIdentity = do
  identityLeg <- refIncl [1, 2, 3] [1, 2, 3]
  rule <- assertRight (identityTypedRule RefCat ("identity-action" :: String) identityLeg identityLeg)
  monicMatch <-
    maybe (assertFailure "expected identity match to be monic") pure (witnessMonic RefCat identityLeg)
  untypedStep <- assertRight (applyPBPO RefCat rule monicMatch)
  typedStep <-
    assertRight
      ( applyPBPOPlus
          RefCat
...
testUntypedStep = do
  leftLeg <- refIncl [1] [1, 2]
  rightLeg <- refIncl [1] [1, 4]
  rule <- assertRight (identityTypedRule RefCat ("delete-2-add-4" :: String) leftLeg rightLeg)
  matchArrow <- refIncl [1, 2] [1, 2, 3]
  monicMatch <-
    maybe (assertFailure "expected monic match witness") pure (witnessMonic RefCat matchArrow)
  step <- assertRight (applyPBPO RefCat rule monicMatch)
...
testTypedStepPermissive = do
  ...
  match <- typedRefMatch
  step <- assertRight (applyTyped match)
...
testAdmitPBPOUnit :: Assertion
testAdmitPBPOUnit = do
  case admitPBPOUnit TestHostFailure ("unit-admission" :: String) (41 :: Int) Right of
    Right admittedHost ->
      admittedHost @?= 41
    Left _ ->
      assertFailure "expected unit admission to accept the host"
  case admitPBPOUnit TestHostFailure ("unit-admission" :: String) (41 :: Int) (const (Left TestHostFailure)) of
    Left TestHostFailure ->
      pure ()
    Right _ ->
      assertFailure "expected unit admission to propagate the rejection"

testAdmitPBPOUnitAdmissibilityLaw :: Assertion
testAdmitPBPOUnitAdmissibilityLaw = do
  let admitLicensedHost hostValue =
        if hostValue == ("licensed" :: String)
          then Right hostValue
          else Left TestHostFailure
  admitPBPOUnit TestHostFailure ("unit-law" :: String) "licensed" admitLicensedHost
    @?= Right "licensed"
  admitPBPOUnit TestHostFailure ("unit-law" :: String) "unlicensed" admitLicensedHost
    @?= Left TestHostFailure
...
stepTestRule =
  assertRight (mkPBPORule (StepTestCat StepPermit) ("step-law" :: String) stepTestLegs)
...
testTermCatPushoutAgreesWithUnifier = do
  ...
  unifierWitness <-
    assertRight (unifyPatterns leftPattern rightPattern)
...
compositionLawTests =
  testGroup
    ...
    [ testCase "left identity" testCompositionLeftIdentity,
      ...
      testCase "invalid composed interfaces are reported explicitly" testCompositionInvalidInterfaceFailure,
...
testCompositionAssociativeModuloCanonicalization = do
  ...
  rs <- assertRight (composeRewriteOnly rewriteR rewriteS)
  leftAssociated <- assertRight (composeRewriteOnly rs rewriteT)

  st <- assertRight (composeRewriteOnly rewriteS rewriteT)
  rightAssociated <- assertRight (composeRewriteOnly rewriteR st)
...
testCompositionInvalidInterfaceFailure :: Assertion
testCompositionInvalidInterfaceFailure = do
  leftRewrite <-
    assertRight
      ( mkPatternRewrite
          (RewriteAtomic "left")
  ...
  rightRewrite <-
    assertRight
      ( mkPatternRewrite
          (RewriteAtomic "right")
          (patternVar 2)
          (Set.singleton (EGraph.mkPatternVar 2))
          (patternVar 2)
          UnitDecoration
      )
  let forgedLeftRewrite =
        leftRewrite
          { prInterface = mkPatternInterface (Set.singleton (EGraph.mkPatternVar 1))
          }
  case composePatternRewrites forgedLeftRewrite rightRewrite of
    Left (InvalidComposedInterface (PatternInterfaceLegNotContained [_])) ->
      pure ()
    Left otherError ->
      assertFailure ("expected invalid composed interface, got " <> show otherError)
    Right _ ->
      assertFailure "expected invalid composed interface failure"
...
testDecorationProjectionBeforeComposition = do
  ...
  compositionResult <-
    assertRight (composePatternRewrites leftRewrite rightRewrite)
...
assertRight =
  either (assertFailure . ("expected Right, got Left " <>) . show) pure
