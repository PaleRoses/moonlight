    ...
    [ testCase "exposes a closed primitive magic vocabulary" closedPrimitiveVocabulary,
      ...
      testGroup
        ...
        [ testCase "scorches an unwarded target" unwardedFireballScorchesTarget,
          ...
          testCase "rejects a stale match after host replacement" staleMatchRejectedAfterHostReplacement,
          testCase "revalidates a stale match that still exists after host growth" staleMatchRevalidatedAfterHostGrowth,
...
closedPrimitiveVocabulary = do
  ...
  fmap vocabularyLabelText (fmap atomKindVocabularyLabel allAtomKinds)
    @?= [ "Fire",
          "Oil",
          "Ice",
          "Cold",
  ...
  traverse decodeAtomKind (fmap atomKindVocabularyId allAtomKinds)
    @?= Right allAtomKinds

  traverse decodeOutcomeKind (fmap outcomeKindVocabularyId allOutcomeKinds)
    @?= Right allOutcomeKinds

  traverse decodeEntityKind (fmap entityKindVocabularyId allEntityKinds)
    @?= Right allEntityKinds

  traverse decodeStatusKind (fmap statusKindVocabularyId allStatusKinds)
    @?= Right allStatusKinds

  traverse decodeComboKind (fmap comboKindVocabularyId allComboKinds)
    @?= Right allComboKinds

  traverse decodeAtlasContextKind (fmap atlasContextKindVocabularyId allAtlasContextKinds)
...
staleMatchRejectedAfterHostReplacement = do
  ...
  (replacementHost, _roots) <-
    expectRightHost (hostFromTerms 17 [])
  ...
  let staleMatch =
      ...
      replacementRevision =
        hostRevision (engineHost engineWithReplacement)

  (_engineAfterApply, applyResult) <-
    expectRightProgram
      (apply defaultApplyConfig staleMatch engineWithReplacement)

  applyResultStatus applyResult
    @?= ApplyRejected (RejectedStaleMatch (matchRevision staleMatch) replacementRevision)
...
staleMatchRevalidatedAfterHostGrowth = do
  ...
  (grownHost, _roots) <-
    expectRightHost
      (hostFromTerms 23 (fmap HostTerm [fireballAt goblin, oil]))

  let engineWithGrowth =
        replaceHost grownHost (sfcEngine staleCase)
...
prepareStaleFireballCase initialTerms = do
  ...
  (host0, _roots) <-
    expectRightHost (hostFromTerms 0 (fmap HostTerm initialTerms))
...
newtype AtlasDiscoveryId = AtlasDiscoveryId
  { atlasDiscoveryIdText :: String
  }
  deriving stock (Eq, Show)
...
newtype ArchetypeMinerId = ArchetypeMinerId
  { archetypeMinerIdText :: String
  }
  deriving stock (Show)
...
data AtlasBuildError
  = AtlasMissingWorld !AtlasWorld
  | AtlasMissingSourceWitness !AtlasDiscovery
    ...
    { archetypeMinerId = contextPreparedCatalystMinerId,
      runArchetypeMiner =
        \config _outcomes _discovery legalWorlds ->
          if not (amcBaseWorld config `elem` legalWorlds)
...
fuseDepth (Fix (Node sigNode)) =
  ...
  where
    childFuseDepth =
      foldr max 0 (fmap fuseDepth (nodeChildren sigNode))
...
expectRightAtlas :: Either AtlasBuildError a -> IO a
expectRightAtlas =
  \case
    Left atlasError ->
      assertFailure (show atlasError)
