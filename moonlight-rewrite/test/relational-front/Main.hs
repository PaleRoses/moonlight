  ...
  defaultMain
    ( testGroup
        ...
        [ termHostMatchesNamedBindings,
          ...
          contextRemovalAndReinstallRotatesGeneration,
          variableBearingMacrosCompileAndMatchRelationally,
...
termHostMatchesNamedBindings =
  testCase "hostFromTerm builds a host and match returns named sorted bindings" $ do
    (host, rootKey) <-
      expectRight (hostFromTerm (wrap litA))
    ...
    (_engine1, matches) <-
      expectRight (matchBaseRule unwrapRuleName Nothing engine0)
...
twoLiteralNodeHost :: Either HostBuildError (Host TinySig)
twoLiteralNodeHost =
  hostFromNodeClasses
    ( IntMap.singleton
        0
...
nodeClassesSupportMultipleNodesPerKey =
  testCase "hostFromNodeClasses supports multiple nodes per key" $ do
    host <-
      expectRight twoLiteralNodeHost
    ...
    isBRuleName <-
      expectRuleName "is-b"
    (engine1, aMatches) <-
      expectRight (matchBaseRule isARuleName Nothing engine0)
    (_engine2, bMatches) <-
      expectRight (matchBaseRule isBRuleName Nothing engine1)
...
saturationNodeBudgetCountsNodesNotClasses =
  testCase "saturation node budget counts nodes in multi-node classes" $ do
    ...
    (_engine1, result) <-
      expectRight (saturate RewriteBase config engine0)
...
emptyHostProgramIsIdentity =
  testCase "empty host rewrite program is the identity on the public host face" $ do
    (host, _rootKey) <-
      expectRight (hostFromTerm (wrap (wrap litA)))
    ...
    HostProgramResult
      ...
      expectRight (runHostRewriteProgram emptyHostRewriteProgram host)
...
saturationIsIdempotentAtFace =
  testCase "saturating an already saturated public host makes no further host change" $ do
    ...
    (engine1, saturatedOnce) <-
      expectRight (saturate RewriteBase (defaultSaturationConfig :: SaturationConfig TinySig) engine0)
    (engine2, saturatedTwice) <-
      expectRight (saturate RewriteBase (defaultSaturationConfig :: SaturationConfig TinySig) engine1)
...
hostProgramExecutionIsDeterministic =
  testCase "same host rewrite program on the same public host is deterministic" $ do
    (host, rootKey) <-
      expectRight (hostFromTerm litA)

    firstResult <-
      expectRight (runHostRewriteProgram (deterministicHostProgram rootKey) host)
    secondResult <-
      expectRight (runHostRewriteProgram (deterministicHostProgram rootKey) host)
...
extractionReturnsTypedCostedRepresentative =
  testCase "extract returns a typed costed representative for a host class" $ do
    (host, rootKey) <-
      expectRight (hostFromTerm (wrap litB))
...
extractionExhaustionPreservesConvergenceReport =
  testCase "extract preserves every coverage count when its work budget is exhausted" $ do
    (host, rootClass) <-
      expectRight (hostFromTerm litA)
    case
      extractWith
        ExtractConfig {extractWorkLimit = ExtractMaxWorkSteps 0}
        (sortWitness @"Expr")
        tinyCost
        rootClass
        host ::
      Either ExtractError (Extracted TinySig Int "Expr") of
      Left (ExtractWorkBudgetExhausted budgetExhaustion) -> do
        ebeBudget budgetExhaustion @?= ExtractionWorkBudget 0
        ebeConsumedWorkSteps budgetExhaustion @?= 0
        ebeTotalClassCount budgetExhaustion @?= 1
        ebeResolvedClassCount budgetExhaustion @?= 0
        ebeUnresolvedClassCount budgetExhaustion @?= 1

      Left otherError ->
        assertFailure ("expected work-budget report, got " <> show otherError)
...
extractionReportsTypedObstructions =
  testCase "extract and host construction report typed sort/finite-representative obstructions" $ do
    case hostFromNodeClasses (IntMap.singleton 0 [] :: IntMap.IntMap [Node TinySig Int]) of
    ...
    case hostFromNodeClasses (IntMap.singleton 0 [Node LitA, Node LitFlag]) of
    ...
    flagHost <-
      expectRight (hostFromNodeClasses (IntMap.singleton 0 [Node LitFlag]))
    flagRoot <-
    ...
    cycleHost <-
      expectRight (hostFromNodeClasses (IntMap.singleton 0 [Node (Wrap (K 0))]))
    cycleRoot <-
      expectHostClassId cycleHost 0
    case
      extractWith
        ExtractConfig {extractWorkLimit = ExtractMaxWorkSteps 4}
        (sortWitness @"Expr")
        tinyCost
        cycleRoot
        cycleHost ::
...
hostProgramCanonicalizationLaws =
  testCase "host rebuild barrier follows observable egraph quotient laws" $ do
    ...
    missingHost <-
      expectRight (hostFromNodeClasses (IntMap.singleton 99 [Node LitA]))
    missingClass <- expectHostClassId missingHost 99
    HostProgramResult {hprHost = mergedHost} <-
      expectRight (runHostRewriteProgram (mergeClasses class0 class2) host)
    ...
    HostRebuildResult {hrrHost = rebuiltHost} <-
      expectRight (rebuildHostBarrier mergedHost)
    ...
    HostProgramResult {hprHost = idempotentHost} <-
      expectRight (runHostRewriteProgram (mergeClasses class1 class3) rebuiltHost)
    ...
    litAName <-
      expectRuleName "is-a"
    (engine1, aMatches) <-
      expectRight (matchBaseRule litAName Nothing engine0)
    litBName <-
      expectRuleName "is-b"
    (_engine2, bMatches) <-
      expectRight (matchBaseRule litBName Nothing engine1)
...
hostTermVariablesAreRejected =
  testCase "host terms reject variables as typed host build errors" $
    case hostFromTerm (var (symbolToken @"x") (symbolToken @"Expr") :: Term TinySig "Expr") of
...
existsQueriesDoNotNeedMatchKeys =
  testCase "exists and existsAtRoot work without public match keys" $ do
    ...
    unwrapRuleName <-
      expectRuleName "unwrap"
    (engine1, anyExists) <-
      expectRight (existsBaseRule unwrapRuleName Nothing engine0)
    anyExists @?= True

    (engine2, rootExists) <-
      expectRight (existsBaseRule unwrapRuleName (Just rootKey) engine1)
    rootExists @?= True

    (engine3, matches) <-
      expectRight (matchBaseRule unwrapRuleName Nothing engine2)

    case matches >>= (Map.elems . matchBindings) of
      [childKey] -> do
        (_engine4, childExists) <-
          expectRight (existsBaseRule unwrapRuleName (Just childKey) engine3)
        childExists @?= False
...
contextualRuntimeSupportsRepeatedContextUpdates =
  testCase "engine matches repeated changing contextual state without user-built backend state" $ do
    ...
    (engine2, contextAHasA) <-
      expectRight (existsContextRule contextA ctxWrapARuleName Nothing engine1)
    contextAHasA @?= True

    (engine3, contextAHasB0) <-
      expectRight (existsContextRule contextA ctxWrapBRuleName Nothing engine2)
    contextAHasB0 @?= False

    (engine4, contextBHasB0) <-
      expectRight (existsContextRule contextB ctxWrapBRuleName (Just contextBRoot0) engine3)
    contextBHasB0 @?= True
    ...
    (engine6, contextAHasA1) <-
      expectRight (existsContextRule contextA ctxWrapARuleName (Just contextARoot0) engine5)
    contextAHasA1 @?= False

    (engine7, contextAHasB1) <-
      expectRight (existsContextRule contextA ctxWrapBRuleName (Just contextARoot1) engine6)
    contextAHasB1 @?= True

    (_engine8, contextBStillHasB) <-
      expectRight (existsContextRule contextB ctxWrapBRuleName (Just contextBRoot0) engine7)
    contextBStillHasB @?= True

contextRemovalAndReinstallRotatesGeneration :: TestTree
contextRemovalAndReinstallRotatesGeneration =
  testCase "removing and reinstalling a context invalidates matches even at the same host revision" $ do
    (contextHost, _contextRoot) <-
      expectRight (hostFromTerm (wrap litA))
    ...
    let engineWithContexts =
          setContext contextB contextHost (setContext contextA contextHost engine0)

    (engineAfterMatch, matches) <-
      expectRight
        ( match
            MatchQuery
              { matchQueryTarget = RewriteContext contextA,
                matchQueryRule = ctxWrapARuleName,
    ...
    staleMatch <-
      case matches of
        ...
        [] ->
          assertFailure "expected a contextual match before removal"

    let reinstalledEngine =
          setContext contextA contextHost (removeContext contextA engineAfterMatch)

    (_engineAfterApply, applyResult) <-
      expectRight (apply defaultApplyConfig staleMatch reinstalledEngine)

    case applyResultStatus applyResult of
      ApplyRejected (RejectedTargetGeneration observedGeneration installedGeneration) -> do
        observedGeneration @?= matchGeneration staleMatch
        assertBool
          "context reinstall must allocate a fresh generation"
          (installedGeneration /= observedGeneration)

      otherStatus ->
        assertFailure ("expected replaced-context rejection, got " <> show otherStatus)

variableBearingMacrosCompileAndMatchRelationally :: TestTree
variableBearingMacrosCompileAndMatchRelationally =
  testCase "variable-bearing and nested macros retain names and sorts through relational compilation" $ do
    (host, _rootClass) <-
      expectRight (hostFromTerm (wrap litA))

    engine0 <-
      prepareProgram variableMacroProgram host

    unwrapWrapName <-
      expectRuleName "unwrap-wrap"
    (engine1, firstMacroMatches) <-
      expectRight (matchBaseRule unwrapWrapName Nothing engine0)
    assertNamedExprBinding firstMacroMatches

    nestedName <-
      expectRuleName "nested-unwrap"
    (_engine2, nestedMatches) <-
      expectRight (matchBaseRule nestedName Nothing engine1)
    assertNamedExprBinding nestedMatches
  where
    assertNamedExprBinding matches =
      case matches of
        [matchValue] ->
          case Map.keys (matchBindings matchValue) of
            [matchVariable] -> do
              matchVarName matchVariable @?= "x"
              fmap sortNameString (matchVarSort matchVariable) @?= Just "Expr"

            variables ->
              assertFailure ("expected one macro variable, got " <> show (length variables))

        observedMatches ->
          assertFailure ("expected one macro match, got " <> show (length observedMatches))
...
variableMacroProgram :: Program TinySig NoGuardAtom
  ...
  program $ do
    ...
    rule
      ...
      ( forall_
          ...
          (var (symbolToken @"y") (symbolToken @"Expr") ==> wrap (var (symbolToken @"y") (symbolToken @"Expr")))
      )
    macro "unwrap-wrap" (compose "unwrap-x" >>> compose "wrap-y")
    macro "nested-unwrap" (compose "unwrap-wrap" >>> compose "unwrap-x")
...
prepareProgram programValue host = do
  rulesValue <-
    expectRight (compile programValue)
...
existsBaseRule ruleNameValue maybeRoot engineValue =
  fmap (not . null) <$> matchBaseRule ruleNameValue maybeRoot engineValue
...
expectRuleName =
  expectRight . mkRuleName

expectContextName :: String -> IO ContextName
expectContextName =
  expectRight . contextName
