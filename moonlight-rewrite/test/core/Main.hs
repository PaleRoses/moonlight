...
applicationConditionTests =
  testGroup
    ...
    [ testCase "static plan extraction sees all possible atom effects" $ do
        condition <-
          assertCompiledCondition
            (And [Atom testExtension, Or [Atom testExtension]])

        fmap cpePath (compiledApplicationConditionExtensions condition)
          @?= [ApplicationConditionPath [0], ApplicationConditionPath [1, 0]],
  ...
  where
    assertShortCircuit label mkCondition firstDecision secondDecision =
      testCase label $ do
        ...
        condition <-
          assertCompiledCondition
            (mkCondition [Atom testExtension, Atom testExtension])
...
testExtension :: PatternExtension () TestNode
testExtension =
  globalPatternExtension (singlePatternQuery testPattern)

assertCompiledCondition ::
  ConstraintExpr (PatternExtension () TestNode) ->
  IO (CompiledApplicationCondition () TestNode)
assertCompiledCondition expression =
  case
    compileApplicationCondition
      combineUnitGuards
      (\_ () -> Right ())
      Set.empty
      (ApplicationCondition expression)
    of
    Left compileError ->
      assertFailure (show (compileError :: ApplicationConditionCompileError))
    Right condition ->
      pure condition

combineUnitGuards :: [()] -> Maybe ()
combineUnitGuards guards =
  case guards of
    [] -> Nothing
    _ : _ -> Just ()
