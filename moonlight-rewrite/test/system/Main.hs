...
import Moonlight.Core (Substitution, emptySubstitution)
...
import Moonlight.Rewrite.System (checkedRuleNames)
import Moonlight.Rewrite.System
...
import Moonlight.Rewrite.System (RuleName, RuleNameError (..), mkRuleName, ruleNameString)
import Moonlight.Rewrite.System (RuleSet, ruleSet, ruleWithId)
...
naiveUnsatisfiedAtom :: NaiveAtomAssessment
naiveUnsatisfiedAtom =
  NaiveAtomAssessment
    { naaSatisfied = False,
      naaPositiveEvidence = [],
...
assertGuardCanonicalWordsInvariances = do
  ...
  combinedForward <- assertJust "two compiled guards combine" (combineCompiledGuards [factSeven, factNine])
  combinedBackward <- assertJust "two compiled guards combine" (combineCompiledGuards [factNine, factSeven])
  assertEqual
...
assertJust :: String -> Maybe value -> IO value
assertJust label =
  maybe (fail (label <> " expected Just, got Nothing")) pure
...
assertDuplicateRuleIdsReject = do
  firstName <- assertRight (mkRuleName "first")
  secondName <- assertRight (mkRuleName "second")
...
assertMissingRuleReportsTypedError = do
  knownName <- assertRight (mkRuleName "known")
  missingName <- assertRight (mkRuleName "missing")
  checkedSystem <- assertRight (checkRuleSet (singleRuleSet knownName))
...
assertAliasSeededDerivationsProduceNoPhantomRounds = do
  compiledRule <-
    assertRight (compileFactRule (mkClosureRule 1 [] Nothing (FactId 51)))
  (_matchCalls, closure) <-
    assertRight
      ( gatedClosureWith
          aliasCanonicalClass
...
assertRight :: Show err => Either err value -> IO value
assertRight value =
  case value of
