...
import Moonlight.Core (ClassId, GuideEvidence, RewriteRuleId)
...
contextualSupportModelRewritesAt contextLatticeValue contextValue =
  fmap (fmap srRewrite)
    . filterM
      (\supportedRewrite -> supportContains contextLatticeValue (srSupport supportedRewrite) contextValue)
    . unContextualSupportModel
...
supportedFactFamilyRulesAt contextLatticeValue contextValue =
  fmap (fmap sfrRule)
    . filterM
      (\supportedRule -> supportContains contextLatticeValue (sfrSupport supportedRule) contextValue)
    . unSupportedFactFamily
...
supportedFactStoreAt contextLatticeValue contextValue =
  fmap
    ( foldr
        (\(factWitness, _support) -> insertFact (fwFactId factWitness) (fwTuple factWitness))
        emptyFactStore
    )
    . filterM
      (\(_factWitness, supportValue) -> supportContains contextLatticeValue supportValue contextValue)
    . Map.toAscList
...
supportedFactDerivationIndexAt contextLatticeValue contextValue =
  fmap
    (foldMap (singletonFactDerivationIndex . fst))
    . filterM
      (\(_derivation, supportValue) -> supportContains contextLatticeValue supportValue contextValue)
    . Map.toAscList
