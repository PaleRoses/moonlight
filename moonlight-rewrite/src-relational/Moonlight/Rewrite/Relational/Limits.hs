...
-- | Typed metric and limit record for relational rewrite runs.
-- Owns the closed metric vocabulary, indexed 'Limit'/'Count' rows, and
-- first-exceeded checking for match, support, proof, and saturation stats.
-- Contracts: @Nothing@ is unbounded, defaults are protective bounds, and the
-- overflow sentinel asks Flow for one row past a finite result limit.
...
data RewriteRunMetrics value = RewriteRunMetrics
  { rrmResultRows :: !(value 'ResultRows),
    ...
    rrmProofSteps :: !(value 'ProofSteps),
  ...
  bmap transform metrics =
    RewriteRunMetrics
      { rrmResultRows = transform (rrmResultRows metrics),
        ...
        rrmProofSteps = transform (rrmProofSteps metrics),
...
instance TraversableB RewriteRunMetrics where
  btraverse transform metrics =
    RewriteRunMetrics
      ...
      <*> transform (rrmProofSteps metrics)
  ...
  bprod leftMetrics rightMetrics =
    RewriteRunMetrics
      { rrmResultRows = Pair (rrmResultRows leftMetrics) (rrmResultRows rightMetrics),
        ...
        rrmProofSteps = Pair (rrmProofSteps leftMetrics) (rrmProofSteps rightMetrics),
...
defaultRewriteRunLimits =
  RewriteRunMetrics
    { rrmResultRows = Limit (Just 1000000),
      ...
      rrmProofSteps = Limit (Just 100000),
...
statsForProofSteps :: Natural -> RewriteRunStats
statsForProofSteps proofSteps =
  emptyRewriteRunStats
    { rrmProofSteps = Count proofSteps
    }
...
rewriteRunLimitChecks =
  RewriteRunMetrics
    { rrmResultRows = RewriteRunLimitCheck (exceededNatural MaxResultRowsExceeded),
      ...
      rrmProofSteps = RewriteRunLimitCheck (exceededNatural MaxProofStepsExceeded),
