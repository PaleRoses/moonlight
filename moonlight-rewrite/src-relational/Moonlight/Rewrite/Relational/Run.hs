...
-- | Stateful relational execution boundary for prepared rewrite systems.
-- Owns prepared host caches, context snapshots, support checks, restrictions,
-- pending patch deltas, telemetry, result stats, and run-limit failures.
-- Contracts: base/context support is enforced before execution; host repairs
-- patch or evict prepared caches, then limits check projected result stats.
...
composeAtomDeltas newer older =
  normalizeAtomDeltas (IntMap.unionWith composePlainRowPatch newer older)
