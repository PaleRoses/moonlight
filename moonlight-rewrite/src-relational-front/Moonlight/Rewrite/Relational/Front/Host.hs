...
hostClassWitness ::
  ...
  Host sig ->
  Maybe (Fix (Node sig))
hostClassWitness fuel classId host =
  ...
  where
    witnessClass remaining currentClass
      ...
      | otherwise = do
          ...
          Fix <$> traverse (witnessClass (remaining - 1)) witnessNode
...
hostFromTerm revision termValue =
  case runStateT (internTerm termValue) emptyHostInternState of
    Left hostError ->
      Left hostError

    Right (rootKey, internState) ->
      Right (hostFromInternState revision internState, rootKey)
...
hostFromNodes revision nodes =
  hostFromNodeClasses revision (fmap (: []) nodes)
...
hostFromNodeClasses ::
  ...
  IntMap [Node sig Int] ->
  Either HostBuildError (Host sig)
hostFromNodeClasses revision nodeClasses = do
  let knownKeys =
...
hostFromInternState ::
  ...
  HostInternState sig ->
  Host sig
hostFromInternState revision internState =
  materializeHost
    revision
...
hostMergeClasses leftClass rightClass host = do
  ...
  if leftRoot == rightRoot
    then Right (host, leftRoot, False)
      ...
      mergedRoot <-
        requireHostClass host' leftRoot
