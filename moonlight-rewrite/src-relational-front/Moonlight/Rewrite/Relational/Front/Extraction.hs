...
data ExtractWorkLimit
  = ExtractAutoWork
  | ExtractMaxWorkSteps !Natural
  deriving stock (Eq, Ord, Show, Read)

data ExtractConfig = ExtractConfig
  { extractWorkLimit :: !ExtractWorkLimit
  }
  deriving stock (Eq, Ord, Show, Read)
...
defaultExtractConfig =
  ExtractConfig
    { extractWorkLimit = ExtractAutoWork
    }
...
data ExtractError
  ...
  | ExtractWorkBudgetExhausted !ExtractionBudgetExhaustion
...
extractSomeWith config costAlgebra classId host = do
  ...
  let workLimit =
        effectiveExtractWorkLimit config host
      extractionBudget =
        ExtractionWorkBudget workLimit
  extractionResult <-
    case extractFromTableBounded
      ...
      Left budgetExhaustion ->
        Left (ExtractWorkBudgetExhausted budgetExhaustion)
...
effectiveExtractWorkLimit :: ExtractConfig -> Host sig -> Natural
effectiveExtractWorkLimit config host =
  case extractWorkLimit config of
    ExtractAutoWork ->
      -- Allow one class-finalization phase plus one whole-table improvement
      -- phase; the host topology, not a decorative magic number, sets the
      -- default.
      fromIntegral (2 * hostClassCount host + 1)

    ExtractMaxWorkSteps maxWorkSteps ->
      maxWorkSteps
