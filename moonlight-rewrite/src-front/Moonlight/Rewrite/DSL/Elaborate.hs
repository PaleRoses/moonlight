...
elaborateRuleDecl ruleDecl = do
  ...
  let variables =
        ruleVariablesFromState finalState
  pure
    ElaboratedRule
      { elaboratedRuleName = ruleNameValue,
        elaboratedRuleCallSite = rdCallSite ruleDecl,
        elaboratedRuleSpec = Rewrite.withRuleVariables variables ruleSpec,
...
ruleVariablesFromState :: RuleElabState -> Rewrite.RuleVariables
ruleVariablesFromState state =
  Rewrite.typedRuleVariables
    ( Map.fromList
        [ ( bbPatternVar binding,
            ( someTypedVarName (bbTypedVar binding),
              someTypedVarSort (bbTypedVar binding)
            )
          )
