  ...
  bgroup
    ...
    [ bgroup "host" (benchSizes >>= hostBenchmarksForSize),
      env publicFixture $ \fixture ->
        bgroup
          "pipeline"
          ( pipelineBenchmarks "" fixture
              <> proofRetentionBenchmarks fixture
              <> [bench "extract" (nf extractWeight fixture)]
          ),
...
publicBenchmarkPreflight = do
  ...
  sequence_
    [ pipelinePreflight "public" publicPipelineFixture,
      proofRetentionPreflight publicPipelineFixture,
...
hostSizePreflight size =
  sequence_
    [ void (expectBench (caseLabel "hostFromTerm" size) (hostFromTerm (nestedTerm size))),
      void (expectBench (caseLabel "hostFromTerms" size) (hostFromTerms (benchTerms size)))
    ]
...
proofRetentionBenchmarks :: PipelineFixture -> [Benchmark]
proofRetentionBenchmarks fixture =
  fmap
    ( \(retentionLabel, retention) ->
        bench
          ("saturate/proof-retention/" <> retentionLabel)
          (nf (baseSaturateWeightWith retention) fixture)
    )
    proofRetentionCases

proofRetentionPreflight :: PipelineFixture -> IO ()
proofRetentionPreflight fixture = do
  semanticWeights <-
    traverse
      ( \(retentionLabel, retention) ->
          expectMaybeBench
            ("public saturation proof retention " <> retentionLabel)
            (baseSaturateWeightWith retention fixture)
      )
      proofRetentionCases
  case semanticWeights of
    [] ->
      fail "proof retention benchmark registry is empty"
    referenceWeight : remainingWeights ->
      if all (== referenceWeight) remainingWeights
        then pure ()
        else fail "proof retention modes changed the saturation semantic digest"

proofRetentionCases :: [(String, ProofRetention)]
proofRetentionCases =
  [ ("none", KeepNoProof),
    ("summary", KeepProofSummary),
    ("recent-8", KeepRecentProofSteps 8),
    ("full", KeepFullProof)
  ]
...
publicFixture =
  pipelineFixture "public" benchProgram (hostFromTerms (benchTerms 16)) "unwrap"
...
applicationConditionFixture =
  pipelineFixture
    ...
    (fmap (\(host, root) -> (host, [root])) (hostFromTerm (wrap (leaf 0))))
    "requires.leaf"
...
contextFixture = do
  (host, _roots) <- expectBench "context hostFromTerms" (hostFromTerms (benchTerms 16))
  (updatedHost, _updatedRoot) <- expectBench "context updated hostFromTerm" (hostFromTerm (wrap (leaf 0)))
...
hostFromTermWeight size =
  eitherWeight (hostClassCount . fst) (hostFromTerm (nestedTerm size))

hostFromTermsWeight :: Int -> Maybe Int
hostFromTermsWeight size =
  eitherWeight (\(host, roots) -> hostClassCount host + length roots) (hostFromTerms (benchTerms size))
...
baseSaturateWeightWith :: ProofRetention -> PipelineFixture -> Maybe Int
baseSaturateWeightWith retention fixture =
  saturateWeightWith
    RewriteBase
    defaultSaturationConfig {scProofRetention = retention}
    (pipelineEngine fixture)
...
saturateWeight target engineValue =
  saturateWeightWith target defaultSaturationConfig engineValue

saturateWeightWith ::
  RewriteTarget ->
  SaturationConfig BenchSig ->
  Engine BenchSig NoGuardAtom ->
  Maybe Int
saturateWeightWith target config engineValue =
  eitherWeight saturatedWeight (saturate target config engineValue)
