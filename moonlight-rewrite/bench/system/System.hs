...
data SystemFixture = SystemFixture
  { systemRootClass :: !ClassId,
    ...
    systemCheckedBase :: !(CheckedSystem Capability []),
    systemCheckedAppend :: !(CheckedRewrite Capability []),
...
instance NFData SystemFixture where
  rnf fixture =
    systemRootClass fixture
      ...
      `seq` systemCheckedAppend fixture
  ...
  env systemFixture $ \fixture ->
    bgroup
      ...
      [ bench "semi-naive closure" (nf semiNaiveClosureWeight fixture),
        ...
        bench "checked/append 1024" (nf checkedSystemAppendWeight fixture),
  ...
  systemFixture >>= \fixture ->
    sequence_
      [ void (expectBench "semi-naive closure" (deriveSeededFactClosureWithStateAndConfig (closureRun fixture))),
        ...
        void
          ( expectBench
              "checked system append"
              (appendCheckedRewrite 1025 (systemCheckedAppend fixture) (systemCheckedBase fixture))
          ),
...
systemFixture = do
  (_host, rootClass) <-
    expectBench "hostFromTerm fact root" (hostFromTerm 0 (leaf 0))
  firstName <-
  ...
  checkedBase <-
    expectBench "checked system base" (checkRawRewriteSystem (fmap rawRule [0 .. 1023]))
  appendedSystem <-
    expectBench "checked system appended rule" (checkRawRewriteSystem [rawRule 1024])
  appendedRewrite <-
    expectMaybeBench "checked system appended rewrite" (listToMaybe (checkedRewrites appendedSystem))
  deepGuard <-
    expectBench
      "deep guard"
      ( compileGuard
  ...
  pure
    SystemFixture
      { systemRootClass = rootClass,
        ...
        systemCheckedAppend = appendedRewrite,
...
checkedSystemAppendWeight :: SystemFixture -> Maybe Int
checkedSystemAppendWeight fixture =
  either
    (const Nothing)
