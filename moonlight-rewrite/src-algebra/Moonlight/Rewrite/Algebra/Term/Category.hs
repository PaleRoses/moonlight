...
data TermMor f = TermMor !(Pattern f) !(Pattern f) !(TermSubst f)
...
termMorSource :: TermMor f -> Pattern f
termMorSource (TermMor sourcePattern _targetPattern _substitution) =
  sourcePattern

termMorTarget :: TermMor f -> Pattern f
termMorTarget (TermMor _sourcePattern targetPattern _substitution) =
  targetPattern

termMorSubst :: TermMor f -> TermSubst f
termMorSubst (TermMor _sourcePattern _targetPattern substitution) =
  substitution
...
termMor sourcePattern rawSubst =
  let normalSubst =
   ...
   in TermMor sourcePattern (applyTermSubst normalSubst sourcePattern) normalSubst

termIdentity :: Pattern f -> TermMor f
termIdentity patternValue =
  TermMor patternValue patternValue emptyTermSubst
