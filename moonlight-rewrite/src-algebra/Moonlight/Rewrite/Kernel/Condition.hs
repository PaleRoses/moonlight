...
-- | Application-condition algebra over query extensions.
-- It owns scoped anchors, boolean constraint planning, and selective evaluation
-- effects; compilation rejects unbound guard variables, invalid anchors, and
-- unanchored local extensions before the runtime supplies host-state truth.
...
data CompiledPatternExtension compiledGuard f = CompiledPatternExtension
  { cpePath :: !ApplicationConditionPath,
    cpeQuery :: !(CompiledPatternQuery compiledGuard f),
    cpeAnchorVars :: !(Set PatternVar),
    cpeScope :: !PatternExtensionScope
  }
...
compiledApplicationCondition expression =
  CompiledApplicationCondition
    { compiledApplicationConditionExpression = expression,
      compiledApplicationConditionPlan = applicationConditionPlan expression
    }
...
applicationConditionPlan =
  \case
    ...
    And children ->
      foldr (<&&>) (pure True) (fmap applicationConditionPlan children)

    Or children ->
      foldr (<||>) (pure False) (fmap applicationConditionPlan children)
...
compilePatternExtension combineCompiledGuards compileGuard lhsVariables path extension = do
  ...
  Right
    CompiledPatternExtension
      { cpePath = path,
        cpeQuery = compiledQuery,
        cpeAnchorVars = anchorVars,
        cpeScope = peScope extension
      }
