-- | Query normalization stratum for pattern matching.
-- It owns conjunctive query flattening, canonical pattern ordering and
-- deduplication, primary-pattern selection, and guard compilation under bound variables.
...
data CompiledPatternQuery compiledGuard f = CompiledPatternQuery
  { cpqQuery :: PatternQuery compiledGuard f,
    cpqPrimaryPattern :: Pattern f,
    cpqCondition :: Maybe compiledGuard
  }
  ...
  CompiledPatternQuery
    { cpqQuery =
        maybe
          ...
          condition,
      cpqPrimaryPattern = patternValue,
      cpqCondition = condition
    }
...
mapCompiledPatternQuery mapPatternValue mapGuard compiledQuery =
  CompiledPatternQuery
    { cpqQuery = mapPatternQuery mapPatternValue mapGuard (cpqQuery compiledQuery),
      cpqPrimaryPattern = mapPatternValue (cpqPrimaryPattern compiledQuery),
      cpqCondition = mapGuard <$> cpqCondition compiledQuery
    }
...
compilePatternQueryWithScope combineCompiledGuards compileGuard extraBoundVariables patternQuery =
  let flattenedPatterns@(primaryPattern :| _) =
        canonicalizePatterns (patternQueryPatterns patternQuery)
   ...
   in do
        ...
        pure
          CompiledPatternQuery
            { cpqQuery = compiledQuery,
              cpqPrimaryPattern = primaryPattern,
              cpqCondition = compiledCondition
            }
