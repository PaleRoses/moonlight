    ...
    [ testCase "smart constructor rejects truncation bounds below 2" $ do
      ...
      testCase "backend collapses commutative enode permutations inside one e-class" $
        let graph0 = emptyEGraph analysisSpec
            (_, graph1) = addTerm (numTerm 1) graph0
            (_, graph2) = addTerm (numTerm 2) graph1
            (leftRootClass, graph3) = addTerm (addTermNode (numTerm 1) (numTerm 2)) graph2
            (rightRootClass, graph4) = addTerm (addTermNode (numTerm 2) (numTerm 1)) graph3
            graph5 = rebuild (merge leftRootClass rightRootClass graph4)
            patternValue = PatternNode (Add (PatternVar (EGraph.mkPatternVar 0)) (PatternVar (EGraph.mkPatternVar 1)))
         in do
              parallelBlockRegistry <- expectParallelBlockRegistry (mkParallelBlockRegistry arithParallelBlockDeclarations)
              simplicialBackend <-
