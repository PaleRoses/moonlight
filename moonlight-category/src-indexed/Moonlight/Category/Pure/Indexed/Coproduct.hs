{-# LANGUAGE DerivingStrategies, GeneralizedNewtypeDeriving, TypeFamilies, TypeOperators, UndecidableInstances, GADTs, FlexibleContexts, NoImplicitPrelude #-}
...
module Moonlight.Category.Pure.Indexed.Coproduct where
...
-- | t'Inj1' is a functor which injects into the left category.
...
-- | t'Inj2' is a functor which injects into the right category.
...
-- | t'CodiagCoprod' is the codiagonal functor for coproducts.
...
-- | t'Cotuple1' projects out to the left category, replacing a value from the right category with a fixed object.
...
-- | t'Cotuple2' projects out to the right category, replacing a value from the left category with a fixed object.
...
newtype (c1 :>>: c2) a b = DC (Cograph c1 c2 (Const (Op c1 :**: c2) (->) ()) a b) deriving newtype Category
