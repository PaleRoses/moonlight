...
module Moonlight.Category.Pure.Indexed.KanExtension where
...
import Moonlight.Category.Pure.Indexed.Limit
...
newtype RanHask p (f :: Type) a = RanHask (forall c. Obj (Dom p) c -> Cod p a (p :% c) -> f :% c)
data RanHaskF p (f :: Type) = RanHaskF
...
data LanHask p (f :: Type) a where
  LanHask :: Obj (Dom p) c -> Cod p (p :% c) a -> f :% c -> LanHask p f a
data LanHaskF p (f :: Type) = LanHaskF
