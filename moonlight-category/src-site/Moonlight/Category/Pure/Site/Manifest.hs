      ...
      [0 .. objectCount - 1]
        >>= ( \targetIndex ->
                case objectVector Vector.!? targetIndex of
                  Nothing -> []
                  Just targetObject ->
                    let reachableBits = fromMaybe 0 (reachabilityRows Vector.!? targetIndex)
                        coverBits = fromMaybe 0 (coverRows Vector.!? targetIndex)
      ...
      [0 .. objectCount - 1]
        >>= ( \targetIndex ->
                case objectVector Vector.!? targetIndex of
                  Nothing -> []
                  Just targetObject ->
                    let coverBits = fromMaybe 0 (coverRows Vector.!? targetIndex)
  ...
  bitsToAscList (Vector.length objectVector) coverBits
    >>= ( \coveredIndex ->
            case objectVector Vector.!? coveredIndex of
              Nothing -> []
              Just covered ->
                let coveredCoverBits = fromMaybe 0 (coverRows Vector.!? coveredIndex)
...
rowsUnionForBits objectCount rows bits =
  bitsToAscList objectCount bits
    & foldr (\rowIndex unionBits -> unionBits .|. fromMaybe 0 (rows Vector.!? rowIndex)) 0
