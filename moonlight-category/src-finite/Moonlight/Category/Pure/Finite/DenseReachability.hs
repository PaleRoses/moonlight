  ...
  let vertexCount = Vector.length inputRows
      rows =
        UVector.generate vertexCount $
          integerRowChunkWord vertexCount 0 . integerRowAt inputRows
...
integerRowAt :: Vector Integer -> Int -> Integer
integerRowAt rows index =
  fromMaybe 0 (rows Vector.!? index)
{-# INLINE integerRowAt #-}
  ...
  PackedRows chunkCount $
    UVector.generate (bitCount * chunkCount) $ \flatIndex ->
      let (rowIndex, chunkIndex) = flatIndex `quotRem` chunkCount
       in integerRowChunkWord bitCount chunkIndex (integerRowAt rows rowIndex)
  where
...
writeMutablePackedRowChunk (MutablePackedRows chunkCount chunks) rowIndex chunkIndex word =
  UMVector.write chunks (rowChunkOffset chunkCount rowIndex chunkIndex) word
