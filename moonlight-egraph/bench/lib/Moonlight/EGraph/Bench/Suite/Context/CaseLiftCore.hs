...
caseLiftCoreBench (branchCount, termCount) =
  env (prepareCaseLiftCoreFixture branchCount termCount) $ \fixture ->
    bgroup
...
data CaseLiftCoreFixture = CaseLiftCoreFixture
  { clcfBranchCount :: !Int,
    clcfTermCount :: !Int,
    clcfSite :: !(PreparedContextSite CaseContext),
...
instance NFData CaseLiftCoreFixture where
  rnf fixture =
...
prepareCaseLiftCoreFixture :: Int -> Int -> IO CaseLiftCoreFixture
prepareCaseLiftCoreFixture branchCount termCount = do
  fixture <-
    either
      (ioError . userError . ("case-lift-core fixture failed: " <>))
      pure
      (mkCaseLiftCoreFixture branchCount termCount)
  validatedDigest <-
    either
...
mkCaseLiftCoreFixture :: Int -> Int -> Either String CaseLiftCoreFixture
mkCaseLiftCoreFixture branchCount termCount = do
  site <- first show (fromPowersetAtoms [0 .. branchCount - 1])
...
validateCaseLiftCoreFixture :: CaseLiftCoreFixture -> Either String Int
...
validateIncompleteBranchCoverage :: CaseLiftCoreFixture -> Either String ()
...
validateParentLiftNonVacuity ::
  CaseLiftCoreFixture ->
  ContextEGraph ArithF Depth CaseContext ->
  Either String Int
...
stageAllBranchFacts :: CaseLiftCoreFixture -> Either String (ContextEGraph ArithF Depth CaseContext)
...
stageAllBranchProofs ::
  CaseLiftCoreFixture ->
  ContextEGraph ArithF Depth CaseContext ->
  Either String (ContextEGraph ArithF Depth CaseContext)
...
stageOursParentLift :: CaseLiftCoreFixture -> Either String (ContextEGraph ArithF Depth CaseContext)
...
stageOneProductBranchFact :: CaseLiftCoreFixture -> CaseContext -> Either String Int
...
stageOneProductBranchProof :: CaseLiftCoreFixture -> CaseContext -> Either String Int
...
baseContextGraph :: CaseLiftCoreFixture -> ContextEGraph ArithF Depth CaseContext
baseContextGraph fixture =
  emptyContextEGraphFromSite (clcfSite fixture) (clcfBaseGraph fixture)

data CaseLiftConstruction = CaseLiftConstruction
  { clcBatch :: !(ContextRebaseBatch ArithF Depth CaseContext),
    clcMergePlansReversed :: ![ContextMergePlan CaseContext]
  }

beginCaseLiftConstruction :: ContextEGraph ArithF Depth CaseContext -> CaseLiftConstruction
beginCaseLiftConstruction contextGraph =
...
commitCaseLiftConstruction :: CaseLiftConstruction -> Either String (ContextEGraph ArithF Depth CaseContext)
commitCaseLiftConstruction construction =
  foldM
...
commitGraph :: ContextRebaseBatch ArithF Depth CaseContext -> Either String (ContextEGraph ArithF Depth CaseContext)
...
branchLocalDigest :: ContextEGraph ArithF Depth CaseContext -> Either String Int
branchLocalDigest contextGraph =
  fmap
    (graphShapeDigest contextGraph +)
    (contextDigest parentContext contextGraph)

liftedDigest :: ContextEGraph ArithF Depth CaseContext -> Either String Int
liftedDigest contextGraph =
  fmap
    ((graphShapeDigest contextGraph +) . (17 *))
    (contextDigest parentContext contextGraph)

productLiftDigest :: ContextEGraph ArithF Depth CaseContext -> Either String Int
productLiftDigest contextGraph =
  fmap
    ((graphShapeDigest contextGraph +) . (31 *))
    (contextDigest parentContext contextGraph)

graphShapeDigest :: ContextEGraph ArithF Depth CaseContext -> Int
graphShapeDigest contextGraph =
  eGraphNodeCount (cegBase contextGraph)
    + eGraphClassCount (cegBase contextGraph)

contextDigest :: CaseContext -> ContextEGraph ArithF Depth CaseContext -> Either String Int
