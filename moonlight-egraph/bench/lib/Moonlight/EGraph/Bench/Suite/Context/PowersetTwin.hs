...
powersetTwinDigestIO atomCount =
  either
    ...
    (powersetTwinWorkload (powersetBenchmarkAtoms atomCount) powersetTwinDigest)

powersetBenchmarkAtoms :: Int -> [Char]
powersetBenchmarkAtoms atomCount =
  take atomCount ['a' .. 'z']

powersetTwinDigest :: PowersetTwinWorkload denseOwner symbolicOwner -> Either String Int
powersetTwinDigest workload = do
  denseDigest <- graphDigest classA classB contexts pairs (ptwDenseGraph workload)
  symbolicDigest <- graphDigest classA classB contexts pairs (ptwSymbolicGraph workload)
  pure (denseDigest + symbolicDigest)
  where
    classA =
      ptwClassA workload
    classB =
