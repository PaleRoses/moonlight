...
data ParameterReplayState owner = ParameterReplayState
  { prsGraph :: !(ContextEGraph owner ParameterF () VersionContext),
...
replayVersionedParameterTrace trace@(ParameterTrace actions) = do
  ...
  first
    ...
    ( withPreparedContextSiteFromPowersetAtoms [0 .. branchCount - 1] $ \site -> do
        replayState <-
          foldM
...
initialParameterReplayState ::
  PreparedContextSite owner VersionContext ->
  EGraph ParameterF () ->
  ParameterReplayState owner
...
applyParameterReplayAction ::
  ...
  ParameterReplayState owner ->
  LocatedParameterAction ->
  Either ParameterReplayObstruction (ParameterReplayState owner)
