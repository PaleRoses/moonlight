...
module Moonlight.EGraph.Bench.Suite.Relational
  ( relationalBenchmarks,
    ...
    annotatedComposedColdSharedDigest,
  ) where
...
data AnnotatedMatchingFixture = AnnotatedMatchingFixture
  { amfBuckets :: !(AnnotatedDeltaBuckets Arith.ArithF),
    amfBaseGraph :: !(EGraph Arith.ArithF Arith.NodeCount),
    amfContextGraph :: !(ContextEGraph Arith.ArithF Arith.NodeCount AnatomyRegion),
    amfContextKeys :: ![ContextObjectKey],
    amfContextRevision :: !Natural,
    amfQuery :: !ArithCompiledQuery,
    amfWarmState :: !(EGraphPreparedMatchState SurfaceKind Arith.ArithF),
    amfHeldStates :: ![(ContextObjectKey, EGraphPreparedMatchState SurfaceKind Arith.ArithF)],
    amfRoundMergeRegion :: !AnatomyRegion,
    amfRoundMergePair :: !(ClassId, ClassId)
  }

instance NFData AnnotatedMatchingFixture where
  rnf fixture =
...
contextAnnotatedMatchingBenches =
  [ annotatedMatchingBench label digest gridPoint
    | (label, digest) <-
        [ ("composed-cold", annotatedComposedColdDigest),
          ("composed-cold-shared", annotatedComposedColdSharedDigest),
          ("variant-warm", annotatedVariantWarmDigest),
          ("post-merge-round", annotatedPostMergeRoundDigest),
          ("post-merge-round-fresh", annotatedPostMergeRoundFreshDigest)
        ],
      gridPoint <- (,) <$> [1, 2, 4, 8] <*> [1000, 4000]
  ]

annotatedMatchingBench :: String -> (AnnotatedMatchingFixture -> Int) -> (Int, Int) -> Benchmark
annotatedMatchingBench label digest (contextCount, termCount) =
  env (prepareAnnotatedMatchingFixture contextCount termCount) $ \fixture ->
    bench
      (label <> "/" <> caseLabel [("K", contextCount), ("N", termCount)])
      (nf digest fixture)

prepareAnnotatedMatchingFixture :: Int -> Int -> IO AnnotatedMatchingFixture
prepareAnnotatedMatchingFixture contextCount termCount = do
  ...
  evaluate (annotatedComposedColdDigest fixture + annotatedVariantWarmDigest fixture) *> pure fixture

annotatedComposedColdDigest :: AnnotatedMatchingFixture -> Int
annotatedComposedColdDigest fixture =
  foldl'
    ( \total contextKey ->
...
annotatedComposedColdSharedDigest :: AnnotatedMatchingFixture -> Int
...
annotatedVariantWarmDigest :: AnnotatedMatchingFixture -> Int
...
annotatedPostMergeRoundDigest :: AnnotatedMatchingFixture -> Int
...
annotatedPostMergeRoundFreshDigest :: AnnotatedMatchingFixture -> Int
...
annotatedVariantMatchCount :: AnnotatedMatchingFixture -> ContextObjectKey -> Int
