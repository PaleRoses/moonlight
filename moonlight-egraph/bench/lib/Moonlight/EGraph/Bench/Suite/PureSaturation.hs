...
type SupportedEmissionU =
  EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion

type SupportedEmissionMatchState =
  SatMatchState SupportedEmissionU

data SupportedEmissionFixture = SupportedEmissionFixture
  { sefContextGraph :: !(ContextEGraph Arith.ArithF Arith.NodeCount AnatomyRegion),
...
instance NFData SupportedEmissionFixture where
  rnf fixture =
...
contextSupportedEmissionBenches =
  [ supportedEmissionBench label digest gridPoint
  | (label, digest) <-
      [ ("region-native", supportedEmissionRegionNativeDigest),
        ("per-context-oracle", supportedEmissionPerContextOracleDigest)
      ],
    gridPoint <- (,) <$> [1, 2, 4, 8] <*> [1000, 4000]
  ]

supportedEmissionBench :: String -> (SupportedEmissionFixture -> Int) -> (Int, Int) -> Benchmark
supportedEmissionBench label digest (contextCount, termCount) =
  env (prepareSupportedEmissionFixture contextCount termCount) $ \fixture ->
    bench
      (label <> "/" <> caseLabel [("K", contextCount), ("N", termCount)])
      (nf digest fixture)

prepareSupportedEmissionFixture :: Int -> Int -> IO SupportedEmissionFixture
prepareSupportedEmissionFixture contextCount termCount = do
  ...
  evaluate (supportedEmissionRegionNativeDigest fixture + supportedEmissionPerContextOracleDigest fixture) *> pure fixture
...
compileSupportedEmissionRule ruleId patternValue conditionValue =
  case compileRewriteRules @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion) [rawRule] of
    Right [compiledRule] ->
      pure compiledRule
    Right compiledRules ->
...
supportedEmissionStartingState :: SupportedEmissionMatchState
supportedEmissionStartingState =
  initialMatchState @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion)
    GenericJoinMatching
    emptyRewriteRuntimeCapabilities

supportedEmissionGraph ::
  SupportedEmissionFixture ->
  SaturatingContextEGraph SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion
supportedEmissionGraph =
  emptySaturatingContextEGraph . sefContextGraph

supportedEmissionRegionNativeDigest :: SupportedEmissionFixture -> Int
supportedEmissionRegionNativeDigest fixture =
  either
    (const (-1))
    (supportedEmissionMatchesDigest . snd)
    ( contextSupportedMatchesPrepared @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion)
        emptyRewriteRuntimeCapabilities
        supportedEmissionCapabilityResolver
        0
        Delta.fullDelta
...
supportedEmissionPerContextOracleDigest :: SupportedEmissionFixture -> Int
...
supportedEmissionOracleStep fixture (matchState, supportedMap) (contextValue, (factStore, factIndex, rules)) = do
  (nextState, rawMatches) <-
    first
      (const ())
      ( rawContextMatchesPrepared @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion)
          emptyRewriteRuntimeCapabilities
          contextValue
          0
          Delta.fullDelta
  ...
  supportedMatches <-
    first
      (const ())
      ( materializeRawMatchesAtContextView @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion)
          emptyRewriteRuntimeCapabilities
          supportedEmissionCapabilityResolver
          contextValue
          factStore
  ...
  case existing of
    ...
    Just held ->
      Just
        <$> first
          (const ())
          (mergeSupportedMatch @(EGraphU SurfaceKind Arith.ArithF Arith.NodeCount AnatomyRegion) (supportedEmissionGraph fixture) held candidate)
...
data CompiledRingFrontFixture = CompiledRingFrontFixture
  { crfCompiled :: !(CompiledEGraphFront BenchRingSig Ring.NodeCount () ()),
...
ringSaturationProgram ::
  ...
  RulesetM BenchRingSig () ->
  Term BenchRingSig "Expr" ->
  EGraphFront 'Authored BenchRingSig Ring.NodeCount () ()
...
ringFrontReportDigest ::
  EGraphFrontReport BenchRingSig Ring.NodeCount () () ->
  Int
...
emptyRingFrontGraph :: SaturatingContextEGraph SurfaceKind (PackedNode BenchRingSig) Ring.NodeCount ()
emptyRingFrontGraph =
  emptySaturatingContextEGraph $
