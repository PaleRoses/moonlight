...
runScalePoint point =
  either
    (pure . Left)
    id
    ( first labelFailure $ do
        siteAndProbes <- buildScaleSite (siteSpecForPoint point)
        prepareScaleFixture siteAndProbes scopedFixtureSpec $ \fixture ->
          either
            (pure . Left . labelFailure . show)
  ...
  where
    labelFailure failure =
      scopedPointLabel point <> ": " <> failure
...
scopedProbe :: Probe (PreparedSupportFixture owner ScopedF ()) (ScaleReport owner ScopedF ())
scopedProbe =
  requireFixedPoint
