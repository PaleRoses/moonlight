...
type ChimeraPlainU = EGraphU UnitContextSiteOwner () Chimera.TissueF Chimera.TissueCount TrivialContext
type ChimeraSupportU owner = EGraphU owner () Chimera.TissueF Chimera.TissueCount Scale.ScaleContext
type ChimeraPlainReport = Saturation.EGraphSaturationReport () Chimera.TissueF Chimera.TissueCount
type ChimeraContextGraph owner = ContextEGraph owner Chimera.TissueF Chimera.TissueCount Scale.ScaleContext
type ChimeraScaleReport owner = ScaleDigest.ScaleReport owner Chimera.TissueF Chimera.TissueCount
type ChimeraDriverFailure owner = Driver.DriverFailure owner Chimera.TissueF Chimera.TissueCount
type PlainRuleAssignment =
  ([FactRule () Chimera.TissueF], [RawRewriteRule (RewriteCondition () Chimera.TissueF) Chimera.TissueF])

data PointFixture owner = PointFixture
  { pfScaleFixture :: !(Fixture.ScaleFixture owner Chimera.TissueF Chimera.TissueCount),
    pfKeyedContexts :: ![(ContextObjectKey owner, Scale.ScaleContext)],
...
data SemanticPointFixture owner = SemanticPointFixture
  { spfPointFixture :: !(PointFixture owner),
    spfMergedContextQueries :: ![Digest.SemanticQuery owner Scale.ScaleContext],
    spfDeltaContextQueries :: ![Digest.SemanticQuery owner Scale.ScaleContext],
    spfProductComparisonQueries :: ![Digest.SemanticQuery owner Scale.ScaleContext]
  }
...
data CriterionKernelFixture owner = CriterionKernelFixture
  { ckfPointFixture :: !(PointFixture owner),
    ckfMergedContextQueries :: ![Digest.SemanticQuery owner Scale.ScaleContext],
    ckfDeltaContextQueries :: ![Digest.SemanticQuery owner Scale.ScaleContext],
    ckfDeltaGraph :: !(ChimeraContextGraph owner)
  }

instance NFData (CriterionKernelFixture owner) where
  rnf fixture =
...
data SupportDriverReceiptObstruction owner
  = SupportDriverPreparationObstruction !(ChimeraDriverFailure owner)
  | SupportDriverRuntimeObstruction !(ChimeraDriverFailure owner)
  | SupportDriverSemanticObstruction !(Digest.SemanticDigestObstruction owner Scale.ScaleContext)
  deriving stock (Show)

data SupportDriverObservation owner = SupportDriverObservation
  { sdoReport :: !(ChimeraScaleReport owner),
...
criterionEquivalenceKernelBench contextCount =
  criterionKernelBench
    contextCount
    criterionEquivalenceKernelDigest

criterionRepresentativeQueryBench :: Int -> Benchmark
criterionRepresentativeQueryBench contextCount =
  criterionKernelBench
    contextCount
    criterionRepresentativeQueryDigest

criterionKernelBench ::
  NFData result =>
  Int ->
  (forall owner. CriterionKernelFixture owner -> result) ->
  Benchmark
criterionKernelBench contextCount digestFixture =
  either setupFailure id $ do
    corpus <-
      Fixture.prepareScaleCorpus
        Chimera.tissueAnalysis
        (Population.tissueTermsAtDepth populationDepth populationSize)
    preparePointFixture contextCount corpus $ \pointFixture ->
      Criterion.env (prepareCriterionKernelFixture pointFixture) $ \fixture ->
        Criterion.bench benchmarkName (Criterion.nf digestFixture fixture)
  where
    benchmarkName =
      "K=" <> show contextCount
    ...
    setupFailure failure =
      Criterion.env
        ( (ioError (userError ("chimera Criterion fixture failed: " <> failure))) ::
            IO ()
        )
        (const (Criterion.bench benchmarkName (Criterion.whnf id ())))

prepareCriterionKernelFixture ::
  PointFixture owner ->
  IO (CriterionKernelFixture owner)
prepareCriterionKernelFixture pointFixture =
  either
...
criterionEquivalenceKernelDigest :: CriterionKernelFixture owner -> Either String (Int, Int)
...
criterionRepresentativeQueryDigest :: CriterionKernelFixture owner -> Either String Int
...
runContextGridPoint contextCount corpus =
  either
    (pure . Left)
    id
    (preparePointFixture contextCount corpus runPrepared)
  where
    runPrepared fixture =
...
runOursPoint :: SemanticPointFixture owner -> IO (Either Run.BenchFailure [CsvRow])
...
runProductPoint :: SemanticPointFixture owner -> IO (Either Run.BenchFailure [CsvRow])
...
semanticContextSample ::
  Show failure =>
  [Digest.SemanticQuery owner Scale.ScaleContext] ->
  (input -> Either failure (ChimeraContextGraph owner)) ->
  input ->
  Either String (ChimeraContextGraph owner, Int)
...
runDriverPoint :: PointFixture owner -> IO (Either Run.BenchFailure [CsvRow])
...
runSupportDriverReceipt receiptScale corpus =
  either
    (Run.abortBench . ("support-driver receipt fixture: " <>))
    id
    ( preparePointFixture (supportDriverReceiptContextCount receiptScale) corpus $ \fixture ->
      case
        ...
        Right preparedFixture -> do
          measurementResult <-
            PaleMeasure.measureFreshSample
              ...
              (\observation -> sdoSemanticDigest observation `seq` ())
              sdoSemanticDigest
          either
...
measureSupportDriverObservation ::
  PointFixture owner ->
  Driver.PreparedSupportFixture owner Chimera.TissueF Chimera.TissueCount ->
  IO (Either (SupportDriverReceiptObstruction owner) (SupportDriverObservation owner))
...
supportDriverObservationFromObserved ::
  PointFixture owner ->
  RuntimeEngine.RuntimeObservedResult (ChimeraDriverFailure owner) (ChimeraScaleReport owner) ->
  Either (SupportDriverReceiptObstruction owner) (SupportDriverObservation owner)
...
supportDriverSemanticDigest ::
  PointFixture owner ->
  ChimeraScaleReport owner ->
  Either (Digest.SemanticDigestObstruction owner Scale.ScaleContext) Int
...
supportDriverReceiptFromMeasurement ::
  ...
  SupportDriverReceiptScale ->
  PointFixture owner ->
  PaleMeasure.FreshMeasurement (SupportDriverObservation owner) ->
...
supportDriverReceiptFromMeasurement receiptScale fixture measurement =
  SupportDriverReceipt
    { sdrScale = receiptScale,
      ...
      sdrAllocatedBytes =
        PaleMeasure.freshRtsDeltaAllocatedBytes
          (PaleMeasure.freshMeasurementRtsDelta measurement),
      sdrPeakLiveBytes = PaleMeasure.freshMeasurementPeakLiveBytesThroughAction measurement,
  ...
  where
    ...
    metrics =
      Support.supportSaturationMetricsFromReport
        (SaturationSubstrate.proofGraphContext @(ChimeraSupportU owner) @())
        (Fixture.sfProofGraph (pfScaleFixture fixture))
        report

renderSupportDriverMeasurementFailure ::
  PaleMeasure.FreshMeasurementFailure (SupportDriverReceiptObstruction owner) ->
  String
renderSupportDriverMeasurementFailure = \case
  ...
  PaleMeasure.FreshMeasurementRtsDeltaFailed obstruction ->
    "support-driver receipt observed an RTS counter regression: " <> show obstruction
...
chimeraDriverFixpointProbe ::
  Measure.Probe
    (Driver.PreparedSupportFixture owner Chimera.TissueF Chimera.TissueCount)
    (ScaleDigest.ScaleReport owner Chimera.TissueF Chimera.TissueCount)
chimeraDriverFixpointProbe =
  Driver.supportDriverProbe "production support driver fixpoint"

runProductDriverPoint :: PointFixture owner -> IO (Either Run.BenchFailure [CsvRow])
runProductDriverPoint fixture =
...
sampledValue =
  PaleMeasure.timedSampleValue . Measure.sampledFirst
measuredContextKernelRow ::
  PointFixture owner ->
  Phase ->
  ChimeraContextGraph owner ->
...
withRegionalStructure :: ChimeraContextGraph owner -> CsvRow -> CsvRow
...
measuredKernelRow :: Arm -> PointFixture owner -> Phase -> Measure.Sampled value -> CsvRow
...
timeoutRow :: Arm -> PointFixture owner -> Phase -> CsvRow
timeoutRow arm fixture phase =
  pointRow KernelRegister arm fixture phase NotSaturationRun CellTimeout
pointRow :: Register -> Arm -> PointFixture owner -> Phase -> TerminationCell -> CellValue -> CsvRow
...
pointContextCount :: PointFixture owner -> Int
pointContextCount =
  Scale.scaleSiteContextCount . Fixture.sfSite . pfScaleFixture
preparePointFixture ::
  Int ->
  Fixture.ScaleCorpus Chimera.TissueF Chimera.TissueCount ->
  (forall owner. PointFixture owner -> result) ->
  Either Run.BenchFailure result
preparePointFixture contextCount corpus usePointFixture = do
...
prepareSemanticPointFixture :: PointFixture owner -> Either Run.BenchFailure (SemanticPointFixture owner)
...
semanticDigestSensitivityLaw =
  first
    (\lookupError -> "chimera semantic digest fixture: " <> show lookupError)
    (contextObjectKeyFor unitPreparedContextSite ())
    >>= semanticDigestSensitivityLawAt

semanticDigestSensitivityLawAt ::
  ContextObjectKey UnitContextSiteOwner ->
  Either Run.BenchFailure ()
semanticDigestSensitivityLawAt contextKey =
...
buildOursMergedGraph ::
  PointFixture owner ->
  Either (ContextDeltaError Chimera.TissueF Scale.ScaleContext) (ChimeraContextGraph owner)
buildOursMergedGraph fixture =
  foldM
    ...
    ( emptyContextEGraphFromSite
        (cegSite (Fixture.sfContextGraph scaleFixture))
...
productRegionScopedMerges :: PointFixture owner -> Either String (Map Scale.ScaleContext (EGraph Chimera.TissueF Chimera.TissueCount))
