...
runSdfPoint siteAndProbes populationSize =
  either
    (pure . Left)
    id
    ( Fixture.prepareScaleFixture siteAndProbes (sdfSpec populationSize) $ \fixture ->
        case
          first show
...
sdfProbe ::
  Measure.Probe
    (Driver.PreparedSupportFixture owner SDF.SDFF SDF.Depth)
    (ScaleDigest.ScaleReport owner SDF.SDFF SDF.Depth)
sdfProbe =
...
sdfRow ::
  PopulationSize ->
  Fixture.ScaleFixture owner SDF.SDFF SDF.Depth ->
