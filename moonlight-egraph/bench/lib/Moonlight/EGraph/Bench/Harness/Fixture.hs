...
data ScaleFixture owner f analysis = ScaleFixture
  { sfSite :: !ScaleSite,
    sfProbes :: !ScaleProbes,
    sfTermCount :: !Int,
    sfBaseGraph :: !(EGraph f analysis),
    sfClassIds :: ![ClassId],
    sfContextGraph :: !(Context.ContextEGraph owner f analysis ScaleContext),
    sfProofGraph :: !(SaturationState.SaturatingProofEGraph owner () f analysis ScaleContext ()),
    sfRuleBook ::
      !(SheafTwist.SupportedRuleBook owner ScaleContext (ScaleRewrite f)),
    sfFactBook :: !(SheafTwist.SupportedFactBook owner ScaleContext (FactRule () f)),
    sfDigest :: !Int
  }
...
prepareScaleFixture ::
  ...
  (forall owner. ScaleFixture owner f analysis -> result) ->
  Either BenchFailure result
prepareScaleFixture (site, probes) spec useFixture = do
  corpus <- sfsCorpus spec
  Context.withEmptyContextEGraph
    (Scale.scaleSiteLattice site)
    (scBaseGraph corpus)
    ( \emptyContextGraph -> do
        ...
        fixture `seq` pure (useFixture fixture)
    )
