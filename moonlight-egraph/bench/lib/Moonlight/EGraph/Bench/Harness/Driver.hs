...
type DriverGraph owner f analysis =
  Substrate.EGraphU owner () f analysis Scale.ScaleContext

type DriverFailure owner f analysis =
  Error.SaturationError
    (DriverGraph owner f analysis)
    (Support.SupportScheduleGroup (DriverGraph owner f analysis))

data PreparedSupportFixture owner f analysis = PreparedSupportFixture
  { preparedSupportPlan ::
      !( Plan
           (DriverGraph owner f analysis)
...
prepareSupportDriverFixture ::
  ...
  Fixture.ScaleFixture owner f analysis ->
  Either (DriverFailure owner f analysis) (PreparedSupportFixture owner f analysis)
...
supportDriverProbe ::
  ( Logic.EGraphLogicConstraints () f analysis Scale.ScaleContext,
    Core.ZipMatch f,
    Show (DriverFailure owner f analysis)
  ) =>
  String ->
  Probe (PreparedSupportFixture owner f analysis) (ScaleReport owner f analysis)
...
runSupportDriverObserved ::
  ...
  PreparedSupportFixture owner f analysis ->
  IO
    ( RuntimeEngine.RuntimeObservedResult
        (DriverFailure owner f analysis)
        (ScaleReport owner f analysis)
    )
...
requireFixedPoint :: Probe input (ScaleReport owner f analysis) -> Probe input (ScaleReport owner f analysis)
