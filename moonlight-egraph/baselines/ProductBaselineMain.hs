...
type RegionGraph owner = ContextEGraph owner ArithF NodeCount ContextIndex
...
measureRegionArm workloadSpec =
  withPreparedContextSiteFromFiniteLattice (workloadLattice workloadSpec) $ \contextSite -> do
    coldstart <- timePhase regionColdDigest (pure (buildRegionArm contextSite workloadSpec))
    case coldstart of
      Left failure ->
        pure (Left failure)
      Right timedColdstart -> do
...
buildRegionArm ::
  PreparedContextSite owner ContextIndex ->
  WorkloadSpec ->
  Either BaselineError (RegionArm owner)
buildRegionArm contextSite workloadSpec = do
  builtGraph <-
    first (GraphClassIdAllocationFailed (workloadConfig workloadSpec)) $
...
commitRegionProbe :: RegionArm owner -> Either BaselineError (RegionArm owner)
commitRegionProbe regionArm =
  commitRegionMerge (regionArmProbeMerge regionArm) regionArm

runRegionRound :: RegionArm owner -> Either BaselineError (RegionArm owner)
...
commitRegionMerge :: AuthoredMerge -> RegionArm owner -> Either BaselineError (RegionArm owner)
...
regionColdDigest :: RegionArm owner -> Int
...
regionCommitDigest :: RegionArm owner -> Int
...
regionRoundDigest :: RegionArm owner -> Int
regionRoundDigest =
  regionCommitDigest

regionSemanticQueryDigest :: RegionArm owner -> Either BaselineError Int
regionSemanticQueryDigest regionArm =
  fmap sum
    (traverse (contextSemanticDigest regionArm) (regionArmContexts regionArm))

contextSemanticDigest :: RegionArm owner -> ContextIndex -> Either BaselineError Int
...
regionMergeDigest :: RegionArm owner -> Int
regionMergeDigest =
