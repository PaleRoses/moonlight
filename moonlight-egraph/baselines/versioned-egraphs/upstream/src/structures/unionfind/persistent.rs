...
pub struct UnionFind<A: Clone> {
    /// A map from [Id] to [UnionFindNode].
    universe: im::Vector<UnionFindNode<A>>,
}
...
impl<A: Clone> super::UnionFindView for UnionFind<A> {
    type ElementData = A;
    #[inline]
...
impl<A: Clone> super::UnionFind for UnionFind<A> {
    #[cfg_attr(
        feature = "trace-unionfind-persistent",
    ...
    fn new_element(&mut self, element: A) -> Id {
        ...
        self.universe.push_back(node);
        id
    }
    #[cfg_attr(feature = "trace-unionfind-persistent", tracing::instrument(skip(self), ret))]
...
impl<A: Clone + std::fmt::Debug> GraphViz for UnionFind<A> {
    #[cfg_attr(feature = "trace-unionfind-persistent", tracing::instrument(skip(self), ret))]
