...
use crate::structures::unionfind::persistent;
...
pub struct EGraph<A: Analysis> {
    ...
    dependants: im::HashMap<EClass, im::Vector<EClass>>,
    ...
    eclasses: im::HashMap<EClass, im::Vector<EClass>>,
...
impl<A: Analysis<EClassDatas: Debug>> Debug for EGraph<A> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("PersistentEGraph")
...
impl<A: Analysis> super::EGraph for EGraph<A> {
    ...
    #[cfg_attr(feature = "trace-egraph-persistent", tracing::instrument(skip(self), ret))]
        ...
        match self.get_class(&xn) {
            ...
            None => {
                ...
                for arg in xn.args.iter() {
                    self.dependants.entry(*arg).or_default().push_back(xc)
                }
                let op_i = xn.op.to_i();
                // resize with default
                while op_i >= self.enodes_by_op.len() {
                    self.enodes_by_op.push_back(Default::default());
                }
                self.enodes_by_op[op_i].insert(xn, xc);
                if self.ematching_cache {
                    self.eclasses.entry(xc).or_default().push_back(xc);
                }
                A::make_data(self, &xc);
                xc
            },
    ...
    fn union_canonical(&mut self, xc: EClass, yc: EClass) -> EClass {
        ...
        self.dirty = true;
        if self.ematching_cache {
            let child_classes = self.eclasses.remove(&child).unwrap();
            self.eclasses.get_mut(&parent).unwrap().append(child_classes);
        }
    ...
    #[cfg_attr(feature = "trace-egraph-persistent", tracing::instrument(skip(self), ret))]
    fn rebuild(&mut self) {
        while let Some(child) = self.worklist.pop_front() {
...
impl<A: Analysis> EGraph<A> {
    ...
    fn init_ematching_cache(&mut self) {
        ...
        let mut cache: im::HashMap<EClass, im::Vector<EClass>> = Default::default();
        ...
        for xns in self.enodes_by_op.iter() {
            for xn in xns.keys() {
                ...
                cache.entry(xcc).or_default().push_back(*xc);
            }
        }
        self.eclasses = cache;
    }
    ...
    fn rebuild_ematching_cache(&mut self) {
        ...
        if self.ematching_cache {
            ...
            for (&_xc, xns) in eclasses.iter_mut() {
                ...
                // sort by operator
                xns.sort_by(|xc, yc| {
                    let xop = self.unionfind.get_element(*xc).op;
                    let yop = self.unionfind.get_element(*yc).op;
                    xop.cmp(&yop)
                });
                // dedup
                let mut temp: im::Vector<EClass> = std::mem::take(xns);
                let mut last: Option<EClass> = None;
                while let Some(xnc) = temp.pop_front() {
                    if Some(xnc) != last {
                        xns.push_back(xnc);
                        last = Some(xnc);
                    }
...
impl<A: Analysis> MachineRequirements for EGraph<A> {
    #[cfg_attr(
        all(feature = "trace-egraph-persistent", feature = "trace-machine"),
    ...
    fn for_each_matching_node<Err>(
        ...
        if classes.len() < 50 {
            ...
            let mut start = classes
                .binary_search_by_key(&enode.op, |c| self.unionfind.get_element(*c).op)
                .unwrap_or_else(|i| i);
            let discrim = enode.op;
            while start > 0 {
                let op = self.unionfind.get_element(classes[start - 1]).op;
                if op == discrim {
                    start -= 1;
                } else {
                    break;
