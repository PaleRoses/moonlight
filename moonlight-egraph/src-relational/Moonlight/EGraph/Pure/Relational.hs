...
data PreparedAlphaCache f = PreparedAlphaCache
  { pacRevision :: !EGraphRevision,
    ...
    pacStores :: !(Map.Map (PreparedPlanKey f) Store),
...
type PreparedRegionalSection :: Type
data PreparedRegionalSection = PreparedRegionalSection
  { prsRowsByAtom :: !(IntMap (Map.Map RowTupleKey ContextRegion)),
...
type PreparedSectionBuild :: Type -> (Type -> Type) -> Type -> Type
data PreparedSectionBuild capability f section = PreparedSectionBuild
  { psbState :: !(EGraphPreparedMatchState capability f),
    psbSections :: !(IntMap section)
  }

type EGraphPreparedMatchState :: Type -> (Type -> Type) -> Type
data EGraphPreparedMatchState capability f = EGraphPreparedMatchState
  { epmsPendingDeltas :: !(Map.Map (PreparedQueryKey f) PreparedMatchDelta),
    ...
    epmsPreparedBases :: !(Map.Map (PreparedPlanKey f) (EGraphPreparedBase capability f)),
    epmsFactorCaches :: !(Map.Map (PreparedPlanKey f) FactorCache),
    epmsRegionalSections :: !(Map.Map (PreparedPlanKey f) PreparedRegionalSection),
...
emptyEGraphPreparedMatchState :: EGraphPreparedMatchState capability f
emptyEGraphPreparedMatchState =
  EGraphPreparedMatchState
    { epmsPendingDeltas = Map.empty,
      ...
      epmsPreparedBases = Map.empty,
      epmsFactorCaches = Map.empty,
      epmsRegionalSections = Map.empty,
      epmsSharedTagRows = Nothing,
      epmsAlphaCache = Nothing
    }

resetEGraphPreparedMatchState :: EGraphPreparedMatchState capability f -> EGraphPreparedMatchState capability f
...
preparedPlanTemplate :: EGraphPreparedMatchState capability f -> EGraphPreparedMatchState capability f
preparedPlanTemplate state =
  emptyEGraphPreparedMatchState {epmsPlanCache = epmsPlanCache state}

preparedPlanCacheSize :: EGraphPreparedMatchState capability f -> Int
preparedPlanCacheSize =
...
wcojPreparedMatchCompiledWithRootFilter ::
  ...
  EGraph f a ->
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, [(ClassId, Substitution)])
...
-- section. Flow's prepared factor cache supplies the join provenance; the
-- region algebra interprets that provenance into the exact contexts where an
-- assignment exists.
wcojPreparedRegionalDeltaMatchCompiledWithRootFilter ::
  (Language f, Show (f ()), Show capability, Ord (f ())) =>
  RootClassFilter ->
  ...
  Either
    ...
    (EGraphPreparedMatchState capability f, [(ClassId, Substitution, ContextRegion)])
...
wcojPreparedSharedBaseDeltaMatchCompiledWithRoots ::
  ...
  Either
    EGraphRelationalMatchObstruction
    (PreparedBaseMatchMemo f, EGraphPreparedMatchState capability f, [(ClassId, Substitution)])
wcojPreparedSharedBaseDeltaMatchCompiledWithRoots =
...
preparedPatternPlans ::
  ...
  CompiledPatternQuery (CompiledGuard capability f) f ->
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (PreparedPatternPlans capability f, EGraphPreparedMatchState capability f)
...
flowPreparedStructuralMatches rootClassFilter graph preparedPlans state = do
  ...
  let preparedBase =
        buildPreparedBase (pppQueryPlan preparedPlans) graph
      baseState =
        recordPreparedBase preparedPlans preparedBase sourcesState
  (storeState, store) <-
    preparedStoreForBase graph preparedPlans preparedBase baseState
  (seededState, _factorMatches) <-
    flowStructuralMatchesWithFactorCache
      rootClassFilter
      graph
      preparedPlans
      store
...
preparedSourcesForQuery ::
  ...
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, [RelRuntime.DenseArrangement])
...
recordPreparedBase ::
  ...
  EGraphPreparedMatchState capability f ->
  EGraphPreparedMatchState capability f
recordPreparedBase preparedPlans preparedBase state =
  state
    { epmsPreparedBases =
        Map.insert (pppPlanKey preparedPlans) preparedBase (epmsPreparedBases state)
    }
{-# INLINE recordPreparedBase #-}

preparedStoreForBase ::
  Language f =>
  EGraph f a ->
  PreparedPatternPlans capability f ->
  EGraphPreparedBase capability f ->
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, Store)
preparedStoreForBase graph preparedPlans preparedBase state =
  case Map.lookup (pppPlanKey preparedPlans) (pacStores alphaCache) of
    Just store ->
      Right (state {epmsAlphaCache = Just alphaCache}, store)
    Nothing -> do
      sections <-
        first
          (EGraphRelationalRuntimeQueryObstruction . RelRuntime.RuntimeQueryPlanRowBuildObstruction)
          (preparedBaseRowBlocks (eGraphRevisionValue (eGraphRevision graph)) preparedBase)
      store <-
        first EGraphRelationalRuntimeQueryObstruction $
          RelRuntime.evalPlanPreparedStore (pppCompiledStoragePlan preparedPlans) sections
      let nextAlphaCache =
            alphaCache
              { pacStores = Map.insert (pppPlanKey preparedPlans) store (pacStores alphaCache)
              }
      Right (state {epmsAlphaCache = Just nextAlphaCache}, store)
  where
    alphaCache =
      preparedAlphaCacheForGraph graph state
{-# INLINE preparedStoreForBase #-}
...
flowStructuralMatchesWithFactorCache ::
  ...
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, [(ClassId, Substitution)])
flowStructuralMatchesWithFactorCache rootClassFilter graph preparedPlans store atomDeltas op state = do
  (matches, maybeCache) <-
    first EGraphRelationalRuntimeQueryObstruction $
      RelRuntime.evalPlanOutputsFromPreparedStoreCached
        (pppQueryPlan preparedPlans)
        (pppDecomp preparedPlans)
        (flowRootSelection rootClassFilter graph)
        store
        (Map.findWithDefault emptyFactorCache (pppPlanKey preparedPlans) (epmsFactorCaches state))
        atomDeltas
        op
  pure
    ( maybe
        state
        (\cache -> state {epmsFactorCaches = Map.insert (pppPlanKey preparedPlans) cache (epmsFactorCaches state)})
        maybeCache,
      matches
    )
{-# INLINE flowStructuralMatchesWithFactorCache #-}
...
flowPreparedStructuralDeltaMatches rootClassFilter dirtyResults graph preparedPlans state = do
  case Map.lookup (pppPlanKey preparedPlans) (epmsPreparedBases state) of
    Nothing ->
      flowPreparedStructuralDeltaMatchesWithoutBase rootClassFilter dirtyResults graph preparedPlans state
    Just preparedBase0 -> do
      let (preparedBase1, atomDeltas) =
            patchPreparedBaseWith graph dirtyResults preparedBase0
          baseState =
            recordPreparedBase preparedPlans preparedBase1 state
      if IntMap.null atomDeltas
        then pure (baseState, [])
        else do
          (storeState, store) <-
            preparedStoreForBase graph preparedPlans preparedBase1 baseState
          (factorState, _newMatches) <-
            flowStructuralMatchesWithFactorCache
              rootClassFilter
              graph
              preparedPlans
              ...
              storeState
          flowPreparedStructuralDeltaMatchesWithoutBase rootClassFilter dirtyResults graph preparedPlans factorState
{-# INLINE flowPreparedStructuralDeltaMatches #-}

flowPreparedStructuralDeltaMatchesWithoutBase ::
  Language f =>
  RootClassFilter ->
  IntSet ->
  EGraph f a ->
  ...
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, [(ClassId, Substitution)])
flowPreparedStructuralDeltaMatchesWithoutBase rootClassFilter dirtyResults graph preparedPlans state = do
...
{-# INLINE flowPreparedStructuralDeltaMatchesWithoutBase #-}
...
regionalRowsByAtom ::
  ...
  EGraph f a ->
  QueryPlan capability f ->
  IntMap (Map.Map RowTupleKey ContextRegion)
regionalRowsByAtom regionTable buckets graph queryPlan =
  IntMap.fromList
    [ ( atomKey,
        Map.fromListWith
...
regionalPhysicalRowsForTag ::
  ...
  EGraph f a ->
  f () ->
  Map.Map RowTupleKey ContextRegion
...
preparedRegionalSection ::
  ...
  Either EGraphRelationalMatchObstruction PreparedRegionalSection
...
regionalMatchesForOutputs ::
  ...
  Either EGraphRelationalMatchObstruction [(ClassId, Substitution, ContextRegion)]
...
regionalAssignmentRegion ::
  ...
  IntMap (Map.Map RowTupleKey ContextRegion) ->
  RowTupleKey ->
  Either EGraphRelationalMatchObstruction ContextRegion
...
preparedQuerySections ::
  ...
  QueryPlan capability f ->
  EGraphPreparedMatchState capability f ->
  Either Row.RowBuildError (EGraphPreparedMatchState capability f, IntMap (RowBlock 'Canonical))
...
insertPreparedAtomSection ::
  ...
  EGraph f a ->
  QueryPlan capability f ->
  PreparedSectionBuild capability f (RowBlock 'Canonical) ->
  RelPlan.AtomSpec (f ()) (ENode f) ClassId ->
  Either Row.RowBuildError (PreparedSectionBuild capability f (RowBlock 'Canonical))
...
preparedAlphaBlockForAtom ::
  ...
  RelPlan.AtomSpec (f ()) (ENode f) ClassId ->
  EGraphPreparedMatchState capability f ->
  Either Row.RowBuildError (EGraphPreparedMatchState capability f, RowBlock 'Canonical)
...
preparedAlphaRowsForAtom ::
  ...
  RelPlan.AtomSpec (f ()) (ENode f) ClassId ->
  EGraphPreparedMatchState capability f ->
  (EGraphPreparedMatchState capability f, IntMap [RowTupleKey])
preparedAlphaRowsForAtom graph atomSpec state =
...
flowQuerySections ::
  ...
  QueryPlan capability f ->
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, IntMap (RowBlock 'Canonical))
...
flowRowsForDirtyResults ::
  ...
  QueryPlan capability f ->
  EGraphPreparedMatchState capability f ->
  Either EGraphRelationalMatchObstruction (EGraphPreparedMatchState capability f, IntMap (IntMap [RowTupleKey]))
