...
type ProofEGraph :: Type -> (Type -> Type) -> Type -> Type -> Type -> Type
type ProofEGraph owner f a c p = ProofGraph (ContextEGraph owner f a c) f c p
...
emptyProofEGraph :: ContextEGraph owner f a c -> ProofEGraph owner f a c p
emptyProofEGraph =
  emptyProofGraph

emptyProofEGraphWithRetention :: ProofRetention -> ContextEGraph owner f a c -> ProofEGraph owner f a c p
emptyProofEGraphWithRetention =
  emptyProofGraphWithRetention
...
proofBetween :: ClassId -> ClassId -> ProofEGraph owner f a c p -> Either ProofQueryError (ProofStep f c p)
...
canonicalizeProofClass :: ProofEGraph owner f a c p -> ClassId -> ClassId
...
proofBaseGraph :: ProofEGraph owner f a c p -> EGraph f a
proofBaseGraph =
  cegBase . pgGraph
