...
emptyAnnotatedDeltaBuckets :: AnnotatedDeltaBuckets owner f
...
freshAnnotatedDeltaCache ::
  ...
  Either (RegionalClosureObstruction c) (AnnotatedDeltaCache owner f)
...
contextAnnotatedDeltaBuckets :: ContextEGraph owner f a c -> AnnotatedDeltaBuckets owner f
contextAnnotatedDeltaBuckets =
...
deriveAnnotatedDeltaBuckets ::
  (Language f, Ord c) =>
  ContextEGraph owner f a c ->
  Either (RegionalClosureObstruction c) (AnnotatedDeltaBuckets owner f)
...
combinedSeedRegions :: [RegionalSeed owner] -> Map (ClassId, ClassId) (ContextRegion owner)
combinedSeedRegions =
...
type RegionalSeed :: Type -> Type
data RegionalSeed owner = RegionalSeed
  { rsRegion :: !(ContextRegion owner),
...
authoredRegionalSeeds ::
  Ord c =>
  ContextEGraph owner f a c ->
  Either (RegionalClosureObstruction c) [RegionalSeed owner]
...
applyRegionalSeed :: RegionTable owner -> RegionalUnionFind owner -> RegionalSeed owner -> RegionalUnionFind owner
...
seedInhabitedRegion :: ContextRegion owner -> RegionalSeed owner -> ContextRegion owner
...
type RegionalRow :: Type -> (Type -> Type) -> Type
data RegionalRow owner f = RegionalRow
  { rrTag :: !(f ()),
    rrBaseRoot :: !Int,
    rrBaseChildren :: ![Int],
    rrCanonicalRoot :: !Int,
...
regionalForestRegion :: RegionalUnionFind owner -> ContextRegion owner
...
foldRegionalRepresentativeCoverage :: RegionalUnionFind owner -> ContextRegion owner
foldRegionalRepresentativeCoverage =
...
combineRegionalChoices ::
  ContextRegion owner ->
  [[(Int, ContextRegion owner)]] ->
  [([Int], ContextRegion owner)]
combineRegionalChoices activeRegion =
  foldr combineOne [([], activeRegion)]
  where
    combineOne :: [(Int, ContextRegion owner)] -> [([Int], ContextRegion owner)] -> [([Int], ContextRegion owner)]
...
type RegionalMerge :: Type -> Type
type RegionalMerge owner = (ContextRegion owner, Int, Int)

regionalCollisionMerges :: Ord (f ()) => [RegionalRow owner f] -> [RegionalMerge owner]
regionalCollisionMerges rows =
  ...
  where
    ...
    collisionsForForm :: [(Int, ContextRegion owner)] -> [RegionalMerge owner]
...
groupRegionalRows ::
  Ord (f ()) =>
  [(f (), Int, [Int], ContextRegion owner)] ->
  Map (f ()) [AnnotatedRow owner]
...
bucketFrontierBetween contextKeys oldBuckets newBuckets =
  IntMap.fromList
    [ (contextObjectKeyValue contextKey, frontierValue)
...
bucketFrontierAt contextKey oldBuckets newBuckets =
  Store.AnnotatedDeltaFrontier
    { Store.adfRepresentativeKeys =
        repKeyDelta
          (annotatedRepresentativeMapAt contextKey oldBuckets)
          (annotatedRepresentativeMapAt contextKey newBuckets),
...
rowsAtContext :: ContextObjectKey owner -> [AnnotatedRow owner] -> Set.Set RowForm
...
annotatedRowsAtKey ::
  ...
  AnnotatedDeltaBuckets owner f ->
  [(Int, [Int])]
annotatedRowsAtKey tag contextKey buckets =
...
absorbedRowsAtKey ::
  ...
  AnnotatedDeltaBuckets owner f ->
  [(Int, [Int])]
absorbedRowsAtKey tag contextKey buckets =
...
annotatedRowsByTagAt ::
  ...
  AnnotatedDeltaBuckets owner f ->
  Map (f ()) [(Int, [Int])]
annotatedRowsByTagAt contextKey =
...
absorbedRowsByTagAt ::
  ...
  AnnotatedDeltaBuckets owner f ->
  Map (f ()) [(Int, [Int])]
absorbedRowsByTagAt contextKey =
...
rowsByTagAt :: ContextObjectKey owner -> Map (f ()) [AnnotatedRow owner] -> Map (f ()) [(Int, [Int])]
...
annotatedInhabitedRegion :: AnnotatedDeltaBuckets owner f -> ContextRegion owner
annotatedInhabitedRegion = Store.adbInhabitedRegion
{-# INLINE annotatedInhabitedRegion #-}

annotatedDeltaMetrics :: RegionTable owner -> AnnotatedDeltaBuckets owner f -> AnnotatedDeltaMetrics
    ...
    rowWords row =
      Store.arRootKey row
        : Store.arChildKeys row
          <> fmap contextObjectKeyValue (regionGeneratorKeys regionTable (Store.arRegion row))
