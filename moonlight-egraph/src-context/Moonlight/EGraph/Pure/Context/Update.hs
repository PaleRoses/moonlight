...
import Moonlight.EGraph.Pure.Analysis.Spec (asMake)
...
import Moonlight.EGraph.Pure.Types (ClassId (..), ENode (..), classIdKey, canonicalizeClassId, eGraphRevision)
...
type RebaseSupportDemand :: Type -> Type
data RebaseSupportDemand c
  = RebaseNoSupport
  | RebaseAllSupport !(Set.Set c)
  | RebaseLimitedSupport !(SupportCarrier c) !(Set.Set c)

type RebaseScope :: Type -> Type
data RebaseScope c = RebaseScope
  { rsExplicitContexts :: !(Set.Set c),
    rsSupportDemand :: !(RebaseSupportDemand c)
  }

type ContextRebaseBatch :: (Type -> Type) -> Type -> Type -> Type
data ContextRebaseBatch f a c = ContextRebaseBatch
  { crbGraph :: !(ContextEGraph f a c),
    crbScope :: !(RebaseScope c),
    crbTrace :: !(ContextMutationTrace c f),
    crbOriginContextRevision :: !Natural
  }

type ContextRebaseReport :: (Type -> Type) -> Type -> Type
data ContextRebaseReport f c = ContextRebaseReport
  { crrScope :: !(RebaseScope c),
    crrDirtyContexts :: !(Set.Set c),
    crrTrace :: !(ContextMutationTrace c f),
    crrContextRevisionBefore :: !Natural,
    crrContextRevisionAfter :: !Natural
  }

type ContextMutationTrace :: Type -> (Type -> Type) -> Type
data ContextMutationTrace c f = ContextMutationTrace
  { cmtBaseTrace :: !(EGraphMutationTrace f),
    ...
    cmtSupportDelta :: !(ClassSupportDelta c)
  }
...
emptyContextEGraph :: Ord c => ContextLattice c -> EGraph f a -> ContextEGraph f a c
emptyContextEGraph contextLatticeValue baseGraph =
  emptyContextEGraphFromSite
    (fromFiniteLattice contextLatticeValue)
    baseGraph
{-# INLINE emptyContextEGraph #-}

emptyContextEGraphFromSite :: PreparedContextSite c -> EGraph f a -> ContextEGraph f a c
...
refreshContextDerivedState ::
  (Language f, Ord c) =>
  ContextEGraph f a c ->
  Either (ContextDeltaError f c) (ContextEGraph f a c)
refreshContextDerivedState contextGraph = do
  ...
  let refreshedAnalysisDeltas = fmap snd refreshedContextSections
      stepFrontier =
        bucketFrontierBetween
          [contextKey | ContextObjectKey contextKey <- fmap fst (Map.elems refreshedContextSections)]
...
bumpSupportContextRevision :: ContextEGraph f a c -> ContextEGraph f a c
...
stageFiberUnion ::
  ...
  ClassId ->
  ContextEGraph f a c ->
  Either EquivalenceRelationError (ContextEGraph f a c)
stageFiberUnion contextValue canonicalLeft canonicalRight contextGraph
  ...
  | otherwise = do
      let baseGraph =
          ...
          storedFiber =
            Map.findWithDefault
              (ContextFiber emptyContextFiberRelation (eGraphRevision baseGraph))
              contextValue
              (cegContextFibers contextGraph)
      repairedFiber <- repairContextFiber baseGraph storedFiber
      let extendedRelation =
...
type ContextMergeStaging :: (Type -> Type) -> Type -> Type -> Type
data ContextMergeStaging f a c = ContextMergeStaging
  { cmsGraph :: !(ContextEGraph f a c),
...
stageContextMerges ::
  ...
  ContextMergePlan c ->
  ContextRebaseBatch f a c ->
  Either (ContextDeltaError f c) (ContextRebaseBatch f a c)
...
stageContextMergeFiber ::
  ...
  ClassId ->
  ClassId ->
  ContextMergeStaging f a c ->
  (c, Set.Set c) ->
  Either (ContextDeltaError f c) (ContextMergeStaging f a c)
...
beginContextRebaseBatch :: ContextEGraph f a c -> ContextRebaseBatch f a c
...
emptyRebaseScope :: RebaseScope c
emptyRebaseScope =
  RebaseScope
...
-- 'commitContextRebaseBatch'.
contextRebaseBatchBaseGraph :: ContextRebaseBatch f a c -> EGraph f a
contextRebaseBatchBaseGraph =
  cegBase . crbGraph

contextRebaseBatchSite :: ContextRebaseBatch f a c -> PreparedContextSite c
contextRebaseBatchSite =
  cegSite . crbGraph

contextRebaseBatchClassSupportIndex :: ContextRebaseBatch f a c -> ClassSupportIndex c
contextRebaseBatchClassSupportIndex =
  cegClassSupport . crbGraph

contextRebaseBatchTrace :: ContextRebaseBatch f a c -> ContextMutationTrace c f
contextRebaseBatchTrace =
  crbTrace
...
applyBaseMutationToBatch ::
  ...
  ContextRebaseBatch f a c ->
  Either (ContextDeltaError f c) (result, ContextRebaseBatch f a c)
...
normalizeClassSupport :: Ord c => PreparedContextSite c -> SupportBasis c -> Either (ContextDeltaError f c) (SupportBasis c)
normalizeClassSupport site =
  first ContextSupportSiteFailed . normalizePreparedSupport site

unionClassSupport :: Ord c => PreparedContextSite c -> SupportBasis c -> SupportBasis c -> Either (ContextDeltaError f c) (SupportBasis c)
unionClassSupport site leftSupport =
  first ContextSupportSiteFailed . unionPreparedSupport site leftSupport

classSupportForChecked :: (Language f, Ord c) => ClassId -> ContextEGraph f a c -> Either (ContextDeltaError f c) (SupportBasis c)
...
clearAnnotatedDeltaFrontier :: ContextEGraph f a c -> ContextEGraph f a c
...
appendRebaseSupport ::
  ...
  SupportBasis c ->
  RebaseScope c ->
  Either (ContextDeltaError f c) (RebaseScope c)
...
rebaseSupportDemandFromSupport :: Ord c => PreparedContextSite c -> SupportBasis c -> Either (ContextDeltaError f c) (RebaseSupportDemand c)
rebaseSupportDemandFromSupport site supportValue = do
  globalSupport <- isGlobalSupport site supportValue
  let supportSeeds =
        Set.fromList (supportGenerators supportValue)
  if globalSupport
...
rebaseSupportDemandSeeds :: RebaseSupportDemand c -> Set.Set c
...
stageENodeWithSupport ::
  ...
  a ->
  ContextRebaseBatch f a c ->
  Either (ContextDeltaError f c) (ClassId, ContextRebaseBatch f a c)
stageENodeWithSupport supportValue enodeValue analysisValue batchValue = do
  ...
  normalizedSupport <- normalizeClassSupport site supportValue
  globalSupport <- isGlobalSupport site normalizedSupport
  readableBatch <-
...
contextMutationTraceHasMerges :: ContextMutationTrace c f -> Bool
  ...
  where
    ...
    canonicalChildAnalysis graphValue (classId, fallbackAnalysis) =
      maybe
        fallbackAnalysis
        id
        (IntMap.lookup (classIdKey classId) (egAnalysis graphValue))
...
commitContextRebaseBatch ::
  (Language f, Ord c) =>
  ContextRebaseBatch f a c ->
  Either (ContextDeltaError f c) (ContextRebaseReport f c, ContextEGraph f a c)
      ...
      report =
        ContextRebaseReport
          { crrScope = crbScope batchValue,
            crrDirtyContexts = dirtyContexts,
            ...
            let EGraphMutationResult
                ...
                EGraphMutationResult
                  { emrTrace = rebuildTrace,
                    ...
                    rebuildTracked Nothing dirtyBase
             in ( localMergeTrace,
      ...
      ( \contextValue existingDelta ->
          case adcaAdvanceMode bucketAdvance of
            ...
            AnnotatedDeltaAdvancedRegionally -> do
              contextKey@(ContextObjectKey contextKeyIndex) <-
                first
                  ContextSupportSiteFailed
                  (contextObjectKeyFor (cegSite graphWithBuckets) contextValue)
              if IntMap.member contextKeyIndex (adcaFrontierByContextKey bucketAdvance)
...
advanceAnnotatedDeltaBuckets ::
  ...
  ContextMutationTrace c f ->
  ContextEGraph f a c ->
  Either (RegionalClosureObstruction c) (AnnotatedDeltaCacheAdvance f)
...
isGlobalSupport :: Eq c => PreparedContextSite c -> SupportBasis c -> Either (ContextDeltaError f c) Bool
isGlobalSupport site supportValue =
  fmap
    (== supportValue)
    (first ContextSupportSiteFailed (defaultPreparedSupport site))
