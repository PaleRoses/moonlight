...
import Moonlight.EGraph.Pure.Analysis.Spec (AnalysisSpec (asJoin, asJoinChanged))
...
cegBase :: ContextEGraph f a c -> EGraph f a
cegBase =
  Store.cegBase

cegSite :: ContextEGraph f a c -> PreparedContextSite c
cegSite =
  Store.cegSite

cegLattice :: ContextEGraph f a c -> ContextLattice c
cegLattice =
  preparedContextLattice . Store.cegSite

cegContextFibers :: ContextEGraph f a c -> Map c Store.ContextFiber
cegContextFibers =
...
contextAuthoredUnionPairs :: Ord c => c -> ContextEGraph f a c -> [(ClassId, ClassId)]
...
cegClassSupportIndex :: ContextEGraph f a c -> ClassSupportIndex c
cegClassSupportIndex =
  Store.cegClassSupport

cegContextAnalysisDeltas :: ContextEGraph f a c -> Map c (IntMap a)
cegContextAnalysisDeltas =
  Store.cegContextAnalysisDeltas

cegContextRevision :: ContextEGraph f a c -> Natural
cegContextRevision =
  Store.cegContextRevision

cegRuntimeState :: ContextEGraph f a c -> Store.ContextRuntimeState c
cegRuntimeState =
  Store.cegRuntimeState

contextPreparedObjects :: Ord c => ContextEGraph f a c -> [c]
contextPreparedObjects =
  contextEnumerableObjectsForExecution
{-# INLINE contextPreparedObjects #-}

contextEnumerableObjectsForExecution :: Ord c => ContextEGraph f a c -> [c]
...
contextEnumerableSeedObjects :: Ord c => ContextEGraph f a c -> Set.Set c
...
contextSupportGeneratorObjects :: Ord c => ContextEGraph f a c -> Set.Set c
contextSupportGeneratorObjects contextGraph =
  either
    (const Set.empty)
...
contextCachedObjectsForExecution :: ContextEGraph f a c -> [c]
contextCachedObjectsForExecution =
...
activateContext ::
  ...
  c ->
  ContextEGraph f a c ->
  Either (PreparedContextSupportError c) (ContextEGraph f a c)
...
graphLiveCanonicalClassMap :: ContextEGraph f a c -> EGraph f a -> IntMap ClassId
...
baseCanonicalClassMap :: ContextEGraph f a c -> IntMap ClassId
...
contextRepresentativeAtKey :: ContextObjectKey -> ClassId -> ContextEGraph f a c -> ClassId
...
instance (Language f, Ord c) => ContextAlgebraSite (ContextEGraph f a c) c ClassId a where
...
contextRestrictionMismatchesWith ::
  Eq a =>
  (c -> ContextEGraph f a c -> Either errorValue (IntMap (ContextClassPayload ClassId a))) ->
  (c -> ContextEGraph f a c -> Either errorValue [c]) ->
  c ->
  ContextEGraph f a c ->
