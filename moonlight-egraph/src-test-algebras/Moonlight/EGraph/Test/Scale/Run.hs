...
runAtlasProgram ::
  ...
  [context] ->
  ( SaturatingProofEGraph capability f analysis context proof ->
    Either
      errorValue
      (SupportSaturationReportFor universe (SaturatingProofEGraph capability f analysis context proof))
  ) ->
  SaturatingProofEGraph capability f analysis context proof ->
  Either
...
runAtlasReferences ::
  forall capability f analysis context.
  ( Ord capability,
    Show capability,
    HasConstructorTag f,
  ...
  PlanSpec
    (EGraphU capability f analysis TrivialContext)
    (SatGraph (EGraphU capability f analysis TrivialContext))
    RewriteRuleId ->
  SheafTwist.SupportedRuleBook context (RawRewriteRule (RewriteCondition capability f) f) ->
  SheafTwist.SupportedFactBook context (FactRule capability f) ->
  ContextEGraph f analysis context ->
  [context] ->
  Either
    ( AtlasReferenceObstruction
        context
        (SaturationError (EGraphU capability f analysis TrivialContext) RewriteRuleId)
    )
...
assertReferenceAgreement ::
  forall f analysis context cost.
  (Language f, Ord context, Ord cost) =>
  CostAlgebra f cost ->
  [context] ->
  [ClassId] ->
