...
import Moonlight.EGraph.Pure.Context (ContextDeltaError (..), ContextEGraph, contextCachedObjectsForExecution, rebaseContextGraphAtContexts, withEmptyContextEGraph)
...
baseFixture ::
  Ord c =>
  ContextLattice c ->
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF Depth c) -> result) ->
  Either (ContextDeltaError ArithF c) result
baseFixture contextLatticeValue useFixture = do
  ...
  pure
    ( withEmptyContextEGraph contextLatticeValue graph2 $ \contextGraph ->
        useFixture (classA, classB, contextGraph)
    )

diagnosticFullContextRebase :: Ord c => ContextEGraph owner ArithF Depth c -> EGraph ArithF Depth -> Either (ContextDeltaError ArithF c) (ContextEGraph owner ArithF Depth c)
...
extendFixture :: Ord c => ContextEGraph owner ArithF Depth c -> Either (ContextDeltaError ArithF c) (ClassId, ClassId, ContextEGraph owner ArithF Depth c)
