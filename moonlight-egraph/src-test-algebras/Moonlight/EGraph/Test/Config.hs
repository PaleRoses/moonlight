...
testConfig :: (HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord capability, Show capability, Ord c) => TestBudget -> PlanSpec (EGraphU owner capability f a c) (SatGraph (EGraphU owner capability f a c)) RewriteRuleId
testConfig =
  TestSaturation.genericJoinSaturationConfig . toBudget

testConfigWith :: TestBudget -> MatchingStrategy owner c capability f a -> PlanSpec (EGraphU owner capability f a c) (SatGraph (EGraphU owner capability f a c)) RewriteRuleId
testConfigWith budget strategy =
  withSchedulerConfig
    deterministicSchedulerConfig
    (planSpec (toBudget budget) strategy emptyRewriteRuntimeCapabilities)

tracingTestConfig :: (HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord capability, Show capability, Ord c) => TestBudget -> PlanSpec (EGraphU owner capability f a c) (SatGraph (EGraphU owner capability f a c)) RewriteRuleId
