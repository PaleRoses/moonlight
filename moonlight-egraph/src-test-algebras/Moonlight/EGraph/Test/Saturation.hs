...
import Moonlight.EGraph.Pure.Context.Core (cegBase, cegSite)
...
type EGraphSaturationConfig :: Type -> (Type -> Type) -> Type -> Type -> Type
type EGraphSaturationConfig capability f a c =
  PlanSpec (EGraphU capability f a c) (SatGraph (EGraphU capability f a c)) RewriteRuleId
...
type EGraphSaturationReport capability f a =
  SaturationReport (EGraphU capability f a TrivialContext)

type EGraphProofSaturationReport :: Type -> (Type -> Type) -> Type -> Type -> Type -> Type
type EGraphProofSaturationReport capability f a c p =
  ProofSaturationReport (EGraphU capability f a c) (SaturatingProofEGraph capability f a c p)
...
data ContextualRewriteSaturationError capability f a c
  ...
  | ContextualRewriteSaturationFailed !(SaturationError (EGraphU capability f a TrivialContext) RewriteRuleId)
...
type EGraphContextIterationState :: Type -> (Type -> Type) -> Type -> Type -> Type
type EGraphContextIterationState capability f a c =
  ContextReport.RuntimeState
    (EGraphU capability f a c)
    (SatGraph (EGraphU capability f a c))
    RewriteRuleId

type PlainEGraphRuntimeState :: Type -> (Type -> Type) -> Type -> Type
type PlainEGraphRuntimeState capability f a =
  EGraphContextIterationState capability f a TrivialContext

type SaturatingProofEGraphRuntimeState :: Type -> (Type -> Type) -> Type -> Type -> Type -> Type
type SaturatingProofEGraphRuntimeState capability f a c p =
  ContextReport.RuntimeState
    (EGraphU capability f a c)
    (SaturatingProofEGraph capability f a c p)
    RewriteRuleId

type EGraphGuidance :: Type -> (Type -> Type) -> Type -> Type -> Type
type EGraphGuidance capability f a c =
  Gate
    (SaturationGuidanceView (EGraphU capability f a c))
    ()
    (SatSupportedMatch (EGraphU capability f a c))
    GuideRoundTrace
    RewriteRuleId

type EGraphStrategyPhase :: Type -> (Type -> Type) -> Type -> Type -> Type
data EGraphStrategyPhase capability f a c = EGraphStrategyPhase
  { spName :: !String,
    spConfig :: !(EGraphSaturationConfig capability f a c),
    spGuidance :: !(EGraphGuidance capability f a c)
  }

srIterations :: ContextReport.SaturationReportOf (EGraphU capability f a c) carrier schedulerGroup tracePayload -> Int
srIterations =
  ContextReport.reportIterationCount

srMatchesApplied :: ContextReport.SaturationReportOf (EGraphU capability f a c) carrier schedulerGroup tracePayload -> Int
srMatchesApplied =
  ContextReport.reportMatchesApplied

srGuideTrace :: ContextReport.SaturationReportOf (EGraphU capability f a c) carrier schedulerGroup tracePayload -> [GuideRoundTrace]
srGuideTrace =
  ContextReport.reportGuideTrace

srTrace :: ContextReport.SaturationReportOf (EGraphU capability f a c) carrier RewriteRuleId tracePayload -> SaturationTrace RewriteRuleId
srTrace =
...
psrProofGraph :: ProofSaturationReport (EGraphU capability f a c) (SaturatingProofEGraph capability f a c p) -> SaturatingProofEGraph capability f a c p
psrProofGraph =
  ContextReport.srCarrier

saturationReportHasFacts ::
  ContextReport.SaturationReportOf (EGraphU capability f a c) carrier schedulerGroup tracePayload ->
  Bool
saturationReportHasFacts =
...
runCompiledPlainPhaseWithRuntimeBuilder ::
  ...
  (SatGraph (EGraphU capability f a TrivialContext) -> RewriteContextSnapshot (EGraphU capability f a TrivialContext)) ->
  EGraphGuidance capability f a TrivialContext ->
  EGraphSaturationConfig capability f a TrivialContext ->
  [RulePlan (CompiledGuard capability f) f] ->
  [CompiledFactRule capability f] ->
  EGraph f a ->
  Either
    (SaturationError (EGraphU capability f a TrivialContext) RewriteRuleId)
    (SaturationReport (EGraphU capability f a TrivialContext))
runCompiledPlainPhaseWithRuntimeBuilder schedulerRefinement rewriteContextSnapshotForCarrier phaseGuidance saturationConfig compiledRules compiledFactRules baseGraph = do
  let compiledRuntimeProgram =
        baseProgram
          ...
          @(EGraphU capability f a TrivialContext)
          (rewriteRuleId @(EGraphU capability f a TrivialContext))
          compiledRules
          compiledFactRules
      phaseSpec =
        withRewriteContext
  ...
  planValue <-
    first SaturationCompileFailure $
      ContextProgram.planFromCompiledProgram @(EGraphU capability f a TrivialContext)
        phaseSpec
        compiledRuntimeProgram
  let runtimePolicy =
        ContextReport.plainRuntimePolicyWith @(EGraphU capability f a TrivialContext)
          ( \_state matches ->
              candidateSpaceForSupportedMatches
                @(EGraphU capability f a TrivialContext)
                (supportedMatchRuleKey @(EGraphU capability f a TrivialContext))
                (compareSupportedMatches @(EGraphU capability f a TrivialContext))
                matches
          )
          ( scheduleRefinedRoundSupportedMatches
              @(EGraphU capability f a TrivialContext)
              @(SatGraph (EGraphU capability f a TrivialContext))
              @RewriteRuleId
              schedulerRefinement
          )
  bimap SaturationRunFailure snd $
    ContextReport.runPlanWithPolicy @(EGraphU capability f a TrivialContext)
      runtimePolicy
      planValue
      (embedBaseGraph @(EGraphU capability f a TrivialContext) @(SatGraph (EGraphU capability f a TrivialContext)) baseGraph)

runCompiledProofPhaseWithRuntimeBuilder ::
  forall capability f a c p.
  (Ord capability, Show capability, HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord a, JoinSemilattice a, Ord c) =>
  SchedulerRefinement (SaturatingProofEGraphRuntimeState capability f a c p) RewriteRuleId ->
  (SaturatingProofEGraph capability f a c p -> RewriteContextSnapshot (EGraphU capability f a c)) ->
  ProofAnnotationBuilder c p ->
  Maybe c ->
  EGraphGuidance capability f a c ->
  EGraphSaturationConfig capability f a c ->
  [RulePlan (CompiledGuard capability f) f] ->
  [CompiledFactRule capability f] ->
  SaturatingProofEGraph capability f a c p ->
  Either
    (SaturationError (EGraphU capability f a c) RewriteRuleId)
    (EGraphProofSaturationReport capability f a c p)
runCompiledProofPhaseWithRuntimeBuilder schedulerRefinement rewriteContextSnapshotForCarrier proofAnnotationBuilder maybeActiveContext phaseGuidance saturationConfig compiledRules compiledFactRules proofGraph = do
  let compiledRuntimeProgram =
        baseProgram
          @'CompiledProgramStage
          @(EGraphU capability f a c)
          (rewriteRuleId @(EGraphU capability f a c))
          compiledRules
          compiledFactRules
      phaseSpec =
        withRewriteContext
  ...
  planValue <-
    first SaturationCompileFailure $
      ContextProgram.planFromCompiledProgram @(EGraphU capability f a c)
        phaseSpec
        compiledRuntimeProgram
  let runtimePolicy =
        ContextProof.proofRuntimePolicyWith @(EGraphU capability f a c) @p
          ( \_state matches ->
              candidateSpaceForSupportedMatches
                @(EGraphU capability f a c)
                (supportedMatchRuleKey @(EGraphU capability f a c))
                (compareSupportedMatches @(EGraphU capability f a c))
                matches
          )
          ( scheduleRefinedRoundSupportedMatches
              @(EGraphU capability f a c)
          ...
          maybeActiveContext
  bimap SaturationRunFailure snd $
    ContextReport.runPlanWithPolicy @(EGraphU capability f a c)
      runtimePolicy
      planValue
      proofGraph

mkStrategyPhaseSummary ::
  EGraphStrategyPhase capability f a c ->
...
phaseProgressed initialGraph saturationReport
  | ContextReport.reportMatchesApplied saturationReport == 0 && not (saturationReportHasFacts saturationReport) =
      False
  | otherwise =
      not (baseGraphEquals @(EGraphU capability f a c) initialGraph (saturationReportBaseGraph saturationReport))
...
type ProofSaturationSpec :: Type -> (Type -> Type) -> Type -> Type -> Type -> Type
data ProofSaturationSpec capability f a c p = ProofSaturationSpec
  { pssSaturation :: !(PlanSpec (EGraphU capability f a c) (SatGraph (EGraphU capability f a c)) RewriteRuleId),
...
compileEGraphProgram ::
  ...
  Either (SaturationError (EGraphU capability f a c) RewriteRuleId) (CompiledEGraphProgram capability f)
compileEGraphProgram graph _planSpecValue rawRules = do
  compiledRewrites <-
    first (SaturationCompileFailure . SaturationRewriteRulesFailed BaseProgramSite)
      (compileRewriteRules @(EGraphU capability f a c) (expandEGraphRulesByTheory (eGraphTheorySpec graph) rawRules))
...
supportPlanSpec ::
  ...
  PlanSpec
    (EGraphU capability f a c)
    (SaturatingProofEGraph capability f a c p)
    (SupportScheduleGroup (EGraphU capability f a c))
...
type EGraphSupportPlan capability f a c p =
  Plan
    (EGraphU capability f a c)
    (SaturatingProofEGraph capability f a c p)
    (SupportScheduleGroup (EGraphU capability f a c))

type EGraphSupportRunResult capability f a c p =
  ContextRunResult
    (EGraphU capability f a c)
    (SaturatingProofEGraph capability f a c p)
    (SupportScheduleGroup (EGraphU capability f a c))
    (SupportSaturationReportFor (EGraphU capability f a c) (SaturatingProofEGraph capability f a c p))
...
prepareEGraphSupportPlan ::
  ...
  (SaturatingProofEGraph capability f a c p -> RewriteContextSnapshot (EGraphU capability f a c)) ->
  PlanSpec (EGraphU capability f a c) (SatGraph (EGraphU capability f a c)) RewriteRuleId ->
  SheafTwist.SupportedRuleBook c (RawRewriteRule (RewriteCondition capability f) f) ->
  SheafTwist.SupportedFactBook c (FactRule capability f) ->
  SaturatingProofEGraph capability f a c p ->
  Either
    (SaturationError (EGraphU capability f a c) (SupportScheduleGroup (EGraphU capability f a c)))
    (EGraphSupportPlan capability f a c p)
...
runEGraphSupportPlan ::
  ...
  (Ord capability, Show capability, HasConstructorTag f, ZipMatch f, Show (ConstructorTag f), Show (f ()), Ord a, JoinSemilattice a, Ord c) =>
  ProofAnnotationBuilder c p ->
  TerminationGoal (SaturatingProofEGraph capability f a c p) ->
  EGraphSupportPlan capability f a c p ->
  SaturatingProofEGraph capability f a c p ->
  Either
    (SaturationError (EGraphU capability f a c) (SupportScheduleGroup (EGraphU capability f a c)))
    (EGraphSupportRunResult capability f a c p)
...
runContextualRewriteProofSaturation ::
  ...
  SheafTwist.SupportedRuleBook c (RawRewriteRule (RewriteCondition capability f) f) ->
  SaturatingProofEGraph capability f a c p ->
  Either (SaturationError (EGraphU capability f a c) RewriteRuleId) (EGraphProofSaturationReport capability f a c p)
...
saturate ::
  ...
  [RawRewriteRule (RewriteCondition capability f) f] ->
  EGraph f a ->
  Either (SaturationError (EGraphU capability f a TrivialContext) RewriteRuleId) (EGraphSaturationReport capability f a)
saturate budget =
  runSaturationSpec
...
saturateWithSchedulerRefinement schedulerRefinement saturationConfig factRules rawRules initialGraph = do
  compiledRules <-
    first
      ...
      (compileRewriteRules @(EGraphU capability f a TrivialContext) rawRules)
  compiledFactRules <-
    first
      (SaturationCompileFailure . SaturationFactRulesFailed BaseProgramSite)
      (compileFactRules @(EGraphU capability f a TrivialContext) factRules)
  runCompiledPlainPhaseWithRuntimeBuilder
    schedulerRefinement
...
saturateByStrategyWithSchedulerRefinement ::
  ...
  (Ord capability, Show capability, HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord a, JoinSemilattice a) =>
  SchedulerRefinement (PlainEGraphRuntimeState capability f a) RewriteRuleId ->
  Program () (EGraphStrategyPhase capability f a TrivialContext) ->
  [FactRule capability f] ->
  [RawRewriteRule (RewriteCondition capability f) f] ->
  EGraph f a ->
  Either
  ...
  where
    runStrategyPhase compiledRules compiledFactRules guidePhase graph = do
      ...
      pure
        ( guidePhase,
          Execution
            { seState = saturationReportBaseGraph @(EGraphU capability f a TrivialContext) saturationReport,
...
runSaturationSpec ::
  forall capability f a.
  (Ord capability, Show capability, HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord a, JoinSemilattice a) =>
  PlanSpec (EGraphU capability f a TrivialContext) (SatGraph (EGraphU capability f a TrivialContext)) RewriteRuleId ->
  [RawRewriteRule (RewriteCondition capability f) f] ->
  EGraph f a ->
  Either (SaturationError (EGraphU capability f a TrivialContext) RewriteRuleId) (EGraphSaturationReport capability f a)
runSaturationSpec planSpecValue rawRules initialGraph =
  let saturationConfig = planSpecValue
   in do
        compiledProgram <-
          compileEGraphProgram @capability @f @a @TrivialContext initialGraph planSpecValue rawRules
        runCompiledPlainPhaseWithRuntimeBuilder
          identitySchedulerRefinement
...
runProofSaturationSpec ::
  forall capability f a c p.
  (Ord capability, Show capability, HasConstructorTag f, ZipMatch f, Show (ConstructorTag f), Show (f ()), Ord a, JoinSemilattice a, Ord c) =>
  ProofSaturationSpec capability f a c p ->
  [RawRewriteRule (RewriteCondition capability f) f] ->
  SaturatingProofEGraph capability f a c p ->
  Either (SaturationError (EGraphU capability f a c) RewriteRuleId) (EGraphProofSaturationReport capability f a c p)
runProofSaturationSpec proofSpec rawRules proofGraph =
  let saturationConfig = pssSaturation proofSpec
   in do
        compiledProgram <-
          compileEGraphProgram @capability @f @a @c (cegBase (sceContextGraph (pgGraph proofGraph))) (saturationConfig) rawRules
        runCompiledProofPhaseWithRuntimeBuilder
          identitySchedulerRefinement
