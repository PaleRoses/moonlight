...
contextGlobalSection :: (Language f, Ord c) => ClassId -> ClassId -> ContextEGraph owner f a c -> Bool
...
contextRestrictionComposition :: (Language f, Ord c) => c -> c -> c -> ContextEGraph owner f a c -> Bool
...
contextRestrictionIdentityLaw :: (Language f, Ord c) => c -> ContextEGraph owner f a c -> Bool
contextRestrictionIdentityLaw contextValue =
  checkedLaw . contextRestrictionIdentity contextValue

contextMorphismLeftIdentityLaw :: Ord c => c -> ContextEGraph owner f a c -> Bool
...
contextMorphismRightIdentityLaw :: Ord c => c -> ContextEGraph owner f a c -> Bool
...
contextMorphismAssociativeLaw :: Ord c => c -> c -> c -> c -> ContextEGraph owner f a c -> Bool
...
contextRestrictionFunctorialActionLaw :: (Language f, Ord c) => c -> c -> c -> ContextEGraph owner f a c -> Bool
...
contextGlobalSectionInvariantLaw :: (Language f, Ord c) => c -> c -> ContextEGraph owner f a c -> Bool
...
contextMergeMonotone :: (Language f, JoinSemilattice a, Ord c) => c -> ClassId -> ClassId -> ContextEGraph owner f a c -> Bool
...
proofContextConsistency :: (Language f, Ord c) => (graph -> ContextEGraph owner f a c) -> ProofGraph graph f c p -> Bool
...
obstructionComplete :: (Language f, Ord c, Eq a) => ClassId -> ClassId -> c -> ContextEGraph owner f a c -> Bool
...
sameGraphState leftGraph rightGraph =
  UnionFind.canonicalMap (eGraphUnionFind leftGraph) == UnionFind.canonicalMap (eGraphUnionFind rightGraph)
    && materializeEGraphClasses leftGraph == materializeEGraphClasses rightGraph
    && materializeEGraphHashCons leftGraph == materializeEGraphHashCons rightGraph
