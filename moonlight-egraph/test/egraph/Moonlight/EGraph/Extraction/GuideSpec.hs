...
import Moonlight.Core (Language, Pattern (..), UnionFindAllocationError)
...
import Moonlight.EGraph.Pure.Extraction.Guide (guidedExtractionResult)
...
import Moonlight.EGraph.Pure.Types (ClassId (..), EGraph, ENode (..), RewriteRuleId (..), classIdKey)
import Moonlight.EGraph.Test.Arith.Core (ArithF (..), NodeCount (..), negTermNode, numTerm, viewArithTerm)
...
import Moonlight.Rewrite.Runtime (RulePlan, rpId, rpQuery)
...
tests =
  testGroup "guide" . hunitCases $
    [ HUnitCase "applyGuidance retains only matches that preview into preferred checkpoint classes" $ do
      ...
      HUnitCase "front-authored companion reaches saturation through public EDSL" $
        Tiny.withEmptyFrontGraph $ \emptyFrontGraph -> do
          frontReport <-
            Tiny.expectFront (Front.runEGraphFront frontAuthoredGuidanceCompanion emptyFrontGraph)
        ...
        rewriteCommit <-
          expectSaturation $
            runExecutableRewriteMatchEGraphCommitted
              ...
              initialMatch
              graph
        let (rebuildDelta, rebuiltGraph) = rebuildWithDelta (emrGraph rewriteCommit)
      ...
      HUnitCase "guided extraction selects a required nested descendant alternative" $ do
        table <- requireMaybe "expected a valid nested guidance table" nestedGuidanceTable
        let section = extractChoiceSection (liftCostAlgebra arithCost) table
            extractionResult =
              guidedExtractionResult
                (nestedGuidance GuideRequire 2)
                section
                (ClassId 0)
        fmap (viewArithTerm . erTerm) extractionResult
          @?= Just (viewArithTerm (negTermNode (numTerm 2))),
      HUnitCase "required extraction checkpoint is a hard filter" $ do
        table <- requireMaybe "expected a valid nested guidance table" nestedGuidanceTable
        let section = extractChoiceSection (liftCostAlgebra arithCost) table
        case
            guidedExtractionResult
              (nestedGuidance GuideRequire 3)
              section
              (ClassId 0)
          of
            Nothing -> pure ()
            Just _ -> assertFailure "required checkpoint admitted a non-matching extraction"
    ]

nestedGuidance :: GuideMode -> Int -> GuidanceConfig (Pattern ArithF)
nestedGuidance guideMode leafValue =
  GuidanceConfig
    [ GuideCheckpoint
        { gcName = "nested-negation",
          gcMode = guideMode,
          gcTarget = PatternNode (Neg (PatternNode (Num leafValue)))
        }
    ]

nestedGuidanceTable :: Maybe (ExtractionTable ArithF NodeCount)
nestedGuidanceTable =
  extractionTable
    ( IntMap.fromList
        [ (0, extractionClass (NodeCount 1) [ENode (Neg (ClassId 1))]),
          (1, extractionClass (NodeCount 1) [ENode (Num 1), ENode (Num 2)])
        ]
    )
    (\classId -> if classId == ClassId 0 || classId == ClassId 1 then Just classId else Nothing)

frontAuthoredGuidanceCompanion ::
  EGraphFront 'Authored owner Tiny.FrontTinySig Tiny.NodeCount Tiny.FrontTinyContext Bool
frontAuthoredGuidanceCompanion =
  Front.egraph $ do
    simplificationRules <-
...
frontMatchesApplied ::
  EGraphFrontReport owner Tiny.FrontTinySig Tiny.NodeCount Tiny.FrontTinyContext result ->
  Int
frontMatchesApplied =
...
compiledArithMatches rules graph = do
  compiledRewrites <- expectCompile (compileRewriteRules @(EGraphU UnitContextSiteOwner SurfaceKind ArithF NodeCount ()) rules)
...
requireMaybe _ (Just value) =
  pure value
requireMaybe failureMessage Nothing =
  assertFailure failureMessage
...
guideProofSpec :: ProofSaturationSpec UnitContextSiteOwner SurfaceKind ArithF NodeCount TrivialContext GuideNote
guideProofSpec =
  ProofSaturationSpec
...
guideProofGraph :: EGraph ArithF NodeCount -> SaturatingProofEGraph UnitContextSiteOwner SurfaceKind ArithF NodeCount TrivialContext GuideNote
guideProofGraph =
  emptySaturatingProofEGraph . emptyContextEGraphFromSite unitPreparedContextSite

priorityConfig :: EGraphSaturationConfig UnitContextSiteOwner SurfaceKind ArithF NodeCount ()
priorityConfig =
  withSchedulerConfig
    ( withPriorityProfile
        ( priorityProfileFromList
...
tracingConfig :: EGraphSaturationConfig UnitContextSiteOwner SurfaceKind ArithF NodeCount ()
tracingConfig =
