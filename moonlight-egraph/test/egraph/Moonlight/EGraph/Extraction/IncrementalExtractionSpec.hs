...
import Moonlight.EGraph.Pure.Extraction.Cache (ExtractionChoiceCache)
import Moonlight.EGraph.Pure.Extraction.Worklist (choiceKey)
...
import Moonlight.EGraph.Pure.Types (ClassId, EGraph, ENode, emptyEGraph)
...
genStableStart = do
  ...
  pure $
    fmap
      (\(graph, classes) -> (emrGraph (rebuildTracked Nothing graph), classes))
      ( foldM
...
choicesView =
  fmap (fmap choiceKey) . eccChoices
...
runScript cacheValue graph classes (op : remainingOps) =
  case applyEditOp op (graph, classes) of
    ...
    Right (stepTrace, nextGraph, nextClasses) ->
      let advanced =
            advanceExtractionChoiceCache cacheBudget costAlgebraValue stepTrace nextGraph cacheValue
          fresh =
       ...
       in case (advanced, fresh) of
        (Right advancedCache, Right freshCache) ->
          QC.conjoin
            [ QC.counterexample
                ("advanced choices diverge from fresh build after " <> show op)
                (choicesView advancedCache QC.=== choicesView freshCache),
              QC.counterexample
