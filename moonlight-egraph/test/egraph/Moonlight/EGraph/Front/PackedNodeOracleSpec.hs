...
compiledGuardFromExpr guardExpr =
  mapCompiledGuard (const guardExpr) mempty
...
genCompiledPatternExtension =
  CompiledPatternExtension
    <$> (ApplicationConditionPath <$> vectorOf 2 (chooseInt (0, 2)))
    <*> genCompiledPatternQuery
    <*> (Set.fromList . fmap mkPatternVar <$> listOf (chooseInt (0, 2)))
    <*> elements [ExtensionLocal, ExtensionRoot, ExtensionGlobal]
...
genCompiledApplicationCondition =
  compiledApplicationCondition <$> exprAtDepth (2 :: Int)
  where
    exprAtDepth ::
      Int ->
      Gen (ConstraintExpr (CompiledPatternExtension (CompiledGuard OracleCapability (Node OracleSig)) (Node OracleSig)))
    exprAtDepth depth
      | depth <= 0 =
          Atom <$> genCompiledPatternExtension
      | otherwise =
          frequency
            [ (4, Atom <$> genCompiledPatternExtension),
              (1, Not <$> exprAtDepth (depth - 1)),
              (1, And <$> vectorOf 2 (exprAtDepth (depth - 1))),
              (1, Or <$> vectorOf 2 (exprAtDepth (depth - 1)))
            ]
...
genCompiledFactRule =
  (\factRuleId factRuleName patternValue factId ->
     CompiledFactRule
       { cfrId = factRuleId,
         cfrName = factRuleName,
         cfrQuery = SinglePatternQuery patternValue,
         cfrCompiledQuery = compiledSinglePatternQuery patternValue Nothing,
         cfrProjection = [GuardRoot],
         cfrFactId = factId
       }
  )
    <$> (FactRuleId <$> chooseInt (0, 5))
    <*> elements ["packed.compiled.fact.a", "packed.compiled.fact.b"]
    <*> genOraclePattern
    <*> genFactId
...
planSetRoundTripProperty =
  case (mkRuleName "packed.plan.a", mkRuleName "packed.plan.b") of
    (Right leftName, Right rightName) ->
      case checkedSystemFromRewrites 2 [checkedIdentity 0 leftName, checkedIdentity 1 rightName] of
        Left checkedError ->
          counterexample ("plan set construction failed: " <> show checkedError) False
        Right checkedSystem ->
          let planSet = planRuleSet checkedSystem
           in case packRulePlanSet planSet of
                Left err ->
    ...
    _ ->
      property False
  where
    checkedIdentity rewriteId name =
      CheckedRewrite
        { checkedRewriteId = RewriteRuleId rewriteId,
          checkedRewriteName = name,
          checkedRewriteAlgebra =
            identityPatternRewrite (PatternNode (Node (OracleLeaf rewriteId)))
              :: PatternRewrite RuleOrigin (LogicalDecoration OracleCapability) (Node OracleSig)
        }
