...
withDiamondFixture ::
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF Depth DiamondCtx) -> Assertion) ->
  Assertion
withDiamondFixture useFixture =
  withResult (baseFixture diamondLattice useFixture) id

withAnatomyFixture ::
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF Depth AnatomyRegion) -> Assertion) ->
  Assertion
withAnatomyFixture useFixture =
  withResult (baseFixture coarseAnatomyLattice useFixture) id
...
sectionMonotonicityViolations ::
  ...
  Show c =>
  ContextLattice c ->
  ContextEGraph owner ArithF Depth c ->
...
monotonicityTests =
  testGroup
    ...
    [ testCase "diamond: sections are monotone along <= across merge scripts" $ withDiamondFixture $ \(termA, termB, ceg) -> do
        ...
        withEmptyContextEGraph diamondLattice graph3 $ \conflictBase -> do
      ...
      testCase "anatomy: sections are monotone with local generators and narrowed carriers" $ withAnatomyFixture $ \(termA, termB, ceg) -> do
        ...
        withSupportSensitiveAdmissibilityFixture $ \(fixtureA, _fixtureB, localHead, extended) -> do
...
diamondUnitTests =
  testGroup
    ...
    [ testCase "unmerged diamond: descent satisfied everywhere" $ withDiamondFixture $ \(_, _, ceg) -> do
        let report = fullDescentCheck ceg
        drSatisfied report @?= True
        drObstructionCount report @?= 0,
      testCase "compatible merge (both sides): descent satisfied at DTop" $ withDiamondFixture $ \(termA, termB, ceg) -> do
      ...
      testCase "overlap agreement across incomparable branches does not force gluing at DBottom" $ do
        ...
        withEmptyContextEGraph diamondLattice graph3 $ \contextGraph ->
          withResult (contextMerge DLeft termA termB contextGraph) $ \leftMerged ->
      ...
      testCase "one-sided merge propagates to top — descent still holds (propagation resolves)" $ withDiamondFixture $ \(termA, termB, ceg) -> do
        withResult (contextMerge DLeft termA termB ceg) $ \merged ->
          descentAt DTop merged @?= SearchAccepted,
      testCase "after propagation converges, descent is always satisfied" $ withDiamondFixture $ \(termA, termB, ceg) -> do
      ...
      testCase "conflicting merges at DLeft and DRight — descent at DBottom" $ do
        (a, b, c, graph3) <- expectRight seedThreeLiterals
        withEmptyContextEGraph diamondLattice graph3 $ \contextGraph ->
          withResult (contextMerge DLeft a b contextGraph) $ \leftMerged ->
      ...
      testCase "leaf context: descent trivially satisfied" $ withDiamondFixture $ \(termA, termB, ceg) -> do
        withResult (contextMerge DLeft termA termB ceg) $ \merged -> do
              descentAt DLeft merged @?= SearchAccepted
              descentAt DRight merged @?= SearchAccepted,
      testCase "bottom context with no children above: descent trivially satisfied" $ withDiamondFixture $ \(_, _, ceg) -> do
        descentAt DBottom ceg @?= SearchAccepted,
      testCase "global merge: descent satisfied everywhere" $ withDiamondFixture $ \(termA, termB, ceg) -> do
          ...
          withResult
              ( do
                  (parentB, stagedParentB) <-
                    stageTermGlobally (plus (lit 2) (lit 2)) (beginContextRebaseBatch contextGraph)
...
anatomyIntegrationTests =
  testGroup
    ...
    [ testCase "unmerged anatomy: descent satisfied everywhere" $ withAnatomyFixture $ \(_, _, ceg) -> do
        drSatisfied (fullDescentCheck ceg) @?= True,
      testCase "merge at Head: descent at Upper checks k=4 cover" $ withAnatomyFixture $ \(termA, termB, ceg) -> do
      ...
      testCase "descent accepts invisible local generators across k-ary covers" $
        withSupportSensitiveAdmissibilityFixture $ \(termA, _termB, localHead, extended) ->
          withResult (contextMerge Head localHead termA extended) $ \merged ->
            descentAt Upper merged @?= SearchAccepted,
      testCase "global merge: descent satisfied at every level" $ withAnatomyFixture $ \(termA, termB, ceg) -> do
        withResult (globalMerge termA termB ceg) $ \merged -> do
              let report = fullDescentCheck merged
              drSatisfied report @?= True
              drObstructionCount report @?= 0,
      testCase "merge at Upper: descent at Whole checks k=2 cover {Upper, Lower}" $ withAnatomyFixture $ \(termA, termB, ceg) -> do
      ...
      testCase "regional cache survives unrelated base growth" $
        withSupportSensitiveAdmissibilityFixture $ \(_termA, _termB, _localHead, extended) ->
...
withSupportSensitiveAdmissibilityFixture ::
  (forall owner. (ClassId, ClassId, ClassId, ContextEGraph owner ArithF Depth AnatomyRegion) -> Assertion) ->
  Assertion
withSupportSensitiveAdmissibilityFixture useFixture =
  withAnatomyFixture $ \(termA, termB, contextGraph) ->
    withResult
      ( do
          (localHead, stagedBatch) <-
            stageTermAtContext Head (lit 7) (beginContextRebaseBatch contextGraph)
