...
import Moonlight.EGraph.Pure.Types
    ( classIdKey, ClassId, EClass(eClassData), eGraphClasses )
...
contextAnalysisFor :: (Language f, Ord c) => c -> ContextEGraph f a c -> Either (PreparedContextSupportError c) (IntMap.IntMap a)
contextAnalysisFor =
...
flatColorTests =
  testGroup
    ...
    [ testCase "merge at Red is visible at Red but not Blue" $
        withResult flatFixture $ \(termA, termB, ceg) ->
      ...
      testCase "restriction between incomparable colors is typed failure" $
        withResult flatFixture $ \(_, _, ceg) ->
          do
...
sheafPropagationTests =
  testGroup
    ...
    [ testCase "merge at Upper propagates to Head, Torso, ArmLeft, ArmRight, Local" $
        withResult anatomyFixture $ \(termA, termB, ceg) ->
      ...
      testCase "restriction maps exist along lattice chains" $
        withResult anatomyFixture $ \(_, _, ceg) ->
          do
  ...
  testGroup
    ...
    [ testCase "sheaf: transitive propagation — merge at Whole flows through Upper to Head" $
      ...
      testCase "flat: no intermediate structure to compose through" $
        withResult flatFixture $ \(_, _, ceg) -> do
...
contextAnalysisTests =
  testGroup
    ...
    [ testCase "analysis at bottom context matches base graph" $
        withResult anatomyFixture $ \(_, _, ceg) ->
          let baseAnalysis = IntMap.map eClassData (eGraphClasses (cegBase ceg))
...
flatFixture :: Either (ContextDeltaError ArithF FlatColor) (ClassId, ClassId, ContextEGraph ArithF Depth FlatColor)
flatFixture = baseFixture flatLattice

flatFixtureExtended :: ContextEGraph ArithF Depth FlatColor -> Either (ContextDeltaError ArithF FlatColor) (ClassId, ClassId, ContextEGraph ArithF Depth FlatColor)
...
anatomyFixture = baseFixture preciseAnatomyLattice

anatomyFixtureExtended :: ContextEGraph ArithF Depth AnatomyRegion -> Either (ContextDeltaError ArithF AnatomyRegion) (ClassId, ClassId, ContextEGraph ArithF Depth AnatomyRegion)
