...
type ContextEGraphAnatomy =
  ContextEGraph Arith.ArithF Arith.NodeCount AnatomyRegion

type ArithEGraphU = EGraphU () Arith.ArithF Arith.NodeCount AnatomyRegion

data CacheLineageCheckpoint = CacheLineageCheckpoint
  { clcLabel :: String,
    clcGraph :: ContextEGraphAnatomy
  }
...
cacheLineageCheckpoints :: ArithFixture -> Bool -> Either String [CacheLineageCheckpoint]
cacheLineageCheckpoints fixture preferAddA = do
  let graph0 =
        emptyContextEGraph coarseAnatomyLattice (afBase fixture)
      (firstCompound, secondCompound) =
        if preferAddA
...
factClosureVirtualFiberOracleLaw preferAddA =
  case arithFixture of
    ...
    Right fixture ->
      case (cacheLineageCheckpoints fixture preferAddA, compileFactRules @ArithEGraphU factClosureOracleRules) of
        (Left failure, _) ->
          QC.counterexample failure False
        (_, Left compileFailure) ->
          QC.counterexample ("fact closure oracle rule compilation failed: " <> show compileFailure) False
        (Right checkpoints, Right compiledRules) ->
...
factClosureCheckpointLaw fixture compiledRules checkpoint =
  let contextGraph = clcGraph checkpoint
      ...
      baseContext = graphBaseContext @ArithEGraphU graph
      contextValues = filter (/= baseContext) (graphExecutionContexts @ArithEGraphU graph)
   in case
        traverse
          ( \contextValue ->
              fmap
...
factClosureCheckpointLawWithViews ::
  ...
  [CompiledFactRule () Arith.ArithF] ->
  CacheLineageCheckpoint ->
  SaturatingContextEGraph () Arith.ArithF Arith.NodeCount AnatomyRegion ->
  [AnatomyRegion] ->
...
factClosureCheckpointLawWithViews fixture compiledRules checkpoint graph contextValues materializedViews =
  let
      perContextResults =
        Map.fromList
          [ ( contextValue,
              deriveFactClosureAtContext @ArithEGraphU
                emptyGuardCapabilityResolver
                emptyFactStore
                compiledRules
                graph
      ...
      batchedResult =
        deriveFactClosuresAtContexts @ArithEGraphU
          emptyGuardCapabilityResolver
          graph
          batchInputs
      independentResult =
      ...
      localCanonicalFacts =
        [ hasFact
          ...
          | contextValue <- contextValues,
            Just materializedView <- [Map.lookup contextValue materializedViews],
            let baseCanonicalClass = canonicalizeClassId (graphBase @ArithEGraphU graph) (afNumB fixture)
...
factClosureContextLaw checkpoint graph compiledRules contextValue materializedView =
  let optimizedResult =
        deriveFactClosureAtContext @ArithEGraphU
          ...
          emptyFactDerivationIndex
      defaultResult =
        deriveFactClosure @ArithEGraphU
          emptyGuardCapabilityResolver
          emptyFactStore
          compiledRules
          materializedView
...
testRecursiveGuardCollapseBoundary = do
  ...
  where
    ...
    recursiveFactAt fixture compiledRules graph contextValue =
      case deriveFactClosureAtContext @ArithEGraphU
        emptyGuardCapabilityResolver
        emptyFactStore
        compiledRules
        graph
  ...
  contextGraph <-
    either
      ...
      ( foldlM
          ...
          (emptyContextEGraph coarseAnatomyLattice (afBase fixture) :: ContextEGraphAnatomy)
          (armScript fixture)
      )
...
visibleAt ::
  ContextEGraphAnatomy ->
  ClassId ->
  AnatomyRegion ->
  Either String Bool
visibleAt contextGraph stagedClass region = do
  let site = fromFiniteLattice coarseAnatomyLattice
  regionKey <-
    either (Left . show) Right (contextObjectKeyFor site region)
  case classSupportExplicitCarrierForKey (cegClassSupportIndex contextGraph) (classIdKey stagedClass) of
    Nothing ->
      pure True
    Just carrier ->
      pure (supportCarrierContainsKey site carrier regionKey)

stageProbeAt ::
  AnatomyRegion ->
  ContextEGraphAnatomy ->
  Either String (ClassId, ContextEGraphAnatomy)
...
testStagedTermSupportScope = do
  fixture <- expectArithFixture
  let contextGraph = emptyContextEGraph coarseAnatomyLattice (afBase fixture)
...
testContextInsertionDerivedViewOracle = do
  fixture <- expectArithFixture
  let contextGraph = emptyContextEGraph coarseAnatomyLattice (afBase fixture)
      leftTerm = Arith.addTermNode (Arith.numTerm 1) (Arith.numTerm 1)
  ...
  oracleSupportedGraph <-
    expectStep
      ( stageSupportOracle
          ...
          (emptyContextEGraph coarseAnatomyLattice oracleBase2)
      )
  oracleGraph <-
    expectStep
...
testGlobalStagingDegeneracy = do
  fixture <- expectArithFixture
  let contextGraph = emptyContextEGraph coarseAnatomyLattice (afBase fixture)
      termValue = Arith.mulTermNode (Arith.addTermNode (Arith.numTerm 1) (Arith.numTerm 2)) (Arith.numTerm 3)
...
stageSupportOracle ::
  ...
  [ClassId] ->
  ContextEGraphAnatomy ->
  Either String ContextEGraphAnatomy
