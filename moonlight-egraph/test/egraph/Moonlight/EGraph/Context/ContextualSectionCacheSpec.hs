...
type CacheView = Map Scope (IntMap (Maybe (Int, Int, ENode ArithF)))
...
cacheView ::
  ...
  CacheView
cacheView =
  fmap (fmap (fmap choiceKey) . ecsChoices . cesChoiceSection) . cscSections

advanceEqualsFreshBuild :: Assertion
advanceEqualsFreshBuild = do
  ...
  advancedCache <-
    expectRight
      (advanceContextualSections cacheBudget contextScope costOverlay costAlgebraValue report1 contextGraph1 cache0)
  freshCache <-
    expectRight
      (contextualSectionCacheBounded cacheBudget contextScope costOverlay costAlgebraValue contextGraph1)
  cacheView advancedCache @?= cacheView freshCache

compositionAcrossTwoCommits :: Assertion
compositionAcrossTwoCommits = do
  (sumClassId, oneClassId, contextGraph0) <- expectRight fixtureContextGraph
  let initialBatch = beginContextRebaseBatch contextGraph0
  cache0 <- expectRight (contextualSectionCacheBounded cacheBudget contextScope costOverlay costAlgebraValue contextGraph0)
  mergePlan <- expectRight (planContextMerges [ModuleCtx] sumClassId oneClassId initialBatch)
  ...
  advancedCache2 <-
    expectRight
      (advanceContextualSections cacheBudget contextScope costOverlay costAlgebraValue report2 contextGraph2 advancedCache1)
  freshCache <-
    expectRight
      (contextualSectionCacheBounded cacheBudget contextScope costOverlay costAlgebraValue contextGraph2)
  cacheView advancedCache2 @?= cacheView freshCache

foreignLineageObstructs :: Assertion
foreignLineageObstructs = do
  (sumClassId, oneClassId, contextGraph0) <- expectRight fixtureContextGraph
  let initialBatch = beginContextRebaseBatch contextGraph0
  cache0 <- expectRight (contextualSectionCacheBounded cacheBudget contextScope costOverlay costAlgebraValue contextGraph0)
  mergePlan <- expectRight (planContextMerges [ModuleCtx] sumClassId oneClassId initialBatch)
  ...
  (report2, contextGraph2) <-
    expectRight (commitContextRebaseBatch secondBatch)
  case advanceContextualSections cacheBudget contextScope costOverlay costAlgebraValue report2 contextGraph2 cache0 of
    Left (ContextualSectionContextLineageMismatch _ _) ->
      pure ()
    Left obstruction ->
      assertFailure ("expected lineage mismatch, got " <> show obstruction)
    Right cacheValue ->
      assertFailure ("expected lineage mismatch, got Right " <> show (cacheView cacheValue))
...
contextLocalMergeKeepsUntouchedFiberVerbatim = do
  (sumClassId, oneClassId, contextGraph0) <- expectRight fixtureContextGraph
  let initialBatch = beginContextRebaseBatch contextGraph0
  cache0 <- expectRight (contextualSectionCacheBounded cacheBudget contextScope costOverlay costAlgebraValue contextGraph0)
  mergePlan <- expectRight (planContextMerges [ModuleCtx] sumClassId oneClassId initialBatch)
...
cachedAndEphemeralAgreeForKeys ::
  Scope ->
  ContextEGraph ArithF NodeCount Scope ->
  [Int] ->
  QC.Property
