...
perContextUnionFindIsolationProperty scopes =
  withFixtureContextProperty $ \(sumClassId, oneClassId, contextGraph0) ->
...
contextCanonicalMapsAgree :: ContextEGraph owner ArithF NodeCount Scope -> Scope -> QC.Property
...
applyScopeMerge :: ClassId -> ClassId -> ContextEGraph owner ArithF NodeCount Scope -> Scope -> Either (ContextDeltaError ArithF Scope) (ContextEGraph owner ArithF NodeCount Scope)
...
fixtureContext useFixture = do
  ...
  pure $
    withEmptyContextEGraph incidenceScopeLattice graph3 $ \contextGraph ->
      useFixture (sumClassId, oneClassId, contextGraph)

withFixtureContextProperty ::
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF NodeCount Scope) -> QC.Property) ->
  QC.Property
withFixtureContextProperty useFixture =
  either
    (\contextError -> QC.counterexample (show contextError) False)
    id
    (fixtureContext useFixture)
