...
buildContextGraph :: ArithFixture -> AuthoredScript -> Either String ContextEGraphAnatomy
buildContextGraph fixture =
  foldlM
...
contextKeyAt ::
  ...
  c ->
  ContextEGraph f a c ->
  Either String ContextObjectKey
...
regionalCompositeRows ::
  ContextEGraph Arith.ArithF analysis c ->
...
testRegionalRowsAgreeWithOracle = do
  ...
  traverse_
    ( \script -> do
        contextGraph <- either assertFailure pure (buildContextGraph fixture script)
...
testRegionalRepresentativesAgreeWithOracle = do
  fixture <- expectRight arithFixture
  contextGraph <- either assertFailure pure (buildContextGraph fixture (layeredScript fixture))
  buckets <- expectRight (deriveAnnotatedDeltaBuckets contextGraph)
  traverse_
    ( \contextValue -> do
        contextKey <- either assertFailure pure (contextKeyAt contextValue contextGraph)
...
testEquivalentRegionPointwiseLaw = do
  ...
  contextGraph <- either assertFailure pure (buildContextGraph fixture (armScript fixture))
  buckets <- expectRight (deriveAnnotatedDeltaBuckets contextGraph)
  ...
  traverse_
    ( \contextValue -> do
        ...
        regionMemberKey equivalentRegion (contextObjectKeyIndex contextKey)
...
data BatchAnalysisFixture = BatchAnalysisFixture
  { bafGraph :: !(ContextEGraph Arith.ArithF BatchAnalysis AnatomyRegion),
    bafClassA :: !ClassId,
    bafClassB :: !ClassId,
    bafClassC :: !ClassId,
    bafAddAB :: !ClassId,
...
batchAnalysisFixture :: Either UnionFindAllocationError BatchAnalysisFixture
batchAnalysisFixture = do
  ...
  pure
    BatchAnalysisFixture
      { bafGraph = emptyContextEGraph coarseAnatomyLattice graph5,
        bafClassA = classA,
        bafClassB = classB,
        bafClassC = classC,
        bafAddAB = addAB,
...
batchAnalysisClasses :: BatchAnalysisFixture -> [ClassId]
...
assertAnalysisAgreesAt ::
  AnatomyRegion ->
  [ClassId] ->
  ContextEGraph Arith.ArithF BatchAnalysis AnatomyRegion ->
  Assertion
...
testContextualAnalysisOracle = do
  fixture <- expectRight batchAnalysisFixture
  contextGraph <-
    expectRight
      ( contextMerge
          ArmLeft
...
testBulkAuthorDescent = do
  ...
  let leftClass = afClassA fixture
      ...
      initialGraph = emptyContextEGraph coarseAnatomyLattice (afBase fixture)
...
testConstructionAfterMergeRejected = do
  fixture <- expectRight batchAnalysisFixture
  let initialGraph = bafGraph fixture
      initialBatch = beginContextRebaseBatch initialGraph
      assertConstructionRejected :: String -> ContextRebaseBatch Arith.ArithF a AnatomyRegion -> IO ()
...
testBaseRevisionSubsumesLocalTrace = do
  fixture <- expectRight batchAnalysisFixture
...
testSingletonDegeneration = do
  fixture <- expectRight arithFixture
  let emptyContextGraph = emptyContextEGraph soleLattice (afBase fixture)
  ...
  regionMemberKey (annotatedInhabitedRegion emptyBuckets) (contextObjectKeyIndex soleKey) @?= False
...
contextObjectKeyIndex :: ContextObjectKey -> Int
contextObjectKeyIndex (ContextObjectKey keyValue) = keyValue
