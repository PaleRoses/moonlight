...
data RegionalFixture context = RegionalFixture
  { rfClasses :: !(IntMap ClassId),
    rfGraph :: !(ContextEGraph Arith.ArithF Arith.NodeCount context)
  }
...
diamondFixture :: Either String (RegionalFixture DiamondCtx)
diamondFixture = do
  ...
  pure
    RegionalFixture
      { rfClasses = classes,
        rfGraph = emptyContextEGraph contextLattice baseGraph
      }
...
buildGeneratedScaleGeometries ::
  ...
  Either
    String
    [ ( Scale.ScaleSiteShape,
        Int,
        NonEmpty Scale.ScaleContext,
        RegionalFixture Scale.ScaleContext
      )
    ]
    ...
    buildGeometry (shape, extent) = do
      ...
      let contexts = Scale.scaleSiteContexts site
          initialFixture =
            RegionalFixture
              { rfClasses = classes,
                rfGraph = emptyContextEGraph (Scale.scaleSiteLattice site) baseGraph
              }
      finalFixture <- applyGeneratedMerges contexts initialFixture (ggsMerges script)
      pure (shape, extent, contexts, finalFixture)
...
buildGeneratedPowersetGeometries ::
  GeneratedGeometryScript ->
  Either String [(Int, NonEmpty (Set Char), RegionalFixture (Set Char))]
buildGeneratedPowersetGeometries script =
  ...
  where
    buildGeometry atomCount = do
      ...
      (classes, baseGraph) <- first show arithBase
      symbolicSite <- first show (symbolicPowersetSite atoms)
      contextCover <-
        maybe
      ...
      finalFixture <-
        applyGeneratedMerges
          ...
          (ggsMerges script)
      pure (atomCount, contextCover, finalFixture)

applyGeneratedMerges ::
  (Ord context, Show context) =>
  NonEmpty context ->
  RegionalFixture context ->
  [GeneratedMerge] ->
  Either String (RegionalFixture context)
...
emptyGeometrySaturatingGraph ::
  ContextEGraph Arith.ArithF Arith.NodeCount context ->
  SaturatingContextEGraph () Arith.ArithF Arith.NodeCount context
emptyGeometrySaturatingGraph =
  emptySaturatingContextEGraph

classAt :: Int -> RegionalFixture context -> Either String ClassId
...
applyStep ::
  RegionalFixture DiamondCtx ->
  RegionalStep ->
  Either String (RegionalFixture DiamondCtx)
...
fixtureAfter :: [RegionalStep] -> IO (RegionalFixture DiamondCtx)
fixtureAfter steps =
  expectRight (diamondFixture >>= (`applyScript` steps))

testSiblingIsolation :: Assertion
testSiblingIsolation = do
  fixture <- fixtureAfter [RegionalStep DLeft 0 1]
...
testDiamondOverlap = do
  fixture <- fixtureAfter overlapScript
  expectEquivalent True DLeft 0 1 fixture
...
testResidualCompression = do
  forwardFixture <- fixtureAfter overlapScript
  reverseFixture <- fixtureAfter (reverse overlapScript)
  traverse_
    ( \contextValue ->
        traverse_
          ( \(leftKey, rightKey) ->
...
testChildAncestorOrderIndependence = do
  baseFixture <- expectRight diamondFixture
  let childFirst =
        [ RegionalStep DLeft 0 1,
...
testSingletonBehavior = do
  ...
  let fixture =
        RegionalFixture
          { rfClasses = classes,
            rfGraph = emptyContextEGraph (singletonContextLattice SoleContext) baseGraph
          }
...
testDenseSymbolicAgreement = do
  workload <- expectRight (powersetTwinWorkload powersetTwinAtoms)
...
testTwentyAtomCompactness = do
  symbolicSite <- expectRight (fromPowersetAtoms ([0 .. 19] :: [Int]))
  preparedSiteEnumerability symbolicSite @?= SiteImplicitPowerset 20
  (classes, baseGraph) <- expectRight arithBase
  let graph0 :: ContextEGraph Arith.ArithF Arith.NodeCount TwentyAtomContext
      graph0 = emptyContextEGraphFromSite symbolicSite baseGraph
...
partitionSignature :: RegionalFixture DiamondCtx -> Either String [[Int]]
...
partitionLaw :: RegionalFixture DiamondCtx -> Either String Bool
...
regionalCompositeRowsForTag ::
  ContextEGraph Arith.ArithF analysis context ->
  AnnotatedDeltaBuckets Arith.ArithF ->
  ContextObjectKey ->
  Arith.ArithF () ->
  [(Int, [Int])]
...
propRegionalScripts (RegionalScript steps) =
  case diamondFixture of
    Left fixtureError -> QC.counterexample fixtureError False
    Right baseFixture ->
      case
        ...
        ( fmap
            ( \(shape, extent, contexts, fixture) ->
                QC.counterexample
                  ...
                  (geometryLawReport contexts fixture QC.=== Right completeGeometryLawReport)
            )
            geometries
        )
