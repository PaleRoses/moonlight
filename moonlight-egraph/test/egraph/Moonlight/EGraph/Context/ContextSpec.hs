...
tests =
  testGroup
    ...
    [ testCase "context merge propagates upward but not downward" $
        withResult
          ( fixtureModuleMergedContextGraph $ \(sumClassId, oneClassId, mergedContextGraph) -> do
      ...
      testCase "context refresh produces sheaf propagation report" $
        withResult
          ( fixtureModuleMergedContextGraph $ \(_, _, mergedContextGraph) -> do
      ...
      testCase "enumerable objects are the inhabited join closure, not the site universe" $
        withResult
          ( fixtureContextGraph $ \(_, _, contextGraph) ->
