...
classesEquivalentAt :: BodyRegion -> ClassId -> ClassId -> ContextEGraph owner ArithF NodeCount BodyRegion -> Bool
...
contextPropagationTests =
  testGroup
    "context propagation"
    [ testCase "merge at creature propagates to all regions" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        ...
        assertBool "creature merge should be visible at every region"
          (all
             ...
             allRegions),
      testCase "merge at paw-front stays local to paw-front and body-local" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
      ...
      testCase "merge at torso propagates to ribcage, spine, and body-local but not head" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        ...
        classesEquivalentAt Creature termA termB mergedGraph @?= False,
      testCase "merge at head propagates to jaw, cranium, body-local but not torso subtree" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        ...
        classesEquivalentAt Creature termA termB mergedGraph @?= False,
      testCase "global merge makes equivalence visible everywhere" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
      ...
      testCase "two independent context merges do not interfere" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
      ...
      testCase "chained merges at increasing specificity accumulate" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        (termC, termD, contextGraphWithExtra) <- expectRight (fixtureBodyGraphExtended baseContextGraph)
      ...
      testCase "context find returns canonical representative" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
        contextClassAt Torso termA mergedGraph @?= contextClassAt Torso termB mergedGraph,
      testCase "propagation report is present after context merge" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Head termA termB baseContextGraph)
        contextPropagationSettled mergedGraph @?= True
        contextPropagationFailed mergedGraph @?= False,
      testCase "central context merge applies context merge semantics" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
        classesEquivalentAt Torso termA termB mergedGraph @?= True
        classesEquivalentAt Head termA termB mergedGraph @?= False,
      testCase "context incidence nerve site materializes on demand from snapshot" $ withFixtureBodyGraph $ \(_, _, baseContextGraph) -> do
      ...
      testCase "context incidence representatives agree with direct graph after local merge" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
      ...
      testCase "context incidence category reflects context-local insertion before merge" $ withFixtureBodyGraph $ \(termA, _, baseContextGraph) -> do
      ...
      testCase "global multi-node term insertion reports every fresh node" $
        withEmptyContextEGraph bodyLattice (emptyEGraph analysisSpec) $ \baseContextGraph -> do
      ...
      testCase "context trace records per-context local unions without overcounting propagated effect" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
      ...
      testCase "context trace records support touches only when support changes" $
        withEmptyContextEGraph bodyLattice (emptyEGraph analysisSpec) $ \baseContextGraph -> do
      ...
      testCase "context term staging supports unchanged compound footprint" $
        withEmptyContextEGraph bodyLattice (emptyEGraph analysisSpec) $ \baseContextGraph -> do
        let compoundTerm =
...
restrictionMapTests =
  testGroup
    "restriction maps"
    [ testCase "restriction from ribcage to torso exists (torso leq ribcage)" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
        assertBool "restriction ribcage -> torso should exist"
          (isRight (restrictionMap Ribcage Torso mergedGraph)),
      testCase "restriction from torso to head is typed failure (incomparable)" $ withFixtureBodyGraph $ \(_, _, baseContextGraph) -> do
        isLeft (restrictionMap Torso Head baseContextGraph) @?= True,
      testCase "restriction from paw-front to creature exists" $ withFixtureBodyGraph $ \(_, _, baseContextGraph) -> do
        assertBool "restriction paw-front -> creature should exist"
          (isRight (restrictionMap PawFront Creature baseContextGraph)),
      testCase "restriction from jaw to paw-hind is typed failure (incomparable)" $ withFixtureBodyGraph $ \(_, _, baseContextGraph) -> do
        isLeft (restrictionMap Jaw PawHind baseContextGraph) @?= True,
      testCase "restriction maps compose across creature-torso-ribcage chain" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
      ...
      testCase "restrict-to-context produces a coherent base graph" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
...
obstructionTests =
  testGroup
    "obstructions"
    [ testCase "obstruction report non-empty when classes merged only at leaf" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge PawFront termA termB baseContextGraph)
      ...
      testCase "context barrier present when equivalence is context-dependent" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
      ...
      testCase "no obstructions when classes are equivalent at queried context" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
        assertBool "expected empty obstruction report" (null (obstructionReport termA termB Ribcage mergedGraph)),
      testCase "obstruction completeness holds for torso-scoped merge queried at creature" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge Torso termA termB baseContextGraph)
        obstructionComplete termA termB Creature mergedGraph @?= True,
      testCase "restriction barrier present when sheaf condition detects inconsistency" $ withFixtureBodyGraph $ \(termA, termB, baseContextGraph) -> do
        mergedGraph <- expectRight (contextMerge PawFront termA termB baseContextGraph)
...
fixtureBodyGraph ::
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF NodeCount BodyRegion) -> result) ->
  Either (ContextDeltaError ArithF BodyRegion) result
fixtureBodyGraph useFixture = do
  ...
  pure $
    withEmptyContextEGraph bodyLattice graph3 $ \contextGraph ->
      useFixture (sumClassId, oneClassId, contextGraph)

withFixtureBodyGraph ::
  (forall owner. (ClassId, ClassId, ContextEGraph owner ArithF NodeCount BodyRegion) -> Assertion) ->
  Assertion
withFixtureBodyGraph useFixture =
  either (assertFailure . show) id (fixtureBodyGraph useFixture)

diagnosticFullContextRebase :: ContextEGraph owner ArithF NodeCount BodyRegion -> EGraph ArithF NodeCount -> Either (ContextDeltaError ArithF BodyRegion) (ContextEGraph owner ArithF NodeCount BodyRegion)
...
fixtureBodyGraphExtended :: ContextEGraph owner ArithF NodeCount BodyRegion -> Either (ContextDeltaError ArithF BodyRegion) (ClassId, ClassId, ContextEGraph owner ArithF NodeCount BodyRegion)
...
isGlobalEquivalence :: ClassId -> ClassId -> ContextEGraph owner f a c -> Bool
