...
import Moonlight.EGraph.Pure.Context (activateContext, contextMerge, withEmptyContextEGraph)
  ...
  testGroup "diagnostic" . hunitCases $
    [ HUnitCase "snapshotEGraph reports class and pending merge structure" $ do
      ...
      HUnitCase "summarizeProofLog measures provenance compression" $ do
        ...
        withEmptyContextEGraph diagnosticLattice graph $ \contextGraph -> do
          let canonicalize = canonicalizeClassId (cegBase contextGraph)
      ...
      HUnitCase "contextDiagnostic reports sheaf propagation state" $ do
        ...
        withEmptyContextEGraph diagnosticLattice graph $ \emptyContextGraph ->
          withResult (contextMerge ModuleCtx sumClassId oneClassId emptyContextGraph) $ \mergedGraph ->
...
frontSeedGraph :: (forall owner. FrontDiagnosticProgram owner ()) -> IO FrontDiagnosticGraph
frontSeedGraph program =
  FrontTiny.withEmptyFrontGraph $ \emptyFrontGraph -> do
    report <- FrontTiny.expectFront (runEGraphFront program emptyFrontGraph)
...
tracedArithConfig :: SaturationBudget -> EGraphSaturationConfig UnitContextSiteOwner SurfaceKind ArithF NodeCount ()
...
frontOneSeedProgram :: FrontDiagnosticProgram owner ()
frontOneSeedProgram =
  egraph $ do
    _ <- def @"one" FrontTiny.one
    pure done

frontOneSumSeedProgram :: FrontDiagnosticProgram owner ()
...
frontThreeSeedProgram :: FrontDiagnosticProgram owner ()
...
type FrontDiagnosticProgram owner result =
  EGraphFront 'Authored owner FrontTiny.FrontTinySig FrontTiny.NodeCount FrontTiny.FrontTinyContext result
