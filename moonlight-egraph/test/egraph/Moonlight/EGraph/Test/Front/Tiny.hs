...
import Moonlight.EGraph.Pure.Types (EGraph, emptyEGraphWithTheory)
...
withEmptyFrontGraph ::
  (forall owner. SaturatingContextEGraph owner SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext -> IO result) ->
  IO result
withEmptyFrontGraph useGraph =
  withEmptyContextEGraph
    (singletonContextLattice BaseOnly)
    emptyFrontBaseGraph
...
withContextualFrontGraph ::
  (forall owner. SaturatingContextEGraph owner SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext -> IO result) ->
  IO result
withContextualFrontGraph useGraph =
  case latticeContext of
    Right latticeValue ->
      withEmptyContextEGraph
        ...
        (useGraph . emptySaturatingContextEGraph)
    Left compileError ->
      assertFailure ("invalid FrontTinyContext lattice fixture: " <> show compileError)

emptyFrontBaseGraph :: EGraph (PackedNode FrontTinySig) NodeCount
emptyFrontBaseGraph =
...
expectFront :: Either (EGraphFrontError owner FrontTinySig NodeCount FrontTinyContext) value -> IO value
expectFront =
  \case
    Right value -> pure value
    Left err -> assertFailure (frontErrorMessage err)

expectCompiled :: EGraphFront 'Authored owner FrontTinySig NodeCount FrontTinyContext result -> Assertion
