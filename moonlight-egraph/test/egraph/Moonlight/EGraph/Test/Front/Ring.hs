...
emptyRingGraph :: SaturatingContextEGraph SurfaceKind (PackedNode RingSig) NodeCount ()
emptyRingGraph =
  emptySaturatingContextEGraph $
...
type RingSaturationReport =
  SaturationReport (EGraphU SurfaceKind (PackedNode RingSig) NodeCount ())
...
boundedRingExtraction :: EGraphFrontReport RingSig NodeCount () () -> IO (Maybe RingExtraction)
boundedRingExtraction frontReport =
...
ringExtractionBudget :: ExtractionFixpointBudget
ringExtractionBudget =
  ExtractionFixpointBudget 4096

singleSaturationReport :: EGraphFrontReport RingSig NodeCount () result -> IO RingSaturationReport
singleSaturationReport frontReport =
...
type RingFrontProgram result =
  EGraphFront 'Authored RingSig NodeCount () result

expectFront :: Either (EGraphFrontError RingSig NodeCount ()) value -> IO value
expectFront =
  \case
