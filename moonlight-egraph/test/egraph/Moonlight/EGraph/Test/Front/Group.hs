...
assertGroupEquivalent left right = do
  report <- expectFront (runEGraphFront (equivalenceProgram left right) emptyGroupGraph)
  efrResult report @?= True

equivalenceProgram :: Term GroupSig "Group" -> Term GroupSig "Group" -> EGraphFront 'Authored GroupSig GroupDepth GroupContext Bool
...
emptyGroupGraph :: SaturatingContextEGraph SurfaceKind (PackedNode GroupSig) GroupDepth GroupContext
emptyGroupGraph =
  emptySaturatingContextEGraph $
    emptyContextEGraph (singletonContextLattice GroupContext) $
...
expectFront :: Either (EGraphFrontError GroupSig GroupDepth GroupContext) value -> IO value
expectFront =
  \case
