...
mkRequest ::
  MatchingAlgebra state UnitContextSiteOwner String SurfaceKind RingF NodeCount ->
  GenericMatching.MatchSite String ->
  Maybe (AnnotatedContextSource UnitContextSiteOwner RingF) ->
  CompiledPatternQuery (CompiledGuard SurfaceKind RingF) RingF ->
  EGraph RingF NodeCount ->
  MatchingRequest UnitContextSiteOwner String SurfaceKind RingF NodeCount
...
mkConfig :: MatchingStrategy UnitContextSiteOwner String SurfaceKind RingF NodeCount -> SaturationConfig (EGraphU UnitContextSiteOwner SurfaceKind RingF NodeCount String) RewriteRuleId
mkConfig strategy =
  SaturationConfig
    { scBudget =
        SaturationBudget
