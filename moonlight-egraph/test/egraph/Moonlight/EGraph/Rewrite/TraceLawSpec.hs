...
tests =
  testGroup
    ...
    [ testCase "matching projection is a mutation-trace homomorphism" $ do
      ...
      testCase "matching projection is a context-trace homomorphism" $ do
        (sumClassId, oneClassId, contextGraph0) <- expectRight fixtureContextGraph
      ...
      testCase "saturation summary preserves application trace lineage order" $ do
        (sumClassId, oneClassId, contextGraph0) <- expectRight fixtureContextGraph
        let initialBatch = beginContextRebaseBatch contextGraph0
        mergePlan <- expectRight (planContextMerges [ModuleCtx] sumClassId oneClassId initialBatch)
        firstBatch <- expectRight (stageContextMerges mergePlan initialBatch)
      ...
      testCase "saturation summary projects rebuild dirty keys to context fibers" $ do
        (_sumClassId, _oneClassId, contextGraph0) <- expectRight fixtureContextGraph
      ...
      testCase "saturation summary adds proof artifact construction counts" $
        let firstSummary :: EGraphSaturationChangeSummary Scope ArithF
            firstSummary =
              mempty
                { egscProofRestrictionRegistryConstructions = 2,
                  egscProofExtractionTableConstructions = 3
                }
            secondSummary :: EGraphSaturationChangeSummary Scope ArithF
            secondSummary =
              mempty
      ...
      testCase "extraction cache advance composes over appended traces" $ do
        ...
        let appendedAdvance =
              advanceExtractionChoiceCache cacheBudget arithChoiceAlgebra appendedTrace graph2 cacheValue
            sequentialAdvance =
              advanceExtractionChoiceCache cacheBudget arithChoiceAlgebra firstTrace graph1 cacheValue
                >>= advanceExtractionChoiceCache cacheBudget arithChoiceAlgebra secondTrace graph2
        assertBool
      ...
      testCase "empty extraction traces preserve caches under append" $ do
        ...
        cacheValue <- expectCache (extractionChoiceCacheFromStableGraph cacheBudget arithChoiceAlgebra graph0)
        advancedCache <- expectCache (advanceExtractionChoiceCache cacheBudget arithChoiceAlgebra appendedTrace graph0 cacheValue)
        assertBool
      ...
      testCase "proof projection accepts insertions and obstructs proofless unions" $ do
        ...
        let EGraphMutationResult
            ...
            proofGraph :: ProofEGraph ArithF NodeCount Scope ()
            proofGraph = emptyProofEGraph (emptyContextEGraph traceScopeLattice graph2)
