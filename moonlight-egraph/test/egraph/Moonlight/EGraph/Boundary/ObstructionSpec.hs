...
tests =
  testGroup
    ...
    [ testCase "obstruction report is non-empty when classes only merge contextually" $
        withResult fixtureModuleMergedContextGraph $ \(sumClassId, oneClassId, contextGraph) -> do
      ...
      testCase "obstruction report includes restriction barrier when gluing fails across contexts" $
        withResult fixtureModuleMergedContextGraph $ \(sumClassId, oneClassId, contextGraph) ->
