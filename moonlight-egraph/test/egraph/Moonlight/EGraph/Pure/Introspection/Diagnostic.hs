  ...
  EGraphSnapshot
    { egsClasses =
        fmap
          ...
          (IntMap.elems (eGraphClasses graph)),
...
graphDiagnostic graph =
  let classSizes = fmap (Set.size . eClassNodes) (eGraphClasses graph)
   in EGraphDiagnostic
        { egdClassCount = eGraphClassCount graph,
          egdNodeCount = eGraphNodeCount graph,
          egdHashConsCount = Map.size (eGraphHashCons graph),
  ...
  let cachedContexts =
   ...
   in ContextDiagnostic
        { cdCachedContexts = cachedContexts,
          cdRestrictionCount = length (preparedRestrictionPairs (cegSite contextGraph)),
...
contextStructureSnapshot :: (Language f, Ord c) => ContextEGraph f a c -> Either (PreparedContextSupportError c) ContextStructureSnapshot
  ...
  pure
    ContextStructureSnapshot
        { cssCachedContextCount =
          ...
          cssRestrictionCount =
            length (preparedRestrictionPairs (cegSite contextGraph)),
...
renderEGraphDotWith renderAnalysis graph =
  unlines
    ( ["digraph egraph {"]
        <> (IntMap.elems (eGraphClasses graph) >>= renderEClass renderAnalysis)
