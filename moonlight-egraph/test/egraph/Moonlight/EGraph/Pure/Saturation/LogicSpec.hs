...
type TestU = EGraphU SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext

type TestGraph = SaturatingContextEGraph SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext
...
mutationOwnerGuard = do
  report <- expectLogicReport
  assertBool
    "seed facts must not mutate the graph carrier"
    (graphConvergenceStateEquals @TestU contextualFrontGraph (srCarrier (elrSaturation report)))

runObservationLogic :: EGraphLogic SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext
runObservationLogic =
  logic $ do
...
runObservationPlumbingGuard = do
...
expectLogicReport :: IO (EGraphLogicReport SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext)
expectLogicReport =
...
testLogic :: EGraphLogic SurfaceKind (PackedNode FrontTinySig) NodeCount FrontTinyContext
testLogic =
  logic $ do
    seedFacts BaseSite (factStoreFor 10)
    seedFacts (ContextSite Rain) (factStoreFor 20)
...
testRunSpec ::
  ContextRunSpec
    ...
    TestGraph
    RewriteRuleId
    (SaturationReport TestU)
testRunSpec =
  plainContextRunSpec testPlanSpec mempty

testPlanSpec :: PlanSpec TestU TestGraph RewriteRuleId
testPlanSpec =
  defaultPlanSpec @TestU @RewriteRuleId defaultBudget GenericJoinMatching

seedOnlyObservationProgram :: EGraphFront 'Authored FrontTinySig NodeCount FrontTinyContext (MaybeExtraction Int)
