              ...
              Right (graph0, [c1, c2, _]) -> do
                compiledQuery <-
                  case compileRingPatternQuery addXYPattern of
                    ...
                    Right query ->
                      pure query
                let (rebuildDelta, repairIndex, rebuiltGraph) =
                      ...
                      GenericMatching.maAdvanceState
                        ...
                        MatchingAdvanceCtx
                          { macGraph = rebuiltGraph,
                            macRepairIndex = Just repairIndex,
                            macCanonicalize = id,
...
regionalPreparedRootFilterLaw =
  ...
  of
    Right (baseGraph, firstRoot : _) -> do
      ...
      let contextGraph :: ContextEGraph RingF NodeCount ()
          contextGraph =
            emptyContextEGraph trivialLattice baseGraph
    ...
    Right (baseGraph, variableClass : oneClass : zeroClass : _) -> do
      ...
      firstContextGraph <-
        expectRegionalRight
          ...
          (contextMerge () variableClass oneClass (emptyContextEGraph trivialLattice baseGraph))
      secondContextGraph <-
        expectRegionalRight
