...
requireScenario ::
  LambdaBindingHarness ctx f a ->
  LambdaGoalScenario ->
  IO (LambdaGoalRun ctx f a)
requireScenario harness scenario =
  requireRight
    ("scenario " <> show scenario)
    (lbhRunScenario harness scenario)

requireContext :: String -> LambdaGoalRun ctx f a -> IO ctx
requireContext label runValue =
  requireRight ("context " <> show label) (lookupNamedContext label runValue)

requireClass :: String -> String -> LambdaGoalRun ctx f a -> IO ClassId
...
requireNormalForm :: String -> String -> LambdaGoalRun ctx f a -> IO String
...
requireMetricIO :: String -> LambdaGoalRun ctx f a -> IO Int
...
contextCount :: (Language f, Ord ctx) => LambdaGoalRun ctx f a -> Int
contextCount =
  Map.size . lgrNamedContexts

runCore :: LambdaGoalRun ctx f a -> LambdaGoalCore ctx f a
runCore =
  lgrCore

runGraph :: LambdaGoalRun ctx f a -> ContextEGraph f a ctx
runGraph =
  lgcGraph . lgrCore

equivalentAt ::
  (Language f, Ord ctx) =>
  LambdaGoalRun ctx f a ->
  String ->
  String ->
  String ->
...
test_alphaEquivalenceAsSheafCondition ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_alphaEquivalenceAsSheafCondition harness = do
  runValue <- requireScenario harness AlphaEquivalenceScenario
...
test_dynamicBetaAndLatticeGrowth ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_dynamicBetaAndLatticeGrowth harness = do
  runValue <- requireScenario harness DynamicBetaScenario
...
test_captureAvoidanceViaBindingObstruction ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_captureAvoidanceViaBindingObstruction harness = do
  runValue <- requireScenario harness CaptureAvoidanceScenario
...
test_etaPositiveAndNegative ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_etaPositiveAndNegative harness = do
  runValue <- requireScenario harness EtaScenario
...
test_letFloatPositiveAndNegative ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_letFloatPositiveAndNegative harness = do
  runValue <- requireScenario harness LetFloatScenario
...
test_latticeGrowthPreservesEqualities ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_latticeGrowthPreservesEqualities harness = do
  runValue <- requireScenario harness LatticeGrowthScenario
...
test_deepNestingContextGrowth ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_deepNestingContextGrowth harness = do
  let depth = 32
  runValue <- requireScenario harness (DeepNestingScenario depth)
...
test_profiledScopeSensitiveWorkload ::
  ...
  LambdaBindingHarness ctx f a ->
  Assertion
test_profiledScopeSensitiveWorkload harness = do
  ...
  runValue <- requireScenario harness (ProfiledScopeSensitiveScenario binderCount)
