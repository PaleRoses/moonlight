...
type GoalRun = LambdaGoalRun Ctx (PackedNode ScopedLambdaSig) LamAnalysis

type GoalGraph = ContextEGraph (PackedNode ScopedLambdaSig) LamAnalysis Ctx

bindingHarness :: LambdaBindingHarness Ctx (PackedNode ScopedLambdaSig) LamAnalysis
bindingHarness =
  LambdaBindingHarness
    { lbhRunScenario = \case
        AlphaEquivalenceScenario -> runAlpha
        DynamicBetaScenario -> runBeta
        CaptureAvoidanceScenario -> runCapture
        EtaScenario -> runEta
        LetFloatScenario -> runLetFloat
        LatticeGrowthScenario -> runLatticeGrowth
        DeepNestingScenario depth -> runDeepNesting depth
        ProfiledScopeSensitiveScenario binderCount -> runProfiledScopeSensitive binderCount
    }

runAlpha :: Either String GoalRun
runAlpha = do
...
runCapture :: Either String GoalRun
runCapture = do
...
runLetFloat :: Either String GoalRun
runLetFloat = do
  lattice <- chainLattice []
  runFrontGoal
    lattice
...
runLatticeGrowth :: Either String GoalRun
runLatticeGrowth = do
...
runDeepNesting :: Int -> Either String GoalRun
runDeepNesting depth = do
...
runProfiledScopeSensitive :: Int -> Either String GoalRun
runProfiledScopeSensitive binderCount = do
...
runFrontGoal ::
  ...
  [((String, String), String)] ->
  [(String, Int)] ->
  EGraphFront 'Authored ScopedLambdaSig LamAnalysis Ctx () ->
  Either String GoalRun
runFrontGoal lattice contexts classes norms metrics =
  runFrontGoalWith lattice contexts classes norms metrics (const (Right []))
...
runFrontGoalWith ::
  ...
  EGraphFront 'Authored ScopedLambdaSig LamAnalysis Ctx result ->
  Either String GoalRun
runFrontGoalWith lattice contexts classes norms metrics extraMetrics program = do
  report <-
    first frontErrorMessage $
      runEGraphFront program (emptyScopedLambdaGraph lattice)
...
seedClassEntry ::
  GoalGraph ->
  EGraphFrontReport ScopedLambdaSig LamAnalysis Ctx result ->
  ((String, String), String) ->
...
seedClass :: GoalGraph -> EGraphFrontReport ScopedLambdaSig LamAnalysis Ctx result -> String -> Either String ClassId
...
scheduleIterations :: EGraphFrontReport ScopedLambdaSig LamAnalysis Ctx result -> Int
scheduleIterations =
  sum . fmap (reportIterationCount . elrSaturation) . efrScheduleReports

scheduleMatches :: EGraphFrontReport ScopedLambdaSig LamAnalysis Ctx result -> Int
scheduleMatches =
