...
import Moonlight.EGraph.Pure.Change (EGraphMutationResult (emrGraph))
...
import Moonlight.Graph.Pure.LocalTopology (cyclicCellsFromChildren)
        ...
        let graph0 = emptyEGraph seededAnalysisSpec
         in withResult (addTerm onePlusTwo graph0) $ \(leftParent, graph1) ->
              withResult (addTerm threePlusFour graph1) $ \(rightParent, graph2) ->
                withResult (fst <$> addTerm one graph2) $ \oneClass ->
                  withResult (fst <$> addTerm two graph2) $ \twoClass ->
                    withResult (fst <$> addTerm three graph2) $ \threeClass ->
                      withResult (fst <$> addTerm four graph2) $ \fourClass ->
                        withResult (seedExternalEvidence (ENode (Add oneClass twoClass)) 101 graph2) $ \graph3 ->
                          withResult (seedExternalEvidence (ENode (Add threeClass fourClass)) 202 graph3) $ \seededGraph ->
                            let rebuiltGraph = rebuild (merge leftParent rightParent seededGraph)
...
seedExternalEvidence :: ENode ArithF -> Int -> EGraph ArithF SeededAnalysis -> Either UnionFindAllocationError (EGraph ArithF SeededAnalysis)
seedExternalEvidence enode evidence graph =
  fmap emrGraph
    ( insertENodeTracked
        enode
        SeededAnalysis
          { saStructuralObservations = Set.empty,
            saExternalEvidence = Set.singleton evidence
          }
        graph
    )
...
assertCyclicKeys expectedKeys childEdges =
  cyclicCellsFromChildren (childMulti childEdges) @?= Right (IntSet.fromList expectedKeys)
...
data RoundRebuildSeed owner = RoundRebuildSeed
  { rsClass1 :: !ClassId,
    rsClass2 :: !ClassId,
    rsClass3 :: !ClassId,
    rsParent1 :: !ClassId,
    rsParent3 :: !ClassId,
    rsGraphWithPendingMerge :: !(ContextEGraph owner ArithF NodeCount Scope)
  }

data RoundRebuildPair owner = RoundRebuildPair
  { rrpLeftReport :: !(RoundRebuildReport owner SurfaceKind ArithF NodeCount Scope),
    rrpRightReport :: !(RoundRebuildReport owner SurfaceKind ArithF NodeCount Scope)
  }

roundRebuildSeed ::
  (forall owner. (RoundRebuildSeed owner, [(ClassId, ClassId)]) -> result) ->
  Either UnionFindAllocationError result
roundRebuildSeed useSeed = do
  ...
  pure $
    moduleContextGraph (merge class4 class5 graph) $ \contextGraph ->
      useSeed
        ( RoundRebuildSeed
            { rsClass1 = class1,
              rsClass2 = class2,
              ...
              rsParent1 = parent1,
              rsParent3 = parent3,
              rsGraphWithPendingMerge = contextGraph
            },
          [(class2, class1), (class3, class1)]
        )

withRoundRebuildSeed ::
  (forall owner. (RoundRebuildSeed owner, [(ClassId, ClassId)]) -> Assertion) ->
  Assertion
withRoundRebuildSeed useSeed =
  withResult (roundRebuildSeed useSeed) id

runRoundRebuildPair :: RoundRebuildSeed owner -> Either String (RoundRebuildPair owner)
...
leftLocalUnions :: RoundRebuildSeed owner -> [(ClassId, ClassId)]
leftLocalUnions seed =
  [(rsClass1 seed, rsClass2 seed), (rsClass2 seed, rsClass3 seed)]

rightLocalUnions :: RoundRebuildSeed owner -> [(ClassId, ClassId)]
rightLocalUnions seed =
  [(rsClass2 seed, rsClass3 seed), (rsClass1 seed, rsClass2 seed)]

rebuiltContextGraph :: RoundRebuildReport owner SurfaceKind ArithF NodeCount Scope -> ContextEGraph owner ArithF NodeCount Scope
rebuiltContextGraph =
  sceContextGraph . rrrGraph

roundRebuildContextGraph ::
  [(ClassId, ClassId)] ->
  RoundRebuildSeed owner ->
  Either String (ContextEGraph owner ArithF NodeCount Scope)
