...
scopedLambdaGraph ::
  ...
  Either String (SaturatingContextEGraph SurfaceKind (PackedNode ScopedLambdaSig) LamAnalysis ScopedLambdaContext)
scopedLambdaGraph bindingPlan =
  emptyScopedLambdaGraph <$> scopedLambdaLattice (bindingPlanContexts bindingPlan)

emptyScopedLambdaGraph ::
  Ord context =>
  ContextLattice context ->
  SaturatingContextEGraph SurfaceKind (PackedNode ScopedLambdaSig) LamAnalysis context
emptyScopedLambdaGraph lattice =
  emptySaturatingContextEGraph
    (emptyContextEGraph lattice (emptyEGraphWithTheory (packAnalysisSpec (monoAnalysisSpec lamAnalysisSpec)) emptyTheorySpec))
