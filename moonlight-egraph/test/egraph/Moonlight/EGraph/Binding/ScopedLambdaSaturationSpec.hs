...
scopedLambdaGoldenPath = do
  ...
  observation <- expectRight "scoped lambda graph and program" (runScopedLambdaProgram bindingPlan)
  let result = sloResult observation
      iterations = sloIterations observation
      matches = sloMatches observation
      flagSummary =
        "captureAlpha="
  ...
  assertBool
    ...
    (all (`elem` sloPreparedContexts observation) [ScopedBinder, ScopedBody])

data ScopedLambdaObservation = ScopedLambdaObservation
  { sloResult :: !ScopedLambdaResult,
    sloIterations :: !Int,
    sloMatches :: !Int,
    sloPreparedContexts :: ![ScopedLambdaContext]
  }

runScopedLambdaProgram ::
  BindingPlan ScopedLambdaSig ScopedLambdaContext ->
  Either String ScopedLambdaObservation
runScopedLambdaProgram bindingPlan =
  scopedLambdaGraph
    bindingPlan
    ( \graph -> do
        report <- first frontErrorMessage (runEGraphFront scopedLambdaProgram graph)
        result <- first show (efrResult report)
        pure
          ScopedLambdaObservation
            { sloResult = result,
              sloIterations = scheduleIterations report,
              sloMatches = scheduleMatches report,
              sloPreparedContexts = contextSiteWitness report
            }
    )
    >>= id

scopedLambdaProgram :: EGraphFront 'Authored owner ScopedLambdaSig LamAnalysis ScopedLambdaContext (Either ScopedLambdaProgramError ScopedLambdaResult)
scopedLambdaProgram =
  egraph $ do
...
scheduleIterations :: EGraphFrontReport owner ScopedLambdaSig LamAnalysis ScopedLambdaContext result -> Int
scheduleIterations =
  sum . fmap (reportIterationCount . elrSaturation) . efrScheduleReports

scheduleMatches :: EGraphFrontReport owner ScopedLambdaSig LamAnalysis ScopedLambdaContext result -> Int
scheduleMatches =
  sum . fmap (reportMatchesApplied . elrSaturation) . efrScheduleReports

contextSiteWitness :: EGraphFrontReport owner ScopedLambdaSig LamAnalysis ScopedLambdaContext result -> [ScopedLambdaContext]
contextSiteWitness =
...
expectFrontReport :: Either (EGraphFrontError owner ScopedLambdaSig LamAnalysis ScopedLambdaContext) value -> IO value
expectFrontReport =
  either
