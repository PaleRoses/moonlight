...
contextGraphDigest :: ContextGraph.ContextEGraph owner f analysis context -> Int
...
data SemanticQuery owner context = SemanticQuery
  { semanticQueryContextKey :: !(Site.ContextObjectKey owner),
...
data SemanticDigestObstruction owner context
  = SemanticContextSupportObstruction !(Site.PreparedContextSupportError context)
  | SemanticContextPayloadRepresentativeMissing !(Site.ContextObjectKey owner) !Int
  | SemanticProductContextMissing !(Site.ContextObjectKey owner) !context
  deriving stock (Eq, Show)
...
data QuotientObservation owner = QuotientObservation
  !(Site.ContextObjectKey owner)
  ![(Int, Int)]
  ![(Int, Maybe Int)]
  deriving stock (Eq, Show)
...
data ContextRowObservation owner = ContextRowObservation
  { contextRowContextKey :: !(Site.ContextObjectKey owner),
...
data ContextSemanticObservation owner = ContextSemanticObservation
  { contextObservedQuotients :: ![QuotientObservation owner],
    contextObservedVariantRows :: ![ContextRowObservation owner],
    contextObservedAbsorbedRows :: ![ContextRowObservation owner],
    contextObservedRegionalStructure :: !RegionalStructureObservation
  }
  deriving stock (Eq, Show)

semanticQueriesForClasses :: [(Site.ContextObjectKey owner, context)] -> [ClassId] -> [SemanticQuery owner context]
...
deriveContextSemanticQueries ::
  Ord context =>
  [(Site.ContextObjectKey owner, context)] ->
  ContextGraph.ContextEGraph owner f analysis context ->
  Either (SemanticDigestObstruction owner context) [SemanticQuery owner context]
...
contextQuotientObservations ::
  ...
  (analysis -> Int) ->
  [SemanticQuery owner context] ->
  ContextGraph.ContextEGraph owner f analysis context ->
  Either (SemanticDigestObstruction owner context) [QuotientObservation owner]
...
contextQuotientDigest ::
  ...
  [SemanticQuery owner context] ->
  ContextGraph.ContextEGraph owner f analysis context ->
  Either (SemanticDigestObstruction owner context) Int
...
contextSemanticObservation ::
  ...
  [SemanticQuery owner context] ->
  ContextGraph.ContextEGraph owner f analysis context ->
  Either (SemanticDigestObstruction owner context) (ContextSemanticObservation owner)
...
semanticObservationDigest :: ContextSemanticObservation owner -> Int
...
contextRowObservations ::
  (Site.ContextObjectKey owner -> Annotated.AnnotatedDeltaBuckets owner f -> Map.Map (f ()) [(Int, [Int])]) ->
  Annotated.AnnotatedDeltaBuckets owner f ->
  [SemanticQuery owner context] ->
  [ContextRowObservation owner]
    ...
    taggedRows =
      Map.fromListWith (<>)
        [ ( void nodeShape,
          ...
          | (rootKey, graphClass) <- IntMap.toAscList (Graph.materializeEGraphClasses graph),
...
quotientObservationDigest :: QuotientObservation owner -> Int
...
contextRowDigest :: ContextRowObservation owner -> Int
