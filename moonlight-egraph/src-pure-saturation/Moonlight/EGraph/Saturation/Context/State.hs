...
type SaturatingContextEGraph :: Type -> (Type -> Type) -> Type -> Type -> Type
data SaturatingContextEGraph capability f a c = SaturatingContextEGraph
  { sceContextGraph :: !(ContextEGraph f a c),
    sceSaturationState :: !(ContextSaturationState capability f)
  }

type SaturatingProofEGraph :: Type -> (Type -> Type) -> Type -> Type -> Type -> Type
type SaturatingProofEGraph capability f a c p =
  ProofGraph (SaturatingContextEGraph capability f a c) f c p
...
mapSaturatingContextGraph ::
  (ContextEGraph f a c -> ContextEGraph f a c) ->
  SaturatingContextEGraph capability f a c ->
  SaturatingContextEGraph capability f a c
