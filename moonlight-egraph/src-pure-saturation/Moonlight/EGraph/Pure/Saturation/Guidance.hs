...
egraphSupportGuidance ::
  forall owner capability f a c schedulerKey.
...
egraphSupportGuidance runtimeCapabilities =
  ...
  where
    projectSupportedByInner rawMatches supportedMatches =
      let retainedMatches = Map.fromList [(matchKey @(EGraphU owner capability f a c) match, match) | match <- rawMatches]
       in mapMaybe
            ( \sm ->
                fmap
                  (\m -> setSupportedMatchInner @(EGraphU owner capability f a c) m sm)
                  (Map.lookup (matchKey @(EGraphU owner capability f a c) (supportedMatchInner @(EGraphU owner capability f a c) sm)) retainedMatches)
            )
            supportedMatches
