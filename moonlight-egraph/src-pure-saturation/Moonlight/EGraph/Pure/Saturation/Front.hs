...
requireFrontNameInput ::
  (String -> error -> EGraphFrontError sig analysis context) ->
  FrontNameInput name error ->
  Either (EGraphFrontError sig analysis context) name
...
type EGraphFront :: FrontPhase -> (Symbol -> (Symbol -> Type) -> Type) -> Type -> Type -> Type -> Type
data EGraphFront phase sig analysis context result where
  AuthoredFront :: !(FrontProgram sig analysis context) -> !(FrontOutput sig analysis context result) -> EGraphFront 'Authored sig analysis context result
  CompiledFront :: !(CompiledFrontState sig analysis context) -> !(FrontOutput sig analysis context result) -> EGraphFront 'Compiled sig analysis context result

type CompiledEGraphFront sig analysis context result =
  EGraphFront 'Compiled sig analysis context result
...
type FrontOutput :: (Symbol -> (Symbol -> Type) -> Type) -> Type -> Type -> Type -> Type
newtype FrontOutput sig analysis context result = FrontOutput
  { stageFrontOutput ::
      ObservationSeedState sig analysis context ->
      Either
        (EGraphFrontError sig analysis context)
        (ObservationSeedState sig analysis context, FrontResolvedOutput sig analysis context result)
  }

type FrontResolvedOutput :: (Symbol -> (Symbol -> Type) -> Type) -> Type -> Type -> Type -> Type
newtype FrontResolvedOutput sig analysis context result = FrontResolvedOutput
  { resolveFrontOutput ::
      SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context ->
      Either (EGraphFrontError sig analysis context) result
  }

instance Functor (FrontOutput sig analysis context) where
...
instance Applicative (FrontOutput sig analysis context) where
  pure value =
    FrontOutput $ \state ->
...
instance Functor (FrontResolvedOutput sig analysis context) where
  fmap transform resolved =
    FrontResolvedOutput $ \graph ->
      transform <$> resolveFrontOutput resolved graph

instance Applicative (FrontResolvedOutput sig analysis context) where
  pure value =
...
data FrontRuntimeSchedulePhase sig analysis context = FrontRuntimeRunRuleset
  { frspPlan :: !(FrontCompiledPlan sig analysis context),
    frspGoal :: !(TerminationGoal (SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context))
  }

type FrontCompiledProgram sig analysis context =
  Program 'CompiledProgramStage (EGraphU SurfaceKind (PackedNode sig) analysis context)

type FrontCompiledPlan sig analysis context =
  Plan
    (EGraphU SurfaceKind (PackedNode sig) analysis context)
    (SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context)
    RewriteRuleId
...
type FrontRuntimeSchedule :: (Symbol -> (Symbol -> Type) -> Type) -> Type -> Type -> Type
newtype FrontRuntimeSchedule sig analysis context = FrontRuntimeSchedule
  { frontRuntimeScheduleTree :: ControlProgram.Program () (FrontRuntimeSchedulePhase sig analysis context)
  }

data CompiledFrontState sig analysis context = CompiledFrontState
  { cfCanonical :: !(CanonicalProgram sig FrontGuardAtom),
    ...
    cfRulesetPrograms :: !(Map FrontRulesetName (FrontCompiledProgram sig analysis context)),
    cfRulesetPlans :: !(Map FrontRulesetName (Map SaturationBudget (FrontCompiledPlan sig analysis context)))
  }
...
data EGraphFrontError sig analysis context
  ...
  | EGraphFrontPackedPlanClosureMiss ![EGraph.PatternVar]
  ...
  | EGraphFrontLogicError !(EGraphLogicError SurfaceKind (PackedNode sig) analysis context)
...
data EGraphFrontReport sig analysis context result = EGraphFrontReport
  { efrResult :: !result,
    efrFinalGraph :: !(SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context),
    efrScheduleReports :: ![EGraphLogicReport SurfaceKind (PackedNode sig) analysis context],
    efrSeedClasses :: !(Map FrontSeedName ClassId),
    efrSeedTrace :: !(ContextMutationTrace context (Node sig))
  }

data EGraphFrontObservedReport sig analysis context result = EGraphFrontObservedReport
  { eforReport :: !(EGraphFrontReport sig analysis context result),
    eforScheduleTimings :: ![RuntimeIOTiming]
  }

data PreparedFrontRun sig analysis context result = PreparedFrontRun
  { pfrCompiled :: !(CompiledFrontState sig analysis context),
    pfrSeeded :: !(SeededFrontState sig analysis context result)
  }

egraph :: EGraphFrontM sig analysis context (FrontOutput sig analysis context result) -> EGraphFront 'Authored sig analysis context result
egraph action =
  let (output, builder) =
...
done :: FrontOutput sig analysis context ()
done =
  pure ()

check :: forall name sig analysis context. (KnownSymbol name, RewriteSignature sig, Ord (NodeTag sig), Ord context) => FrontCheck sig -> EGraphFrontM sig analysis context (FrontOutput sig analysis context Bool)
check =
  pure . checkOutput (frontName @name) Nothing

checkAt :: forall name sig analysis context. (KnownSymbol name, RewriteSignature sig, Ord (NodeTag sig), Ord context) => ContextRef context -> FrontCheck sig -> EGraphFrontM sig analysis context (FrontOutput sig analysis context Bool)
checkAt contextRef =
  pure . checkOutput (frontName @name) (Just contextRef)

extract :: forall name sig analysis context cost sort. (KnownSymbol name, RewriteSignature sig, Ord (NodeTag sig), Ord context, Ord cost) => AnalysisCostAlgebra (Node sig) analysis cost -> TermRef sig sort -> EGraphFrontM sig analysis context (FrontOutput sig analysis context (Maybe (ExtractionResult (Node sig) cost)))
extract =
  extractOutput (frontName @name) Nothing

extractAt :: forall name sig analysis context cost sort. (KnownSymbol name, RewriteSignature sig, Ord (NodeTag sig), Ord context, Ord cost) => ContextRef context -> AnalysisCostAlgebra (Node sig) analysis cost -> TermRef sig sort -> EGraphFrontM sig analysis context (FrontOutput sig analysis context (Maybe (ExtractionResult (Node sig) cost)))
...
compileEGraphFront ::
  forall sig analysis context result.
  ...
  EGraphFront 'Authored sig analysis context result ->
  Either (EGraphFrontError sig analysis context) (EGraphFront 'Compiled sig analysis context result)
compileEGraphFront (AuthoredFront frontProgramValue authoredResult) = do
  ...
  packedRulePlanSet <-
    first EGraphFrontPackedPlanClosureMiss $
...
prepareFrontRun ::
  ...
  EGraphFront 'Authored sig analysis context result ->
  SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context ->
  Either (EGraphFrontError sig analysis context) (PreparedFrontRun sig analysis context result)
...
frontReportFromRun ::
  forall sig analysis context result.
  SeededFrontState sig analysis context result ->
  SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context ->
  [EGraphLogicReport SurfaceKind (PackedNode sig) analysis context] ->
  Either (EGraphFrontError sig analysis context) (EGraphFrontReport sig analysis context result)
...
initialScheduleRunState ::
  timing ->
  SeededFrontState sig analysis context result ->
  ScheduleRunState timing sig analysis context
...
runPreparedFrontWith ::
  Monad m =>
  timing ->
  ( CompiledFrontState sig analysis context ->
    ScheduleRunState timing sig analysis context ->
    FrontRuntimeSchedulePhase sig analysis context ->
    m (Either (EGraphFrontError sig analysis context) (ScheduleRunState timing sig analysis context))
  ) ->
  (EGraphFrontReport sig analysis context result -> ScheduleRunState timing sig analysis context -> output) ->
  PreparedFrontRun sig analysis context result ->
  m (Either (EGraphFrontError sig analysis context) output)
...
runPreparedFrontObserved ::
  ...
  PreparedFrontRun sig analysis context result ->
  IO (Either (EGraphFrontError sig analysis context) (EGraphFrontObservedReport sig analysis context result))
runPreparedFrontObserved =
  runPreparedFrontWith
    []
...
runCompiledEGraphFront ::
  ...
  CompiledEGraphFront sig analysis context result ->
  SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context ->
  [FrontSeedTerm sig] ->
  Either (EGraphFrontError sig analysis context) (EGraphFrontReport sig analysis context result)
...
frontErrorMessage =
  \case
    ...
    EGraphFrontPackedPlanClosureMiss missingVars ->
      "front packed plan closure missed " <> show (length missingVars) <> " variables"
...
compiledFrontValue :: EGraphFront 'Compiled sig analysis context result -> (CompiledFrontState sig analysis context, FrontOutput sig analysis context result)
compiledFrontValue (CompiledFront compiled authoredResult) =
  (compiled, authoredResult)

authoredFrontSeedTerms :: EGraphFront 'Authored sig analysis context result -> [FrontSeedTerm sig]
...
checkOutput :: (RewriteSignature sig, Ord (NodeTag sig), Ord context) => String -> Maybe (ContextRef context) -> FrontCheck sig -> FrontOutput sig analysis context Bool
...
extractOutput ::
  ...
  TermRef sig sort ->
  EGraphFrontM sig analysis context (FrontOutput sig analysis context (Maybe (ExtractionResult (Node sig) cost)))
...
unpackContextMutationTrace ::
  ...
  ContextMutationTrace context (PackedNode sig) ->
  ContextMutationTrace context (Node sig)
...
contextRegistry :: FrontProgram sig analysis context -> Either (EGraphFrontError sig analysis context) (Map DSLRule.ContextName context)
contextRegistry frontProgramValue =
  fmap Map.fromList $
    traverse
      ( \contextDecl -> do
...
validateFrontProgram :: FrontProgram sig analysis context -> Either (EGraphFrontError sig analysis context) ()
validateFrontProgram frontProgramValue =
...
rejectDuplicateName ::
  Ord name =>
  (name -> EGraphFrontError sig analysis context) ->
  (value -> Either (EGraphFrontError sig analysis context) name) ->
  [value] ->
  Either (EGraphFrontError sig analysis context) ()
...
contextDeclName :: ContextDecl context -> Either (EGraphFrontError sig analysis context) DSLRule.ContextName
contextDeclName =
  requireContextName . cdFrontName

rulesetDeclName :: RulesetDecl sig -> Either (EGraphFrontError sig analysis context) FrontRulesetName
rulesetDeclName =
  requireRulesetName . rsdName

seedDeclName :: SeedDecl sig context -> Either (EGraphFrontError sig analysis context) FrontSeedName
seedDeclName (SeedDecl nameInput _ _) =
  requireSeedName nameInput

relationDeclName :: SomeRelationDecl -> Either (EGraphFrontError sig analysis context) FrontRelationName
relationDeclName (SomeRelationDecl relationRef) =
  requireRelationName (relationRefName relationRef)

validateRulesetRuleDecls :: RulesetDecl sig -> Either (EGraphFrontError sig analysis context) ()
validateRulesetRuleDecls rulesetDecl =
  traverse_ ruleDeclName (rsdRules rulesetDecl)

requireContextName :: FrontNameInput DSLRule.ContextName DSLRule.ContextNameError -> Either (EGraphFrontError sig analysis context) DSLRule.ContextName
requireContextName =
  requireFrontNameInput EGraphFrontInvalidContextName

requireRulesetName :: FrontNameInput FrontRulesetName FrontNameError -> Either (EGraphFrontError sig analysis context) FrontRulesetName
requireRulesetName =
  requireFrontNameInput EGraphFrontInvalidRulesetName

requireSeedName :: FrontNameInput FrontSeedName FrontNameError -> Either (EGraphFrontError sig analysis context) FrontSeedName
requireSeedName =
  requireFrontNameInput EGraphFrontInvalidSeedName

requireRelationName :: FrontNameInput FrontRelationName FrontNameError -> Either (EGraphFrontError sig analysis context) FrontRelationName
requireRelationName =
  requireFrontNameInput EGraphFrontInvalidRelationName

requireRuleName :: FrontNameInput RuleName RuleNameError -> Either (EGraphFrontError sig analysis context) RuleName
requireRuleName input =
  first (const (EGraphFrontInvalidRuleName (frontNameInputRaw input))) (frontNameInputParsed input)

rewriteDslProgram :: forall sig analysis context. FrontProgram sig analysis context -> Either (EGraphFrontError sig analysis context) (DSLProgram.Program sig FrontGuardAtom)
rewriteDslProgram frontProgramValue = do
  ...
  where
    rulesetDslRuleDecls :: RulesetDecl sig -> Either (EGraphFrontError sig analysis context) [(RuleName, DSLRule.RuleBody sig FrontGuardAtom)]
    rulesetDslRuleDecls rulesetDecl =
      fold <$> traverse rewriteDeclForDsl (rsdRules rulesetDecl)

    rewriteDeclForDsl :: RuleDecl sig -> Either (EGraphFrontError sig analysis context) [(RuleName, DSLRule.RuleBody sig FrontGuardAtom)]
    rewriteDeclForDsl =
      \case
...
traverseRulesetRuleNames :: forall sig analysis context. FrontProgram sig analysis context -> Either (EGraphFrontError sig analysis context) (Map FrontRulesetName [RuleName])
traverseRulesetRuleNames frontProgramValue =
  fmap Map.fromList $
    traverse
      ( \rulesetDecl -> do
...
ruleDeclName :: RuleDecl sig -> Either (EGraphFrontError sig analysis context) RuleName
ruleDeclName =
  \case
...
traverseRulesetFactRules ::
  forall sig analysis context.
  (RewriteSignature sig, Ord (NodeTag sig)) =>
  FrontProgram sig analysis context ->
  Either (EGraphFrontError sig analysis context) (Map FrontRulesetName [CompiledFactRule SurfaceKind (PackedNode sig)])
traverseRulesetFactRules frontProgramValue =
  fmap Map.fromList $
    traverse
      ( \rulesetDecl -> do
...
compileRulesetFacts ::
  ...
  FrontRulesetName ->
  RulesetDecl sig ->
  Either (EGraphFrontError sig analysis context) [CompiledFactRule SurfaceKind (PackedNode sig)]
compileRulesetFacts rulesetName rulesetDecl = do
  ...
  where
    factRuleSource :: FrontRulesetName -> (Int, RuleDecl sig) -> Either (EGraphFrontError sig analysis context) [FactRule SurfaceKind (Node sig)]
...
termPattern :: RewriteSignature sig => Term sig sort -> Either (EGraphFrontError sig analysis context) (Pattern (Node sig))
termPattern termValue =
  ...
  where
    ...
    lowerWithVars :: RewriteSignature sig => Map SomeTypedVar EGraph.PatternVar -> Term sig sort' -> Either (EGraphFrontError sig analysis context) (Pattern (Node sig))
    lowerWithVars vars =
      \case
        TVar typedVariable ->
          maybe
...
compiledFrontSchedulePlanKeys ::
  ...
  Either (EGraphFrontError sig analysis context) (Set FrontPlanKey)
...
frontPlanSpec budget =
  defaultPlanSpec @(EGraphU SurfaceKind (PackedNode sig) analysis context) @RewriteRuleId
    budget
    GenericJoinMatching
...
compileRulesetPlans ::
  ...
  Either
    (EGraphFrontError sig analysis context)
    (Map FrontRulesetName (Map SaturationBudget (FrontCompiledPlan sig analysis context)))
compileRulesetPlans compiled keys =
  ...
  where
    compileRulesetPlan ::
      ...
      Either
        (EGraphFrontError sig analysis context)
        (FrontRulesetName, Map SaturationBudget (FrontCompiledPlan sig analysis context))
    compileRulesetPlan key = do
      ...
      pure
        ( fpkRuleset key,
          Map.singleton
            (fpkBudget key)
            (mkPlan (frontPlanSpec @sig @analysis @context (fpkBudget key)) compiledProgram)
        )
...
compileProgramForRulesetName ::
  ...
  CompiledFrontState sig analysis context ->
  FrontRulesetName ->
  Either (EGraphFrontError sig analysis context) (FrontCompiledProgram sig analysis context)
...
traverseContextRules ::
  forall sig analysis context.
  Ord context =>
  (RuleName -> Maybe (SatRule (EGraphU SurfaceKind (PackedNode sig) analysis context))) ->
  [RuleName] ->
  RuleSupportIndex DSLRule.ContextName ->
  Map DSLRule.ContextName context ->
  Either (EGraphFrontError sig analysis context) (Map context [SatRule (EGraphU SurfaceKind (PackedNode sig) analysis context)])
...
selectRulePlans ::
  ...
  [RuleName] ->
  Set RuleName ->
  Either (EGraphFrontError sig analysis context) [SatRule (EGraphU SurfaceKind (PackedNode sig) analysis context)]
...
compiledRulesetPlan ::
  ...
  FrontRulesetName ->
  SaturationBudget ->
  Either (EGraphFrontError sig analysis context) (FrontCompiledPlan sig analysis context)
...
runSchedulePhase ::
  ...
  CompiledFrontState sig analysis context ->
  ScheduleRunState () sig analysis context ->
  FrontRuntimeSchedulePhase sig analysis context ->
  Either (EGraphFrontError sig analysis context) (ScheduleRunState () sig analysis context)
...
foldFrontSchedulesWith ::
  Monad m =>
  (state -> FrontRuntimeSchedule sig analysis context -> m (Either (EGraphFrontError sig analysis context) state)) ->
  state ->
  [FrontRuntimeSchedule sig analysis context] ->
  m (Either (EGraphFrontError sig analysis context) state)
...
runFrontScheduleWith ::
  Monad m =>
  (state -> FrontRuntimeSchedulePhase sig analysis context -> m (Either (EGraphFrontError sig analysis context) state)) ->
  state ->
  FrontRuntimeSchedule sig analysis context ->
  m (Either (EGraphFrontError sig analysis context) state)
...
runSchedulePhaseObserved ::
  ...
  CompiledFrontState sig analysis context ->
  ScheduleRunState [RuntimeIOTiming] sig analysis context ->
  FrontRuntimeSchedulePhase sig analysis context ->
  IO (Either (EGraphFrontError sig analysis context) (ScheduleRunState [RuntimeIOTiming] sig analysis context))
...
data ScheduleRunState timing sig analysis context = ScheduleRunState
  { srsGraph :: !(SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context),
    srsFactStore :: !FactStore,
    srsReports :: ![EGraphLogicReport SurfaceKind (PackedNode sig) analysis context],
    srsTimings :: !timing
  }
...
data SeededFrontState sig analysis context result = SeededFrontState
  { sfsGraph :: !(SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context),
    sfsSeeds :: !(Map FrontSeedName ClassId),
    sfsFactStore :: !FactStore,
    sfsSchedules :: ![FrontRuntimeSchedule sig analysis context],
    sfsOutput :: !(FrontResolvedOutput sig analysis context result),
    sfsTrace :: !(ContextMutationTrace context (Node sig))
  }

stageCompiledFrontSeeds ::
  forall sig analysis context result.
  (RewriteSignature sig, Ord (NodeTag sig), Ord context) =>
  FrontOutput sig analysis context result ->
  SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context ->
  CompiledFrontState sig analysis context ->
  [FrontSeedTerm sig] ->
  Either (EGraphFrontError sig analysis context) (SeededFrontState sig analysis context result)
...
data StagingState sig analysis context = StagingState
  { stBatch :: !(ContextRebaseBatch (PackedNode sig) analysis context),
...
data SeedBatchState sig analysis context = SeedBatchState
  { sbsPendingGlobalSeeds :: ![GlobalSeedDecl sig],
    sbsStagingState :: !(StagingState sig analysis context)
  }
...
runtimeSeedDecls ::
  ...
  CompiledFrontState sig analysis context ->
  [FrontSeedTerm sig] ->
  Either (EGraphFrontError sig analysis context) [RuntimeSeedDecl sig context]
...
runtimeSeedTermMap ::
  ...
  [FrontSeedTerm sig] ->
  Either (EGraphFrontError sig analysis context) (Map FrontSeedName (SomeTerm sig))
...
seedSlotName :: FrontSeedSlot context -> Either (EGraphFrontError sig analysis context) FrontSeedName
seedSlotName =
...
stageFrontSchedules ::
  ...
  Either
    (EGraphFrontError sig analysis context)
    (ObservationSeedState sig analysis context, [FrontRuntimeSchedule sig analysis context])
...
stageFrontSchedule compiled schedule initialState =
  ...
  where
    stageScheduleTree ::
      ...
      Either
        (EGraphFrontError sig analysis context)
        (ObservationSeedState sig analysis context, ControlProgram.Program () (FrontRuntimeSchedulePhase sig analysis context))
    stageScheduleTree =
      foldProgram
        ProgramAlgebra
...
stageFrontGoal ::
  ...
  Either
    (EGraphFrontError sig analysis context)
    (ObservationSeedState sig analysis context, TerminationGoal (SaturatingContextEGraph SurfaceKind (PackedNode sig) analysis context))
stageFrontGoal =
  \case
    FrontCheckEq termRef termValue ->
      \state -> do
...
stageRuntimeSeedDecls ::
  forall sig analysis context.
  (RewriteSignature sig, Ord (NodeTag sig), Ord context) =>
  StagingState sig analysis context ->
  [RuntimeSeedDecl sig context] ->
  Either (EGraphFrontError sig analysis context) (StagingState sig analysis context)
...
stageRuntimeSeedDecl ::
  forall sig analysis context.
  (RewriteSignature sig, Ord (NodeTag sig), Ord context) =>
  SeedBatchState sig analysis context ->
  RuntimeSeedDecl sig context ->
  Either (EGraphFrontError sig analysis context) (SeedBatchState sig analysis context)
...
stageContextSeedDecl ::
  forall sig analysis context sort.
  (RewriteSignature sig, Ord (NodeTag sig), Ord context) =>
  StagingState sig analysis context ->
  FrontSeedName ->
  ContextRef context ->
...
prepareGlobalSeedDecl ::
  RewriteSignature sig =>
  GlobalSeedDecl sig ->
  Either (EGraphFrontError sig analysis context) (FrontSeedName, Fix (PackedNode sig))
...
resolveSeedFact ::
  ...
  FactStore ->
  SeedFactDecl sig ->
  Either (EGraphFrontError sig analysis context) FactStore
...
resolveFactRefArgs ::
  ...
  Either (EGraphFrontError sig analysis context) [ClassId]
...
data ObservationSeedState sig analysis context = ObservationSeedState
  { ossBatch :: !(ContextRebaseBatch (PackedNode sig) analysis context),
...
stageObservationTerm ::
  ...
  Maybe (ContextRef context) ->
  Term sig sort ->
  ContextRebaseBatch (PackedNode sig) analysis context ->
  Either (EGraphFrontError sig analysis context) (ClassId, ContextRebaseBatch (PackedNode sig) analysis context)
...
groundTermFix ::
  RewriteSignature sig =>
  Term sig sort ->
  Either (EGraphFrontError sig analysis context) (Fix (PackedNode sig))
groundTermFix =
...
groundNodeTermFix ::
  RewriteSignature sig =>
  Term sig sort ->
  Either (EGraphFrontError sig analysis context) (Fix (Node sig))
groundNodeTermFix =
  \case
    TVar typedVariable ->
      Left
