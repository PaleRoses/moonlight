...
module Moonlight.EGraph.Pure.Saturation.Rebuild.Internal
  ( RoundRebuildReport (..),
...
type RoundRebuildReport :: Type -> (Type -> Type) -> Type -> Type -> Type
data RoundRebuildReport capability f a c = RoundRebuildReport
  { rrrGraph :: !(SaturatingContextEGraph capability f a c),
    rrrRepairIndex :: !(BaseRepairIndex f),
...
runRoundRebuildReport ::
  ...
  Either (ContextDeltaError f c) (RoundRebuildReport capability f a c)
runRoundRebuildReport saturatingGraph =
  let contextGraph = sceContextGraph saturatingGraph
      (rebuildDelta, repairIndex, rebuiltBase) = rebuildWithDelta (cegBase contextGraph)
