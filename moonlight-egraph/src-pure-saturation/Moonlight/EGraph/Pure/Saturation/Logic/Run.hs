...
data EGraphLogic owner capability f a c = EGraphLogic
  { eglSource :: !(ProgramFragment (EGraphU owner capability f a c)),
    eglSeedFacts :: !(SeedFacts (EGraphU owner capability f a c)),
    eglObservations :: ![SomeStableObservation f a c],
    eglRunObservations :: ![SomeRunObservation c]
  }

emptyEGraphLogic :: EGraphLogic owner capability f a c
emptyEGraphLogic =
  EGraphLogic
...
appendEGraphLogic :: EGraphLogic owner capability f a c -> EGraphLogic owner capability f a c -> EGraphLogic owner capability f a c
...
newtype EGraphLogicM owner capability f a c result = EGraphLogicM
  { runEGraphLogicM :: (result, EGraphLogic owner capability f a c)
  }

instance Functor (EGraphLogicM owner capability f a c) where
  fmap transform action =
    let (value, logicValue) = runEGraphLogicM action
     in EGraphLogicM (transform value, logicValue)

instance Applicative (EGraphLogicM owner capability f a c) where
  pure value =
    EGraphLogicM (value, emptyEGraphLogic)

  (<*>) = ap

instance Monad (EGraphLogicM owner capability f a c) where
  action >>= continue =
...
logic :: EGraphLogicM owner capability f a c () -> EGraphLogic owner capability f a c
logic action =
  snd (runEGraphLogicM action)
{-# INLINE logic #-}

source :: ProgramM (EGraphU owner capability f a c) () -> EGraphLogicM owner capability f a c ()
source =
  sourceFragment . program
{-# INLINE source #-}

sourceFragment :: ProgramFragment (EGraphU owner capability f a c) -> EGraphLogicM owner capability f a c ()
...
seedFacts ::
  MatchSite c ->
  SatFactStore (EGraphU owner capability f a c) ->
  EGraphLogicM owner capability f a c ()
...
observe :: StableObservation f a c result -> EGraphLogicM owner capability f a c ()
observe observation =
  EGraphLogicM
    ( (),
      emptyEGraphLogic
...
observeRun :: RunObservation c result -> EGraphLogicM owner capability f a c ()
...
data EGraphLogicError owner capability f a c
  = EGraphLogicCompileError !(SaturationCompileError (EGraphU owner capability f a c) RewriteRuleId)
  | EGraphLogicRunError !(SaturationRunError (EGraphU owner capability f a c))
  | EGraphLogicObservationError !(StableObservationError c)

data EGraphLogicReport owner capability f a c = EGraphLogicReport
  { elrRuntimeState :: !(RuntimeState (EGraphU owner capability f a c) (SaturatingContextEGraph owner capability f a c) RewriteRuleId),
    elrSaturation :: !(SaturationReport (EGraphU owner capability f a c)),
    elrObservations :: ![SomeStableObservationResult f],
    elrRunObservations :: ![SomeRunObservationResult c]
  }

data EGraphLogicObservedReport owner capability f a c = EGraphLogicObservedReport
  { elorTiming :: !RuntimeIOTiming,
    elorReport :: !(EGraphLogicReport owner capability f a c)
  }

compileEGraphLogic ::
  forall owner capability f a c.
  EGraphLogicConstraints capability f a c =>
  PlanSpec (EGraphU owner capability f a c) (SaturatingContextEGraph owner capability f a c) RewriteRuleId ->
  EGraphLogic owner capability f a c ->
  Either
    (SaturationCompileError (EGraphU owner capability f a c) RewriteRuleId)
    (Plan (EGraphU owner capability f a c) (SaturatingContextEGraph owner capability f a c) RewriteRuleId)
compileEGraphLogic planSpec logicValue =
  compileFragment @(EGraphU owner capability f a c)
    planSpec
...
runEGraphLogic ::
  ...
  EGraphLogic owner capability f a c ->
  SaturatingContextEGraph owner capability f a c ->
  Either (EGraphLogicError owner capability f a c) (EGraphLogicReport owner capability f a c)
runEGraphLogic runSpec logicValue graph = do
  plan <-
    first EGraphLogicCompileError $
      compileEGraphLogic @owner @capability @f @a @c
        (crsPlanSpec runSpec)
        logicValue
...
runCompiledEGraphLogic executionSpec plan logicValue graph = do
  let seedFactsByContext =
        resolveSeedFacts @(EGraphU owner capability f a c)
          graph
          (eglSeedFacts logicValue)
      seededState =
        seedRuntimeStateFacts @(EGraphU owner capability f a c)
          seedFactsByContext
          (runtimeStateFromCarrier @(EGraphU owner capability f a c) plan graph)

  (finalState, saturationReport) <-
    first EGraphLogicRunError $
      runRuntime @(EGraphU owner capability f a c)
...
runCompiledEGraphLogicObserved ::
  ...
  EGraphLogic owner capability f a c ->
  SaturatingContextEGraph owner capability f a c ->
  IO (Either (EGraphLogicError owner capability f a c) (EGraphLogicObservedReport owner capability f a c))
runCompiledEGraphLogicObserved executionSpec plan logicValue graph = do
  ...
  observedRuntime <-
    runRuntimeObserved @(EGraphU owner capability f a c)
      (cesPolicy executionSpec)
      plan
      (cesGoal executionSpec)
      seededState
