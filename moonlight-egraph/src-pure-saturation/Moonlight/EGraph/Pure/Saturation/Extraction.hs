...
data ContextualExtractionObstruction c
  ...
  | ContextualExtractionDidNotConverge !ExtractionConvergenceReport
...
contextualExtractWithAnalysisAndMetricsBounded budget contextValue overlayValue baseCost classId contextGraph =
  contextualExtractionSectionWithAnalysisBounded
    ...
    baseCost
    contextGraph
    >>= contextualExtractFromSection classId
...
contextualExtractionSectionWithAnalysisBounded budget contextValue overlayValue baseCost contextGraph = do
  ...
  choiceSection <-
    first ContextualExtractionDidNotConverge
      ( extractChoiceSectionBounded
          budget
...
emptyContextualSectionCache :: ContextEGraph f a c -> ContextualSectionCache f a c cost
emptyContextualSectionCache contextGraph =
  ContextualSectionCache
    { cscBaseRevision = eGraphRevision (cegBase contextGraph),
      cscContextRevision = cegContextRevision contextGraph,
      cscSections = Map.empty
    }
...
advanceContextualSections ::
  ...
  CostOverlay c (AnalysisCostAlgebra f a cost) ->
  AnalysisCostAlgebra f a cost ->
  ContextRebaseReport f c ->
  ContextEGraph f a c ->
  ContextualSectionCache f a c cost ->
  Either (ContextualSectionObstruction c) (ContextualSectionCache f a c cost)
advanceContextualSections budget contextScope overlayValue baseCost reportValue contextGraph cacheValue
  ...
  where
    ...
    advanceAt contextValue =
      case Map.lookup contextValue (cscSections cacheValue) of
        Just section
          | not baseChanged && not (contextSectionDirty contextValue) ->
        ...
        || any
          ( \sourceContext ->
              either
                (const False)
                id
                (preparedContextRestrictsTo (cegSite contextGraph) contextValue sourceContext)
          )
...
contextualExtractionSectionMetrics section extractionResult =
  ContextualExtractionMetrics
    { cemLocalizedNodeCount =
        sum (fmap (length . extractionClassNodes) (IntMap.elems (extractionClasses (cesTable section)))),
      cemLocalizedClassCount = IntMap.size (extractionClasses (cesTable section)),
      cemLocalizedContextCount = cesContextCount section,
