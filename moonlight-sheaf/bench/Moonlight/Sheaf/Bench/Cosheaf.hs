...
buildBenchCoverFixture benchmarkCase = do
  ...
  cosheaf <- first show (buildBenchCoverCosheaf benchmarkCase)
  pure
    BenchCoverFixture
      { bcfCover = coverValue,
        bcfCosheaf = cosheaf
...
buildBenchCoverCosheaf benchmarkCase =
  mkFiniteCosheaf
    (BenchCoverSite (bvcCoverArity benchmarkCase))
    benchCoverAlgebra
...
newtype BenchCoverSite = BenchCoverSite
  { benchCoverArity :: Int
  }
  deriving stock (Eq, Ord, Show)
...
instance Site BenchCoverSite where
  ...
  coversAt site objectValue =
    case objectValue of
      BenchCoverRoot -> either (const []) pure (benchmarkCoverFamily (BenchCoverCase "site-cover" (benchCoverArity site) 1))
      BenchCoverLeaf _ -> []
