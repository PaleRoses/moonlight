...
benchEither label benchmarkValue =
  either
    (\failureMessage -> bench label (whnf (const (Left failureMessage :: Either String ())) ()))
    (bench label . benchmarkValue)

benchWeight :: String -> (value -> Either String Int) -> value -> Benchmark
benchWeight label weightValue =
  bench label . whnf (measuredWeight . weightValue)
...
unitMatrixFromCells dimensionValue cells = do
  ...
  pure
    CoboundaryMatrix
      { cmDimension = dimensionValue,
        cmEntries = entries,
        cmSourceBasis = mkSheafBasis cells,
        cmTargetBasis = mkSheafBasis cells
      }
...
packedResidualBenchData objectCount = do
  model <-
    showSuccess
      ( prepareSheafModel
          (SheafModelVersion 0)
...
sheafSpectralComparisonSuite label objectCount =
  env
    ...
    ( \ ~(ForcedNerveSite _ siteValue) ->
        bgroup
          label
          [ bench "raw-unreduced-site-spectral" (whnf rawSheafSpectralWeight siteValue),
            bench "spectral-ready-filtered-morse" (whnf sheafSpectralReadyWeight siteValue)
          ]
      )
...
sheafSpectralReducedOnlySuite label objectCount =
  env
    ...
    ( \ ~(ForcedNerveSite _ siteValue) ->
        bgroup
          label
          [ bench "site-scaffold" (whnf nerveScaffoldWeight siteValue),
            bench "scaffold-plus-filtered-morse" (whnf filteredMorseWeight siteValue),
            bench "spectral-ready-filtered-morse" (whnf sheafSpectralReadyWeight siteValue)
          ]
      )
    <$ totalOrderNerveSite objectCount
...
nerveScaffoldWeight :: NerveSite BenchSiteTag -> MeasuredWeight
nerveScaffoldWeight =
  measuredWeight . fmap scaffoldWeight . first show . mkNerveComplexScaffold

filteredMorseWeight :: NerveSite BenchSiteTag -> MeasuredWeight
filteredMorseWeight siteValue =
  measuredWeight $ do
    scaffoldValue <- first show (mkNerveComplexScaffold siteValue)
    filteredMorseValue <-
...
rawSheafSpectralWeight :: NerveSite BenchSiteTag -> MeasuredWeight
rawSheafSpectralWeight =
  measuredWeight . rawSheafSpectralWeightResult

sheafSpectralReadyWeight :: NerveSite BenchSiteTag -> MeasuredWeight
sheafSpectralReadyWeight =
  measuredWeight . sheafSpectralReadyWeightResult
...
sheafSpectralReadyWeightResult siteValue =
  case prepareNerveCochainSpectralWith (\_ _ -> 0) trivialFiltration siteValue of
    ...
    Right (SpectralReadyIteration spectralValue) ->
      let reducedComplex =
            rmcReducedComplex
