...
buildGuardedTriangleTrie config = do
  vertexCount <-
    either
      (die . ("invalid dense triangle benchmark domain: " <>) . show)
      pure
      (triangleVertexCount config)
  trieResult <-
    evaluate
      ( force
          ( buildDenseTriangleTrie
  ...
  trie <-
    either
      (die . ("dense triangle construction failed: " <>) . show)
      pure
      trieResult
...
data TriangleBenchConfigError
  = TriangleBenchScaleOutsideIntDomain !Int !Int
  deriving stock (Eq, Show)

triangleVertexCount :: TriangleBenchConfig -> Either TriangleBenchConfigError Int
triangleVertexCount config =
  let scale = tbcScale config
      maximumScale = finiteBitSize (0 :: Int) - 2
   in if scale > maximumScale
        then Left (TriangleBenchScaleOutsideIntDomain scale maximumScale)
        else Right (1 `shiftL` scale)
