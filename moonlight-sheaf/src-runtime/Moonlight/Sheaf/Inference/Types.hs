...
data FactorCompileError pid obj
  ...
  | FactorLocalInvalidLogWeight !pid !obj !LogWeightError
  | FactorTupleInvalidLogWeight !Int !LogWeightError
  | FactorAssignmentSpaceOverflow !Integer
...
data BlueprintError pid obj
  ...
  | BlueprintAssignmentSpaceOverflow !Integer
...
type AssignmentKey :: Type
newtype AssignmentKey = AssignmentKey
  { assignmentKeyValue :: Int
  }
  deriving stock (Eq, Ord, Show)

-- | A checked log-domain weight. The constructor is internal: NaN is never a
-- member, while both infinities are deliberate values handled by total
-- semiring operations.
type LogWeight :: Type
...
type LogWeightError :: Type
data LogWeightError = LogWeightNaN
  deriving stock (Eq, Ord, Show)
...
type TopKCount :: Type
newtype TopKCount = TopKCount
  { topKCountValue :: Int
  }
  deriving stock (Eq, Ord, Show)

type TopKCountError :: Type
data TopKCountError = TopKCountNegative !Int
  deriving stock (Eq, Ord, Show)
