...
inferLogZWithOrder order index factors0 = do
  ...
  if isNegInf const0
    then Right negInf
      ...
      pure (addProductLogWeights constN tailConst)
...
inferLogZAndMarginalsWithOrder
  ...
  -> DomainIndex pid obj
  -> [WeightedFactor]
  -> Either InferenceExecutionError (Double, IntMap [Double])
inferLogZAndMarginalsWithOrder order index factors0 =
  let (const0, _nonConst0) = pullConstantFactors factors0
   ...
   in if isNegInf const0
        then Right (negInf, zeroMarginals (diDomainSizes index) varCount)
        else do
          trace <- buildSumProductTrace order (diDomainSizes index) factors0
          marginals <- sumProductMarginals (diDomainSizes index) varCount trace
          pure (sptLogZ trace, marginals)
...
inferMapWithOrder order index factors0 = do
  let (const0, nonConst0) = pullConstantFactors factors0
  if isNegInf const0
    then Right (MapSolution Map.empty negInf)
    else do
...
sumProductEliminateOne
  :: IntMap Int
  -> VU.Vector Int
  -> (Double, [TaggedFactor], [SumProductBucket], Int)
  -> Int
  -> Either InferenceExecutionError (Double, [TaggedFactor], [SumProductBucket], Int)
sumProductEliminateOne orderPositions domainSizes (!constAcc, !factors, !buckets, !bucketIndex) !varIdx
  | isNegInf constAcc = Right (constAcc, factors, buckets, bucketIndex + 1)
  | otherwise =
...
sumProductMarginals
  ...
  -> Int
  -> SumProductTrace
  -> Either InferenceExecutionError (IntMap [Double])
sumProductMarginals domainSizes varCount trace
  | isNegInf (sptLogZ trace) = Right (zeroMarginals domainSizes varCount)
  | otherwise = do
      outside <- outsideFactorsForTrace domainSizes trace
      IntMap.fromList
        <$> traverse
          (sumProductMarginalForVariable domainSizes trace outside)
          [0 .. varCount - 1]
...
sumProductMarginalForVariable
  ...
  -> Int
  -> Either InferenceExecutionError (Int, [Double])
sumProductMarginalForVariable domainSizes trace outside variable =
  case IntMap.lookup variable (sptBucketsByVariable trace) of
    Nothing ->
      pure (variable, normalizeLogMasses (replicate (domainSizes VU.! variable) 0.0))
    Just bucket -> do
      ...
      pure (variable, normalizeLogMasses (unaryLogWeights (domainSizes VU.! variable) marginal))

outsideFactorsForTrace
  :: VU.Vector Int
  -> SumProductTrace
  -> Either InferenceExecutionError (IntMap WeightedFactor)
outsideFactorsForTrace domainSizes trace =
  foldM
    propagateFromParent
    IntMap.empty
    (reverse (IntMap.keys (sptBucketsByIndex trace)))
  where
    childrenByParent =
      traceChildrenByParent trace

    propagateFromParent outside parentIndex =
      case IntMap.lookup parentIndex (sptBucketsByIndex trace) of
    ...
    insertChildOutside parentBucket outside childIndex =
      case IntMap.lookup childIndex (sptBucketsByIndex trace) of
        Nothing -> Right outside
        Just childBucket -> do
          childOutside <-
...
traceChildrenByParent trace =
  foldl'
    insertChild
    IntMap.empty
    (IntMap.elems (sptBucketsByIndex trace))
  where
...
unaryLogWeights :: Int -> WeightedFactor -> [Double]
  ...
  case factorConstantLog factor of
    ...
    Nothing ->
      let !rowWeights =
       ...
       in [ IntMap.findWithDefault negInf value rowWeights
          | value <- [0 .. domainSize - 1]
          ]

zeroMarginals :: VU.Vector Int -> Int -> IntMap [Double]
zeroMarginals domainSizes varCount =
  IntMap.fromList
    [ (variable, replicate (domainSizes VU.! variable) 0.0)
    | variable <- [0 .. varCount - 1]
    ]
...
reconstructAssignment domainSizes =
  ...
  where
    assignOne acc DecisionFactor{..} = do
      ...
      let code = encodeAssignment scopeDims scopeAssignment
      valueIx <-
...
eliminateOneVariable
  ...
  -> (aux -> [aux] -> [aux])
  -> VU.Vector Int
  -> (Double, [WeightedFactor], [aux])
  -> Int
  -> Either InferenceExecutionError (Double, [WeightedFactor], [aux])
eliminateOneVariable eliminator auxCons domainSizes (!constAcc, !factors, !auxes) !varIdx
  | isNegInf constAcc = Right (constAcc, factors, auxes)
  | otherwise =
...
unitFactor =
  WeightedFactor
    { wfScope = VU.empty
    , wfRows  = VB.singleton (FactorRow VU.empty 0.0)
    }
...
indicatorFactor varIdx valueIx =
  WeightedFactor
    ...
    , wfRows  = VB.singleton (FactorRow (VU.singleton valueIx) 0.0)
    }
...
joinWithMatches plan indexed bigPos smallIsA !acc !bigRow =
  let !key = encodeProjectionWithStrides (jpOverlapDims plan) (jpOverlapStrides plan) bigPos (frAssign bigRow)
  ...
  where
    combineOne !acc0 !smallRow =
      let (!rowA, !rowB) =
          ...
          !code = encodeJoinedAssignment plan (frAssign rowA) (frAssign rowB)
          !weight = frLogWeight rowA + frLogWeight rowB
            ...
            VB.foldl'
              ( \acc FactorRow{..} ->
                  let !projected = deleteAt frAssign pos
                      !code = encodeAssignment newDims projected
...
scaleFactor :: Double -> WeightedFactor -> WeightedFactor
scaleFactor c factor
  | isNegInf c = impossibleFactor (wfScope factor)
  | c == 0.0   = factor
  | otherwise  =
      factor
        { wfRows =
            VB.map
              (\row -> row { frLogWeight = frLogWeight row + c })
              (wfRows factor)
        }
...
factorConstantLog :: WeightedFactor -> Maybe Double
factorConstantLog WeightedFactor{..}
  | not (VU.null wfScope) = Nothing
  | VB.null wfRows        = Just negInf
  | otherwise             = Just (VB.foldl' (\acc row -> logAddExp acc (frLogWeight row)) negInf wfRows)

pullConstantFactors :: [WeightedFactor] -> (Double, [WeightedFactor])
pullConstantFactors =
  foldl'
    step
    (0.0, [])
...
collapseConstantProduct :: VU.Vector Int -> [WeightedFactor] -> Either InferenceExecutionError Double
collapseConstantProduct _ [] = Right 0.0
  ...
  let !strides = stridesFor dims
   in WeightedFactor
        ...
        , wfRows  =
            VB.fromList
              [ FactorRow
                  { frAssign = decodeAssignmentWithStrides dims strides code
                  , frLogWeight = weight
                  }
...
buildProjectionIndex overlapDims overlapStrides overlapPos =
  VB.foldl'
    ( \acc row ->
        let !key = encodeProjectionWithStrides overlapDims overlapStrides overlapPos (frAssign row)
...
encodeAssignment :: VU.Vector Int -> VU.Vector Int -> Int
encodeAssignment dims assignment =
  encodeAssignmentFrom dims (VU.length assignment) (assignment VU.!)

decodeAssignment :: VU.Vector Int -> Int -> VU.Vector Int
decodeAssignment dims code0 =
  decodeAssignmentWithStrides dims (stridesFor dims) code0

decodeAssignmentWithStrides :: VU.Vector Int -> VU.Vector Int -> Int -> VU.Vector Int
decodeAssignmentWithStrides dims strides code0 =
...
encodeAssignmentFrom :: VU.Vector Int -> Int -> (Int -> Int) -> Int
encodeAssignmentFrom dims count f =
  encodeAssignmentFromWithStrides dims (stridesFor dims) count f

encodeAssignmentFromWithStrides :: VU.Vector Int -> VU.Vector Int -> Int -> (Int -> Int) -> Int
...
normalizeLogMasses :: [Double] -> [Double]
normalizeLogMasses [] = []
normalizeLogMasses xs
  | all isNegInf xs = replicate (length xs) 0.0
  | otherwise =
      let !m = maximum xs
          !exps = [ exp (x - m) | x <- xs ]
          !z = sum exps
       in if z <= 0.0 || isNaN z || isInfinite z
            then replicate (length xs) 0.0
            else [ e / z | e <- exps ]

sanitizeLogWeight :: Double -> Double
sanitizeLogWeight w
  | isNaN w   = negInf
  | otherwise = w
...
logAddExp a b
  | isNegInf a = b
  | isNegInf b = a
  | a < b      = b + log (1.0 + exp (a - b))
  | otherwise  = a + log (1.0 + exp (b - a))

addProductLogWeights :: Double -> Double -> Double
addProductLogWeights a b
  | isNegInf a = negInf
  | isNegInf b = negInf
  | otherwise  = a + b
