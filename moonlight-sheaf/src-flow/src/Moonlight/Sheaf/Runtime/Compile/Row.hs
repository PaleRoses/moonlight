...
data RowRuntimeResolutionSpec site ctx prop cell stalk report failure =
  RowRuntimeResolutionSpec
    { rrsInitialDirtyCells :: !(Set cell),
      ...
      rrsRebuildResolvedSection ::
        TotalSectionStore cell stalk ->
        Map cell stalk ->
        Either failure (TotalSectionStore cell stalk),
...
dirtyCellsToPatch ::
  RowRuntimeResolutionSpec site ctx prop cell stalk report failure ->
  Set cell ->
  site ->
  TotalSectionStore cell stalk ->
  Either failure (site, Patch)
...
observeResolvedSection ::
  ...
  Either
    failure
    (TotalSectionStore cell stalk, report)
