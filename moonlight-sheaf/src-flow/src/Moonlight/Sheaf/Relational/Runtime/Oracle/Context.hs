...
data ContextOraclePort site ctx fresh section report failure prop = ContextOraclePort
  { coaRuntime :: !(ContextRuntime site ctx fresh section report failure),
    coaProjectSection ::
      ctx ->
      section ->
...
projectContextOracle ::
  Ord ctx =>
  ContextOraclePort site ctx fresh section report failure prop ->
  site ->
