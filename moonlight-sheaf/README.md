...
The propagation toy is a fixture-backed validation and benchmark harness under `Moonlight.Sheaf.TestFixture.PropagationToy`, consumed by `moonlight-sheaf-propagation-toy-test` and the propagation benchmark groups. The relational bridge belongs to the adjacent `moonlight-sheaf-flow` package, whose `relational` sublibrary owns flow-backed sheaf integration.
