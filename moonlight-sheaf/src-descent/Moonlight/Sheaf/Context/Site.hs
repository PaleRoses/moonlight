...
type MaterializedContextSite :: Type -> Type -> Type
data MaterializedContextSite owner c = MaterializedContextSite
  { pcsObjects :: !(Set c),
    pcsObjectKeys :: !(Map c (ContextObjectKey owner)),
    ...
    pcsRegionTable :: !(RegionTable owner)
  }

type role MaterializedContextSite nominal nominal

type PowersetContextSite :: Type -> Type -> Type
data PowersetContextSite owner c = PowersetContextSite
  { pwsAtomCount :: !Int,
...
type role PowersetContextSite nominal nominal

type PreparedContextSite :: Type -> Type -> Type
data PreparedContextSite owner c
  = SiteMaterialized !(MaterializedContextSite owner c)
  | SitePowerset !(PowersetContextSite owner c)

type role PreparedContextSite nominal nominal

type PreparedContextFragment :: Type -> Type -> Type
newtype PreparedContextFragment owner c = PreparedContextFragment
  (PreparedContextSite owner c)

type role PreparedContextFragment nominal nominal
...
type SupportCarrier :: Type -> Type -> Type
newtype SupportCarrier owner c = SupportCarrier
  { supportCarrierGeneratorKeys :: IntSet
  }
  deriving stock (Eq, Ord, Show)

type role SupportCarrier nominal representational

type ClassSupportIndex :: Type -> Type -> Type
data ClassSupportIndex owner c = ClassSupportIndex
  { csiCarrierByClassKey :: !(IntMap (SupportCarrier owner c)),
    csiClassesByGeneratorKey :: !(IntMap IntSet)
  }
  deriving stock (Eq, Show)

type role ClassSupportIndex nominal representational

type ClassSupportDelta :: Type -> Type -> Type
data ClassSupportDelta owner c = ClassSupportDelta
  { csdTouchedClassKeys :: !IntSet,
    csdTouchedCarriers :: !(IntMap (SupportCarrier owner c)),
    csdTouchedGeneratorKeys :: !IntSet
  }
  deriving stock (Eq, Show)

type role ClassSupportDelta nominal representational

instance Semigroup (ClassSupportDelta owner c) where
  (<>) = appendClassSupportDelta

instance Monoid (ClassSupportDelta owner c) where
  mempty = emptyClassSupportDelta

-- | Nominal owner of the unique one-object context site over @()@.
--
-- The constructor is deliberately hidden.  Unlike an arbitrary finite site,
-- this site has exactly one lawful realization, so a stable owner is sound and
-- lets context-free consumers retain nominal site identity without existential
-- plumbing.
type UnitContextSiteOwner :: Type
data UnitContextSiteOwner

unitPreparedContextSite :: PreparedContextSite UnitContextSiteOwner ()
unitPreparedContextSite =
  preparedContextSiteFromFiniteLattice (singletonContextLattice ())
{-# NOINLINE unitPreparedContextSite #-}

withPreparedContextSiteFromFiniteLattice ::
  Ord c =>
  ContextLattice c ->
  (forall owner. PreparedContextSite owner c -> result) ->
  result
withPreparedContextSiteFromFiniteLattice lattice continuation =
  continuation (preparedContextSiteFromFiniteLattice lattice)
{-# INLINE withPreparedContextSiteFromFiniteLattice #-}

preparedContextSiteFromFiniteLattice ::
  Ord c =>
  ContextLattice c ->
  PreparedContextSite owner c
preparedContextSiteFromFiniteLattice lattice =
  SiteMaterialized
...
{-# INLINE preparedContextSiteFromFiniteLattice #-}
...
withPreparedContextSiteFromPowersetAtoms ::
  Ord a =>
  [a] ->
  (forall owner. PreparedContextSite owner (Set a) -> result) ->
  Either (PowersetSitePreparationError a) result
withPreparedContextSiteFromPowersetAtoms atomValues continuation =
    ...
    duplicatedAtoms =
      [ atomValue
        | (atomValue, occurrenceCount) <-
            Map.toAscList atomOccurrences,
          occurrenceCount > 1
      ]

    sortedAtoms =
      Map.keys atomOccurrences
...
preparedSiteEnumerability :: PreparedContextSite owner c -> SiteEnumerability
...
preparedDefaultContext :: PreparedContextSite owner c -> c
...
-- | The finite local section visible without enumerating an implicit site.
-- Materialized sites expose their whole object family; implicit powerset sites
-- expose only the bottom singleton. Full enumeration uses the refusing
-- 'preparedMaterialized*' operations below.
preparedContextFragment :: PreparedContextSite owner c -> PreparedContextFragment owner c
preparedContextFragment = PreparedContextFragment
{-# INLINE preparedContextFragment #-}

contextFragmentObjects :: PreparedContextFragment owner c -> [c]
contextFragmentObjects (PreparedContextFragment site) =
  case site of
    SiteMaterialized denseSite -> IntMap.elems (pcsObjectsByKey denseSite)
    SitePowerset powersetSite -> [pwsBottomObject powersetSite]
{-# INLINE contextFragmentObjects #-}

contextFragmentKeyedObjects :: PreparedContextFragment owner c -> [(ContextObjectKey owner, c)]
contextFragmentKeyedObjects (PreparedContextFragment site) =
  case site of
    ...
    SitePowerset powersetSite -> [(ContextObjectKey 0, pwsBottomObject powersetSite)]
{-# INLINE contextFragmentKeyedObjects #-}

contextFragmentObjectSet :: PreparedContextFragment owner c -> Set c
contextFragmentObjectSet (PreparedContextFragment site) =
  case site of
    SiteMaterialized denseSite -> pcsObjects denseSite
    SitePowerset powersetSite -> Set.singleton (pwsBottomObject powersetSite)
{-# INLINE contextFragmentObjectSet #-}

contextFragmentLattice :: PreparedContextFragment owner c -> ContextLattice c
contextFragmentLattice (PreparedContextFragment site) =
  case site of
...
{-# INLINE contextFragmentLattice #-}

contextFragmentJoinClosure ::
  PreparedContextFragment owner c ->
  ([c], [(c, c, ContextLatticeLookupError c)])
contextFragmentJoinClosure (PreparedContextFragment site) =
  case site of
    SiteMaterialized denseSite -> (pcsJoinClosureContexts denseSite, pcsJoinFailures denseSite)
    SitePowerset powersetSite -> ([pwsBottomObject powersetSite], [])
{-# INLINE contextFragmentJoinClosure #-}

contextFragmentRestrictionPairs :: PreparedContextFragment owner c -> [(c, c)]
contextFragmentRestrictionPairs (PreparedContextFragment site) =
  case site of
    SiteMaterialized denseSite -> materializedRestrictionPairs denseSite
    SitePowerset _ -> []
{-# INLINE contextFragmentRestrictionPairs #-}

preparedRegionTable :: PreparedContextSite owner c -> RegionTable owner
...
preparedRegionAt ::
  ...
  PreparedContextSite owner c ->
  c ->
  Either (PreparedContextSupportError c) (ContextRegion owner)
preparedRegionAt site contextValue =
  fmap
    (Region.regionAtKey (preparedRegionTable site))
...
supportCarrierRegion :: PreparedContextSite owner c -> SupportCarrier owner c -> ContextRegion owner
supportCarrierRegion site carrier =
  fromGeneratorKeys
    (preparedRegionTable site)
    (fmap ContextObjectKey (IntSet.toAscList (supportCarrierGeneratorKeys carrier)))
...
preparedContextAtKey :: PreparedContextSite owner c -> ContextObjectKey owner -> Maybe c
...
preparedMaterializedObjects :: PreparedContextSite owner c -> Either SiteEnumerationRefusal [c]
...
preparedMaterializedKeyedObjects :: PreparedContextSite owner c -> Either SiteEnumerationRefusal [(ContextObjectKey owner, c)]
...
preparedMaterializedObjectSet :: PreparedContextSite owner c -> Either SiteEnumerationRefusal (Set c)
...
preparedMaterializedRestrictionPairs :: PreparedContextSite owner c -> Either SiteEnumerationRefusal [(c, c)]
preparedMaterializedRestrictionPairs site =
  case site of
    SiteMaterialized denseSite -> Right (materializedRestrictionPairs denseSite)
...
preparedMaterializedLattice :: PreparedContextSite owner c -> Either SiteEnumerationRefusal (ContextLattice c)
...
preparedContextRestrictsTo :: Ord c => PreparedContextSite owner c -> c -> c -> Either (PreparedContextSupportError c) Bool
...
preparedContextKeyRestrictsTo :: PreparedContextSite owner c -> ContextObjectKey owner -> ContextObjectKey owner -> Bool
...
preparedRestrictionSources :: Ord c => c -> PreparedContextSite owner c -> Either (PreparedContextSupportError c) [c]
...
preparedRestrictionTargets :: Ord c => c -> PreparedContextSite owner c -> Either (PreparedContextSupportError c) [c]
...
materializedRestrictionPairs :: MaterializedContextSite owner c -> [(c, c)]
materializedRestrictionPairs denseSite =
...
{-# INLINE materializedRestrictionPairs #-}
    ...
    SiteMaterialized denseSite -> do
      ...
      registryFromScopedEdges
        ...
        [ ContextRestrictionEdge sourceValue targetValue
          | (sourceValue, targetValue) <- materializedRestrictionPairs denseSite,
...
validateMaterializedContextObject ::
  Ord c =>
  MaterializedContextSite owner c ->
  c ->
...
emptyClassSupportIndex :: ClassSupportIndex owner c
emptyClassSupportIndex =
  ClassSupportIndex
...
classSupportIndexFromEntries ::
  ...
  Either (PreparedContextSupportError c) (ClassSupportIndex owner c)
...
contextObjectKeyFor ::
  ...
  PreparedContextSite owner c ->
  c ->
  Either (PreparedContextSupportError c) (ContextObjectKey owner)
...
supportCarrierFromSupport ::
  ...
  PreparedContextSite owner c ->
  SupportBasis c ->
  Either (PreparedContextSupportError c) (SupportCarrier owner c)
...
supportCarrierContainsKey :: PreparedContextSite owner c -> SupportCarrier owner c -> ContextObjectKey owner -> Bool
...
supportCarrierGeneratorCount :: SupportCarrier owner c -> Int
supportCarrierGeneratorCount =
  IntSet.size . supportCarrierGeneratorKeys
{-# INLINE supportCarrierGeneratorCount #-}

supportCarrierUnion :: PreparedContextSite owner c -> SupportCarrier owner c -> SupportCarrier owner c -> SupportCarrier owner c
...
supportCarrierMeet :: PreparedContextSite owner c -> SupportCarrier owner c -> SupportCarrier owner c -> SupportCarrier owner c
...
defaultSupportCarrier site =
  supportCarrierFromSupport site (defaultPreparedSupport site)
{-# INLINE defaultSupportCarrier #-}

classKeysVisibleAtKey :: PreparedContextSite owner c -> ClassSupportIndex owner c -> ContextObjectKey owner -> IntSet
...
classSupportIndexExplicitClassKeys :: ClassSupportIndex owner c -> IntSet
classSupportIndexExplicitClassKeys =
  IntMap.keysSet . csiCarrierByClassKey
{-# INLINE classSupportIndexExplicitClassKeys #-}

classSupportIndexSupportEntryCount :: ClassSupportIndex owner c -> Int
classSupportIndexSupportEntryCount =
  IntMap.size . csiCarrierByClassKey
{-# INLINE classSupportIndexSupportEntryCount #-}

classSupportIndexCarrierGeneratorCount :: ClassSupportIndex owner c -> Int
classSupportIndexCarrierGeneratorCount =
  sum . fmap supportCarrierGeneratorCount . IntMap.elems . csiCarrierByClassKey
{-# INLINE classSupportIndexCarrierGeneratorCount #-}

classSupportIndexGeneratorBucketCount :: ClassSupportIndex owner c -> Int
classSupportIndexGeneratorBucketCount =
  IntMap.size . csiClassesByGeneratorKey
{-# INLINE classSupportIndexGeneratorBucketCount #-}

classSupportExplicitCarrierForKey :: ClassSupportIndex owner c -> Int -> Maybe (SupportCarrier owner c)
...
defaultPreparedSupport :: PreparedContextSite owner c -> SupportBasis c
defaultPreparedSupport site =
  case site of
    SiteMaterialized denseSite ->
      principalSupport (pcsDefaultObject denseSite)
    SitePowerset powersetSite ->
      principalSupport (pwsBottomObject powersetSite)
...
classSupportIndexInsert ::
  ...
  Int ->
  ClassSupportIndex owner c ->
  Either (PreparedContextSupportError c) (ClassSupportIndex owner c, ClassSupportDelta owner c)
...
emptyClassSupportDelta :: ClassSupportDelta owner c
emptyClassSupportDelta =
  ClassSupportDelta
...
appendClassSupportDelta :: ClassSupportDelta owner c -> ClassSupportDelta owner c -> ClassSupportDelta owner c
...
classSupportDeltaEmpty :: ClassSupportDelta owner c -> Bool
...
classSupportDeltaTouchedClassKeys :: ClassSupportDelta owner c -> IntSet
classSupportDeltaTouchedClassKeys =
  csdTouchedClassKeys
{-# INLINE classSupportDeltaTouchedClassKeys #-}

classSupportDeltaTouchedCarriers :: ClassSupportDelta owner c -> IntMap (SupportCarrier owner c)
classSupportDeltaTouchedCarriers =
  csdTouchedCarriers
{-# INLINE classSupportDeltaTouchedCarriers #-}

insertClassCarrier :: Int -> Maybe (SupportCarrier owner c) -> SupportCarrier owner c -> ClassSupportIndex owner c -> ClassSupportIndex owner c
...
supportCarrierReachableKeys :: MaterializedContextSite owner c -> SupportCarrier owner c -> IntSet
...
visibleGeneratorKeysAtKey :: MaterializedContextSite owner c -> ContextObjectKey owner -> IntSet
visibleGeneratorKeysAtKey denseSite (ContextObjectKey keyValue) =
  IntSet.insert
    keyValue
...
normalizeSupportCarrierKeys :: PreparedContextSite owner c -> IntSet -> IntSet
...
upsetKeysFor :: MaterializedContextSite owner c -> ContextObjectKey owner -> IntSet
...
decodeObjectKeys :: Ord c => PreparedContextSite owner c -> IntSet -> Set c
...
decodeObjectKeyList :: MaterializedContextSite owner c -> IntSet -> [c]
...
objectKeyEntriesFromLattice :: ContextLattice c -> [(c, ContextObjectKey owner)]
objectKeyEntriesFromLattice lattice =
...
objectsByKeyFromEntries :: [(c, ContextObjectKey owner)] -> IntMap c
...
restrictionRowsFromObjectKeyPairs :: Int -> [(ContextObjectKey owner, ContextObjectKey owner)] -> IntMap IntSet
restrictionRowsFromObjectKeyPairs objectCount keyPairs =
  ...
  where
    ...
    insertKeyPair ::
      (ContextObjectKey owner, ContextObjectKey owner) ->
      IntMap IntSet ->
      IntMap IntSet
...
swapObjectKeyPair :: (ContextObjectKey owner, ContextObjectKey owner) -> (ContextObjectKey owner, ContextObjectKey owner)
swapObjectKeyPair (leftKey, rightKey) =
  (rightKey, leftKey)
{-# INLINE swapObjectKeyPair #-}

powersetRefusal :: PowersetContextSite owner c -> RefusedSiteEnumeration -> SiteEnumerationRefusal
powersetRefusal powersetSite =
  SiteEnumerationRefusal (pwsAtomCount powersetSite)
{-# INLINE powersetRefusal #-}

powersetUniverseMask :: PowersetContextSite owner c -> Int
powersetUniverseMask powersetSite =
  bit (pwsAtomCount powersetSite) - 1
{-# INLINE powersetUniverseMask #-}

powersetLeqObject :: PowersetContextSite owner c -> c -> c -> Bool
