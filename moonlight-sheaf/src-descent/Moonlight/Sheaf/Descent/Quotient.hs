...
type DescentKernel :: Type -> Type -> Type
data DescentKernel c rep = DescentKernel
  { dkSite :: !(PreparedContextSite c),
...
type PreparedCoverPlan :: Type -> Type -> Type
data PreparedCoverPlan c rep
...
quotientCoverKernel :: DenseKey rep => DescentKernel c rep -> c -> [c] -> CoverMeetSections (ContextClassSection rep) -> CoverKernel.CoverDescentKernel c Int rep (QuotientDescentObstruction c rep)
...
parentRepresentativeImageSet :: DenseKey rep => DescentKernel c rep -> c -> [c] -> Set (Map.Map Int rep)
