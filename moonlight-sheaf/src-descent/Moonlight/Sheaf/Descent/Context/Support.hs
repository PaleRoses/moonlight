...
supportFiberObstructionsWithPreparedCoverPlan ::
  ContextAlgebraSite store ctx classId analysis =>
  ctx ->
  PreparedCoverPlan ctx classId ->
  store ->
  SearchVerdict Void (QuotientDescentObstruction ctx classId)
supportFiberObstructionsWithPreparedCoverPlan parentContext coverPlan store =
  case coverPlan of
    PreparedCoverLookupFailed lookupError ->
      decidedSearchVerdict (rejectedOne (DescentCoverLookupObstruction parentContext lookupError))
    PreparedCoverTrivial ->
      SearchAccepted
    PreparedCoverMeetLookupFailed _activeCoverContexts meetObstructions ->
      decidedSearchVerdict (rejectedFromList meetObstructions)
    PreparedCoverActive activeCoverContexts _meetContexts ->
      decidedSearchVerdict
        ( rejectedFromList
