...
data FiniteCoverBasis site = FiniteCoverBasis
  { fcbSite :: !site,
    ...
    fcbIdentityCoversByTarget ::
      !(Map (SiteObject site) (EffectiveCoverPlan (SiteObject site) (SiteMorphism site))),
    fcbPullbackPlansByTarget ::
      !( Map
           (SiteObject site)
           ( Map
...
mkFiniteCoverBasis site = do
  ...
  pure
    FiniteCoverBasis
      { fcbSite = site,
        fcbCoversByTarget = coversByTarget,
        fcbIdentityCoversByTarget = identities,
        fcbPullbackPlansByTarget = pullbacksByTarget,
        fcbCommonRefinementsByTarget = commonRefinementsByTarget
      }
...
finiteCoversAt basis objectValue =
  Map.findWithDefault [] objectValue (fcbCoversByTarget basis)
...
finiteIdentityCoverAt basis objectValue =
  maybe
    ...
    Right
    (Map.lookup objectValue (fcbIdentityCoversByTarget basis))
...
finitePullbackCoverPlan basis morphismValue coverPlan =
  maybe
    ...
    Right
    ( do
        targetPullbacks <- Map.lookup (cmTarget morphismValue) (fcbPullbackPlansByTarget basis)
...
finiteCommonRefinementPlan basis leftCover rightCover =
  maybe
    ...
    Right
    ( do
        targetRefinements <- Map.lookup (coverTarget (effectiveCoverFamily leftCover)) (fcbCommonRefinementsByTarget basis)
    ...
    pullbackForPair (morphismValue, coverPlan) = do
      ...
      pure
        ( (morphismValue, effectiveCoverFamily coverPlan),
          pullbackPlan {pcpPulledCover = canonicalPulledCover}
        )
