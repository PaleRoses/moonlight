...
module Moonlight.Sheaf.Site.Construction.FiniteMeet
  ( FiniteMeetSiteSpec (..),
    FiniteMeetSite,
    FiniteMeetMorphism,
...
type CellBitSet :: Type
type CellBitSet = Integer

emptyCellBitSet :: CellBitSet
emptyCellBitSet =
  0

type FiniteMeetOrderIndex :: Type -> Type
data FiniteMeetOrderIndex cell = FiniteMeetOrderIndex
  { fmoiCellsByOrdinal :: !(IntMap cell),
    fmoiDownsetsByOrdinal :: !(IntMap CellBitSet),
    fmoiUpsetsByOrdinal :: !(IntMap CellBitSet)
  }
...
buildFiniteMeets ::
  ...
  Either
    ...
    (Map (cell, cell) cell)
buildFiniteMeets cells refinements = do
  orderIndex <- finiteMeetOrderIndex orderedCells refinements
  Map.fromList
    <$> traverse
      ( \(leftOrdinal, leftCell, rightOrdinal, rightCell) ->
          fmap
            (normalizePair leftCell rightCell,)
            (meetOf orderIndex leftCell rightCell leftOrdinal rightOrdinal)
      )
      [ (leftOrdinal, leftCell, rightOrdinal, rightCell)
      | (leftOrdinal, leftCell) <- ordinalCells,
        (rightOrdinal, rightCell) <- ordinalCells,
        leftCell <= rightCell
      ]
  where
    orderedCells =
...
finiteMeetOrderIndex ::
  Ord cell =>
  [cell] ->
  Set (cell, cell) ->
  Either (FiniteMeetSiteBuildError cell) (FiniteMeetOrderIndex cell)
finiteMeetOrderIndex orderedCells refinements = do
  refinementOrdinals <-
    traverse
      (refinementOrdinalPair cellOrdinals)
      (Set.toAscList refinements)
  let cellsByOrdinal =
        IntMap.fromAscList (zip [0 :: Int ..] orderedCells)
      downsetsByOrdinal =
        completeBitsets cellsByOrdinal
          ( bitsetsBy snd fst refinementOrdinals
          )
      upsetsByOrdinal =
        completeBitsets cellsByOrdinal
          ( bitsetsBy fst snd refinementOrdinals
          )
  pure
    FiniteMeetOrderIndex
      { fmoiCellsByOrdinal = cellsByOrdinal,
        fmoiDownsetsByOrdinal = downsetsByOrdinal,
        fmoiUpsetsByOrdinal = upsetsByOrdinal
      }
  where
    cellOrdinals =
      Map.fromList (zip orderedCells [0 :: Int ..])

refinementOrdinalPair ::
  Ord cell =>
  Map cell Int ->
  (cell, cell) ->
  Either (FiniteMeetSiteBuildError cell) (Int, Int)
refinementOrdinalPair cellOrdinals (source, target) =
  case (Map.lookup source cellOrdinals, Map.lookup target cellOrdinals) of
    (Nothing, _) ->
      Left (FiniteMeetUnknownSource source)
    (_, Nothing) ->
      Left (FiniteMeetUnknownTarget target)
    (Just sourceOrdinal, Just targetOrdinal) ->
      Right (sourceOrdinal, targetOrdinal)
...
bitsetsBy ::
  ...
  [(Int, Int)] ->
  IntMap CellBitSet
bitsetsBy keyOf memberOf =
  foldr
    ( \ordinalPair ->
        IntMap.insertWith (.|.) (keyOf ordinalPair) (bit (memberOf ordinalPair))
    )
    IntMap.empty
...
completeBitsets ::
  ...
  IntMap CellBitSet ->
  IntMap CellBitSet
completeBitsets cellsByOrdinal bitsets =
  IntMap.union bitsets (IntMap.map (const 0) cellsByOrdinal)

meetOf ::
  Ord cell =>
  FiniteMeetOrderIndex cell ->
  cell ->
  cell ->
  Int ->
  Int ->
...
meetOf orderIndex leftCell rightCell leftOrdinal rightOrdinal =
  case bitsetCells orderIndex greatestLowerBounds of
    [] ->
  ...
  where
    lowerBounds =
      downsetOf orderIndex leftOrdinal .&. downsetOf orderIndex rightOrdinal

    greatestLowerBounds =
      case maximalLowerBoundOrdinals of
        [greatestLowerBoundOrdinal] ->
          bit greatestLowerBoundOrdinal
        _ ->
          0

    maximalLowerBoundOrdinals =
      [ candidateOrdinal
      | candidateOrdinal <- bitsetOrdinals orderIndex lowerBounds,
        lowerBounds .&. clearBit (upsetOf orderIndex candidateOrdinal) candidateOrdinal == 0
      ]

downsetOf :: FiniteMeetOrderIndex cell -> Int -> CellBitSet
downsetOf orderIndex ordinalValue =
  IntMap.findWithDefault emptyCellBitSet ordinalValue (fmoiDownsetsByOrdinal orderIndex)
{-# INLINE downsetOf #-}

upsetOf :: FiniteMeetOrderIndex cell -> Int -> CellBitSet
upsetOf orderIndex ordinalValue =
  IntMap.findWithDefault emptyCellBitSet ordinalValue (fmoiUpsetsByOrdinal orderIndex)
{-# INLINE upsetOf #-}

bitsetOrdinals :: FiniteMeetOrderIndex cell -> CellBitSet -> [Int]
bitsetOrdinals orderIndex bitsetValue =
  [ ordinalValue
  | ordinalValue <- IntMap.keys (fmoiCellsByOrdinal orderIndex),
    testBit bitsetValue ordinalValue
  ]

bitsetCells :: FiniteMeetOrderIndex cell -> CellBitSet -> [cell]
bitsetCells orderIndex bitsetValue =
  [ cellValue
  | (ordinalValue, cellValue) <- IntMap.toAscList (fmoiCellsByOrdinal orderIndex),
    testBit bitsetValue ordinalValue
  ]
