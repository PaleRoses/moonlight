...
data NerveCell tag = NerveCell
  { nerveCellKeyInternal :: CellKey,
    nerveCellSimplexInternal :: NerveSimplex (NerveCategory tag)
  }
...
data FaceMorphism tag = FaceMorphism
  { faceMorphismSourceInternal :: NerveCell tag,
    faceMorphismTargetInternal :: NerveCell tag,
    faceMorphismKindInternal :: FaceKind,
    faceMorphismFaceIndexInternal :: Natural,
    faceMorphismOrientationInternal :: Int
  }
...
data NerveSite tag = NerveSite
  { nerveSiteDepthInternal :: Natural,
    nerveSiteCategoryInternal :: NerveCategory tag,
    nerveSiteSourceNerveInternal :: TruncatedNormalizedSSet (NerveSimplex (NerveCategory tag)),
    nerveSiteCellsInternal :: [NerveCell tag],
    nerveSiteCellsByDimension :: Map Natural [NerveCell tag],
    nerveSiteBasisInternal :: SheafBasis (NerveCell tag),
    siteFaceMorphismsInternal :: [FaceMorphism tag]
  }

nerveCellKey :: NerveCell tag -> CellKey
nerveCellKey = nerveCellKeyInternal

nerveCellSimplex :: NerveCell tag -> NerveSimplex (NerveCategory tag)
nerveCellSimplex = nerveCellSimplexInternal

faceMorphismSource :: FaceMorphism tag -> NerveCell tag
faceMorphismSource = faceMorphismSourceInternal

faceMorphismTarget :: FaceMorphism tag -> NerveCell tag
faceMorphismTarget = faceMorphismTargetInternal

faceMorphismKind :: FaceMorphism tag -> FaceKind
faceMorphismKind = faceMorphismKindInternal

faceMorphismFaceIndex :: FaceMorphism tag -> Natural
faceMorphismFaceIndex = faceMorphismFaceIndexInternal

faceMorphismOrientation :: FaceMorphism tag -> Int
faceMorphismOrientation = faceMorphismOrientationInternal

nerveSiteDepth :: NerveSite tag -> Natural
nerveSiteDepth = nerveSiteDepthInternal

nerveSiteCategory :: NerveSite tag -> NerveCategory tag
nerveSiteCategory = nerveSiteCategoryInternal

nerveSiteSourceNerve :: NerveSite tag -> TruncatedNormalizedSSet (NerveSimplex (NerveCategory tag))
nerveSiteSourceNerve = nerveSiteSourceNerveInternal

nerveSiteCells :: NerveSite tag -> [NerveCell tag]
nerveSiteCells = nerveSiteCellsInternal

nerveSiteBasis :: NerveSite tag -> SheafBasis (NerveCell tag)
nerveSiteBasis = nerveSiteBasisInternal

siteFaceMorphisms :: NerveSite tag -> [FaceMorphism tag]
siteFaceMorphisms = siteFaceMorphismsInternal
...
nerveSiteFromSkeleton depthValue categoryValue sourceNerve skeletonValue =
  let cells = tssCells skeletonValue
   in NerveSite
        { nerveSiteDepthInternal = depthValue,
          nerveSiteCategoryInternal = categoryValue,
          nerveSiteSourceNerveInternal = sourceNerve,
          nerveSiteCellsInternal = cells,
          nerveSiteCellsByDimension = tssCellsByDimension skeletonValue,
          nerveSiteBasisInternal = mkSheafBasis cells,
          siteFaceMorphismsInternal = tssFaceMorphisms skeletonValue
        }
...
mkNerveCell dimensionValue ordinal simplexValue =
  NerveCell
    { nerveCellKeyInternal = CellKey dimensionValue ordinal,
      nerveCellSimplexInternal = simplexValue
    }
...
mkFaceMorphism sourceCellValue targetCellValue faceKindValue faceIndex orientationValue =
  FaceMorphism
    { faceMorphismSourceInternal = sourceCellValue,
      faceMorphismTargetInternal = targetCellValue,
      faceMorphismKindInternal = faceKindValue,
      faceMorphismFaceIndexInternal = faceIndex,
      faceMorphismOrientationInternal = orientationValue
    }
...
restrictNerveSiteToCellKeys keptKeys siteValue =
  let keptCells =
   ...
   in siteValue
        { nerveSiteCellsInternal = keptCells,
          nerveSiteCellsByDimension = cellsByDimensionFromCells keptCells,
          nerveSiteBasisInternal = mkSheafBasis keptCells,
          siteFaceMorphismsInternal = keptFaceMorphisms
        }
