...
data GradedOperator cell coefficient = GradedOperator
  { gradedOperatorDegree :: HomologicalDegree,
    gradedOperatorSourceBasis :: LinearBasis cell,
    gradedOperatorTargetBasis :: LinearBasis cell,
    gradedOperatorIncidence :: BoundaryIncidence coefficient
  }
  deriving stock (Eq, Show)

data GradedComplex cell coefficient = GradedComplex
  { gradedComplexDirection :: GradedDirection,
    gradedOperatorsByDegree :: Map HomologicalDegree (GradedOperator cell coefficient)
  }
  deriving stock (Eq, Show)
...
emptyGradedComplex direction =
  GradedComplex
    { gradedComplexDirection = direction,
      gradedOperatorsByDegree = Map.empty
    }
...
mkGradedOperator degree sourceBasis targetBasis incidence = do
  ...
  Right
    GradedOperator
      { gradedOperatorDegree = degree,
        gradedOperatorSourceBasis = sourceBasis,
        gradedOperatorTargetBasis = targetBasis,
        gradedOperatorIncidence = incidence
      }
...
mkGradedComplex direction operators = do
  ...
  pure
    GradedComplex
      { gradedComplexDirection = direction,
        gradedOperatorsByDegree = operators
      }
  where
    validateDegreeKey ::
