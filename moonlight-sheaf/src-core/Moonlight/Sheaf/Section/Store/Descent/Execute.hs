...
data DenseDescentSession s cell witness stalk mismatch repairObstruction = DenseDescentSession
  { ddsFastKernelSource :: !(DenseDescentKernelSource cell witness stalk mismatch repairObstruction),
    ddsPreparedDescent :: !(PreparedSectionDescent cell witness),
...
data DenseDescentContext s cell witness stalk mismatch repairObstruction = DenseDescentContext
  { ddcSession :: !(DenseDescentSession s cell witness stalk mismatch repairObstruction),
    ddcRowMode :: !SectionDescentRowMode,
    ddcPinnedTargets :: !(PinnedDescentTarget stalk)
  }

data DenseDescentKernelSource cell witness stalk mismatch repairObstruction
...
descendPreparedLocalKeyed preparedDescent stalkAlgebra observation delta store = do
  validatePreparedStore preparedDescent store
  validateDeltaFingerprint (totalSectionModelFingerprint store) delta
...
prepareSectionProgram ::
  PreparedSectionDescent cell witness ->
  [KeyedSectionEdit stalk] ->
  Either (SectionStoreError cell) (PreparedSectionProgram stalk)
...
preparedSectionProgramLength :: PreparedSectionProgram stalk -> Int
preparedSectionProgramLength =
...
prepareSectionEditInstruction preparedDescent edit
  | kseModelFingerprint edit /= psdModelFingerprint preparedDescent =
      Left
        ( SectionStoreModelFingerprintMismatch
            (psdModelFingerprint preparedDescent)
            (kseModelFingerprint edit)
        )
  | objectOrdinal < 0 || objectOrdinal >= psdObjectCount preparedDescent =
      Left (SectionStoreUnknownObjectKey objectKey)
  | otherwise =
...
data SectionDescentTransaction s cell witness stalk mismatch repairObstruction = SectionDescentTransaction
  { sdtSession :: !(DenseDescentSession s cell witness stalk mismatch repairObstruction),
    sdtAccumulator :: !(STRef s SectionDescentAccumulator),
    sdtModelFingerprint :: !ModelFingerprint,
    sdtObjectCount :: !Int
  }
...
runSectionDescentTransaction ::
  ...
  TotalSectionStore cell stalk ->
  ( forall s.
    SectionDescentTransaction s cell witness stalk mismatch repairObstruction ->
    ST s (Either (SectionDescentError cell stalk mismatch) result)
  ) ->
  Either (SectionDescentError cell stalk mismatch) (result, SectionDescentResult cell stalk)
...
runSectionDescentTransactionWithKernelSource ::
  Ord cell =>
  DenseDescentKernelSource cell witness stalk mismatch repairObstruction ->
  TotalSectionStore cell stalk ->
  ( forall s.
    SectionDescentTransaction s cell witness stalk mismatch repairObstruction ->
    ST s (Either (SectionDescentError cell stalk mismatch) result)
            ...
            transactValue
              SectionDescentTransaction
                { sdtSession =
                  ...
                  sdtModelFingerprint = totalSectionModelFingerprint store,
...
transactKeyedSectionDelta transaction delta =
  case
    validateDeltaFingerprint (sdtModelFingerprint transaction) delta
...
validatePreparedSectionProgram preparedDescent program
  | pspModelFingerprint program /= psdModelFingerprint preparedDescent =
      Left
        ( SectionDescentStoreFailed
            ( SectionStoreModelFingerprintMismatch
                (psdModelFingerprint preparedDescent)
                (pspModelFingerprint program)
            )
        )
...
validatePreparedStore ::
  PreparedSectionDescent cell witness ->
  TotalSectionStore cell stalk ->
  Either (SectionDescentError cell stalk mismatch) ()
validatePreparedStore preparedDescent store
  | psdModelFingerprint preparedDescent /= totalSectionModelFingerprint store =
      Left
        ( SectionDescentStoreFailed
            ( SectionStoreModelFingerprintMismatch
                (psdModelFingerprint preparedDescent)
...
descentProgramForObservation ::
  ...
  TotalSectionStore cell stalk ->
  [KeyedSectionDelta stalk] ->
  Either (SectionDescentError cell stalk mismatch) (PreparedSectionProgram stalk)
descentProgramForObservation observation store deltas =
  case observation of
    ObserveEachStep -> do
      traverse_ (validateDeltaFingerprint (totalSectionModelFingerprint store)) deltas
      pure
        PreparedSectionProgram
          { pspModelFingerprint = totalSectionModelFingerprint store,
            pspModelVersion = totalSectionModelVersion store,
    ...
    ObserveFinalSection ->
      finalSectionDescentProgram
        (totalSectionModelFingerprint store)
        (totalSectionModelVersion store)
...
validateDeltaFingerprint ::
  ModelFingerprint ->
  KeyedSectionDelta stalk ->
  Either (SectionDescentError cell stalk mismatch) ()
validateDeltaFingerprint expectedFingerprint delta
  | ksdModelFingerprint delta /= expectedFingerprint =
      Left
        ( SectionDescentStoreFailed
            ( SectionStoreModelFingerprintMismatch
                expectedFingerprint
...
finalSectionDescentProgram ::
  ...
  [KeyedSectionDelta stalk] ->
  Either (SectionDescentError cell stalk mismatch) (PreparedSectionProgram stalk)
finalSectionDescentProgram expectedFingerprint expectedVersion objectCountValue deltas =
  finalSectionStateToProgram expectedFingerprint expectedVersion objectCountValue
    <$> List.foldl'
      (finalSectionDeltaStep expectedFingerprint)
...
finalSectionDeltaStep ::
  ...
  KeyedSectionDelta stalk ->
  Either (SectionDescentError cell stalk mismatch) (FinalSectionDeltaState stalk)
finalSectionDeltaStep expectedFingerprint stateResult delta =
  case stateResult of
    Left descentError ->
      Left descentError
    Right stateValue -> do
...
finalSectionStateToProgram ::
  ...
  FinalSectionDeltaState stalk ->
  PreparedSectionProgram stalk
finalSectionStateToProgram expectedFingerprint expectedVersion objectCountValue stateValue =
  PreparedSectionProgram
    { pspModelFingerprint = expectedFingerprint,
      pspModelVersion = expectedVersion,
...
deltaInstruction :: KeyedSectionDelta stalk -> PreparedSectionInstruction stalk
deltaInstruction delta =
  PreparedSectionDelta (ksdExtent delta) (ksdAssignments delta)

singletonDeltaShape :: KeyedSectionDelta stalk -> FinalSectionDeltaState stalk
...
coalesceDeltaUnchecked :: CoalescedSectionDelta stalk -> KeyedSectionDelta stalk -> CoalescedSectionDelta stalk
...
runDenseSectionMutationWithKernelSource ::
  ...
  TotalSectionStore cell stalk ->
  ( forall s.
    DenseDescentSession s cell witness stalk mismatch repairObstruction ->
...
closeDenseDescentFrontierFromScope ::
  Ord cell =>
  DenseDescentContext s cell witness stalk mismatch repairObstruction ->
  Scope IntSet ->
  ST s (Either (SectionDescentError cell stalk mismatch) DescentDirtyCoverage)
