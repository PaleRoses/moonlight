...
newtype GlobalSection cell stalk = GlobalSection
  { globalUnderlyingSection :: TotalSectionStore cell stalk
  }
...
certifyPreparedSectionCompatibilityWith ::
  ...
  TotalSectionStore cell stalk ->
  Either (SectionCertificationError cell) (SectionCertification cell mismatch)
certifyPreparedSectionCompatibilityWith preparedDescent stalkAlgebra section = do
  validatePreparedCertificationStore preparedDescent section
...
validatePreparedCertificationStore ::
  PreparedSectionDescent cell witness ->
  TotalSectionStore cell stalk ->
  Either (SectionCertificationError cell) ()
validatePreparedCertificationStore preparedDescent section
  | psdModelFingerprint preparedDescent /= totalSectionModelFingerprint section =
      Left
        ( SectionCertificationStoreFailed
            ( SectionStoreModelFingerprintMismatch
                (psdModelFingerprint preparedDescent)
                (totalSectionModelFingerprint section)
            )
        )
  | otherwise =
      Right ()
...
mkGlobalSection ::
  ...
  StalkAlgebra witness stalk mismatch repairObstruction ->
  TotalSectionStore cell stalk ->
  Either (SectionCertificationFailure cell mismatch) (GlobalSection cell stalk)
...
preparedIncidentRestrictionIds :: PreparedSectionDescent cell witness -> Int -> IntSet
