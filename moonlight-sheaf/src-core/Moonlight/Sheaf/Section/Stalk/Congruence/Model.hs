...
import Moonlight.Sheaf.Section.Congruence.Equivalence.Domain
  ( DomainEndomap,
    DomainEquivalence,
    EquivalenceDomain,
...
type PreparedCongruenceModel :: Type -> Type -> Type -> Type -> Type
type role PreparedCongruenceModel nominal nominal nominal nominal
data PreparedCongruenceModel carrier cell rep atom = PreparedCongruenceModel
  { pcmDomain :: !(EquivalenceDomain carrier rep),
    pcmVisibleByCell :: !(Map.Map cell IntSet),
    pcmSheafModel :: !(SheafModel cell (PreparedCongruenceRestriction carrier rep atom))
  }
  deriving stock (Eq, Show)
...
applyCanonicalCongruenceStalkSeeds ::
  ...
  [(rep, rep)] ->
  CongruenceStalk rep atom ->
  CongruenceStalkMergeDelta rep atom
applyCanonicalCongruenceStalkSeeds canonicalizedUnions stalk =
  let (relationValue, changedKeys, _) =
        applyRuntimeCongruenceMerges canonicalizedUnions (csRelation stalk)
   in CongruenceStalkMergeDelta
...
applyCongruenceStalkMergesCounted repPairs stalk =
  let (relationValue, changedKeys, mergeCount) =
...
prepareCongruenceModelWith ::
  ...
  (forall carrier. PreparedCongruenceModel carrier cell rep atom -> result) ->
...
preparedCongruenceSheafModel ::
  PreparedCongruenceModel carrier cell rep atom ->
  SheafModel cell (PreparedCongruenceRestriction carrier rep atom)
preparedCongruenceSheafModel =
  pcmSheafModel
...
normalizePreparedCongruenceStalk ::
  ...
  PreparedCongruenceStalk carrier rep atom ->
  PreparedCongruenceStalk carrier rep atom
normalizePreparedCongruenceStalk stalk =
  stalk
    { pcsRelation =
        normalizeDomainEquivalence (pcsRelation stalk)
    }
...
normalizeRuntimeRelation ::
  ...
  RuntimeCongruenceRelation rep
normalizeRuntimeRelation (RuntimeCongruenceRelation relationValue) =
  RuntimeCongruenceRelation (normalizeDomainEquivalence relationValue)
...
applyRuntimeCongruenceMerges repPairs (RuntimeCongruenceRelation relationValue) =
  let (domainRelation, changedKeys, mergeCount) =
        applyDomainEquivalenceMergesCounted repPairs relationValue
   in ( RuntimeCongruenceRelation domainRelation,
        changedKeys,
        mergeCount
      )
