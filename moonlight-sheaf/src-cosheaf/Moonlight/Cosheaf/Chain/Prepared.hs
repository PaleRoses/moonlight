...
data PreparedCosheafBoundary cell coefficient provenance = PreparedCosheafBoundary
  { pcbDegree :: !HomologicalDegree,
    pcbSourceBasis :: !(LinearBasis cell),
    pcbTargetBasis :: !(LinearBasis cell),
    pcbIncidence :: !(BoundaryIncidence coefficient),
    pcbEntryProvenance :: !(Map (Int, Int) ProvenanceId),
    pcbProvenanceArena :: !(ProvenanceArena (NonEmpty provenance))
  }
  deriving stock (Eq, Show)
...
data PreparedCosheafChain site cell coefficient provenance = PreparedCosheafChain
  { pccSite :: !site,
    pccMaxDegree :: !HomologicalDegree,
    pccBasisByDegree :: !(Map HomologicalDegree (LinearBasis cell)),
    pccBoundaryByDegree :: !(Map HomologicalDegree (PreparedCosheafBoundary cell coefficient provenance)),
    pccChainComplex :: !(FiniteChainComplex coefficient)
  }
...
buildPreparedCosheafBoundary coefficientOps degreeValue sourceBasis targetBasis rawTerms = do
  ...
  pure
    PreparedCosheafBoundary
      { pcbDegree = degreeValue,
        pcbSourceBasis = sourceBasis,
        pcbTargetBasis = targetBasis,
        pcbIncidence = incidence,
        pcbEntryProvenance = entryProvenanceValue,
        pcbProvenanceArena = arenaValue
      }
  where
    sourceCount =
...
mkPreparedCosheafChain coefficientOps siteValue maxDegree basisByDegree boundaryByDegree = do
  ...
  pure
    PreparedCosheafChain
      { pccSite = siteValue,
        pccMaxDegree = maxDegree,
        pccBasisByDegree = basisByDegree,
        pccBoundaryByDegree = boundaryByDegree,
        pccChainComplex = chainComplex
      }
  where
...
preparedCosheafBoundaryEntryProvenance degreeValue sourceIndexValue targetIndexValue chain = do
  ...
  provenanceId <-
    maybe
      (Left (PreparedCosheafChainBoundaryMissing degreeValue))
      Right
      (Map.lookup (sourceIndexValue, targetIndexValue) (pcbEntryProvenance boundaryValue))
  maybe
    (Left (PreparedCosheafChainBoundaryProvenanceMissing degreeValue sourceIndexValue targetIndexValue provenanceId))
    Right
    (lookupProvenance provenanceId (pcbProvenanceArena boundaryValue))
