...
buildPreparedFiniteCosheafChainDenseFromPreparedSupport preparedSupport = do
  ...
  pure
    PreparedFiniteCosheafChain
      { pfccCosheaf = cosheaf,
        pfccMaxDegree = HomologicalDegree maxDegreeInt,
        pfccBasisByDegree = basisByDegree,
        pfccBoundariesByDegree = boundariesByDegree,
        pfccChainComplex = chainComplexValue
      }
  where
    supportPlan =
...
mkDenseCosheafChainBasisTable cosheaf degreeValue denseCells = do
  ...
  case duplicatesOrd (Vector.toList keys) of
    ...
    [] ->
      pure
        CosheafChainBasisTable
          { ccbtDegree = degreeValue,
            ccbtCells = publicCells,
            ccbtIndexByKey =
              Map.fromList
