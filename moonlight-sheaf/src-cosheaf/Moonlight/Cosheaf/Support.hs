...
data PreparedCosheafSupport site value = PreparedCosheafSupport
  { pcsCosheaf :: !(FiniteCosheaf site value),
    pcsPlan :: !CosheafSupportPlan,
    pcsCorestrictions :: ![CompiledCorestriction (SiteObject site) (SiteMorphism site)],
    pcsCostalkKeysByObject :: !(IntMap IntSet)
  }
...
prepareCosheafSupport cosheaf plan = do
  ...
  pure
    PreparedCosheafSupport
      { pcsCosheaf = cosheaf,
        pcsPlan = plan,
        pcsCorestrictions = retainedCorestrictions,
        pcsCostalkKeysByObject = retainedCostalkKeysByObject
      }
  where
    knownObjectKeys =
