...
tests =
  testGroup
    ...
    [ testCase "restricted support exposes exact retained/pruned footprint" testRestrictedSupportFootprint,
      ...
      testCase "supported chain does not enumerate pruned global composition pairs" testSupportedChainSkipsPrunedGlobalCompositionPairs,
...
testSupportedChainSkipsPrunedGlobalCompositionPairs :: Assertion
testSupportedChainSkipsPrunedGlobalCompositionPairs = do
  cosheaf <- expectRight (chainCosheaf (ChainSite ChainGoodSite) chainGoodAlgebra)
  restrictedSupportPlan <- chainAOnlySupportPlanAtDegree 2 cosheaf
  let trapCosheaf =
        cosheaf {fcSite = ChainSite ChainMissingCompositeSite}
  restrictedPlan <- expectRight (prepareFiniteCosheafChainFromSupportPlan restrictedSupportPlan trapCosheaf)
  assertEqual
    "restricted support reaches degree 2 without consulting the pruned ChainB -> ChainC / ChainA -> ChainB composite"
    0
    (sourceCardinality (cosheafBoundaryIncidenceAt (HomologicalDegree 2) restrictedPlan))
  fullSupportPlan <- expectRight (fullFiniteCosheafChainSupportPlan 2 trapCosheaf)
  case prepareFiniteCosheafChainFromSupportPlan fullSupportPlan trapCosheaf of
    Left (CosheafSupportChainFailed (CosheafChainCompositeUndefined outerMorphism innerMorphism)) -> do
      assertEqual "full support trips the trap on the outer morphism" ChainBC (cmWitness outerMorphism)
      assertEqual "full support trips the trap on the inner morphism" ChainAB (cmWitness innerMorphism)
    Left otherFailure ->
      assertFailure ("unexpected full-support chain failure: " <> show otherFailure)
    Right _ ->
      assertFailure "full support should enumerate the global ChainBC . ChainAB composite and trip the trap"
