...
instance Site CoverSite where
  ...
  coversAt _ objectValue =
    case objectValue of
      CoverRoot -> either (const []) pure coverFamily
      _ -> []
...
coverFamily :: Either String (CoveringFamily CoverObject CoverMorphism)
coverFamily =
  mapLeft show (mkCoveringFamily CoverRoot (coverLeftToRoot :| [coverRightToRoot]))

coverFamilyWithSyntheticLeft :: Either String (CoveringFamily CoverObject CoverMorphism)
coverFamilyWithSyntheticLeft =
  mapLeft show (mkCoveringFamily CoverRoot (coverSyntheticLeftToRoot :| [coverRightToRoot]))
