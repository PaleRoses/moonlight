...
data BlockFailure
  = BlockFailure
  | PrunedBlockForced
  | BlockIncidenceShapeFailure !BoundaryIncidenceShapeError
...
intervalSpec block =
  LinearCosheafChainSpec
    { lccsSite = IntervalSite,
      ...
      lccsCorestrictionBlock = const (blockMatrix block),
...
blockMatrix ::
  (Eq coefficient, Num coefficient, Semiring coefficient) =>
  IntervalBlock coefficient ->
  Either BlockFailure (BoundaryIncidence coefficient)
blockMatrix block
  | intervalBlockCoefficient block == 1 =
      Right (identityBoundaryIncidenceOf (fromIntegral (intervalRank block)))
  | otherwise =
      first BlockIncidenceShapeFailure
        ( mkBoundaryIncidenceFromOrderedEntries
