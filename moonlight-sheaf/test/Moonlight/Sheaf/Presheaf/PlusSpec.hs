...
tests =
  testGroup
    ...
    [ testCase "identity-only basis makes F+ a finite presheaf with base classes" testIdentityOnlyPlusPresheaf,
      ...
      testCase "missing cached pullback plan is typed" testMissingCachedPullbackPlanIsTyped,
      testCase "prepared plus table reports malformed basis during construction" testMalformedBasisIsTypedAtConstruction,
...
testFiniteCoverBasisCachesPullbackPlans = do
  ...
  cachedPlan @?= preparedPlan {pcpPulledCover = canonicalPulledCover}

testMissingCachedPullbackPlanIsTyped :: Assertion
testMissingCachedPullbackPlanIsTyped = do
  basis <- expectRight (mkFiniteCoverBasis coveredSimpleSite)
  rootIdentity <- expectRight (finiteIdentityCoverAt basis Root)
  let malformedBasis = basis {fcbPullbackPlansByTarget = Map.empty}
  case finitePullbackCoverPlan malformedBasis leafToRoot rootIdentity of
    Left (FiniteCoverBasisPullbackPlanMissing morphismValue coverPlan) -> do
      morphismValue @?= leafToRoot
      coverPlan @?= rootIdentity
    Left otherFailure ->
      assertFailure ("expected missing cached pullback plan, received " <> show otherFailure)
    Right _ ->
      assertFailure "expected missing cached pullback plan, received pullback plan"

testMalformedBasisIsTypedAtConstruction :: Assertion
testMalformedBasisIsTypedAtConstruction = do
  basis <- expectRight (mkFiniteCoverBasis coveredSimpleSite)
  leafIdentity <- expectRight (finiteIdentityCoverAt basis Leaf)
  let malformedBasis = removeCoverFromBasis Leaf leafIdentity basis
  presheaf <- expectRight (mkSimplePresheaf coveredSimpleSite [RootSection 0] [LeafSection 0])
  case plusConstruction unboundedBudget malformedBasis presheaf of
    Left
      ( PlusRestrictionTableBuildFailed
          morphismValue
          _classValue
          (PlusRestrictionRepresentativeNotIndexed Leaf coverPlan _sections)
        ) -> do
      morphismValue @?= leafToRoot
      coverTarget (effectiveCoverFamily coverPlan) @?= Leaf
    Left otherFailure ->
      assertFailure ("expected prepared-table construction obstruction, received " <> show otherFailure)
    Right _ ->
      assertFailure "expected prepared-table construction obstruction, received plus construction"
...
removeCoverFromBasis ::
  SimpleObject ->
  EffectiveCoverPlan SimpleObject SimpleMorphism ->
  FiniteCoverBasis SimpleSite ->
  FiniteCoverBasis SimpleSite
removeCoverFromBasis objectValue coverPlan basis =
  basis
    { fcbCoversByTarget =
        Map.adjust
          (filter ((/= effectiveCoverFamily coverPlan) . effectiveCoverFamily))
          objectValue
          (fcbCoversByTarget basis)
    }
