...
testSupportedRuleSpecActivation =
  withPreparedContextSiteFromFiniteLattice twistLattice $ \twistSite ->
    let book =
          fixtureValue
            "twist fixture rule book"
            ( supportedRuleBook
                twistSite
                [ SupportedRuleSpec (support twistSite [BottomCtx]) "everywhere",
                  SupportedRuleSpec (support twistSite [LeftCtx]) "left-only",
                  SupportedRuleSpec (support twistSite [RightCtx]) "right-only"
                ]
            )
     in do
...
support :: PreparedContextSite owner TwistCtx -> [TwistCtx] -> ClassSiteSupport TwistCtx
support site =
