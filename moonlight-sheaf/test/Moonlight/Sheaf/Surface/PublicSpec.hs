...
prepareMiniSite :: (forall owner. PreparedSite owner MiniSite -> Assertion) -> Assertion
prepareMiniSite continue =
  case compile (siteSpec MiniSite) continue of
...
testPrepareRejectsBadSite =
  case compile (siteSpec BadIdentitySite) (const ()) of
...
branchRootCover :: CoveringFamily BranchContext BranchMorphism
branchRootCover =
...
branchRepeatedSourceCover :: CoveringFamily BranchContext BranchMorphism
...
instance Site BranchSite where
  ...
  coversAt _ contextValue =
    case contextValue of
      BranchBase ->
        [branchRootCover, branchRepeatedSourceCover]
      _ -> []
...
branchGluingAlgebra :: GluingAlgebra owner BranchSite BranchStalk BranchGluingFailure
branchGluingAlgebra =
  GluingAlgebra
...
branchRejectingGluingAlgebra :: GluingAlgebra owner BranchSite BranchStalk BranchGluingFailure
...
prepareBranchSite :: (forall owner. PreparedSite owner BranchSite -> Assertion) -> Assertion
prepareBranchSite continue =
  case compile (siteSpec BranchSite) continue of
    Left failure -> assertFailure ("expected branch site preparation to succeed, received " <> show failure)
    Right assertion -> assertion

withBranchRootCoverPlan :: PreparedSite owner BranchSite -> (PreparedCover owner BranchSite -> Assertion) -> Assertion
...
withBranchRepeatedSourceCoverPlan :: PreparedSite owner BranchSite -> (PreparedCover owner BranchSite -> Assertion) -> Assertion
...
branchMismatchSections :: PreparedCover owner BranchSite -> Vector BranchStalk
...
branchAuthoritySections :: PreparedCover owner BranchSite -> Vector BranchStalk
