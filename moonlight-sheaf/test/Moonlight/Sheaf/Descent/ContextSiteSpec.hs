...
testContainmentAgreesWithLattice = do
...
testNormalizeAgreesWithLattice = do
  candidates <- supportCandidates
  mapM_
    ( \supportValue -> do
        compiled <-
...
testVisibleKeysMirrorContainment = do
...
testKeyedVisibilityMirrorsCarrierSupport = do
  entries <- indexedSupportEntries
  builtIndex <-
    requireRight
      (classSupportIndexFromEntries branchSite (IntMap.fromList entries))
...
testCarrierStorageKeepsGenerators = do
...
testSupportDeltaTouchesCarriers = do
  localSupport <- requireRight (supportBasis branchContextLattice [BranchLeft])
