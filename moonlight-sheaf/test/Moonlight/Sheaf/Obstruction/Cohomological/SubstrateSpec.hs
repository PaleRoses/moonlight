...
import Moonlight.Sheaf.Section.Model (SheafModel, emptySheafModel)
import Moonlight.Sheaf.Kernel.Basis (basisCells)
import Moonlight.Sheaf.Section.ObjectIndex (SheafModelVersion (..), mkObjectIndex)
import Moonlight.Sheaf.Section.Restriction (emptyRestrictionIndex)
import Moonlight.Sheaf.Section.Store.State (emptyTotalSectionStoreWith)
  ...
  do
    zeroModel <-
      first (InvalidTopologyInput . show)
        ( emptySheafModel
            (SheafModelVersion 0)
            (mkObjectIndex (basisCells emptySheafBasis))
        )
    fmap
      ( \chainComplex ->
          ObstructionLift
            { olRegion = regionValue,
              ...
              olSection0 = emptyTotalSectionStoreWith zeroModel (const (ExpandedStalk ())),
              olRoot = rootValue,
