...
testSelfLoopKeepsDirectAssignment =
  case selfLoopSectionModel of
    Left indexError ->
      assertFailure ("expected self-loop sheaf model construction to succeed: " <> show indexError)
    Right model -> do
      assignment <- sectionAssignment model (Map.fromList [(Cell0, MiniStalk 2.0)])
      case repairPresheafAssignment assignment of
        Left obstruction ->
...
sectionAssignment ::
  SheafModel MiniCell MiniRestrictionSpec ->
  Map.Map MiniCell MiniStalk ->
  AssertionWithAssignment
sectionAssignment =
...
type AssertionWithAssignment = IO (PresheafAssignment MiniCell MiniStalk () MiniRestrictionSpec ())

withSectionModel ::
  (SheafModel MiniCell MiniRestrictionSpec -> Assertion) ->
  Assertion
withSectionModel useModel =
  case sectionModel of
    Left indexError ->
      assertFailure ("expected sheaf model construction to succeed: " <> show indexError)
    Right model ->
      useModel model
...
withSimpleSectionModel useModel =
  case simpleSectionModel of
    Left indexError ->
      assertFailure ("expected simple sheaf model construction to succeed: " <> show indexError)
    Right model ->
      useModel model
...
sectionModel :: Either (RestrictionIndexError MiniCell) (SheafModel MiniCell MiniRestrictionSpec)
sectionModel =
  first modelRestrictionError $
    prepareSheafModel
      (SheafModelVersion 0)
      ...
      sectionRestrictionSpecs
  where
    modelRestrictionError :: SheafModelBuildError cell -> RestrictionIndexError cell
    modelRestrictionError modelError =
      case modelError of
        SheafModelRestrictionBuildError restrictionError -> restrictionError
...
simpleSectionModel :: Either (RestrictionIndexError MiniCell) (SheafModel MiniCell MiniRestrictionSpec)
simpleSectionModel =
  first modelRestrictionError $
    prepareSheafModel
      (SheafModelVersion 0)
...
selfLoopSectionModel :: Either (RestrictionIndexError MiniCell) (SheafModel MiniCell MiniRestrictionSpec)
selfLoopSectionModel =
  first modelRestrictionError $
    prepareSheafModel
      (SheafModelVersion 0)
