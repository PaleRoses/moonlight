...
tests =
  testGroup
    ...
    [ testCase "partial rep map completes singleton blocks over the declared universe" testSingletonCompletion,
      ...
      testProperty "join is idempotent up to normalization" propJoinIdempotent,
      testProperty "join is commutative up to normalization" propJoinCommutative,
      testProperty "join is associative up to normalization" propJoinAssociative,
        ...
        (equivalenceFromPartialRepMap (IntSet.fromList [1, 2, 3, 4]) (IntMap.fromList [(1, 1 :: Int), (2, 1)])) $ \relationValue ->
          assertRight "expected domain equivalence"
            (mkDomainEquivalence domain relationValue) $ \domainRelation -> do
              let (merged, changed, mergeCount) =
                    applyDomainEquivalenceMergesCounted [(3, 4), (2, 4)] domainRelation
                  closed = domainEquivalenceRaw merged
...
testCanonicalUnionNormalization =
  assertRight "expected relation construction"
    (equivalenceFromPartialRepMap (IntSet.fromList [1, 2, 3]) (IntMap.fromList [(1, 1 :: Int), (3, 1)])) $ \relationValue ->
      canonicalizeEquivalenceUnions relationValue [(1, 2), (3, 1), (2, 1)] @?= [(1, 2)]
...
normalizedRaw =
  domainEquivalenceRaw . normalizeDomainEquivalence
