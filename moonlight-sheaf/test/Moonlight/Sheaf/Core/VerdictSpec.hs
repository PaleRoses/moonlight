...
propAnyAcceptedAccepts =
  forAll genNonEmptyVerdicts $ \verdicts ->
...
propAllRejectedAccumulates =
  forAll genNonEmptyVerdicts $ \verdicts ->
    not (any verdictAllowed verdicts)
      ==> verdictRejectedList (acceptIfAnyAccepted verdicts) === concatMap verdictRejectedList (NonEmpty.toList verdicts)

genNonEmptyVerdicts :: Gen (NonEmpty (ObstructionVerdict Int))
genNonEmptyVerdicts =
  (:|) <$> genVerdict <*> listOf genVerdict
