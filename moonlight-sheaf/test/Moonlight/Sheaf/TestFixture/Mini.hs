...
miniSection :: Either String (TotalSectionStore MiniCell MiniStalk)
miniSection =
  fmap
    (\model -> emptyTotalSectionStoreWith model (const (MiniStalk 0.0)))
    miniSheafModel
...
miniSheafModel :: Either String (SheafModel MiniCell MiniRestriction)
miniSheafModel =
  first show (emptySheafModel (SheafModelVersion 0) (mkObjectIndex [Cell0, Cell1]))
