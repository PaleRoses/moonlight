...
instance ContextCoverBasis SampleSystem where
  contextCoversAt currentSystem contextValue =
    case contextValue of
      RootCtx ->
        buildCover currentSystem RootCtx (LeftCtx :| [RightCtx])
      LeftCtx ->
        buildCover currentSystem LeftCtx (MeetCtx :| [])
      RightCtx ->
        buildCover currentSystem RightCtx (MeetCtx :| [])
      MeetCtx ->
        []
...
buildCover currentSystem targetContext sourceContexts =
  case contextCoverFromSources currentSystem targetContext sourceContexts of
    Left _ ->
      []
    Right coverValue ->
      [coverValue]
