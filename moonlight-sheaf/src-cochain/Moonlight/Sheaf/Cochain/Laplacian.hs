...
data RestrictionGraphPlanKind
  = TarskiRestrictionGraphPlan
  | SemiringRestrictionGraphPlan
  deriving stock (Eq, Ord, Show)

data RestrictionGraphPlan cell witness = RestrictionGraphPlan
  { rgpKind :: RestrictionGraphPlanKind,
    rgpLinearBasis :: LinearBasis cell,
    rgpRestrictions :: [Restriction cell witness]
  }
...
data SheafLaplacian (kind :: LaplacianKind) cell = SheafLaplacian
  { slDegree :: HomologicalDegree,
    slDomainBasis :: LinearBasis cell,
    slCodomainBasis :: LinearBasis cell,
    slIncidence :: BoundaryIncidence Double,
    slHigherStructure :: HigherStructureOf kind
  }
...
mkSheafLaplacian degree domainBasis codomainBasis incidence higherStructure = do
  ...
  Right
    SheafLaplacian
      { slDegree = degree,
        slDomainBasis = domainBasis,
        slCodomainBasis = codomainBasis,
        slIncidence = incidence,
        slHigherStructure = higherStructure
      }
...
prepareRestrictionGraphPlanWithDimensions ::
  ...
  Either (SheafOperatorBuildError cell) (RestrictionGraphPlan cell witness)
...
buildTarskiLaplacianFromPlan ::
  Ord cell =>
  RestrictionGraphPlan cell witness ->
