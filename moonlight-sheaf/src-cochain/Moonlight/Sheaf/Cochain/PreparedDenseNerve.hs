...
-- | Prepared dense-nerve cochain plans: preallocated coboundary layouts for
-- rank-one and general stalks.
...
data PreparedDenseNerveCochainError tag
  ...
  | PreparedDenseNerveScaffoldFailed !HomologyFailure
...
instance Show (PreparedDenseNerveCochainError tag) where
  show errorValue =
    case errorValue of
      ...
      PreparedDenseNerveScaffoldFailed failure ->
        "PreparedDenseNerveScaffoldFailed " <> show failure
...
preparedDenseNerveComplexScaffold plan = do
  ...
  first PreparedDenseNerveScaffoldFailed $
    mkSiteComplexScaffoldFromCells plan cellsByDimensionValue chainComplexValue
