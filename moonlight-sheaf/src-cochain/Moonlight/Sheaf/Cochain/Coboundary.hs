-- | Coboundary operators over oriented nerve bases: contributions, reducers,
-- and assembled specs.
...
data CoboundaryEntry cell witness = CoboundaryEntry
  { coboundaryEntryRestrictionInternal :: Restriction cell witness,
    coboundaryEntrySourceBasisSlotInternal :: !(Maybe Int),
    coboundaryEntryTargetBasisSlotInternal :: !(Maybe Int)
  }
  deriving stock (Eq, Show)

ceRestriction :: CoboundaryEntry cell witness -> Restriction cell witness
ceRestriction = coboundaryEntryRestrictionInternal

ceSourceBasisSlot :: CoboundaryEntry cell witness -> Maybe Int
ceSourceBasisSlot = coboundaryEntrySourceBasisSlotInternal

ceTargetBasisSlot :: CoboundaryEntry cell witness -> Maybe Int
ceTargetBasisSlot = coboundaryEntryTargetBasisSlotInternal
...
data CoboundaryMatrix cell witness = CoboundaryMatrix
  { coboundaryMatrixDimensionInternal :: HomologicalDegree,
    coboundaryMatrixEntriesInternal :: [CoboundaryEntry cell witness],
    coboundaryMatrixSourceBasisInternal :: SheafBasis cell,
    coboundaryMatrixTargetBasisInternal :: SheafBasis cell
  }
  deriving stock (Eq, Show)

cmDimension :: CoboundaryMatrix cell witness -> HomologicalDegree
cmDimension = coboundaryMatrixDimensionInternal

cmEntries :: CoboundaryMatrix cell witness -> [CoboundaryEntry cell witness]
cmEntries = coboundaryMatrixEntriesInternal

cmSourceBasis :: CoboundaryMatrix cell witness -> SheafBasis cell
cmSourceBasis = coboundaryMatrixSourceBasisInternal

cmTargetBasis :: CoboundaryMatrix cell witness -> SheafBasis cell
cmTargetBasis = coboundaryMatrixTargetBasisInternal
...
buildCoboundaryFromIncidenceRestrictions spec incidenceRestrictionValues = do
  ...
  mkCoboundaryMatrix spec entries
...
mkCoboundaryMatrix ::
  ...
  Either (SheafOperatorBuildError cell) (CoboundaryMatrix cell witness)
mkCoboundaryMatrix spec entries = do
  checkedEntries <- traverse (checkEntryAgainstCoboundarySpec spec) entries
  pure
    CoboundaryMatrix
      { coboundaryMatrixDimensionInternal = csDimension spec,
        coboundaryMatrixEntriesInternal = checkedEntries,
        coboundaryMatrixSourceBasisInternal = csSourceBasis spec,
        coboundaryMatrixTargetBasisInternal = csTargetBasis spec
      }

checkEntryAgainstCoboundarySpec ::
  Ord cell =>
  CoboundarySpec cell ->
  CoboundaryEntry cell witness ->
  Either (SheafOperatorBuildError cell) (CoboundaryEntry cell witness)
checkEntryAgainstCoboundarySpec spec entry = do
  sourceSlot <-
    maybe
      (Left (OperatorCellAbsentFromBasis OperatorSourceBasis (ceSourceCell entry)))
      Right
      (basisCellIndex (ceSourceCell entry) (csSourceBasis spec))
  targetSlot <-
    maybe
      (Left (OperatorCellAbsentFromBasis OperatorTargetBasis (ceTargetCell entry)))
      Right
      (basisCellIndex (ceTargetCell entry) (csTargetBasis spec))
  mkCoboundaryEntryAtSlots (ceRestriction entry) sourceSlot targetSlot
...
applyCoboundary ::
  ...
  CoboundaryMatrix cell witness ->
  SheafModel owner cell modelWitness ->
...
mkCoboundaryEntryAtSlotsValue restriction sourceSlot targetSlot =
  case rKind restriction of
    IncidenceRestriction _coefficient ->
      Right
        CoboundaryEntry
          { coboundaryEntryRestrictionInternal = restriction,
            coboundaryEntrySourceBasisSlotInternal = sourceSlot,
            coboundaryEntryTargetBasisSlotInternal = targetSlot
          }
    PortalRestriction ->
