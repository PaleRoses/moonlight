...
filterGroupSelector selectorName keep =
  matchSelectorByDecision selectorName $ \view group match ->
...
matchSelectorByDecision ::
  String ->
  (view -> group -> match -> MatchDecision traceEntry) ->
  MatchSelector view group match traceEntry
matchSelectorByDecision selectorName decide =
  MatchSelector
    { matchSelectorName = selectorName,
      matchSelectorPreservesCount = False,
      runMatchSelector = selectMatchesByDecision decide
    }
