...
contextKeySetUnionImages ::
  ...
  ContextKeySet ->
  ContextKeySet
contextKeySetUnionImages chunkCount image domain =
  contextKeySetFoldr
...
contextKeySetFilter predicate (ContextKeySet chunks) =
  ...
  where
    filterChunk chunkIndex chunkValue =
      retainBits (chunkIndex * contextKeyBitsPerChunk) 0 chunkValue
    ...
    findChunk !chunkIndex
      ...
      | otherwise =
          case findWord
            ...
            (vectorWordAt chunks chunkIndex) of
...
contextKeySetFindDifference (ContextKeySet leftChunks) (ContextKeySet rightChunks) =
  ...
  where
    ...
    findChunk !chunkIndex
      | chunkIndex >= chunkCount = Nothing
      | otherwise =
          let leftChunk = vectorWordAt leftChunks chunkIndex
              rightChunk = vectorWordAt rightChunks chunkIndex
...
contextKeySetFoldr step initial (ContextKeySet chunks) =
  ...
  where
    foldChunk chunkIndex chunkValue rest =
      foldWord (chunkIndex * contextKeyBitsPerChunk) chunkValue rest
