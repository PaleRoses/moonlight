    ...
    upperCoverKeyOrdinals joinIrreducibleIndex
      ...
      | otherwise =
          let ContextKey upperOrdinal =
           ...
           in [upperOrdinal]
      where
        joinIrreducibleKey = ContextKey (vectorIntAt (cdipJoinIrreducibleKeyByIndex plan) joinIrreducibleIndex)
...
maskRowsForKeys size joinIrreducibleKeys upperRows =
  maskRowsGenerate size (UVector.length joinIrreducibleKeys) $ \keyOrdinal joinIrreducibleIndex ->
    contextKeyRelated
      upperRows
      (ContextKey (vectorIntAt joinIrreducibleKeys joinIrreducibleIndex))
      (ContextKey keyOrdinal)
...
upClosureRowsForJoinIrreducibles joinIrreducibleKeys upperRows =
  maskRowsGenerate joinIrreducibleCount joinIrreducibleCount $ \sourceIndex targetIndex ->
    contextKeyRelated
      upperRows
      (ContextKey (vectorIntAt joinIrreducibleKeys sourceIndex))
      (ContextKey (vectorIntAt joinIrreducibleKeys targetIndex))
  where
    joinIrreducibleCount = UVector.length joinIrreducibleKeys
...
maskMember bitIndex (JoinIrreducibleMask chunks)
  ...
  | otherwise = vectorWordAt chunks chunkIndex .&. bit (bitIndex .&. (bitsPerChunk - 1)) /= 0
  where
...
maskUnionImages ::
  ...
  JoinIrreducibleMask ->
  JoinIrreducibleMask
maskUnionImages chunkCount image domain =
  maskFoldr
    (maskUnion . image)
...
maskFoldr step initial (JoinIrreducibleMask chunks) =
  ...
  where
    foldChunk chunkIndex chunkValue rest =
      foldWord (chunkIndex * bitsPerChunk) chunkValue rest
...
maskZipWith combine (JoinIrreducibleMask leftChunks) (JoinIrreducibleMask rightChunks) =
  JoinIrreducibleMask
    ( UVector.generate
        ...
        ( \chunkIndex ->
            combine
              (vectorWordAt leftChunks chunkIndex)
              (vectorWordAt rightChunks chunkIndex)
        )
    )
{-# INLINE maskZipWith #-}
...
maskSignature (JoinIrreducibleMask chunks) =
  case UVector.length chunks of
    ...
    1 -> JoinIrreducibleMaskSignature1 (vectorWordAt chunks 0)
    2 -> JoinIrreducibleMaskSignature2 (vectorWordAt chunks 0) (vectorWordAt chunks 1)
