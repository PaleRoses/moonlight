...
tests =
  testGroup
    ...
    [ testCase "generator-built systems expose the closure as all contexts" testGeneratorClosureAgreement,
      ...
      testCase "resolution source poset is projected from the prepared derived complex" testResolutionSourcePosetMatchesDerived,
...
testResolutionSourcePosetMatchesDerived :: IO ()
testResolutionSourcePosetMatchesDerived =
  withToySpansResolution2 $ \resolutionValue ->
    assertEqual
      "the prepared derived complex owns the resolution source poset"
      (derivedPoset (resolutionDerivedComplex resolutionValue))
