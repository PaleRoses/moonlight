...
import Moonlight.Derived.Site (derivedPosetNodes, mkDerivedPosetFunctor)
    ...
    (Right resolutionValue, Right reductionValue) -> do
      ...
      case explicitGrothendieckCochain (mkGrothendieckSite singleRuleSystem 1) of
        Left constructionFailure ->
          assertFailure ("explicit Grothendieck cochain construction failed: " <> show constructionFailure)
...
data ResolutionSpectralDecisionFailure
  = ResolutionSpectralBidegreeAbsent !BasisCellRef
  | ResolutionSpectralGateFailed !SpectralPruningFailure
  deriving stock (Eq, Show)

resolutionSpectralDecisions ::
  [SpectralPage Rational] ->
  Map.Map BasisCellRef Bidegree ->
  Map.Map RegionNodeId BasisCellRef ->
  Int ->
  [RegionNodeId] ->
  Either ResolutionSpectralDecisionFailure [(RegionNodeId, Bool)]
resolutionSpectralDecisions spectralPages bidegreesByCell basisCellByNode pageNumber =
  traverse decisionAtNode
  where
    decisionAtNode nodeId =
      fmap
        (\kept -> (nodeId, kept))
        ( case Map.lookup nodeId basisCellByNode of
            Nothing -> Right True
            Just basisCellRef -> do
              bidegree <-
                maybe
                  (Left (ResolutionSpectralBidegreeAbsent basisCellRef))
                  Right
                  (Map.lookup basisCellRef bidegreesByCell)
              first ResolutionSpectralGateFailed
                ( spectralPruningGate
                    (mkSpectralPruningOracle spectralPages (const bidegree))
                    pageNumber
                    id
                    basisCellRef
                )
    ...
    Right resolutionValue ->
      case raBoundaryAnalysis (rbAnalysis resolutionValue) of
        ...
        Right boundaryAnalysis -> do
          ...
          case spectralDecisions of
            Left decisionFailure ->
              assertFailure ("spectral decision failed: " <> show decisionFailure)
            Right decisions -> do
              let spectralKeeps =
                    Set.fromList [nodeId | (nodeId, True) <- decisions]
      ...
      case raBoundaryAnalysis (rbAnalysis resolutionValue) of
        ...
        Right boundaryAnalysis -> do
          ...
          case spectralDecisions of
            ...
            Right decisions -> do
              let spectralPrunes =
                    Set.fromList [nodeId | (nodeId, False) <- decisions]
        ...
        Right boundaryAnalysis -> do
          ...
          case spectralDecisions of
            Left decisionFailure ->
              assertFailure ("spectral decision failed: " <> show decisionFailure)
            Right decisions -> do
              let spectralPrunedNodes =
                    [nodeId | (nodeId, False) <- decisions]
              assertBool
...
verdierIndependenceFor rewriteSystem depth =
  case buildResolutionBundle rewriteSystem depth of
    ...
    Right resolutionValue -> do
      let derivedComplex = resolutionDerivedComplex resolutionValue
      verdierPreparation <-
        case prepareVerdierPruning derivedComplex of
          Left failure ->
            assertFailure ("Verdier preparation failed: " <> show failure)
          Right preparation ->
            pure preparation
      verdierDecisions <-
        traverse
          ( \nodeId@(RegionNodeId ordinal) ->
              case
                first show (mkLocalClosed (resolutionSourcePoset resolutionValue) (IS.singleton ordinal))
                  >>= first show . verdierLocalClosedGate verdierPreparation
              of
...
orderComplexLitmusFor rewriteSystem depth =
  case buildResolutionBundle rewriteSystem depth of
    ...
    Right resolutionValue -> do
      let grothendieckDerived = resolutionDerivedComplex resolutionValue
                  ...
                  orderMicrosupport =
                    fmap Map.fromList
                      ( traverse
                          ( \sourceNode ->
                              fmap
                                (\targetNode -> (sourceNode, targetNode))
                                (first show (cocProjection orderComplex sourceNode))
                          )
                          (Vector.toList (derivedPosetNodes (cocSourcePoset orderComplex)))
                      )
                      >>= first show
                        . mkDerivedPosetFunctor
                          (cocSourcePoset orderComplex)
                          contextPoset
                      >>= first show . flip prepareMicrosupport orderDerived
                      >>= first show . computeMicrosupport
              assertBool
