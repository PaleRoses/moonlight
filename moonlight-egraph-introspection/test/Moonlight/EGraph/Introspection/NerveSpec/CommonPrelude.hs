...
import Moonlight.EGraph.Introspection.Analysis.Cohomology
...
import Moonlight.Control.Schedule
import Moonlight.Control.Scheduling.Support
import Moonlight.EGraph.Introspection.Analysis.Summary
import Moonlight.Sheaf.Site hiding (applyDelta)
import Moonlight.EGraph.Introspection.Analysis.Termination
...
genericJoinSaturationConfig ::
  (HasConstructorTag f, Show (ConstructorTag f), Show (f ()), Ord c) =>
  SaturationBudget ->
  PlanSpec (EGraphU owner ScopeCtx f a c) (SatGraph (EGraphU owner ScopeCtx f a c)) RewriteRuleId
