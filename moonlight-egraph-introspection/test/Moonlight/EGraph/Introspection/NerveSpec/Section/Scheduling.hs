...
data IntrospectedSaturation f result where
  PlainIntrospectedSaturation ::
    PlanSpec (EGraphU ScopeCtx f () ()) (SatGraph (EGraphU ScopeCtx f () ())) RewriteRuleId ->
    IntrospectedSaturation f (SaturationReport (EGraphU ScopeCtx f () ()))
  StrategyIntrospectedSaturation ::
    Program () (EGraphStrategyPhase ScopeCtx f () ()) ->
    IntrospectedSaturation
      f
      (Report (EGraph f ()) (SaturationReport (EGraphU ScopeCtx f () ())) (PhaseSummary SaturationBudget SaturationTermination ()))
...
runIntrospectedSaturation ::
  ...
  [RewriteRule ArithF] ->
  EGraph ArithF () ->
  Either (SaturationError (EGraphU ScopeCtx ArithF () ()) RewriteRuleId) result
...
testAdaptiveIntrospectionSaturation =
  let graph0 = emptyEGraph adaptiveAnalysisSpec
      (_, graph1) = addTerm (arithNumTerm 1) graph0
      (_, graph2) = addTerm (arithNumTerm 2) graph1
      saturationConfig :: PlanSpec (EGraphU ScopeCtx ArithF () ()) (SatGraph (EGraphU ScopeCtx ArithF () ())) RewriteRuleId
      saturationConfig =
        withSchedulerConfig
  ...
  let graph0 = emptyEGraph adaptiveAnalysisSpec
      ...
      saturationConfig =
        withSchedulerConfig
          ...
          (planSpec (SaturationBudget 4 32) GenericJoinMatching emptyRewriteRuntimeCapabilities)
      guideStrategy ::
        Program () (EGraphStrategyPhase ScopeCtx ArithF () ())
      guideStrategy =
        phase
