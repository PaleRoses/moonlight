...
import Moonlight.EGraph.Pure.Context (ContextEGraph, cegSite, emptyContextEGraphFromSite)
...
plainRedexCase =
  testCase "a plain redex yields one guarded ground beta and one substitution fact" $ do
    ...
    corpus <- expectRight "plain corpus" (bindingCorpusView convertedModule)
    bcvMetrics corpus
      @?= HsExprBindingRuleMetrics
    ...
    betaRule <- case bcvRules corpus of
      [ruleSpec] ->
...
captureCase =
  testCase "a capture-threatened redex freshens the binder and guards the freshened beta" $ do
    ...
    corpus <- expectRight "capture corpus" (bindingCorpusView convertedModule)
    bcvMetrics corpus
      @?= HsExprBindingRuleMetrics
        { hbrmRedexSiteCount = 1,
          hbrmAllowedCount = 0,
    ...
    (alphaRule, betaRule) <-
      case partition (isNothing . rrCondition . srsRule) (bcvRules corpus) of
        ([alphaSpec], [betaSpec]) ->
...
guardPatternCaptureCase =
  testCase "a pattern guard that would capture the beta argument forces alpha then guarded beta" $ do
    ...
    corpus <- expectRight "guard-capture corpus" (bindingCorpusView convertedModule)
    assertBool
      "guard pattern capture must force freshening"
      (hbrmFresheningCount (bcvMetrics corpus) > 0)
    assertBool
      "guard pattern capture must prevent direct substitution"
      (hbrmAllowedCount (bcvMetrics corpus) == 0)
    (alphaRule, betaRule) <-
      case partition (isNothing . rrCondition . srsRule) (bcvRules corpus) of
        ([alphaSpec], [betaSpec]) ->
          pure (srsRule alphaSpec, srsRule betaSpec)
...
multiIfPatternCaptureCase =
  testCase "a multi-way if pattern guard that would capture the beta argument forces alpha then guarded beta" $ do
    ...
    corpus <- expectRight "multi-if-capture corpus" (bindingCorpusView convertedModule)
    assertBool
...
localPatternCaptureCase =
  testCase "a local pattern bind that would capture the beta argument forces alpha then guarded beta" $ do
    ...
    corpus <- expectRight "local-pattern-capture corpus" (bindingCorpusView convertedModule)
    assertBool
...
clausePatternCaptureCase =
  testCase "a clause argument pattern that would capture the beta argument forces alpha then guarded beta" $ do
    ...
    corpus <- expectRight "clause-capture corpus" (bindingCorpusView convertedModule)
    assertBool
...
disjointIdentifierCase =
  testCase "bridge rule identifiers are disjoint from the site family" $ do
    ...
    corpus <- expectRight "shadow corpus" (bindingCorpusView convertedModule)
    siteRules <- expectRight "site family" (siteRuleSpecs convertedModule)
    let siteIdentifiers = ruleIdentifiers siteRules
        bridgeIdentifiers = ruleIdentifiers (bcvRules corpus)
...
unionContractionCase =
  testCase "the union corpus contracts the shadow binding past what the site family reaches alone" $ do
    convertedModule <- parsedFixture "shadow" shadowSource
    withPreparedBindingCorpus
      "shadow"
      convertedModule
      (unionContractionAssertions convertedModule)

unionContractionAssertions ::
  ConvertedModule ->
  PreparedContextSite owner ScopeCtx ->
  HsExprBindingCorpus owner ->
  SupportedRuleBook owner ScopeCtx HsExprBindingRule ->
  IO ()
unionContractionAssertions convertedModule site corpus siteBook = do
    hbcMetrics corpus
      @?= HsExprBindingRuleMetrics
    ...
    let contextGraph0 = emptyContextEGraphFromSite site (emptyEGraph (specAnalysisSpec (cmScopeIndex convertedModule)))
...
freshenedRuntimeCase =
  testCase "the alpha-then-guarded-beta chain fires on the runtime for a freshened redex" $ do
    ...
    withPreparedBindingCorpus
      "capture"
      convertedModule
      (freshenedRuntimeAssertions convertedModule)

freshenedRuntimeAssertions ::
  ConvertedModule ->
  PreparedContextSite owner ScopeCtx ->
  HsExprBindingCorpus owner ->
  SupportedRuleBook owner ScopeCtx HsExprBindingRule ->
  IO ()
freshenedRuntimeAssertions convertedModule site corpus siteBook = do
    let contextGraph0 = emptyContextEGraphFromSite site (emptyEGraph (specAnalysisSpec (cmScopeIndex convertedModule)))
    (seedClasses, _, _, contextGraph1) <-
...
type SpecUniverse :: Type -> Type
type SpecUniverse owner = EGraphU owner ScopeCtx HsExprF SpecAnalysis ScopeCtx
...
bindingFrontPlanSpec ::
  ContextEGraph owner HsExprF SpecAnalysis ScopeCtx ->
  PlanSpec
    (SpecUniverse owner)
    (SaturatingProofEGraph owner ScopeCtx HsExprF SpecAnalysis ScopeCtx ())
    (SupportScheduleGroup (SpecUniverse owner))
...
saturateAndExtract ::
  ...
  ContextEGraph owner HsExprF SpecAnalysis ScopeCtx ->
  SupportedRuleBook owner ScopeCtx HsExprBindingRule ->
  SupportedFactBook owner ScopeCtx HsExprBindingFactRule ->
  ScopeCtx ->
  ClassId ->
...
saturateAndExtract label contextGraph ruleBook factBook bindingContext seedClass = do
  ...
  compiledProgram <-
    expectRight
      ...
      (compileSupportProgram @(SpecUniverse owner) (cegSite contextGraph) ruleBook factBook)
  supportPlan <-
    expectRight
...
type BindingCorpusView :: Type
data BindingCorpusView = BindingCorpusView
  { bcvRules :: ![SupportedRuleSpec ScopeCtx HsExprBindingRule],
    bcvFacts :: ![SupportedFactSpec ScopeCtx HsExprBindingFactRule],
    bcvMetrics :: !HsExprBindingRuleMetrics
  }

bindingCorpusView :: ConvertedModule -> Either String BindingCorpusView
bindingCorpusView convertedModule = do
  latticeValue <- first show (convertedModuleContextLattice convertedModule)
  withPreparedContextSiteFromFiniteLattice latticeValue $ \site -> do
    corpus <- first show (hsExprBindingCorpus site convertedModule)
    pure
      BindingCorpusView
        { bcvRules = supportedRules (hbcRules corpus),
          bcvFacts = supportedFactSpecs (hbcFacts corpus),
          bcvMetrics = hbcMetrics corpus
        }

siteRuleSpecs :: ConvertedModule -> Either String [SupportedRuleSpec ScopeCtx HsExprBindingRule]
siteRuleSpecs convertedModule = do
  latticeValue <- first show (convertedModuleContextLattice convertedModule)
...
withPreparedBindingCorpus ::
  ...
  IO result
withPreparedBindingCorpus label convertedModule useCorpus = do
  latticeValue <-
    expectRight (label <> " lattice") (first show (convertedModuleContextLattice convertedModule))
  lawBook <- expectRight (label <> " site family") (first show (hsExprSiteLawFamily convertedModule))
  withPreparedContextSiteFromFiniteLattice latticeValue $ \site -> do
    corpus <- expectRight (label <> " corpus") (hsExprBindingCorpus site convertedModule)
    siteBook <-
      expectRight
        (label <> " prepared site family")
        (supportedRuleBook site (fmap lawRule (lawBookEntries lawBook)))
    useCorpus site corpus siteBook

ruleIdentifiers :: [SupportedRuleSpec ScopeCtx HsExprBindingRule] -> Set.Set RewriteRuleId
ruleIdentifiers =
  Set.fromList . fmap (rrId . srsRule)
...
singleFactRule :: BindingCorpusView -> IO HsExprBindingFactRule
singleFactRule corpus =
  case bcvFacts corpus of
    [factSpec] ->
