...
import Moonlight.EGraph.Pure.Types (ClassId, emptyEGraph, materializeEGraphClasses)
...
testBatchedInsertionPreservesContextualSemantics =
  case convertHaskellSource "ScopeTower.hs" (scopeTowerSource 24) of
    ...
    Right convertedModule -> do
      latticeValue <- expectRight (first show (convertedModuleContextLattice convertedModule))
      withEmptyContextEGraph
        latticeValue
        (emptyEGraph trivialAnalysis)
        (\contextGraph0 ->
            case
              ( insertConvertedModuleBatched convertedModule contextGraph0,
                insertConvertedModuleEager convertedModule contextGraph0
              )
            of
              (Right (batchedClassIds, batchedGraph), Right (eagerClassIds, eagerGraph)) ->
                do
                  ...
                  assertEqual
                    ...
                    (materializeEGraphClasses (cegBase eagerGraph))
                    (materializeEGraphClasses (cegBase batchedGraph))
                  assertEqual
              ...
              (Left failureValue, _) ->
                assertFailure ("unexpected batched insertion failure: " <> failureValue)
              (_, Left failureValue) ->
                assertFailure ("unexpected eager insertion failure: " <> failureValue)
        )
...
insertConvertedModuleBatched ::
  ConvertedModule ->
  ContextEGraph owner HsExprF () ScopeCtx ->
  Either String ([ClassId], ContextEGraph owner HsExprF () ScopeCtx)
insertConvertedModuleBatched convertedModule contextGraph0 = do
