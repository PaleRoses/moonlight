...
import Moonlight.EGraph.Pure.Context (cegBase, withEmptyContextEGraph)
import Moonlight.EGraph.Pure.Types (EClass (..), emptyEGraph, lookupEClass, materializeEGraphClasses)
...
resolverOrderCase =
  testCase "the resolver verdict on authored classes is exactly the scope order over the witness chain" $ do
    ...
    withEmptyContextEGraph
      latticeValue
      (emptyEGraph (hsExprFreeScopeAnalysisSpec scopeIndex))
      (\contextGraph0 -> do
          (_, _, _, contextGraph1) <-
            expectRight
          ...
          let resolve = runGuardCapabilityResolver (hsExprScopeGuardCapabilityResolver contextGraph1)
              authoredClassIds = fmap eClassId (IntMap.elems (materializeEGraphClasses (cegBase contextGraph1)))
              witnessFor classId =
