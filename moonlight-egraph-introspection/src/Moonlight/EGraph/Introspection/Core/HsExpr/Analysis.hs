...
import Moonlight.Core (HasConstructorTag (constructorTag), Pattern (..), RewriteRuleId)
...
import Moonlight.EGraph.Pure.Context (ContextEGraph, cegSite, withEmptyContextEGraph)
import Moonlight.EGraph.Pure.Extraction (ExtractionWorkBudget (..), depthCost)
...
data HsExprPipelineFailure
  ...
  | forall owner. HsExprSupportSaturationFailure !(HsExprSupportSaturationError owner)
...
deriving stock instance Show HsExprPipelineFailure

type HsExprSupportSaturationError :: Type -> Type
type HsExprSupportSaturationError owner =
  SaturationError
    (EGraphU owner ScopeCtx HsExprF HsExprMetricAnalysis ScopeCtx)
    (SupportScheduleGroup (EGraphU owner ScopeCtx HsExprF HsExprMetricAnalysis ScopeCtx))
...
data HsExprPipelineMetrics = HsExprPipelineMetrics
  { hpmConversionMetrics :: !ConvertedModuleMetrics,
    ...
    hpmSupportSaturationMetrics :: !(SaturationRunMetrics RewriteRuleId),
...
measureConvertedModuleInsertionMetrics convertedModule = do
  ...
  withEmptyContextEGraph latticeValue graph0 $ \contextGraph0 -> do
...
measureConvertedModulePipeline budgetValue convertedModule = do
  ...
  withEmptyContextEGraph
    latticeValue
    graph0
    (measurePreparedContextPipeline budgetValue convertedModule)

measurePreparedContextPipeline ::
  forall owner.
  SaturationBudget ->
  ConvertedModule ->
  ContextEGraph owner HsExprF HsExprMetricAnalysis ScopeCtx ->
  Either HsExprPipelineFailure HsExprPipelineMetrics
measurePreparedContextPipeline budgetValue convertedModule contextGraph0 = do
...
hsExprMetricExtractionBudget :: ExtractionWorkBudget
hsExprMetricExtractionBudget =
  ExtractionWorkBudget 4194304
