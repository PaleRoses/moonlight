...
import Moonlight.Derived.Complex (Derived)
...
buildResolutionKernel rewriteSystem depthValue = do
  ...
  coreAnalysis <-
    buildResolutionBoundaryAnalysisCore
      interfaceStalkBasisLinearization analysisScaffold originalComplex stalkCache
  pure
    ResolutionKernel
...
deriveAnalysis kernel =
  let scaffold = rkScaffold kernel
      ...
      reductionResult =
        first
          siteComplexScaffoldErrorToHomologyFailure
   ...
   in ResolutionAnalysisAlg
        { raMorse = morseResult,
          ...
          raLerayProfile = do
            derivedValue <- first (BackendFailure . show) (derivedFromFiniteChainComplex complex)
            first (BackendFailure . show) (hypercohomologyDims derivedValue),
...
resolutionDerivedComplex :: ResolutionBundle f -> Either HomologyFailure (Derived GF2)
resolutionDerivedComplex =
  first (BackendFailure . show) . derivedFromFiniteChainComplex . scsChainComplex . rkScaffold . rbKernel
...
resolutionPruningGapAt degreeValue (RegionNodeId ordinalValue) bundle =
  resolutionDerivedComplex bundle
    >>= first (BackendFailure . show)
      . pruningGapAt (resolutionSourcePoset bundle) degreeValue (FinObjectId ordinalValue)
...
sourcePosetFromScaffold ::
  RewriteSiteScaffold f ->
  Either HomologyFailure DerivedPoset
sourcePosetFromScaffold analysisScaffold =
  let siteValue = scsSite analysisScaffold
   in first
        (InvalidTopologyInput . show)
        ( mkDerivedPosetFromOrderEdges
            (fmap (FinObjectId . cellNodeIdFromScaffold analysisScaffold) (grothendieckSiteCells siteValue))
            ( fmap
                ( \faceMorphism ->
                    let (sourceCell, targetCell) = faceCover faceMorphism
                     in ( FinObjectId (cellNodeIdFromScaffold analysisScaffold sourceCell)
                        , FinObjectId (cellNodeIdFromScaffold analysisScaffold targetCell)
                        )
                )
...
buildResolutionBoundaryAnalysisCore stalkLinearization analysisScaffold finiteComplex stalkCache = do
  sourcePoset <- sourcePosetFromScaffold analysisScaffold
  let stalksByNode =
        grothendieckSiteCells (scsSite analysisScaffold)
          & mapMaybe
            ( \cellValue ->
...
siteComplexScaffoldErrorToHomologyFailure :: SiteComplexScaffoldError -> HomologyFailure
siteComplexScaffoldErrorToHomologyFailure =
  BackendFailure . show
...
faceCover :: GrothendieckFaceMorphism (RewriteSystem f) -> (GrothendieckCell (RewriteSystem f), GrothendieckCell (RewriteSystem f))
