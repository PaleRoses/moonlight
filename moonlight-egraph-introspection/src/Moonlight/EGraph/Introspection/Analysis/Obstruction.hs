...
buildObstructionContext rewriteSystem depthValue = do
  ...
  let siteValue = scsSite analysisScaffold
      finiteComplex = scsChainComplex analysisScaffold
      basisRefs = scsBasisRefs analysisScaffold
      basisNodeId = basisCellNodeId finiteComplex
      allCells = grothendieckSiteCells siteValue
      coverPairs =
        fmap
          (\faceMorphism -> (grothendieckFaceMorphismTarget faceMorphism, grothendieckFaceMorphismSource faceMorphism))
          (grothendieckSiteFaceMorphisms siteValue)
  ambientPoset <-
    Generic.ambientPosetFromCellCovers basisRefs basisNodeId allCells coverPairs
  ambientDerived <-
