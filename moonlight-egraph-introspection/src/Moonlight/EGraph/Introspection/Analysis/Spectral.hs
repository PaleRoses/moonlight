...
tarskiRestrictionLaplacian siteValue = do
  ...
  model <- restrictionModelFromRegistry (nerveSiteBasis siteValue) registry
  liftLaplacianBuildError
...
grothendieckTarskiLaplacian rewriteSystem depthValue = do
  ...
  model <- restrictionModelFromRegistry (grothendieckSiteBasis siteValue) registry
  liftLaplacianBuildError
    (buildTarskiLaplacian model)
...
restrictionModelFromRegistry ::
  Ord cell =>
  SheafBasis cell ->
  RestrictionIndex cell witness ->
  Either HomologyFailure (SheafModel cell witness)
restrictionModelFromRegistry basis registry =
...
grothendieckConsistencyProfileWith strategy rewriteSystem depthValue = do
  ...
  model <-
    first (const (BackendFailure "grothendieck sheaf model construction failed")) $
      prepareSheafModel
        (SheafModelVersion 0)
        (mkObjectIndex (basisCells basis))
        presentStoredRestriction
...
buildResolutionProgram ::
  ...
  SheafModel (GrothendieckCell (RewriteSystem f)) (RewriteRestrictionWitness f) ->
...
initialSectionFor ::
  ...
  SheafModel (GrothendieckCell (RewriteSystem f)) witness ->
  TotalSectionStore (GrothendieckCell (RewriteSystem f)) (InterfaceStalk (RewriteTag f))
initialSectionFor strategy stalkBounds canonicalStalkAt model =
  emptyTotalSectionStoreWith
    model
    ( case strategy of
