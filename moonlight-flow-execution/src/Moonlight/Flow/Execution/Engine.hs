...
import Moonlight.Flow.Execution.Factor.Run (FactorRunError)
...
data RelationalRunObstruction backend
  ...
  | RelationalFactorRunObstruction !FactorRunError
   ...
   in do
      ...
      if structuralPlanUsesFactorCache plan
        then do
          ...
          result <-
            firstRelationalFactorRun (runPrepared (preparedSpec (PreparedMeasuredCached cache0)))
          let st1 =
                maybe
                  st0
          ...
          result <-
            firstRelationalFactorRun (runPrepared (preparedSpec PreparedMeasuredFresh))
          finish st0 result

firstRelationalFactorRun ::
  Either FactorRunError a ->
  Either (RelationalRunObstruction backend) a
firstRelationalFactorRun =
  first RelationalFactorRunObstruction
{-# INLINE firstRelationalFactorRun #-}
