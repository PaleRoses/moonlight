   ...
   in foldScope
        ( case payloadValue of
            ...
            Just _ ->
              let stEvicted =
                    evictPreparedKeys
                      (affectedPreparedKeys payloadValue st)
                      st
          ...
          if not (IntSet.null (scopeImpacted dirty))
            then (prunePreparedLRU (clearPreparedCaches st), Map.empty)
              ...
              let dirtyResults =
                  ...
                  stEvicted =
                    evictPreparedKeys
                      (affectedPreparedKeys payloadValue stPatched)
                      stPatched
...
ensureContextPrepared ::
  Ord c =>
  (IntMap relation -> Either obstruction contextPrepared) ->
  c ->
  QueryId ->
  Int ->
  IntMap relation ->
  MatchFootprint ->
  JoinCacheState c plan basePrepared contextPrepared repair ->
  Either obstruction (JoinCacheState c plan basePrepared contextPrepared repair, contextPrepared)
  ...
  let key = ContextPreparedKey ctx qid liveEpoch
   in case Map.lookup key (jcsPrepared st) of
        Just (ContextPreparedEntry prepared _ _) ->
          Right (touchPreparedEntry key st, prepared)
        _ ->
          do
            prepared <- prepareContext rels
            let (t, st1) = nextTick st
                entry = ContextPreparedEntry prepared fp t
                st2 = insertPreparedEntry key entry st1
            Right (prunePreparedLRU st2, prepared)
