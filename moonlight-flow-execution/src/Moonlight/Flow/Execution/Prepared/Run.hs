...
runPrepared ::
  ...
  Either FactorRunError (PreparedResult mode a)
runPrepared spec =
...
runPreparedMeasured ::
  ...
  Either FactorRunError (PreparedResult 'PreparedMeasuredRun a)
...
runPreparedExecution ::
  ...
  PreparedRunSpec mode compiled output guard tag tuple key a ->
  Maybe View ->
  Either FactorRunError (PreparedExecution a)
...
runFactorizedPreparedRequest internalMode fullSchema plan maybeStructuralSources sourceStore restrictedView atomDeltas decomp op =
  case op of
    PreparedRows maybeLimit ->
      case maybeStructuralSources of
        Just structuralSources ->
          fmap
            (staticExecution DispatchAdaptive . limitRows maybeLimit)
            (first FactorRunDenseJoinPlanError (denseStructuralRowsFromSources decomp plan structuralSources))
        Nothing -> do
    ...
    PreparedDeltaRows maybeLimit ->
      case maybeStructuralSources of
        Just structuralSources ->
          fmap
            (staticExecution DispatchAdaptive . limitRows maybeLimit)
            (first FactorRunDenseJoinPlanError (denseStructuralDeltaRowsFromSources decomp plan structuralSources))
        Nothing -> do
          result <- runDemand FactorDemandRows
          let !value =
    ...
    PreparedExists -> do
      ...
      value <- first FactorRunProvenanceError (rootFactorExists (frrPreSealCache result))
      pure (finish result value)
...
denseStructuralRowsFromSources ::
  ...
  QueryPlan compiled output guard tag tuple key ->
  [DenseArrangement] ->
  Either DenseJoinPlanError [RowTupleKey]
denseStructuralRowsFromSources _decomp plan =
  fmap (List.sortOn (canonicalRowBlockOrderKey fullSchema) . denseJoinRows)
    . mkDenseJoinPlan fullSchema fullSchema
  where
    !fullSchema =
      Vector.toList (qpFullSchema plan)
...
denseStructuralDeltaRowsFromSources _decomp plan =
  fmap (List.sortOn (canonicalRowBlockOrderKey fullSchema) . denseJoinDeltaRows)
    . mkDenseJoinPlan fullSchema fullSchema
  where
    !fullSchema =
      Vector.toList (qpFullSchema plan)
