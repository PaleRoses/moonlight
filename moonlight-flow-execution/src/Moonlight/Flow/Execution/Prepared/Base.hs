...
data BasePreparedDB compiled output guard tag tuple key = BasePreparedDB
  { bpdPlan :: !(QueryPlan compiled output guard tag tuple key),
    bpdStore :: !Store,
    bpdRowsByResult :: !(IntMap (IntMap [RowTupleKey]))
  }
...
patchBasePreparedDBWithAtomRows dirtyRowsByAtom dirtyResults baseDb = do
  let plan =
        bpdPlan baseDb

      step (storeAcc, rowsByAtomAcc, deltasAcc) atomSpec = do
        let atomId =
              queryAtomAsAtomId (asQueryAtomId atomSpec)
            atomKey =
              queryAtomKey (asQueryAtomId atomSpec)
            oldRows =
              IntMap.findWithDefault IntMap.empty atomKey rowsByAtomAcc
        let dirtyRows =
              dirtyRowsByResultForAtom
                dirtyRowsByAtom
                dirtyResults
        ...
        let rowIdDelta =
            ...
            rowsByAtomAcc' =
              IntMap.insert atomKey rowsByResult1 rowsByAtomAcc
        pure (patchedStore, rowsByAtomAcc', deltasAcc')

  (store1, rowsByAtom1, atomDeltas) <-
    foldM
      step
      (bpdStore baseDb, bpdRowsByResult baseDb, IntMap.empty)
      (Vector.toList (qpAtoms plan))

  pure
    ( baseDb {bpdStore = store1, bpdRowsByResult = rowsByAtom1},
      atomDeltas
    )
