...
denseJoinRows plan =
  Dense.foldDenseWCOJ
    ...
    ( \leaf rows -> do
        maybeRow <- Dense.denseLeafTupleKey (PrimArray.primArrayToList (djOutputSchema plan)) leaf
        pure $
          case maybeRow of
            Nothing -> rows
            Just row -> row : rows
...
denseJoinDeltaRows plan =
  let !outputSchema =
        PrimArray.primArrayToList (djOutputSchema plan)
...
denseJoinSupportIds plan =
  ...
  where
    ...
    collectSupport leaf support ix = do
      ...
      let src = SmallArray.indexSmallArray (djSources plan) ix
      pure $
...
joinProjectDenseWCOJ outputSchema sources arena0 =
  let (!arena1, !rows) = projectDenseWCOJRows outputSchema sources arena0
   in (arena1, mkFactor outputSchema rows)
          ...
          let !fullSchema =
              ...
              !plan =
                mkDenseJoinPlanWithSupportSources
                  ...
                  Nothing
                  sources
           in foldProjectDenseWCOJDeltaPlanWitnesses config plan arena0 initial step
      ...
      let !fullSchema =
          ...
          !plan =
            mkDenseJoinPlanWithSupportSources
              ...
              Nothing
              sources
       in foldProjectDenseWCOJPlanWitnesses config plan arena0 initial step
...
selectedOutputContains plan key =
  case djSelectedOutput plan of
    Nothing ->
      True
    Just selected ->
      Set.member key (sodKeySet selected)
