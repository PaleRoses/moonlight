...
sourceRowsWitnessWithTelemetry config plan sourceIx src rows arena0
  ...
  where
    !isSupportSource =
      IntSet.member sourceIx (denseJoinPlanSupportSources plan)
...
leafWitnessWithTelemetry config frame plan arena0 =
  ...
  where
    step (!arena, !witness) ix = do
      ...
      let !src =
            SmallArray.indexSmallArray (denseJoinPlanSources plan) ix
...
foldPlanSourceIndexesM plan !initial step =
  ...
  where
    !sourceCount =
      SmallArray.sizeofSmallArray (denseJoinPlanSources plan)
