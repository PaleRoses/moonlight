...
module Moonlight.Flow.Execution.Dense.Plan
  ( DenseArrangementId (..),
    ...
    selectedOutputDenseSelectedOutput,
    denseJoinPlanDenseDeltaProblem,
...
data DenseJoinPlan = DenseJoinPlan
  { djFullSchema :: !(PrimArray Int),
    djOutputSchema :: !(PrimArray Int),
    djSources :: !(SmallArray DenseArrangement),
    djSourcesBySlot :: !(IntMap IntSet),
    djSupportSources :: !IntSet,
    djSelectedOutput :: !(Maybe SelectedOutputDomain),
    djStaticRank :: !(IntMap Int),
    djSlotUniverse :: {-# UNPACK #-} !Int
  }
  deriving stock (Show)
...
mkDenseJoinPlanWithSupportSources fullSchema outputSchema supportSources selectedOutput sources =
  let !sourceCount =
      ...
      !validSupportSources =
        IntSet.filter
          (\ix -> ix >= 0 && ix < sourceCount)
          supportSources
   in DenseJoinPlan
        { djFullSchema = primArrayFromListStrict (fmap slotIdKey fullSchema),
          djOutputSchema = primArrayFromListStrict (fmap slotIdKey outputSchema),
          djSources = SmallArray.smallArrayFromList sources,
          djSourcesBySlot = sourcesBySlot sources,
          djSupportSources = validSupportSources,
          djSelectedOutput = selectedOutput,
          djStaticRank =
            IntMap.fromList
              [ (slotIdKey sid, ix)
                | (ix, sid) <- zip [0 ..] fullSchema
              ],
          djSlotUniverse =
            maybe 0 (+ 1) $
              maximumOf (fmap slotIdKey (fullSchema <> outputSchema))
        }
...
selectedOutputDenseSelectedOutput :: SelectedOutputDomain -> DenseSelectedOutput
selectedOutputDenseSelectedOutput selected =
  DenseSelectedOutput
    { dsoRows = sodRows selected,
      dsoRowsBySlotValue = sodRowsBySlotValue selected
    }
{-# INLINE selectedOutputDenseSelectedOutput #-}

denseJoinPlanDenseDeltaProblem :: DenseJoinPlan -> DenseDeltaProblem
denseJoinPlanDenseDeltaProblem plan =
  DenseDeltaProblem
    { ddpSlotUniverse = djSlotUniverse plan,
      ddpSources = SmallArray.mapSmallArray' denseArrangementDeltaJoinSource (djSources plan),
      ddpSourcesBySlot = djSourcesBySlot plan,
      ddpFullSchema = djFullSchema plan,
      ddpOutputSchema = djOutputSchema plan,
      ddpSelectedOutput = selectedOutputDenseSelectedOutput <$> djSelectedOutput plan,
      ddpStaticRank = djStaticRank plan
    }
{-# INLINE denseJoinPlanDenseDeltaProblem #-}
...
sourcesBySlot :: [DenseArrangement] -> IntMap IntSet
sourcesBySlot arrangements =
  IntMap.fromListWith
    IntSet.union
    [ (slotKey, IntSet.singleton sourceId)
      | (sourceId, arrangement) <- zip [0 ..] arrangements,
        slotKey <- PrimArray.primArrayToList (denseArrangementSchemaKeys arrangement)
    ]
{-# INLINE sourcesBySlot #-}
  ...
  where
    insertKeyedRow eitherRows rowKey row = do
      ...
      case mkRowId rowKey of
        Left _ ->
          Left (DenseArrangementPatchInsertFailed row (IndexedRowsInsertNegativeRowId rowKey))
        Right rowId ->
