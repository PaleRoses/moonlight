...
buildFactorFromSourceBundles ::
  ...
  [SourceBundle] ->
  ProvArena ->
  (ProvArena, Factor, FactorContributionIndex)
  ...
  let !sources =
      ...
      (!arena1, !buildState) =
        foldProjectDenseWCOJWitnesses
          ...
          emptyFactorBuildState
          insertBuiltWitness
   in (arena1, mkFactor outputSchema (fbsRows buildState), fbsContributions buildState)
...
insertBuiltWitness key witness arena state = do
  let !contribution =
      ...
      !rows1 =
        if Set.member key (fccTouchedKeys change)
...
updateFactorIncremental ::
  ...
  FactorContributionIndex ->
  ProvArena ->
  (ProvArena, Factor, FactorContributionIndex, FactorDelta, IncrementalUpdateTrace)
updateFactorIncremental telemetryConfig outputSchema bundles oldFactor oldContributions arena0 =
  let !currentSources =
        fmap sourceBundleArrangement bundles
      !dirtyCells =
   ...
   in if Set.null workKeys
        then
          ...
          let (!arena2, !contributions1, !contributionChange) =
           ...
           in (arena2, fpsFactor patchState, contributions1, delta, traceValue)
...
currentDeltaContributions ::
  ...
  [DenseArrangement] ->
  ProvArena ->
  (ProvArena, FactorRepairState)
currentDeltaContributions telemetryConfig outputSchema sources arena0 =
  case deltaTraversalMode sources of
    DeltaNoDirtySources ->
      (arena0, emptyFactorRepairState telemetryConfig)
    DeltaSingleDirtySource sourceIx ->
      foldProjectDenseWCOJWitnessesWithTelemetry
        telemetryConfig
        outputSchema
...
normalizeAtomDeltas =
  IntMap.mapMaybe
    ( \delta ->
        let normalized =
              normalizePlainRowPatch delta
         in if plainRowPatchNull normalized
              then Nothing
              else Just normalized
    )
{-# INLINE normalizeAtomDeltas #-}
