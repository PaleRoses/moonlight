...
-- | Seat the rendered contribution projection on the factor's authoritative
-- payload map. Construction and repair produce equal maps independently; this
-- gluing step preserves the value while deleting the duplicate physical tree.
coalesceFactorContributionRenderedValues ::
  Factor ->
  FactorContributionIndex ->
  FactorContributionIndex
coalesceFactorContributionRenderedValues factor indexValue =
  indexValue {fciRendered = indexedRowsPayloadMap factor}
{-# INLINE coalesceFactorContributionRenderedValues #-}
...
factorContributionIndexProvRoots indexValue =
  ...
  where
    ...
    rowRoots :: SupportedRow FactorSourceCell ProvAccum -> [ProvVal]
    rowRoots row =
