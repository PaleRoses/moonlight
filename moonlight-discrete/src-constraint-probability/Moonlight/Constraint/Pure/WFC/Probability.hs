                ...
                (\restrictedDistribution ->
                   EntropyCandidate
                     { candidateSlot = slotId,
                       ...
                       candidateCardinality = length outcomes
                     }
                )
      ...
      case wfcProbabilityCandidateSelection options of
        ...
        WeightedSampling ->
          let descendingCandidates =
              ...
              weightedPairs =
                categoricalFoldMap (\(outcome, weight) -> [(outcome, positiveProbValue weight)]) restrictedDistribution
           in case buildAliasTableFromList weightedPairs of
                Left _ -> Right (descendingCandidates, generator)
                Right aliasTable ->
                  let (sampledCandidate, nextGenerator) =
                        runSampleWith (sampleAlias aliasTable) generator
...
liftCompiledDomain priorDomains (compiledSlotId, domainValue) =
  case compiledSlot of
    ...
    CompiledPresenceWitnessSlot _ -> do
      ...
      witnessDistribution <-
        either (const Nothing) Just (uniformCategorical witnessValues)
      pure (compiledSlotId, witnessDistribution)
  where
