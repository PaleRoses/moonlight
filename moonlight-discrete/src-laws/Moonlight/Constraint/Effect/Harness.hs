...
latticeDistributivityAndOverOr first second third =
  equivalent
    (normalize (And [first, Or [second, third]]))
    (normalize (Or [And [first, second], And [first, third]]))
...
latticeDistributivityOrOverAnd first second third =
  equivalent
    (normalize (Or [first, And [second, third]]))
    (normalize (And [Or [first, second], Or [first, third]]))
...
endoPatchActionIdentity :: Ord k => CoFiniteTruth k -> Bool
endoPatchActionIdentity truthValue =
  applyEndoPatch mempty truthValue == truthValue

endoPatchCanonicalAssignments :: Ord k => EndoPatch k -> Bool
endoPatchCanonicalAssignments patch =
  Set.disjoint (endoPatchAdds patch) (endoPatchRemoves patch)
