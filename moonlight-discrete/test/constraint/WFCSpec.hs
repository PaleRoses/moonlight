...
tests =
  testGroup
    ...
    [ testCase "compile_translates_domains_and_arcs" $
      ...
      testCase "checked assignment rejects a missing slot" $ do
        let compiled =
              compileWFCProblem
                WFCProblem
                  { wfcProblemDomains = Map.singleton alphaSlot (domainSingleton Empty),
                    wfcProblemAdjacencyRules = []
                  }
        case assignSlot betaSlot Empty compiled of
          Left err -> err @?= WFCAssignmentMissingSlot betaSlot
          Right _ -> assertFailure "expected assignment to reject a missing slot",
      testCase "checked assignment rejects a value outside the current domain" $ do
        let compiled =
              compileWFCProblem
                WFCProblem
                  { wfcProblemDomains = Map.singleton alphaSlot (domainSingleton Empty),
        ...
        case assignSlot alphaSlot Shrine compiled of
          Left err -> err @?= WFCAssignmentValueOutsideDomain alphaSlot
          Right _ -> assertFailure "expected assignment to reject a value outside the domain",
      testCase "checked assignment restricts an allowed value" $ do
        let compiled =
              compileWFCProblem
                WFCProblem
                  { wfcProblemDomains = Map.singleton alphaSlot (domainFromList [Empty, Hall]),
                    wfcProblemAdjacencyRules = []
                  }
        case assignSlot alphaSlot Hall compiled of
          Left err -> assertFailure ("unexpected assignment obstruction: " <> show err)
          Right assignedProblem ->
            Map.lookup alphaSlot (cspDomains assignedProblem) @?= Just (domainSingleton Hall),
      testCase "custom search cannot widen a domain with a hostile candidate" $ do
        let compiled =
              compileWFCProblem
                WFCProblem
                  { wfcProblemDomains = Map.singleton alphaSlot (domainFromList [Empty, Hall]),
            ...
            hostileSearchContext =
              SearchContext
                { searchSelectSlot = \() _ -> Just alphaSlot,
                  searchCandidates = \() _ _ -> Right ([Shrine], ())
                }
        searchWithContext hostileSearchContext 1 () compiled
          @?= Left (WFCAssignmentValueOutsideDomain alphaSlot),
