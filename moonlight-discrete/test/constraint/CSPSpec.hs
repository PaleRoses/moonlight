...
tests =
  testGroup
    ...
    [ testCase "domainFromList normalizes ordering and duplicates" $
      ...
      testCase "mac3 rejects an isolated pre-existing empty domain" $ do
        let problem =
              ConstraintSatisfactionProblem
                { cspDomains = Map.singleton 'x' (domainFromList ([] :: [Int])),
                  cspConstraints = []
                }
        case mac3 problem of
          Left err -> assertFailure ("unexpected mac3 failure: " <> show err)
          Right Nothing -> pure ()
          Right (Just _) -> assertFailure "expected the empty domain to be inconsistent",
