...
domainMember :: Ord value => value -> Domain value -> Bool
domainMember value = Set.member value . unDomain

domainCardinality :: Domain value -> Int
domainCardinality = Set.size . unDomain

domainSingletonValue :: Domain value -> Maybe value
domainSingletonValue domainValue =
  case Set.minView (unDomain domainValue) of
    Just (value, remainingValues)
      | Set.null remainingValues -> Just value
    _ -> Nothing
...
mac3 problem =
  if any domainNull (Map.elems (cspDomains problem))
    then pure Nothing
    else go problem (allArcs problem)
  where
    go ::
      ConstraintSatisfactionProblem variable value ->
      [Arc variable] ->
