...
filtrationOrderedReduction sourceLevels targetLevels matrixValue = do
  ...
  let columnRows =
        smRows (sparseTransposeMatrix compactMatrix)
      orderedColumns =
        filtrationOrderedIndices sourceLevels
      finalState =
        List.foldl'
          (reduceFiltrationColumn sourceLevels targetLevels columnRows)
          emptyFiltrationReductionState
          orderedColumns
  pure
    FiltrationReduction
      { filtrationReductionSourceDimension = length sourceLevels,
        filtrationReductionTargetDimension = length targetLevels,
...
reduceFiltrationColumn sourceLevels targetLevels columnRows state sourceIndexValue =
  let initialColumn =
        FiltrationReductionColumnState
          { frcsSourceVector = sparseUnitVector (length sourceLevels) sourceIndexValue,
            frcsTargetVector =
              maybe IntMap.empty compactSparseRow (elementAt sourceIndexValue columnRows)
          }
      reducedColumn =
...
filtrationOrderedIndices :: [Int] -> [Int]
filtrationOrderedIndices levels =
  [0 .. length levels - 1]
    & List.sortOn (filtrationOrderKey levels)

filtrationPivotIndex :: [Int] -> AmbientVector -> Maybe Int
