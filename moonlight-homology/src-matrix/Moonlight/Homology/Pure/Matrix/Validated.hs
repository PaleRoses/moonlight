...
validatedColumnAt columnIndexValue matrixValue =
  if columnIndexValue < 0 || columnIndexValue >= matrixColumnCount matrixValue
    then Left (InvalidMatrixShape "column selection index is outside the matrix bounds")
    else
      case safeElementAt columnIndexValue (List.transpose (matrixRows matrixValue)) of
        Just columnValue -> Right columnValue
        Nothing -> Left (InvalidMatrixShape "validated matrix transpose lost a requested column")
