...
materializeIncidence ::
  ...
  [target] ->
  Either HomologyFailure (BoundaryIncidence r)
materializeIncidence boundaryOf sourceBasis targetBasis =
  let targetIndexByBasis = Map.fromList (zip targetBasis [0 :: Int ..])
   in do
        entries <-
          fmap concat $
            traverse
              ( \(sourceIndexValue, sourceBasisElement) ->
                  traverse
                    ...
                    (boundaryOf sourceBasisElement)
              )
              (zip [0 :: Int ..] sourceBasis)
        either
          (Left . InvalidBoundaryIncidence . show)
          Right
          ( mkBoundaryIncidence
              (fromIntegral (length sourceBasis))
