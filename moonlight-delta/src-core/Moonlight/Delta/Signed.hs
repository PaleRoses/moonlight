...
normalizeSigned ::
  Signed key ->
  Signed key
normalizeSigned (Signed rows) =
  Signed (Vector.filter (nonZeroChange . snd) rows)

signedNull ::
  Signed key ->
  Bool
signedNull =
  Vector.null . entries . normalizeSigned

support :: Signed key -> Set key
support =
  Set.fromDistinctAscList . fmap fst . signedToAscList . normalizeSigned
