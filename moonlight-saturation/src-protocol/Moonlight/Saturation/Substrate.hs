...
class SaturationGraph u where
  ...
  graphExecutionContexts ::
    ...
    [SatContext u]
  graphExecutionContexts =
    preparedContextObjects . graphPreparedSite @u
  ...
  where
    ...
    insertMatch ::
      ...
      Either
        (SatObstruction u)
        (Map (SatRuleKey u, SatClassId u, Substitution) (SatSupportedMatch u))
    insertMatch supportedMatchMap supportedMatch =
      Map.alterF
    ...
    mergeAtKey ::
      SatSupportedMatch u ->
      Maybe (SatSupportedMatch u) ->
      Either (SatObstruction u) (Maybe (SatSupportedMatch u))
