...
import Moonlight.Saturation.Test.ObstructionFixture
  ( RegionAggregate (..),
    RegionCache (..),
    RegionRequest (AcceptEvenRegions),
    regionFoldAlgebra,
    regionValues,
  )
import Test.Tasty
...
regionTests =
  testGroup
    ...
    [ testCase "regionFoldForRequest preserves accepted-region order" $
      ...
      testCase "mixed-region fixture rejects odd regions without contaminating the fold" $
        Foldl.fold
          (regionFoldForRequest regionFoldAlgebra (RegionCache 0) AcceptEvenRegions)
          (regionValues 8)
          @?= (RegionCache 4, RegionAggregate 20)
    ]
