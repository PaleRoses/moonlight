...
schedulerTests =
  testGroup
    ...
    [ testCase "deterministic scheduling orders matches by rule id" $
        let matches =
              [ testSupportedMatch 1 10 BaseContext,
                testSupportedMatch 0 11 BaseContext
              ]
            scheduled =
      ...
      testCase "candidate-space scheduling traces candidate groups without matched-list ghosts" $
        let candidateMatches =
              [ testSupportedMatch 1 11 BaseContext,
                testSupportedMatch 2 12 BaseContext
              ]
            result =
              scheduleTestMatches
      ...
      testCase "backoff scheduling suppresses cooled-down rules across rounds" $
        let matches =
              [ testSupportedMatch 0 10 LeftContext,
                testSupportedMatch 0 11 RightContext
              ]
      ...
      testCase "gated scheduling validates only the scheduler-admitted prefix" $
        let graph =
            ...
            staleMatch =
              testSupportedMatch 0 10 BaseContext
            freshMatch =
              testSupportedMatch 0 11 BaseContext
            matches =
      ...
      testCase "gated scheduling records cheap rejection and budget deferral as typed admission outcomes" $
        let graph =
            ...
            matches =
              [ testSupportedMatch 0 10 BaseContext,
                testSupportedMatch 0 11 BaseContext,
                testSupportedMatch 0 12 BaseContext
              ]
