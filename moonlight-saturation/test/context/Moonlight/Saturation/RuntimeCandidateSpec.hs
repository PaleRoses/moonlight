...
import Moonlight.Saturation.Test.ContextWorkload
  ( ContextMatchProfile (OverlappingContextMatches),
...
contextEnumerationScalesByDistinctRoots =
  testCase "context enumeration scales by distinct roots" $ do
    let workload = contextMatchingWorkload OverlappingContextMatches 2
        contextRules =
          fmap (\(_facts, _derivations, rules) -> rules) (contextMatchingInputs workload)
