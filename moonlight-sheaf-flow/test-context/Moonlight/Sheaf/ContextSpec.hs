...
type MockStore :: Type -> Type -> Type
data MockStore owner ctx = MockStore
  { msPreparedSite :: !(PreparedContextSite owner ctx),
    msContexts :: ![ctx],
    msGlobalRep :: !(Int -> Int),
    msSupportIndex :: !(ClassSupportIndex owner ctx),
    msClasses :: !(Map ctx (IntMap Int)),
    msAnalysis :: !(Map ctx (IntMap Int))
  }

instance Ord ctx => ContextAlgebraSite (MockStore owner ctx) ctx Int Int where
  type ContextSiteOwner (MockStore owner ctx) = owner
  contextPreparedSite = msPreparedSite
...
compiledFixtureSupport latticeValue =
  withPreparedContextSiteFromFiniteLattice latticeValue $ \site ->
    either
      (error . ("invalid context fixture support: " <>) . show)
      id
      . preparedSupportFromContexts site

compiledFixtureSupportIndex ::
  (Ord ctx, Show ctx) =>
  PreparedContextSite owner ctx ->
  IntMap (ClassSiteSupport ctx) ->
  ClassSupportIndex owner ctx
compiledFixtureSupportIndex site =
  either
    (error . ("invalid context fixture support index: " <>) . show)
    id
    . classSupportIndexFromEntries site
...
chainStore :: PreparedContextSite owner ChainCtx -> MockStore owner ChainCtx
chainStore site =
  MockStore
    { msPreparedSite = site,
...
supportObstructedStore :: PreparedContextSite owner CoverCtx -> MockStore owner CoverCtx
supportObstructedStore site =
  MockStore
    { msPreparedSite = site,
...
supportVisibleStore :: PreparedContextSite owner CoverCtx -> MockStore owner CoverCtx
supportVisibleStore site =
  (supportObstructedStore site)
    { msSupportIndex =
        compiledFixtureSupportIndex site $
          IntMap.fromList
...
diamondSatisfiedStore :: PreparedContextSite owner BranchContext -> MockStore owner BranchContext
diamondSatisfiedStore site =
  MockStore
    { msPreparedSite = site,
...
diamondObstructedStore :: PreparedContextSite owner BranchContext -> MockStore owner BranchContext
diamondObstructedStore site =
  MockStore
    { msPreparedSite = site,
      msContexts = [BranchBase, BranchLeft, BranchRight, BranchApex],
      msGlobalRep = id,
...
anatomyStore :: PreparedContextSite owner AnatomyCtx -> MockStore owner AnatomyCtx
anatomyStore site =
...
type RefreshSite :: Type -> Type
data RefreshSite owner = RefreshSite
  { rsPreparedSite :: !(PreparedContextSite owner RefreshCtx),
...
refreshRuntimeIdentity ::
  ContextRefreshPrepared siteOwner modelOwner (RefreshSite siteOwner) RefreshCtx MiniStalk ->
  Set RefreshCtx ->
  RefreshSite siteOwner ->
  TotalSectionStore modelOwner RefreshCtx MiniStalk ->
  RowRuntimeIdentity
...
buildRefreshRuntime cacheBudget _prepared _site _dirtyContexts _section0 =
  checkedCacheBudget >>= \cacheBudgetBytes ->
  refreshRuntimeSpec >>= \specValue ->
  firstShow $
    createRuntimeWithOptions
      specValue
      defaultRuntimeCreateOptions
        { rcoVisibleCacheBudgetBytes = cacheBudgetBytes
        }
  where
    checkedCacheBudget
      | cacheBudget <= fromIntegral (maxBound :: Int) =
          Right (fromIntegral cacheBudget)
      | otherwise =
          Left "context row cache budget exceeds Int capacity"
...
initialRefreshSite :: PreparedContextSite owner RefreshCtx -> RefreshSite owner
initialRefreshSite site =
  RefreshSite
    { rsPreparedSite = site,
...
refreshPropagationReport :: RefreshSite owner -> Maybe (ContextPropagationReport RefreshCtx)
...
testRestrictionComposition =
  withPreparedContextSiteFromFiniteLattice chainLattice $ \site -> do
  let store = chainStore site
  localToModule <- expectRight (restrictionMap LocalCtx ModuleCtx store)
  moduleToGlobal <- expectRight (restrictionMap ModuleCtx GlobalCtx store)
  localToGlobal <- expectRight (restrictionMap LocalCtx GlobalCtx store)
  let composed =
        IntMap.map
          (\classId -> IntMap.findWithDefault classId classId moduleToGlobal)
          localToModule
  composed @?= localToGlobal
  restrictionMap ModuleCtx LocalCtx store
    @?= Left (PreparedContextRestrictionUnavailable ModuleCtx LocalCtx)

testPropagationTargets :: Assertion
testPropagationTargets =
  withPreparedContextSiteFromFiniteLattice chainLattice $ \site ->
    expectRight (propagationTargets GlobalCtx 2 3 (chainStore site))
      >>= (@?= [LocalCtx])

testContextWitnessLaws :: Assertion
testContextWitnessLaws =
  withPreparedContextSiteFromFiniteLattice chainLattice $ \site -> do
  let store = chainStore site
  ...
  contextRestrictionIdentity ModuleCtx store @?= Right True
  ...
  contextRestrictionFunctorialAction localToModule moduleToGlobal store @?= Right True
  contextGlobalSectionInvariant localToGlobal store @?= Right True

testAnalysisWitnessLaws :: Assertion
testAnalysisWitnessLaws =
  withPreparedContextSiteFromFiniteLattice chainLattice $ \site -> do
  let store = chainStore site
  localToModule <- expectRightJust "expected LocalCtx -> ModuleCtx morphism" (mkContextMorphism chainLattice LocalCtx ModuleCtx)
  moduleToGlobal <- expectRightJust "expected ModuleCtx -> GlobalCtx morphism" (mkContextMorphism chainLattice ModuleCtx GlobalCtx)
  localToGlobal <- expectRightJust "expected LocalCtx -> GlobalCtx morphism" (mkContextMorphism chainLattice LocalCtx GlobalCtx)
  contextAnalysisRestrictionComposition localToModule moduleToGlobal store @?= Right True
  contextAnalysisGlobalSectionInvariant localToGlobal store @?= Right True
...
testVisibleSupportDescent =
  withPreparedContextSiteFromFiniteLattice coverLattice $ \site ->
  let store = supportVisibleStore site
      report = fullDescentCheck store
   in do
        descentAt RootCtx store @?= SearchAccepted
        drSatisfied report @?= True
        drObstructionCount report @?= 0

testDiamondDescentSatisfied :: Assertion
testDiamondDescentSatisfied =
  withPreparedContextSiteFromFiniteLattice branchContextLattice $ \site ->
  let store = diamondSatisfiedStore site
      report = fullDescentCheck store
   in do
        descentAt BranchBase store @?= SearchAccepted
        drSatisfied report @?= True
        drObstructionCount report @?= 0
...
testDiamondDescentObstructed =
  withPreparedContextSiteFromFiniteLattice branchContextLattice $ \site ->
  case descentAt BranchBase (diamondObstructedStore site) of
    SearchAccepted ->
...
vacuousCoverStore :: PreparedContextSite owner CoverCtx -> MockStore owner CoverCtx
vacuousCoverStore site =
  (supportVisibleStore site)
    { msClasses =
        Map.insert MiddleCtx IntMap.empty (msClasses (supportVisibleStore site))
    }

testVacuousCoverDescent :: Assertion
testVacuousCoverDescent =
  withPreparedContextSiteFromFiniteLattice coverLattice $ \site ->
  case descentAt RootCtx (vacuousCoverStore site) of
    SearchAccepted ->
...
testDiamondWitnessLaws =
  withPreparedContextSiteFromFiniteLattice branchContextLattice $ \site -> do
  let store = diamondSatisfiedStore site
  ...
  contextRestrictionIdentity BranchLeft store @?= Right True
  ...
  contextRestrictionFunctorialAction topToLeft leftToBottom store @?= Right True
  contextGlobalSectionInvariant topToBottom store @?= Right True
...
testAnatomyDescentSatisfied =
  withPreparedContextSiteFromFiniteLattice anatomyLattice $ \site ->
  let store = anatomyStore site
      report = fullDescentCheck store
   in do
        drSatisfied report @?= True
        drObstructionCount report @?= 0
        descentAt AWhole store @?= SearchAccepted
        descentAt AUpper store @?= SearchAccepted

testAnatomyWitnessLaws :: Assertion
testAnatomyWitnessLaws =
  withPreparedContextSiteFromFiniteLattice anatomyLattice $ \site -> do
  let store = anatomyStore site
  ...
  contextRestrictionIdentity AUpper store @?= Right True
  ...
  contextRestrictionFunctorialAction headToUpper upperToWhole store @?= Right True
  contextGlobalSectionInvariant headToWhole store @?= Right True
  contextAnalysisRestrictionComposition headToUpper upperToWhole store @?= Right True

testContextRefreshDiscardsStaleDirtyContexts :: Assertion
testContextRefreshDiscardsStaleDirtyContexts =
  withPreparedContextSiteFromFiniteLattice refreshLattice $ \site -> do
  let refreshedSite =
        bootstrapContextSectionsWithBudget
          1024
          refreshRuntime
...
testContextRefreshRejectsInvalidRestrictionEdges =
  withPreparedContextSiteFromFiniteLattice refreshLattice $ \site -> do
  let refreshedSite =
        bootstrapContextSectionsWithBudget
          1024
...
testClassSupportIndexLocalInsert =
  withPreparedContextSiteFromFiniteLattice chainLattice $ \site -> do
  let localSupport = chainSupport [ModuleCtx]
  (supportIndex, supportDelta) <-
    expectRight
      (classSupportIndexInsertMany site localSupport (IntSet.singleton 42) (compiledFixtureSupportIndex site IntMap.empty))
