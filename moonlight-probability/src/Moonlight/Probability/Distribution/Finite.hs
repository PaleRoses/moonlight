...
uniformFiniteDistribution :: Ord a => NonEmpty a -> Either FiniteDistributionError (FiniteDistribution a)
uniformFiniteDistribution = mkFiniteDistribution . Map.fromList . fmap (,1.0) . NonEmpty.toList
...
finiteRestrict allowed distribution =
  let restrictedWeights =
        foldr
          (\(outcome, probability) ->
             if Set.member outcome allowed
               then Map.insert outcome (positiveProbValue probability)
               else id
          )
          Map.empty
          (finiteWeightedSupport distribution)
   in either (const Nothing) Just (mkFiniteDistribution restrictedWeights)
