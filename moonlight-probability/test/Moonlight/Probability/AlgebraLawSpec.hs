  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testProperty "certain finite is singleton and deterministic" propCertainFiniteConstructor,
          ...
          testCase "uniform finite deduplicates repeated outcomes" testUniformFiniteDeduplicatesOutcomes,
...
testUniformFiniteDeduplicatesOutcomes :: IO ()
testUniformFiniteDeduplicatesOutcomes = do
  let distribution = uniformFiniteDistribution (2 :| [1, 2, 1]) :: FiniteDistribution Int
  finiteSupport distribution @?= Set.fromList [1, 2]
  fmap probValue (finiteLookup 1 distribution) @?= Just 0.5
  fmap probValue (finiteLookup 2 distribution) @?= Just 0.5
...
propUniformFiniteConstructor weightSample =
  let support = supportFromPositiveWeights weightSample
      distribution = uniformFiniteDistribution support
      expectedSupport = Set.fromList (NonEmpty.toList support)
...
propUniformCategoricalConstructor weightSample =
  let support = supportFromPositiveWeights weightSample
      categorical = uniformCategorical support
      expectedSupport = Set.fromList (NonEmpty.toList support)
      expectedProbability = uniformProbability support
