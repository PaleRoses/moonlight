...
getContextRows ::
  ...
  ctx ->
  StateT (ContextRowsCache ctx rows) m rows
getContextRows runtime targetContext = do
  ...
  case cached of
    Just rows -> pure rows
    Nothing -> do
      availableContexts <- gets (cachedContextSetForRuntime runtime)
      maybeSourceContext <-
        lift (crrChooseRestrictionSource runtime availableContexts targetContext)
      rows <-
        case maybeSourceContext of
          Nothing -> lift (crrMaterializeRootRows runtime targetContext)
          Just sourceContext
            | sourceContext == targetContext ->
                lift (crrMaterializeRootRows runtime targetContext)
            | otherwise -> do
                sourceRows <- getContextRows runtime sourceContext
...
cachedContextSetForRuntime ::
  Ord ctx =>
  ContextRowsRuntime m ctx rows ->
  ContextRowsCache ctx rows ->
  Set ctx
cachedContextSetForRuntime runtime =
  Set.fromList
    . fmap contextRowsKeyContextRaw
    . filter currentKey
    . Map.keys
    . contextRowsCacheEntriesRaw
  where
...
{-# INLINE cachedContextSetForRuntime #-}
