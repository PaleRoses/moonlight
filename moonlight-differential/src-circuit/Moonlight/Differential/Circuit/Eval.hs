...
-- native kernel's delta rule must agree with, and the engine under fixpoint
-- bodies. A foreign kernel carries the same obligation under the explicit
-- lawful-foreign-kernel premise.
      ...
      case node of
        ...
        DraftDistinct (parent :: Node s a) ->
          fmap
            ...
            (readParent @(ZSet a weight) (nodeId parent))
        DraftFixpoint budget (seed :: Node s a) feedback result spanIds -> do
          let scope =
                closedScopeForFixpoint selfId feedback result spanIds
