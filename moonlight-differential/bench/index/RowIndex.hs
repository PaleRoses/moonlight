...
import Common (checkedCase, eitherShow, weightAt)
...
rowIdsForSize :: Int -> Either RowIdError PreparedRowIds
rowIdsForSize size =
  PreparedRowIds <$> traverse mkRowId [0 .. size - 1]
...
rowIdSetUnionCase :: Int -> Either RowIdError PreparedRowIdSetPair
rowIdSetUnionCase size = do
  PreparedRowIds leftIds <- rowIdsForSize size
  PreparedRowIds rightIds <- rowIdsForSize (size * 2)
  pure (PreparedRowIdSetPair (rowIdSetFromList leftIds, rowIdSetFromList rightIds))
...
rowIdSetIntersectsWitnessCase :: Int -> Either RowIdError PreparedRowIdSetIntersection
rowIdSetIntersectsWitnessCase size = do
  rowIds <- rowIdsBetween 0 size
  pure
...
rowIdSetIntersectsNoWitnessCase :: Int -> Either RowIdError PreparedRowIdSetIntersection
rowIdSetIntersectsNoWitnessCase size = do
  rowIds <- rowIdsBetween 0 size
  pure
    ( PreparedRowIdSetIntersection
        (rowIdSetFromList rowIds)
...
rowIdSetIntersectsLateWitnessCase :: Int -> Either RowIdError PreparedRowIdSetIntersection
rowIdSetIntersectsLateWitnessCase size = do
  rowIds <- rowIdsBetween 0 size
  pure
    ( PreparedRowIdSetIntersection
        (rowIdSetFromList rowIds)
...
rowSetThresholdBenchmarks =
  [ env (checkedCase "RowSet small-to-sparse fixture" (eitherShow smallToSparseTransition)) $ \transition ->
      bench "small->sparse insert" (nf rowSetTransitionWeight transition),
    env (checkedCase "RowSet sparse-to-dense fixture" (eitherShow sparseToDenseTransition)) $ \transition ->
      bench "sparse->dense insert" (nf rowSetTransitionWeight transition),
    env (checkedCase "RowSet dense-to-sparse fixture" (eitherShow denseToSparseTransition)) $ \transition ->
      bench "dense->sparse delete" (nf rowSetTransitionWeight transition)
  ]

smallToSparseTransition :: Either RowIdError PreparedRowSetTransition
...
sparseToDenseTransition :: Either RowIdError PreparedRowSetTransition
sparseToDenseTransition =
  PreparedRowSetInsertTransition
    (rowSetFromIntSetWithUniverse rowSetDenseMinUniverse (IntSet.fromDistinctAscList [0 .. 126]))
    <$> rowIdsBetween 127 1

denseToSparseTransition :: Either RowIdError PreparedRowSetTransition
...
rowIdsBetween :: Int -> Int -> Either RowIdError [RowId]
rowIdsBetween start count =
  traverse mkRowId [start .. start + count - 1]
...
newtype RowsCacheLookupWeight = RowsCacheLookupWeight
  (Either (ContextRowsSourceSelectionError Int) Int)

instance NFData RowsCacheLookupWeight where
  rnf (RowsCacheLookupWeight result) =
    case result of
      ...
      Left (ContextRowsRestrictionSourceOutsideCachedDomain sourceContext targetContext) ->
        rnf sourceContext `seq` rnf targetContext
      Right weight ->
        rnf weight
...
rowsCacheHitWeight :: PreparedRowsCache -> RowsCacheLookupWeight
rowsCacheHitWeight (PreparedRowsCache cache) =
  RowsCacheLookupWeight
    (runIdentity (evalStateT (getContextRows rowsCacheRuntime 255) cache))

rowsCacheMissWeight :: PreparedRowsCache -> RowsCacheLookupWeight
