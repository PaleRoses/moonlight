...
mkCheckedInputAllocationProbe ::
  NFData input =>
  String ->
  Either String input ->
  (input -> Int) ->
  AllocationProbe
mkCheckedInputAllocationProbe label input measure =
  AllocationProbe
    { allocationProbeLabel = label,
      allocationProbeSetup =
        checkedCase label input >>= evaluate . rnf,
      allocationProbeMeasure =
        checkedCase label input >>= evaluate . measure
    }
...
allocationProbes =
  [ mkAllocationProbe "Batch.fromUpdates duplicate-heavy n=2048" (duplicateHeavyUpdateCase 2048) batchBuildCellCount,
    ...
    mkCheckedInputAllocationProbe "DenseTriangle.count clique n=512" (eitherShow (denseTriangleCliqueCase 512)) denseTriangleCountWeight,
    mkAllocationProbe "DenseTriangle.exact-count clique n=128" (denseTriangleExactCliqueCase 128) exactTriangleCountWeight,
    mkCheckedInputAllocationProbe "DenseTriangle.count skewed n=512" (eitherShow (denseTriangleSkewedCase 512)) denseTriangleCountWeight,
