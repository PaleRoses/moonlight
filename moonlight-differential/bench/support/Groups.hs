...
denseTriangleBenchmarksForSize size =
  [ env (checkedCase "DenseTriangle.count path" (eitherShow (denseTrianglePathCase size))) $ \triangleCase ->
      bench (caseLabel "DenseTriangle.count path" size) (nf denseTriangleCountWeight triangleCase),
    env (checkedCase "DenseTriangle.count star" (eitherShow (denseTriangleStarCase size))) $ \triangleCase ->
      bench (caseLabel "DenseTriangle.count star" size) (nf denseTriangleCountWeight triangleCase),
    env (checkedCase "DenseTriangle.count clique" (eitherShow (denseTriangleCliqueCase size))) $ \triangleCase ->
    ...
    env (checkedCase "DenseTriangle.count skewed" (eitherShow (denseTriangleSkewedCase size))) $ \triangleCase ->
...
rowIdSetBenchmarksForSize size =
  [ env (checkedCase "RowIdSet build fixture" (eitherShow (rowIdsForSize size))) $ \rowIds ->
      bench (caseLabel "build/insert/fold" size) (nf rowIdSetInsertFoldWeight rowIds),
    env (checkedCase "RowIdSet member fixture" (eitherShow (rowIdsForSize size))) $ \rowIds ->
      bench (caseLabel "member sweep" size) (nf rowIdSetMemberSweepWeight rowIds),
    env (checkedCase "RowIdSet union fixture" (eitherShow (rowIdSetUnionCase size))) $ \sets ->
      bench (caseLabel "union" size) (nf rowIdSetUnionWeight sets)
  ]

rowSetBenchmarksForSize :: Int -> [Benchmark]
rowSetBenchmarksForSize size =
  [ env (checkedCase "RowSet build fixture" (eitherShow (rowIdsForSize size))) $ \rowIds ->
    ...
    env (checkedCase "RowIdSet intersection witness fixture" (eitherShow (rowIdSetIntersectsWitnessCase size))) $ \sets ->
      bench (caseLabel "intersects-row-id-set/witness" size) (nf rowIdSetIntersectsWeight sets),
    env (checkedCase "RowIdSet intersection disjoint fixture" (eitherShow (rowIdSetIntersectsNoWitnessCase size))) $ \sets ->
      bench (caseLabel "intersects-row-id-set/no-witness" size) (nf rowIdSetIntersectsWeight sets),
    env (checkedCase "RowIdSet intersection late-witness fixture" (eitherShow (rowIdSetIntersectsLateWitnessCase size))) $ \sets ->
