...
data DenseDeltaProblem = DenseDeltaProblem
  { ddpSlotUniverse :: {-# UNPACK #-} !Int,
    ...
    ddpBitWordCount :: {-# UNPACK #-} !Int,
    ddpUndoCapacity :: {-# UNPACK #-} !Int,
    ddpDeltaUndoCapacity :: {-# UNPACK #-} !Int
  }

data DenseDeltaProblemSlotOwner
  = DenseDeltaFullSchema
  | DenseDeltaOutputSchema
  | DenseDeltaSourceValueIndex !Int
  | DenseDeltaSelectedOutputValueIndex
  deriving stock (Eq, Ord, Show)

data DenseDeltaCapacity
  = DenseDeltaSourceCount
  | DenseDeltaSlotUniverse
  | DenseDeltaFullSchemaVector
  | DenseDeltaOutputSchemaVector
  | DenseDeltaSourceArrays
  | DenseDeltaSlotArrays
  | DenseDeltaBitArrays
  | DenseDeltaSourceUndo
  | DenseDeltaUndoArrays
  | DenseDeltaDirtyUndoArrays
  deriving stock (Eq, Ord, Show)

data DenseDeltaProblemError
  = DenseDeltaNegativeSlot !DenseDeltaProblemSlotOwner !Int
  | DenseDeltaDuplicateFullSchemaSlot !Int
  | DenseDeltaDuplicateOutputSchemaSlot !Int
  | DenseDeltaOutputSlotAbsentFromFullSchema !Int
  | DenseDeltaSourceSlotAbsentFromFullSchema !Int !Int
  | DenseDeltaSelectedOutputSlotsMismatch !IntSet !IntSet
  | DenseDeltaSlotOutsideUniverse !DenseDeltaProblemSlotOwner !Int !Int
  | DenseDeltaCapacityAdditionOverflow !DenseDeltaCapacity !Int !Int
  | DenseDeltaCapacityProductOverflow !DenseDeltaCapacity !Int !Int
  deriving stock (Eq, Ord, Show)
...
mkDenseDeltaProblem ::
  ...
  [Int] ->
  [Int] ->
  Maybe (RowSet, IntMap (IntMap RowIdSet)) ->
  Either DenseDeltaProblemError DenseDeltaProblem
mkDenseDeltaProblem sources fullSchema outputSchema selectedOutputData = do
  sourceCount <- checkedListLength DenseDeltaSourceCount sources
  fullSchemaCount <- checkedListLength DenseDeltaFullSchemaVector fullSchema
  outputSchemaCount <- checkedListLength DenseDeltaOutputSchemaVector outputSchema
  traverse_ (validateNonnegativeSlot DenseDeltaFullSchema) fullSchema
  traverse_ (validateNonnegativeSlot DenseDeltaOutputSchema) outputSchema
  traverse_
    (\(sourceId, source) -> traverse_ (validateNonnegativeSlot (DenseDeltaSourceValueIndex sourceId)) (IntSet.toAscList (deltaJoinSourceSlots source)))
    (zip [0 ..] sources)
  traverse_
    (traverse_ (validateNonnegativeSlot DenseDeltaSelectedOutputValueIndex) . IntMap.keys . snd)
    selectedOutputData
  fullSchemaSlots <- validateUniqueSlots DenseDeltaDuplicateFullSchemaSlot fullSchema
  outputSchemaSlots <- validateUniqueSlots DenseDeltaDuplicateOutputSchemaSlot outputSchema
  traverse_ (validateOutputSlot fullSchemaSlots) outputSchema
  traverse_ (validateSourceSlots fullSchemaSlots) (zip [0 ..] sources)
  validateSelectedOutputSlots outputSchemaSlots selectedOutputData
  let sourceSlots = fmap deltaJoinSourceSlots sources
      selectedSlots = maybe IntSet.empty (IntMap.keysSet . snd) selectedOutputData
      referencedSlots = IntSet.unions (fullSchemaSlots : outputSchemaSlots : selectedSlots : sourceSlots)
  slotUniverse <- slotUniverseFromReferencedSlots referencedSlots
  traverse_ (validateSlotInsideUniverse slotUniverse DenseDeltaFullSchema) fullSchema
  traverse_ (validateSlotInsideUniverse slotUniverse DenseDeltaOutputSchema) outputSchema
  traverse_
    (\(sourceId, source) -> traverse_ (validateSlotInsideUniverse slotUniverse (DenseDeltaSourceValueIndex sourceId)) (IntSet.toAscList (deltaJoinSourceSlots source)))
    (zip [0 ..] sources)
  traverse_
    (traverse_ (validateSlotInsideUniverse slotUniverse DenseDeltaSelectedOutputValueIndex) . IntMap.keys . snd)
    selectedOutputData
  let sourcesBySlot = deltaJoinSourcesBySlot sources
      selectedUndoCapacity = maybe 0 (const outputSchemaCount) selectedOutputData
      bitWordCount = bitWordsForSlots slotUniverse
  sourceUndoCapacity <-
    foldM
      (\capacity slot -> checkedAdd DenseDeltaSourceUndo capacity (IntSet.size (IntMap.findWithDefault IntSet.empty slot sourcesBySlot)))
      0
      fullSchema
  undoCapacity <- checkedAdd DenseDeltaUndoArrays sourceUndoCapacity selectedUndoCapacity
  doubledSourceUndoCapacity <- checkedProduct DenseDeltaDirtyUndoArrays 2 sourceUndoCapacity
  deltaUndoCapacity <- checkedAdd DenseDeltaDirtyUndoArrays doubledSourceUndoCapacity selectedUndoCapacity
  validateVectorAllocation DenseDeltaSourceArrays pointerBytes sourceCount
  validateVectorAllocation DenseDeltaFullSchemaVector intBytes fullSchemaCount
  validateVectorAllocation DenseDeltaOutputSchemaVector intBytes outputSchemaCount
  validateVectorAllocation DenseDeltaSlotArrays intBytes slotUniverse
  validateVectorAllocation DenseDeltaSlotArrays pointerBytes slotUniverse
  validateVectorAllocation DenseDeltaBitArrays word64Bytes bitWordCount
  validateVectorAllocation DenseDeltaUndoArrays intBytes undoCapacity
  validateVectorAllocation DenseDeltaUndoArrays pointerBytes undoCapacity
  validateVectorAllocation DenseDeltaDirtyUndoArrays intBytes deltaUndoCapacity
  validateVectorAllocation DenseDeltaDirtyUndoArrays pointerBytes deltaUndoCapacity
  pure
    DenseDeltaProblem
      { ddpSlotUniverse = slotUniverse,
        ddpSources = SmallArray.smallArrayFromList sources,
        ddpSourcesBySlot = sourcesBySlot,
        ddpFullSchema = PrimArray.primArrayFromList fullSchema,
        ddpOutputSchema = PrimArray.primArrayFromList outputSchema,
        ddpSelectedOutput = uncurry DenseSelectedOutput <$> selectedOutputData,
        ddpStaticRank = IntMap.fromList (zip fullSchema [0 ..]),
        ddpBitWordCount = bitWordCount,
        ddpUndoCapacity = undoCapacity,
        ddpDeltaUndoCapacity = deltaUndoCapacity
      }

denseDeltaProblemFullSchema :: DenseDeltaProblem -> PrimArray Int
denseDeltaProblemFullSchema =
  ddpFullSchema

denseDeltaProblemOutputSchema :: DenseDeltaProblem -> PrimArray Int
denseDeltaProblemOutputSchema =
  ddpOutputSchema

denseDeltaProblemSelectedOutputData :: DenseDeltaProblem -> Maybe (RowSet, IntMap (IntMap RowIdSet))
denseDeltaProblemSelectedOutputData problem =
  (\selected -> (dsoRows selected, dsoRowsBySlotValue selected)) <$> ddpSelectedOutput problem

validateNonnegativeSlot :: DenseDeltaProblemSlotOwner -> Int -> Either DenseDeltaProblemError ()
validateNonnegativeSlot owner slot
  | slot < 0 =
      Left (DenseDeltaNegativeSlot owner slot)
  | otherwise =
      Right ()

validateUniqueSlots :: (Int -> DenseDeltaProblemError) -> [Int] -> Either DenseDeltaProblemError IntSet
validateUniqueSlots duplicateError =
  foldM
    ( \seen slot ->
        if IntSet.member slot seen
          then Left (duplicateError slot)
          else Right (IntSet.insert slot seen)
    )
    IntSet.empty

validateOutputSlot :: IntSet -> Int -> Either DenseDeltaProblemError ()
validateOutputSlot fullSchemaSlots slot
  | IntSet.member slot fullSchemaSlots =
      Right ()
  | otherwise =
      Left (DenseDeltaOutputSlotAbsentFromFullSchema slot)

validateSourceSlots :: IntSet -> (Int, DeltaJoinSource) -> Either DenseDeltaProblemError ()
validateSourceSlots fullSchemaSlots (sourceId, source) =
  traverse_ validateSourceSlot (IntSet.toAscList (deltaJoinSourceSlots source))
  where
    validateSourceSlot slot
      | IntSet.member slot fullSchemaSlots =
          Right ()
      | otherwise =
          Left (DenseDeltaSourceSlotAbsentFromFullSchema sourceId slot)

validateSelectedOutputSlots ::
  IntSet ->
  Maybe (RowSet, IntMap (IntMap RowIdSet)) ->
  Either DenseDeltaProblemError ()
validateSelectedOutputSlots outputSchemaSlots selectedOutputData =
  case selectedOutputData of
    Nothing ->
      Right ()
    Just (_, rowsBySlotValue)
      | selectedSlots == outputSchemaSlots ->
          Right ()
      | otherwise ->
          Left (DenseDeltaSelectedOutputSlotsMismatch outputSchemaSlots selectedSlots)
      where
        selectedSlots =
          IntMap.keysSet rowsBySlotValue

slotUniverseFromReferencedSlots :: IntSet -> Either DenseDeltaProblemError Int
slotUniverseFromReferencedSlots referencedSlots =
  case IntSet.maxView referencedSlots of
    Nothing ->
      Right 0
    Just (largestSlot, _) ->
      checkedAdd DenseDeltaSlotUniverse largestSlot 1

validateSlotInsideUniverse :: Int -> DenseDeltaProblemSlotOwner -> Int -> Either DenseDeltaProblemError ()
validateSlotInsideUniverse slotUniverse owner slot
  | validSlotKey slotUniverse slot =
      Right ()
  | otherwise =
      Left (DenseDeltaSlotOutsideUniverse owner slotUniverse slot)

checkedListLength :: DenseDeltaCapacity -> [value] -> Either DenseDeltaProblemError Int
checkedListLength capacity =
  foldM (\count _ -> checkedAdd capacity count 1) 0

checkedAdd :: DenseDeltaCapacity -> Int -> Int -> Either DenseDeltaProblemError Int
checkedAdd capacity leftCount rightCount
  | rightCount > maxBound - leftCount =
      Left (DenseDeltaCapacityAdditionOverflow capacity leftCount rightCount)
  | otherwise =
      Right (leftCount + rightCount)

checkedProduct :: DenseDeltaCapacity -> Int -> Int -> Either DenseDeltaProblemError Int
checkedProduct capacity leftCount rightCount
  | leftCount == 0 || rightCount == 0 =
      Right 0
  | leftCount > maxBound `quot` rightCount =
      Left (DenseDeltaCapacityProductOverflow capacity leftCount rightCount)
  | otherwise =
      Right (leftCount * rightCount)

validateVectorAllocation :: DenseDeltaCapacity -> Int -> Int -> Either DenseDeltaProblemError ()
validateVectorAllocation capacity elementBytes elementCount =
  () <$ checkedProduct capacity elementCount elementBytes
...
foldDenseWCOJ problem leaf initial =
  runST $ do
    let !slotUniverse = ddpSlotUniverse problem
        ...
        !bitWordCount = ddpBitWordCount problem
