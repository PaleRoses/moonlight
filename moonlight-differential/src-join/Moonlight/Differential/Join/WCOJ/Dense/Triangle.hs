...
instance NFData DenseTriangleTrie where
  rnf
    ( DenseTriangleTrie
        vertexCount
        edgeCount
      ...
      rnf vertexCount
        ...
        `seq` rnf offsets
        `seq` rnf targets
        `seq` rnf orientedSources
        `seq` rnf orientedTargets
        `seq` rnf bitWordCount
        `seq` rnf bitWordOffsets
        `seq` rnf firstNonzeroBitWords
        `seq` rnf lastNonzeroBitWords
        `seq` rnf adjacencyBits

data DenseTriangleAllocation
  = DenseTriangleEdgeCount
  | DenseTriangleDegreeVector
  | DenseTriangleOffsetVector
  | DenseTriangleTargetVector
  | DenseTriangleOrientedEdgeVectors
  | DenseTriangleBitWordOffsetVector
  | DenseTriangleBitBoundsVectors
  | DenseTriangleAdjacencyBits
  deriving stock (Eq, Show, Generic)

instance NFData DenseTriangleAllocation

data DenseTriangleBuildError
  = DenseTriangleNegativeVertexCount !Int
  | DenseTriangleEndpointOutOfRange !Int !(Int, Int)
  | DenseTriangleCapacityAdditionOverflow !DenseTriangleAllocation !Int !Int
  | DenseTriangleCapacityProductOverflow !DenseTriangleAllocation !Int !Int
  deriving stock (Eq, Show, Generic)

instance NFData DenseTriangleBuildError
...
buildDenseTriangleTrie :: Int -> [(Int, Int)] -> Either DenseTriangleBuildError DenseTriangleTrie
buildDenseTriangleTrie vertexCount rawEdges = do
  validateDeclaredVertexCount vertexCount
  traverse_ (validateDenseTriangleEdge vertexCount) normalizedEdges
  edgeCount <- checkedListLength DenseTriangleEdgeCount normalizedEdges
  offsetCount <- checkedAdd DenseTriangleOffsetVector vertexCount 1
  adjacencyBitCount <- checkedProduct DenseTriangleAdjacencyBits vertexCount bitWordCount
  validateVectorAllocation DenseTriangleDegreeVector intBytes vertexCount
  validateVectorAllocation DenseTriangleOffsetVector intBytes offsetCount
  validateVectorAllocation DenseTriangleTargetVector intBytes edgeCount
  validateVectorAllocation DenseTriangleOrientedEdgeVectors intBytes edgeCount
  validateVectorAllocation DenseTriangleBitWordOffsetVector intBytes bitWordCount
  validateVectorAllocation DenseTriangleBitBoundsVectors intBytes vertexCount
  validateVectorAllocation DenseTriangleAdjacencyBits word64Bytes adjacencyBitCount
  let adjacencyBits =
        orientedAdjacencyBitVector adjacencyBitCount bitWordCount orientedEdges
      firstNonzeroBitWords =
...
validateDeclaredVertexCount :: Int -> Either DenseTriangleBuildError ()
validateDeclaredVertexCount vertexCount
  | vertexCount < 0 =
      Left (DenseTriangleNegativeVertexCount vertexCount)
  | otherwise =
      Right ()

validateDenseTriangleEdge :: Int -> (Int, Int) -> Either DenseTriangleBuildError ()
validateDenseTriangleEdge vertexCount edge@(leftVertex, rightVertex)
  | leftVertex >= vertexCount || rightVertex >= vertexCount =
      Left (DenseTriangleEndpointOutOfRange vertexCount edge)
  | otherwise =
      Right ()

checkedListLength :: DenseTriangleAllocation -> [value] -> Either DenseTriangleBuildError Int
checkedListLength allocation =
  foldl'
    (\countResult _ -> countResult >>= \count -> checkedAdd allocation count 1)
    (Right 0)

checkedAdd :: DenseTriangleAllocation -> Int -> Int -> Either DenseTriangleBuildError Int
checkedAdd allocation leftCount rightCount
  | rightCount > maxBound - leftCount =
      Left (DenseTriangleCapacityAdditionOverflow allocation leftCount rightCount)
  | otherwise =
      Right (leftCount + rightCount)

checkedProduct :: DenseTriangleAllocation -> Int -> Int -> Either DenseTriangleBuildError Int
checkedProduct allocation leftCount rightCount
  | leftCount == 0 || rightCount == 0 =
      Right 0
  | leftCount > maxBound `quot` rightCount =
      Left (DenseTriangleCapacityProductOverflow allocation leftCount rightCount)
  | otherwise =
      Right (leftCount * rightCount)

validateVectorAllocation :: DenseTriangleAllocation -> Int -> Int -> Either DenseTriangleBuildError ()
validateVectorAllocation allocation elementBytes elementCount =
  () <$ checkedProduct allocation elementCount elementBytes
...
orientedAdjacencyBitVector adjacencyBitCount wordCount orientedEdges =
  VU.accum
    (.|.)
    (VU.replicate adjacencyBitCount 0)
