...
data RowSetError
  ...
  | RowSetSingletonReserved !Int
  | RowSetSmallTooLarge !Int
  | RowSetNegativeMember !Int
  | RowSetReservedMember !Int
...
rowSetFromIntSetCanonical rows =
  case IntSet.lookupMax validRows of
    Nothing ->
      RowSetEmpty
    Just maxRow ->
      rowSetFromNonnegativeIntSetWithUniverse (exclusiveUniverseForRowId maxRow) validRows
  where
    !validRows =
      validRowIdIntSet rows
{-# INLINE rowSetFromIntSetCanonical #-}

rowSetFromIntSetWithUniverse :: Int -> IntSet -> RowSet
rowSetFromIntSetWithUniverse universe rows =
  rowSetFromNonnegativeIntSetWithUniverse canonicalUniverse validRows
  where
    !validRows =
      validRowIdIntSet rows
    !canonicalUniverse =
      max (max 0 universe) (maybe 0 exclusiveUniverseForRowId (IntSet.lookupMax validRows))
...
shouldUseDense universe rows =
  universe >= rowSetDenseMinUniverse
    && IntSet.size rows >= denseMinimumCount universe
...
rowSetInsertInt rowId set
  ...
  | otherwise =
      case set of
        ...
        RowSetSingleton only ->
          rowSetFromIntSetWithUniverse
            (max (exclusiveUniverseForRowId rowId) (exclusiveUniverseForRowId only))
            (IntSet.fromList [only, rowId])
        RowSetSmall values ->
          rowSetFromIntSetWithUniverse
            (max (exclusiveUniverseForRowId rowId) (smallUniverse values))
            (IntSet.insert rowId (IntSet.fromDistinctAscList (smallIntArrayToAscList values)))
        RowSetDense universe denseWords ->
          rowSetInsertDense rowId universe denseWords
        RowSetSparse rows ->
          rowSetFromIntSetWithUniverse
            (max (exclusiveUniverseForRowId rowId) (sparseUniverse rows))
...
smallUniverse values =
  case PrimArray.sizeofPrimArray values of
    0 -> 0
    count -> exclusiveUniverseForRowId (PrimArray.indexPrimArray values (count - 1))
{-# INLINE smallUniverse #-}

sparseUniverse :: IntSet -> Int
sparseUniverse =
  maybe 0 exclusiveUniverseForRowId . IntSet.lookupMax
...
rowSetInsertDense rowId universe denseWords
  ...
  where
    !newUniverse =
      max universe (exclusiveUniverseForRowId rowId)
    !newCount =
...
validateRowSet = \case
  ...
  RowSetSingleton rowId
    ...
    | rowId == maxBound -> Left (RowSetSingletonReserved rowId)
    | otherwise -> Right ()
  RowSetSmall values ->
  ...
  RowSetSparse rows ->
    let !count = IntSet.size rows
     in case negativeIntSetMinimum rows of
          ...
          Nothing ->
            if IntSet.member maxBound rows
              then Left (RowSetReservedMember maxBound)
              else
...
universeOf = \case
  RowSetEmpty -> 0
  RowSetSingleton rowId -> exclusiveUniverseForRowId rowId
  RowSetSmall values ->
    case PrimArray.sizeofPrimArray values of
      0 -> 0
      n -> exclusiveUniverseForRowId (PrimArray.indexPrimArray values (n - 1))
  RowSetDense universe _ -> universe
  RowSetSparse rows ->
    maybe 0 exclusiveUniverseForRowId (IntSet.lookupMax rows)
{-# INLINE universeOf #-}

wordCount :: Int -> Int
wordCount universe =
  let (!wholeWords, !remainingBits) = max 0 universe `quotRem` wordBits
   in wholeWords + if remainingBits == 0 then 0 else 1
{-# INLINE wordCount #-}
...
shouldUseDenseCount universe count =
  universe >= rowSetDenseMinUniverse
    && count >= denseMinimumCount universe
{-# INLINE shouldUseDenseCount #-}

denseMinimumCount :: Int -> Int
denseMinimumCount universe =
  let (!wholeGroups, !remainingRows) = max 0 universe `quotRem` denseDensityDenominator
   in wholeGroups + if remainingRows == 0 then 0 else 1
{-# INLINE denseMinimumCount #-}
...
validateSmall values =
  case invalidSmallValue values of
    Just invalidMember ->
      Left invalidMember
    Nothing ->
...
invalidSmallValue :: PrimArray Int -> Maybe RowSetError
invalidSmallValue =
  smallIntArrayFoldl'
    ( \found value ->
        case found of
          Just invalidMember ->
            Just invalidMember
          Nothing
            | value < 0 -> Just (RowSetNegativeMember value)
            | value == maxBound -> Just (RowSetReservedMember value)
            | otherwise -> Nothing
    )
    Nothing
{-# INLINE invalidSmallValue #-}
...
validRowIdIntSet :: IntSet -> IntSet
validRowIdIntSet =
  IntSet.filter (\rowId -> rowId >= 0 && rowId < maxBound)
{-# INLINE validRowIdIntSet #-}

exclusiveUniverseForRowId :: Int -> Int
exclusiveUniverseForRowId rowId
  | rowId < 0 = 0
  | rowId >= maxBound = maxBound
  | otherwise = rowId + 1
{-# INLINE exclusiveUniverseForRowId #-}
