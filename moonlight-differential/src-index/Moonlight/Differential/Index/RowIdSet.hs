...
-- IntSets is a specified projection onto the public row-identifier domain, and
...
rowIdSetFromIntSetCanonical rowIds
  | IntSet.size validRowIds <= rowIdSetSmallLimit =
      RowIdSetSmall (smallIntArrayFromAscList (IntSet.toAscList validRowIds))
  | otherwise =
      RowIdSetLarge validRowIds
  where
    !validRowIds =
      validRowIdIntSet rowIds
{-# INLINE rowIdSetFromIntSetCanonical #-}

fromDistinctAscListCanonical :: [Int] -> RowIdSet
fromDistinctAscListCanonical values
  | length validValues <= rowIdSetSmallLimit =
      RowIdSetSmall (smallIntArrayFromAscList validValues)
  | otherwise =
      RowIdSetLarge (IntSet.fromDistinctAscList validValues)
  where
    !validValues =
      filter (\rowId -> rowId >= 0 && rowId < maxBound) values
  ...
  RowIdSetLarge values ->
    let !largeCount = IntSet.size values
     in case negativeIntSetMinimum values of
          ...
          Nothing ->
            if IntSet.member maxBound values
              then Left (RowIdSetReservedMember maxBound)
              else
...
invalidSmallValue :: PrimArray Int -> Maybe RowIdSetError
invalidSmallValue =
  smallIntArrayFoldl'
    ( \found value ->
        case found of
          Just invalidMember ->
            Just invalidMember
          Nothing
            | value < 0 -> Just (RowIdSetNegativeMember value)
            | value == maxBound -> Just (RowIdSetReservedMember value)
            | otherwise -> Nothing
    )
    Nothing
