...
data IndexedRowsInsertError layout key
  = IndexedRowsInsertNegativeRowId !Int
...
indexedRowsFromDistinctAscList format layoutColumnIndex schema payloadByKey rowsAsc =
  case indexedRowsValueIndexFromDistinctAscRows format schema keyedRows of
    Left errors ->
      Left errors
    Right valueIndex ->
      Right
...
indexedRowsNextRowId :: IndexedRows layout key payload -> Int
indexedRowsNextRowId =
  irNextRowId
...
indexedRowsInsertFresh format key payload rows =
  case mkRowId (irNextRowId rows) of
    Left _ ->
      Left (IndexedRowsInsertNegativeRowId (irNextRowId rows))
    Right rowId ->
          ...
          Right
            rows
              { irLiveRows = IntSet.insert rowKey (irLiveRows rows),
                ...
                irValueIx = valueIx,
                irNextRowId = max (irNextRowId rows) (rowKey + 1)
              }
  where
    !rowKey =
      rowIdInt rowId
