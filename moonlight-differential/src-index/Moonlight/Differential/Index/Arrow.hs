...
validateSequentialIds startId =
  fmap (const ()) . List.foldl' step (Right startId)
  where
    step :: Either ArrowIndexError Int -> IndexedArrow endpoint edge -> Either ArrowIndexError Int
    step eitherExpected indexedArrow = do
      expectedId <- eitherExpected
...
validateReplacementIds indexValue =
  fmap (const ()) . List.foldl' step (Right IntSet.empty)
  where
    step eitherSeen indexedArrow = do
      seenIds <- eitherSeen
      let arrowId = iaId indexedArrow
      if IntSet.member arrowId seenIds
