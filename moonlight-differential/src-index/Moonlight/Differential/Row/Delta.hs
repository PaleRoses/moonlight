...
dropEmptyRowDeltas :: IntMap RowDelta -> IntMap RowDelta
dropEmptyRowDeltas =
  IntMap.filter (not . rowDeltaNull)
{-# INLINE dropEmptyRowDeltas #-}
...
rowDeltaToRowBlock identityValue schemaValue rowDelta = do
  packedRows <-
    traverse
      ...
      (Map.toAscList (plainRowPatchChangeMap rowDelta))
...
rowDeltaAffectedClasses rowDelta =
  foldMap tupleKeyClassKeys $
    Map.keys (plainRowPatchChangeMap rowDelta)
