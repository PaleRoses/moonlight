...
-- | Spec/value agreement: the circuit's eager denotation at @output@ over the
-- integrated input equals an independently computed reference. The reference
-- arrives already reduced to an ascending association list, keeping this
-- harness ignorant of the implementation algebra used to obtain it.
circuitEagerAgreesWithReference ::
  (Ord value, Show value, Show fault) =>
  Circuit s fault Int ->
  Node s value ->
  ...
  [(value, Int)] ->
  QC.Property
circuitEagerAgreesWithReference circuit output wholeBatch referenceDenotation =
  case evaluateCircuit wholeBatch circuit of
    Left refusal ->
      QC.counterexample ("eager evaluation refused: " <> show refusal) False
    Right eager ->
      case outputDelta output eager of
        Left obstruction ->
          QC.counterexample ("eager output missing: " <> show obstruction) False
        Right eagerOutput ->
          zsetToAscList eagerOutput QC.=== referenceDenotation
