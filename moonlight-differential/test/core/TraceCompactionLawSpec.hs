...
tests =
  testGroup
    ...
    [ testCase "row trace append, advance, and compaction preserve denotation" rowTraceCompactionLaws,
      ...
      testCase "bounded trace identifiers exhaust without wrapping" boundedTraceIdentifiersExhaust,
...
boundedTraceIdentifiersExhaust :: Assertion
boundedTraceIdentifiersExhaust = do
  traceIdFromKey maxBound @?= Left (TraceIdReserved maxBound)
  largestId <- assertRight "largest valid trace identifier" (traceIdFromKey (maxBound - 1))
  nextTraceId largestId @?= Left TraceIdExhausted
  exhaustedTrace <-
    assertRight
      "largest valid trace insertion"
      ( insertIndexedTraceEntry
          lawTraceIndexOps
          (LawEntry largestId 1 1 (lawTime 1 0) 1)
          (emptyIndexedTraceWithOps lawTraceIndexOps)
      )
  itNextId exhaustedTrace @?= Left IndexedTraceIdsExhausted
  validateIndexedTraceCursor lawTraceIndexOps exhaustedTrace @?= Right ()
  validateIndexedTraceCursor
    lawTraceIndexOps
    (indexedTraceWithNextIdForValidation maxBound (emptyIndexedTraceWithOps lawTraceIndexOps))
    @?= Left (IndexedTraceEntryIdInvalid (TraceIdReserved maxBound))
