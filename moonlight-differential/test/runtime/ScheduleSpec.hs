...
tests =
  testGroup
    ...
    [ testProperty "advance observables agree with an independent model" prop_scheduleAgreesWithModel,
      ...
      customOrderDrain,
      emptyPriorityOrderRejected,
      duplicatePriorityRejected,
...
emptySchedule ::
  [Priority] ->
  Either (ScheduleError Priority) Sched
emptySchedule priorities =
  fmap
    (\priorityPlan -> mkProgressSchedule priorityPlan emptyRuntimeFrontier)
    (mkSchedulePriorityPlan priorities)

withValidSchedule ::
  Either (ScheduleError Priority) Sched ->
  (Sched -> Assertion) ->
  Assertion
withValidSchedule scheduleResult assertion =
  case scheduleResult of
    Left scheduleError ->
      assertFailure ("expected valid schedule: " ++ show scheduleError)
    Right schedule ->
      assertion schedule

enqueueValidPriority ::
  Priority ->
  Capability ->
  Payload ->
  Either (ScheduleError Priority) Sched ->
  Either (ScheduleError Priority) Sched
enqueueValidPriority priority capability payload =
  (>>= scheduleEnqueue priority capability payload)
  ...
  case op of
    OpEnqueue priority tick payload ->
      let capability = mkCap tick
       ...
       in case scheduleEnqueue priority capability payload sched of
            Left scheduleError ->
              ( counterexample
                  ("generated known priority was rejected: " ++ show scheduleError)
                  (property False),
                sched,
                model
              )
...
runOps order ops0 =
  case emptySchedule order of
    Left scheduleError ->
      counterexample
        ("generated valid priority order was rejected: " ++ show scheduleError)
        (property False)
    Right schedule ->
      conjoin (go schedule (emptyModel order) ops0)
  where
    go _ _ [] =
      []
...
mergeSinglePending =
  testCase "same-cell re-enqueue merges payload and mints exactly one pending" $ do
    let capability = mkCap 3
        scheduleResult =
          enqueueValidPriority 0 capability [2] $
            enqueueValidPriority 0 capability [1] $
              emptySchedule [0]
    withValidSchedule scheduleResult $ \sched -> do
...
sharedTimeAcrossPriorities =
  testCase "distinct priorities sharing a time stack the pending refcount" $ do
    let capability = mkCap 8
        scheduleResult =
          enqueueValidPriority 1 capability [20] $
            enqueueValidPriority 0 capability [10] $
              emptySchedule [0, 1]
    withValidSchedule scheduleResult $ \sched -> do
...
dequeuePreservesPending =
  testCase "dequeue consumes the cell but leaves the capability pending" $ do
    let capability = mkCap 4
        scheduleResult =
          enqueueValidPriority 0 capability [7] (emptySchedule [0])
    withValidSchedule scheduleResult $ \sched ->
...
customOrderDrain =
  testCase "dequeue drains in the supplied priority order, not Ord order" $ do
    let order = [2, 0, 3, 1]
        scheduleResult =
          foldr
            (\priority -> enqueueValidPriority priority (mkCap priority) [priority])
            (emptySchedule order)
            order
    withValidSchedule scheduleResult $ \sched ->
...
emptyPriorityOrderRejected :: TestTree
emptyPriorityOrderRejected =
  testCase "empty priority order is rejected before schedule construction" $
    assertEqual
      "typed empty-order obstruction"
      (Left SchedulePriorityOrderEmpty)
      (mkSchedulePriorityPlan ([] :: [Priority]))

duplicatePriorityRejected :: TestTree
duplicatePriorityRejected =
  testCase "duplicate priority is rejected before schedule construction" $
    assertEqual
      "typed duplicate obstruction"
      (Left (SchedulePriorityDuplicate 1))
      (mkSchedulePriorityPlan [0, 1, 1, 2])

unknownPriorityRejected :: TestTree
unknownPriorityRejected =
  testCase "enqueue rejects a priority outside the checked plan" $
    withValidSchedule (emptySchedule [0, 1]) $ \sched ->
      assertEqual
        "typed unknown-priority obstruction"
        (Left (SchedulePriorityUnknown 2))
        (scheduleEnqueue 2 (mkCap 1) [7] sched)
...
lifecycleToQuiescence =
  testCase "quiescence requires both cell drain and pending completion" $ do
    let ticks = [10, 11, 12]
        scheduleResult =
          foldr
            (\tick -> enqueueValidPriority 0 (mkCap tick) [tick])
            (emptySchedule [0, 1])
            ticks
    withValidSchedule scheduleResult $ \sched -> do
...
completeMissingPending =
  testCase "completing a never-pending time fails with the typed error" $ do
    withValidSchedule (emptySchedule [0]) $ \sched ->
...
doubleCompleteFails =
  testCase "second completion of a singly-pending time fails" $ do
    let capability = mkCap 5
        scheduleResult =
          enqueueValidPriority 0 capability [1] (emptySchedule [0])
    withValidSchedule scheduleResult $ \sched ->
      case scheduleComplete capability sched of
        Left err ->
          assertFailure ("first completion should succeed: " ++ show err)
