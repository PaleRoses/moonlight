...
tests =
  testGroup
    ...
    ( fmap fixtureTest fixtures
        <> [ testCase "dense problem rejects malformed slot sections" denseProblemRejectsMalformedSlots,
             testCase "dense problem rejects capacity overflow before allocation" denseProblemRejectsCapacityOverflow
           ]
    )

denseProblemRejectsMalformedSlots :: IO ()
denseProblemRejectsMalformedSlots = do
  assertEqual
    "negative full-schema slot"
    (Just (DenseDeltaNegativeSlot DenseDeltaFullSchema (-1)))
    (denseProblemError (mkDenseDeltaProblem [] [-1] [] Nothing))
  assertEqual
    "duplicate full-schema slot"
    (Just (DenseDeltaDuplicateFullSchemaSlot 0))
    (denseProblemError (mkDenseDeltaProblem [] [0, 0] [] Nothing))
  assertEqual
    "source incidence is derived and must lie within the full schema"
    (Just (DenseDeltaSourceSlotAbsentFromFullSchema 0 1))
    (denseProblemError (mkDenseDeltaProblem [sourceWithSlot 1] [0] [] Nothing))
  assertEqual
    "selected-output incidence must equal the output schema"
    (Just (DenseDeltaSelectedOutputSlotsMismatch (IntSet.singleton 0) (IntSet.singleton 1)))
    (denseProblemError (mkDenseDeltaProblem [] [0, 1] [0] (Just (emptyRowSet, IntMap.singleton 1 IntMap.empty))))
  where
    sourceWithSlot slot =
      DeltaJoinSource
        { deltaSourceRows = emptyRowSet,
          deltaSourceDirtyRows = emptyRowSet,
          deltaSourceValueIndex = IntMap.singleton slot IntMap.empty,
          deltaSourceValueAt = \_ _ -> Nothing
        }

denseProblemRejectsCapacityOverflow :: IO ()
denseProblemRejectsCapacityOverflow = do
  assertEqual
    "slot-universe successor must not wrap"
    (Just (DenseDeltaCapacityAdditionOverflow DenseDeltaSlotUniverse maxBound 1))
    (denseProblemError (mkDenseDeltaProblem [] [maxBound] [] Nothing))
  assertEqual
    "physical slot arrays must fit before ST allocation"
    (Just (DenseDeltaCapacityProductOverflow DenseDeltaSlotArrays overflowingUniverse intBytes))
    (denseProblemError (mkDenseDeltaProblem [] [overflowingSlot] [] Nothing))
  where
    intBytes =
      sizeOf (0 :: Int)
    overflowingSlot =
      maxBound `quot` intBytes
    overflowingUniverse =
      overflowingSlot + 1

denseProblemError :: Either DenseDeltaProblemError problem -> Maybe DenseDeltaProblemError
denseProblemError =
