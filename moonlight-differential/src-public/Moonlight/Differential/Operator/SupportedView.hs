...
data SupportedView cell key value = SupportedView
  { supportedViewRows :: !(Map key (SupportedRow cell value)),
    supportedViewBySupport :: !(Map cell (Set key))
  }
  deriving stock (Eq, Show)
...
supportedViewKeysForCells cells view =
  Set.foldl'
    ( \acc cell ->
        maybe acc (Set.union acc) (Map.lookup cell (supportedViewBySupport view))
    )
    Set.empty
    cells
...
dropEdge :: (Ord cell, Ord key) => key -> Map cell (Set key) -> cell -> Map cell (Set key)
dropEdge key inverted cell =
  Map.update
    ( \keys ->
        let !keys1 = Set.delete key keys
         in if Set.null keys1 then Nothing else Just keys1
    )
    cell
    inverted
{-# INLINE dropEdge #-}

addEdge :: (Ord cell, Ord key) => key -> Map cell (Set key) -> cell -> Map cell (Set key)
addEdge key inverted cell =
  Map.insertWith Set.union cell (Set.singleton key) inverted
{-# INLINE addEdge #-}
