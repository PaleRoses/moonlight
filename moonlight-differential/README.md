...
The [Cabal manifest](moonlight-differential.cabal) owns exact source roots,
module exposure, and component dependencies. The semantic owners are:

| Concept | Owner |
| --- | --- |
| Finite support, signed collections, delta actions, time/frontiers, batches, traces, streams, arrangements, operators, and projection maintenance | default `moonlight-differential` library |
| Row blocks, tuple keys, row patches/deltas, reverse indexes, registries, row projection, arrangements, and relation lifecycle | `moonlight-differential-index` |
| Context restriction, carrier topology, row caching, local facts, and bounded settlement | `moonlight-differential-runtime` |
| Generic, adaptive, delta, and dense WCOJ kernels | `moonlight-differential-join` |
| Region-typed graph construction, sealing, denotation, and differential circuit execution | `moonlight-differential-circuit` |
| Private representation constructors shared by safe views and corruption fixtures | private `moonlight-differential-raw` |
| Intentional invariant-corruption helpers for validation tests | `moonlight-differential-test-support` |
| Named reusable law bundles and harness predicates | `moonlight-differential-laws` |
